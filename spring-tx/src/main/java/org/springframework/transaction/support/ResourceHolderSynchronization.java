...
/**
 * {@link TransactionSynchronization} implementation that manages a {@link ResourceHolder} bound through
...
public abstract class ResourceHolderSynchronization<H extends ResourceHolder, K> implements TransactionSynchronization {
     ...
     *            the ResourceHolder to manage
     * @param resourceKey
     *            the key to bind the ResourceHolder for
    ...
    /**
     * Return whether this holder should be unbound at completion (or should rather be left bound to the thread after
     * the transaction).
     * <p>
    ...
    /**
     * Return whether this holder's resource should be released before transaction completion ({@code true}) or rather
     * after transaction completion ({@code false}).
     * <p>
    ...
    /**
     * Return whether this holder's resource should be released after transaction completion ({@code true}).
     * <p>
     * The default implementation returns {@code !shouldReleaseBeforeCompletion()}, releasing after completion if no
     * attempt was made before completion.
     * 
     * @see #releaseResource
     */
    ...
    /**
     * After-commit callback for the given resource holder. Only called when the resource hasn't been released yet
     ...
     *            the key that the ResourceHolder was bound for
     * @param committed
     *            whether the transaction has committed ({@code true}) or rolled back ({@code false})
     */
