...
/**
 * Abstract base class that implements Spring's standard reactive transaction workflow, serving as basis for concrete
 ...
 * completion time. This is mainly used internally by the data access support classes for R2DBC, MongoDB, etc. The same
...
public abstract class AbstractReactiveTransactionManager implements ReactiveTransactionManager, Serializable {
                ...
                return TransactionContextManager.currentContext().map(TransactionSynchronizationManager::new)
                    .flatMap(nestedSynchronizationManager -> suspend(nestedSynchronizationManager, null)
                        .map(Optional::of).defaultIfEmpty(Optional.empty()).flatMap(suspendedResources -> {
                            ...
                            return Mono.defer(() -> {
                                GenericReactiveTransaction status = newReactiveTransaction(nestedSynchronizationManager,
                                    def, transaction, true, debugEnabled, suspendedResources.orElse(null));
                                return doBegin(nestedSynchronizationManager, transaction, def)
                                    .doOnSuccess(
                                        ignore -> prepareSynchronization(nestedSynchronizationManager, status, def))
                                    .thenReturn(status);
    ...
    private Mono<ReactiveTransaction> handleExistingTransaction(
        TransactionSynchronizationManager synchronizationManager, TransactionDefinition definition, Object transaction,
        ...
        if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NOT_SUPPORTED) {
            ...
            return suspend
                .map(suspendedResources -> prepareReactiveTransaction(synchronizationManager, definition, null, false,
                    debugEnabled, suspendedResources)) //
                .switchIfEmpty(Mono.fromSupplier(() -> prepareReactiveTransaction(synchronizationManager, definition,
                    null, false, debugEnabled, null)))
        ...
        if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NESTED) {
            ...
            GenericReactiveTransaction status =
                newReactiveTransaction(synchronizationManager, definition, transaction, true, debugEnabled, null);
            return doBegin(synchronizationManager, transaction, definition)
                .doOnSuccess(ignore -> prepareSynchronization(synchronizationManager, status, definition))
                .thenReturn(status);
        }
    ...
    /**
     * Create a new ReactiveTransaction for the given arguments, also initializing transaction synchronization as
     * appropriate.
     * 
    ...
    /**
     * Suspend all current synchronizations and deactivate transaction synchronization for the current transaction
     * context.
     * 
     * @param synchronizationManager
     *            the synchronization manager bound to the current transaction
     ...
     */
    private Mono<List<TransactionSynchronization>>
    ...
    /**
     * Reactivate transaction synchronization for the current transaction context and resume all given synchronizations.
     * 
     * @param synchronizationManager
     *            the synchronization manager bound to the current transaction
    ...
    private Mono<Void> processCommit(TransactionSynchronizationManager synchronizationManager,
        ...
        Mono<Object> commit =
            prepareForCommit(synchronizationManager, status).then(triggerBeforeCommit(synchronizationManager, status))
     ...
     * methods (e.g. doBegin and doCommit), either directly or as part of a DefaultReactiveTransactionStatus instance.
     * <p>
     ...
     * <p>
     * A special scenario is a nested transaction: This method will be called to start a nested transaction when
     * necessary. In such a context, there will be an active transaction: The implementation of this method has to
     * detect this and start an appropriate nested transaction.
     * 
     * @param synchronizationManager
     *            the synchronization manager bound to the new transaction
     * @param transaction
