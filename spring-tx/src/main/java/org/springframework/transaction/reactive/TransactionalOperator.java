 ...
 *
 * <p>
 * The central method is {@link #transactional}, supporting transactional wrapping of functional sequences code that.
 * This operator handles the transaction lifecycle and possible exceptions such that neither the
 * ReactiveTransactionCallback implementation nor the calling code needs to explicitly handle transactions.
 *
 * <p>
 * Typical usage: Allows for writing low-level data access objects that use resources such as database connections but
 * are not transaction-aware themselves. Instead, they can implicitly participate in transactions handled by
 * higher-level application services utilizing this class, making calls to the low-level services via an inner-class
 * callback object.
 *
 * <p>
 * <strong>Note:</strong> Transactional Publishers should avoid Subscription cancellation. See the <a href=
...
public interface TransactionalOperator {
    ...
    /**
     * Create a new {@link TransactionalOperator} using {@link ReactiveTransactionManager}, using a default transaction.
     *
     ...
     *
     * @param flux
     *            the Flux that should be executed within the transaction
     ...
     *
     * @param mono
     *            the Mono that should be executed within the transaction
     ...
     * <p>
     * Allows for returning a result object created within the transaction, that is, a domain object or a collection of
     * domain objects. A RuntimeException thrown by the callback is treated as a fatal exception that enforces a
