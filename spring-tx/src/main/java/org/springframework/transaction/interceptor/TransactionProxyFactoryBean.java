 ...
 * AOP {@link org.springframework.aop.framework.ProxyFactoryBean} with a separate {@link TransactionInterceptor}
 * definition.
 *
 * <p>
 * <strong>HISTORICAL NOTE:</strong> This class was originally designed to cover the typical case of declarative
 * transaction demarcation: namely, wrapping a singleton target object with a transactional proxy, proxying all the
 * interfaces that the target implements. However, in Spring versions 2.0 and beyond, the functionality provided here is
 * superseded by the more convenient {@code tx:} XML namespace. See the <a href=
 * "https://docs.spring.io/spring/docs/current/spring-framework-reference/data-access.html#transaction-declarative">declarative
 * transaction management</a> section of the Spring reference documentation to understand modern options for managing
 * transactions in Spring applications. For these reasons, <strong>users should favor the {@code tx:} XML namespace as
 * well as the @{@link org.springframework.transaction.annotation.Transactional Transactional}
 * and @{@link org.springframework.transaction.annotation.EnableTransactionManagement EnableTransactionManagement}
 * annotations.</strong>
 *
 * <p>
...
public class TransactionProxyFactoryBean extends AbstractSingletonProxyFactoryBean implements BeanFactoryAware {
    ...
    /**
     * Set the default transaction manager. This will perform actual transaction management: This class is just a way of
     * invoking it.
     * 
     ...
     * single matching bean of type {@link PlatformTransactionManager} will be fetched from the BeanFactory.
     * 
    ...
    /**
     * As of 4.2, this method adds {@link TransactionalProxy} to the set of proxy interfaces in order to avoid
     * re-processing of transaction metadata.
     */
    @Override
