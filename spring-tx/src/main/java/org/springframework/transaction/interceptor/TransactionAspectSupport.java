 ...
 *
 * <p>
 * This enables the underlying Spring transaction infrastructure to be used easily to implement an aspect for any aspect
 * system.
 *
 * <p>
 ...
 *
 * <p>
 * If no transaction name has been specified in the {@link TransactionAttribute}, the exposed name will be the
 * {@code fully-qualified class name + "." + method name} (by default).
 *
 * <p>
 * Uses the <b>Strategy</b> design pattern. A {@link PlatformTransactionManager} or {@link ReactiveTransactionManager}
 * implementation will perform the actual transaction management, and a {@link TransactionAttributeSource} (e.g.
 * annotation-based) is used for determining transaction definitions for a particular class or method.
 *
 * <p>
 * A transaction aspect is serializable if its {@code TransactionManager} and {@code TransactionAttributeSource} are
 * serializable.
 *
 * @author Rod Johnson
...
public abstract class TransactionAspectSupport implements BeanFactoryAware, InitializingBean {
    ...
    /**
     * Holder to support the {@code currentTransactionStatus()} method, and to support communication between different
     * cooperating advices (e.g. before and after advice) if the aspect involves more than a single method (as will be
     * the case for around advice).
     */
    ...
    /**
     * Subclasses can use this to return the current TransactionInfo. Only subclasses that cannot handle all operations
     * in one method, such as an AspectJ aspect involving distinct before and after advice, need to use this mechanism
     * to get at the current TransactionInfo. An around advice such as an AOP Alliance MethodInterceptor can hold a
     ...
     * <p>
     * To find out about specific transaction characteristics, consider using TransactionSynchronizationManager's
     * {@code isSynchronizationActive()} and/or {@code isActualTransactionActive()} methods.
     *
    ...
    /**
     * Return the transaction status of the current method invocation. Mainly intended for code that wants to set the
     * current transaction rollback-only but not throw an application exception.
     *
     * @throws NoTransactionException
     *             if the transaction info cannot be found, because the method was invoked outside an AOP invocation
     *             context
     */
     ...
     * <p>
     * This can either be a traditional {@link PlatformTransactionManager} or a {@link ReactiveTransactionManager} for
     ...
     * reactive transaction management.
     * <p>
     * The default transaction manager will be used if a <em>qualifier</em> has not been declared for a given
    ...
    /**
     * Set multiple transaction attribute sources which are used to find transaction attributes. Will build a
     * CompositeTransactionAttributeSource for the given sources.
     * 
     ...
     * Able to handle {@link CallbackPreferringPlatformTransactionManager} as well as regular
     * {@link PlatformTransactionManager} implementations and {@link ReactiveTransactionManager} implementations for
    ...
    private TransactionManager determineQualifiedTransactionManager(BeanFactory beanFactory, String qualifier) {
        ...
        if (txManager == null) {
            txManager =
                BeanFactoryAnnotationUtils.qualifiedBeanOfType(beanFactory, TransactionManager.class, qualifier);
    ...
    /**
     * Convenience method to return a String representation of this Method for use in logging. Can be overridden in
     ...
     * @return a TransactionInfo object, whether or not a transaction was created. The {@code hasTransaction()} method
     *         on TransactionInfo can be used to tell if there was a transaction created.
    ...
    /**
     * Execute after successful completion of call, but not after an exception was handled. Do nothing if we didn't
    ...
    /**
     * Simple callback interface for proceeding with the target invocation. Concrete interceptors/aspects adapt this to
     * their invocation mechanism.
     */
    @FunctionalInterface
    ...
    /**
     * Opaque object used to hold transaction information. Subclasses must pass it back to methods on this class, but
     * not see its internals.
     */
