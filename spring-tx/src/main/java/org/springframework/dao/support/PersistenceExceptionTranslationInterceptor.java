 ...
 *
 * <p>
 * Delegates to the given {@link PersistenceExceptionTranslator} to translate a RuntimeException thrown into Spring's
 * DataAccessException hierarchy (if appropriate). If the RuntimeException in question is declared on the target method,
 * it is always propagated as-is (with no translation applied).
 *
 * @author Rod Johnson
...
public class PersistenceExceptionTranslationInterceptor
    ...
    /**
     * Create a new PersistenceExceptionTranslationInterceptor, autodetecting PersistenceExceptionTranslators in the
     * given BeanFactory.
     * 
     * @param beanFactory
     ...
     * <p>
     * Default is to autodetect all PersistenceExceptionTranslators in the containing BeanFactory, using them in a
     * chain.
     * 
    ...
    /**
     * Specify whether to always translate the exception ("true"), or whether throw the raw exception when declared,
     * i.e. when the originating method signature's exception declarations allow for the raw exception to be thrown
     * ("false").
     * <p>
     * Default is "false". Switch this flag to "true" in order to always translate applicable exceptions, independent
     * from the originating method signature.
     * <p>
     * Note that the originating method does not have to declare the specific exception. Any base class will do as well,
     * even {@code throws Exception}: As long as the originating method does explicitly declare compatible exceptions,
     * the raw exception will be rethrown. If you would like to avoid throwing raw exceptions in any case, switch this
     * flag to "true".
     */
     ...
     * 
     * @param bf
     *            the ListableBeanFactory to obtain PersistenceExceptionTranslators from
    ...
    protected PersistenceExceptionTranslator detectPersistenceExceptionTranslators(ListableBeanFactory bf) {
        ...
        Map<String, PersistenceExceptionTranslator> pets =
            BeanFactoryUtils.beansOfTypeIncludingAncestors(bf, PersistenceExceptionTranslator.class, false, false);
