...
/**
 * Abstract base implementation of the JCA 1.7 {@link javax.resource.spi.endpoint.MessageEndpointFactory} interface,
 * providing transaction management capabilities as well as ClassLoader exposure for endpoint invocations.
 *
...
public abstract class AbstractMessageEndpointFactory implements MessageEndpointFactory, BeanNameAware {
    ...
    /**
     * Set the XA transaction manager to use for wrapping endpoint invocations, enlisting the endpoint resource in each
     * such transaction.
     * <p>
     * The passed-in object may be a transaction manager which implements Spring's
     * {@link org.springframework.transaction.jta.TransactionFactory} interface, or a plain
     * {@link javax.transaction.TransactionManager}.
     * <p>
     * If no transaction manager is specified, the endpoint invocation will simply not be wrapped in an XA transaction.
     * Check out your resource provider's ActivationSpec documentation for local transaction options of your particular
     * provider.
     * 
    ...
    /**
     * Set the Spring TransactionFactory to use for wrapping endpoint invocations, enlisting the endpoint resource in
     * each such transaction.
     * <p>
     * Alternatively, specify an appropriate transaction manager through the {@link #setTransactionManager
     * "transactionManager"} property.
     * <p>
     * If no transaction factory is specified, the endpoint invocation will simply not be wrapped in an XA transaction.
     * Check out your resource provider's ActivationSpec documentation for local transaction options of your particular
     * provider.
     * 
     * @see #setTransactionName
     ...
     * <p>
     * Default is none. A specified name will be passed on to the transaction manager, allowing to identify the
     * transaction in a transaction monitor.
     */
     ...
     * <p>
     * Default is -1: rely on the transaction manager's default timeout. Specify a concrete timeout to restrict the
     * maximum duration of each endpoint invocation.
     */
    ...
    /**
     * Set the name of this message endpoint. Populated with the bean name automatically when defined within Spring's
     * bean factory.
     */
    @Override
    ...
    /**
     * Implementation of the JCA 1.7 {@code #getActivationName()} method, returning the bean name as set on this
    ...
    /**
     * Implementation of the JCA 1.7 {@code #getEndpointClass()} method, returning {@code} null in order to indicate a
     * synthetic endpoint type.
     */
    @Override
    @Nullable
    ...
    /**
     * This implementation returns {@code true} if a transaction manager has been specified; {@code false} otherwise.
     * 
     * @see #setTransactionManager
     ...
     * <p>
     * This implementation delegates to {@link #createEndpointInternal()}, initializing the endpoint's XAResource before
     * the endpoint gets invoked.
     */
    @Override
     ...
     * <p>
     * This implementation delegates to {@link #createEndpointInternal()}, ignoring the specified timeout. It is only
     * here for JCA 1.6 compliance.
     */
    @Override
    ...
    /**
     * Create the actual endpoint instance, as a subclass of the {@link AbstractMessageEndpoint} inner class of this
     * factory.
     * 
    ...
    /**
     * Inner class for actual endpoint implementations, based on template method to allow for any kind of concrete
    ...
    protected abstract class AbstractMessageEndpoint implements MessageEndpoint {
         ...
         * 
         * @param xaResource
         *            the XAResource for this endpoint
         */
        ...
        /**
         * This {@code beforeDelivery} implementation starts a transaction, if necessary, and exposes the endpoint
         * ClassLoader as current thread context ClassLoader.
         * <p>
         * Note that the JCA 1.7 specification does not require a ResourceAdapter to call this method before invoking
         * the concrete endpoint. If this method has not been called (check {@link #hasBeforeDeliveryBeenCalled()}), the
         * concrete endpoint method should call {@code beforeDelivery} and its sibling {@link #afterDelivery()}
        ...
        /**
         * Template method for exposing the endpoint's ClassLoader (typically the ClassLoader that the message listener
         * class has been loaded with).
         * 
        ...
        /**
         * Callback method for notifying the endpoint base class that the concrete endpoint invocation led to an
         * exception.
         * <p>
        ...
        /**
         * This {@code afterDelivery} implementation resets the thread context ClassLoader and completes the
         * transaction, if any.
         * <p>
         * Note that the JCA 1.7 specification does not require a ResourceAdapter to call this method after invoking the
         * concrete endpoint. See the explanation in {@link #beforeDelivery}'s javadoc.
         */
        @Override
    ...
    /**
     * Private inner class that performs the actual transaction handling, including enlistment of the endpoint's
     * XAResource.
     */
