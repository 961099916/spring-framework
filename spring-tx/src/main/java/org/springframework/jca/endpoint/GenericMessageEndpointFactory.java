...
/**
 * Generic implementation of the JCA 1.7 {@link javax.resource.spi.endpoint.MessageEndpointFactory} interface, providing
 * transaction management capabilities for any kind of message listener object (e.g. {@link javax.jms.MessageListener}
 * objects or {@link javax.resource.cci.MessageListener} objects.
 *
 * <p>
 * Uses AOP proxies for concrete endpoint instances, simply wrapping the specified message listener object and exposing
 * all of its implemented interfaces on the endpoint instance.
 *
 * <p>
 * Typically used with Spring's {@link GenericMessageEndpointManager}, but not tied to it. As a consequence, this
 * endpoint factory could also be used with programmatic endpoint management on a native
...
public class GenericMessageEndpointFactory extends AbstractMessageEndpointFactory {
    ...
    /**
     * Specify the message listener object that the endpoint should expose (e.g. a {@link javax.jms.MessageListener}
     * objects or {@link javax.resource.cci.MessageListener} implementation).
     */
    ...
    /**
     * Wrap each concrete endpoint instance with an AOP proxy, exposing the message listener's interfaces as well as the
     ...
     * <p>
     * Will only be used if the ResourceAdapter does not invoke the endpoint's {@code beforeDelivery} and
     * {@code afterDelivery} directly, leaving it up to the concrete endpoint to apply those - and to handle any
     * ResourceExceptions thrown from them.
     */
    ...
    /**
     * Private inner class that implements the concrete generic message endpoint, as an AOP Alliance MethodInterceptor
     * that will be invoked by a proxy.
     */
