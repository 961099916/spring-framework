...
/**
 * <b>This is the central class in the CCI core package.</b> It simplifies the use of CCI and helps to avoid common
 * errors. It executes core CCI workflow, leaving application code to provide parameters to CCI and extract results.
 * This class executes EIS queries or updates, catching ResourceExceptions and translating them to the generic exception
 * hierarchy defined in the {@code org.springframework.dao} package.
 *
 * <p>
 * Code using this class can pass in and receive {@link javax.resource.cci.Record} instances, or alternatively implement
 * callback interfaces for creating input Records and extracting result objects from output Records (or CCI ResultSets).
 *
 * <p>
 * Can be used within a service implementation via direct instantiation with a ConnectionFactory reference, or get
...
public class CciTemplate implements CciOperations {
    ...
    /**
     * Construct a new CciTemplate, given a ConnectionFactory to obtain Connections from. Note: This will trigger eager
     * initialization of the exception translator.
     * 
     ...
     *            the CCI ConnectionSpec to obtain Connections for (may be {@code null})
     */
     ...
     * <p>
     * Default is none: When no explicit output Record gets passed into an {@code execute} method, CCI's
     * {@code Interaction.execute} variant that returns an output Record will be called.
     * <p>
     * Specify a RecordCreator here if you always need to call CCI's {@code Interaction.execute} variant with a
     * passed-in output Record. Unless there is an explicitly specified output Record, CciTemplate will then invoke this
    ...
    /**
     * Create a template derived from this template instance, inheriting the ConnectionFactory and other settings but
     * overriding the ConnectionSpec used for obtaining Connections.
     * 
     * @param connectionSpec
     *            the CCI ConnectionSpec that the derived template instance is supposed to obtain Connections for
     ...
     * 
     * @param spec
     *            the CCI InteractionSpec instance that defines the interaction (connector-specific)
     * @param inputRecord
     *            the input record
     * @param outputRecord
     *            output record (can be {@code null})
     * @param outputExtractor
     ...
     * @return the Record
     * @throws DataAccessException
     *             if creation of the Record failed
     ...
     * <p>
     * Default implementation returns the connector's RecordFactory if available, falling back to a
     * NotSupportedRecordFactory placeholder. This allows to invoke a RecordCreator callback with a non-null
    ...
    /**
     * Close the given CCI Interaction and ignore any thrown exception. This is useful for typical finally blocks in
     * manual CCI code.
     * 
     * @param interaction
    ...
    /**
     * Close the given CCI ResultSet and ignore any thrown exception. This is useful for typical finally blocks in
     * manual CCI code.
     * 
     * @param resultSet
     *            the CCI ResultSet to close
