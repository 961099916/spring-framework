...
/**
 * {@link org.springframework.transaction.PlatformTransactionManager} implementation that manages local transactions for
 * a single CCI ConnectionFactory. Binds a CCI Connection from the specified ConnectionFactory to the thread,
 ...
 * {@link ConnectionFactoryUtils#getConnection(ConnectionFactory)} instead of a standard Java EE-style
 * {@link ConnectionFactory#getConnection()} call. Spring classes such as
 * {@link org.springframework.jca.cci.core.CciTemplate} use this strategy implicitly. If not used in combination with
 * this transaction manager, the {@link ConnectionFactoryUtils} lookup strategy behaves exactly like the native
 * DataSource lookup; it can thus be used in a portable fashion.
 *
 * <p>
 * Alternatively, you can allow application code to work with the standard Java EE lookup pattern
 * {@link ConnectionFactory#getConnection()}, for example for legacy code that is not aware of Spring at all. In that
 * case, define a {@link TransactionAwareConnectionFactoryProxy} for your target ConnectionFactory, which will
 * automatically participate in Spring-managed transactions.
 *
