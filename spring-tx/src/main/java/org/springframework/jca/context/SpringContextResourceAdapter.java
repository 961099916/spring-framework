 ...
 * {@link org.springframework.context.ApplicationContext}, starting and stopping Spring-managed beans as part of the
 * ResourceAdapter's lifecycle.
 *
 * <p>
 * Ideal for application contexts that do not need any HTTP entry points but rather just consist of message endpoints
 * and scheduled jobs etc. Beans in such a context may use application server resources such as the JTA transaction
 * manager and JNDI-bound JDBC DataSources and JMS ConnectionFactory instances, and may also register with the
 * platform's JMX server - all through Spring's standard transaction management and JNDI and JMX support facilities.
 *
 * <p>
 * If the need for scheduling asynchronous work arises, consider using Spring's
 * {@link org.springframework.jca.work.WorkManagerTaskExecutor} as a standard bean definition, to be injected into
 * application beans through dependency injection. This WorkManagerTaskExecutor will automatically use the JCA
 * WorkManager from the BootstrapContext that has been provided to this ResourceAdapter.
 *
 * <p>
 * The JCA {@link javax.resource.spi.BootstrapContext} may also be accessed directly, through application components
 * that implement the {@link BootstrapContextAware} interface. When deployed using this ResourceAdapter, the
 * BootstrapContext is guaranteed to be passed on to such components.
 *
 * <p>
 * This ResourceAdapter is to be defined in a "META-INF/ra.xml" file within a Java EE ".rar" deployment unit like as
 * follows:
 *
 * <pre class="code">
 ...
 * </pre>
 *
 * Note that "META-INF/applicationContext.xml" is the default context config location, so it doesn't have to specified
 * unless you intend to specify different/additional config files. So in the default case, you may remove the entire
 * {@code config-property} section above.
 *
 * <p>
 * <b>For simple deployment needs, all you need to do is the following:</b> Package all application classes into a RAR
 * file (which is just a standard JAR file with a different file extension), add all required library jars into the root
 * of the RAR archive, add a "META-INF/ra.xml" deployment descriptor as shown above as well as the corresponding Spring
 * XML bean definition file(s) (typically "META-INF/applicationContext.xml"), and drop the resulting RAR file into your
...
public class SpringContextResourceAdapter implements ResourceAdapter {
    ...
    /**
     * Set the location of the context configuration files, within the resource adapter's deployment unit. This can be a
     * delimited String that consists of multiple resource location, separated by commas, semicolons, whitespace, or
     * line breaks.
     * <p>
    ...
    /**
     * This implementation loads a Spring ApplicationContext through the {@link #createApplicationContext} template
     * method.
     */
    @Override
     ...
     * <p>
     * The default implementation builds a {@link ResourceAdapterApplicationContext} and delegates to
     * {@link #loadBeanDefinitions} for actually parsing the specified configuration files.
     * 
     * @param bootstrapContext
     *            this ResourceAdapter's BootstrapContext
    ...
    public boolean equals(@Nullable Object other) {
        return (this == other
            || (other instanceof SpringContextResourceAdapter && ObjectUtils.nullSafeEquals(getContextConfigLocation(),
