...
/**
 * Mock object based tests for transaction aspects. A true unit test in that it tests how the transaction aspect uses
...
public abstract class AbstractTransactionAspectTests {
    ...
    /**
     * Check that the given exception thrown by the target can produce the desired behavior with the appropriate
    ...
    /**
     * Simulate failure of the underlying transaction infrastructure to commit. Check that the target method was
     * invoked, but that the transaction infrastructure exception was thrown to the client
     */
    @Test
    ...
    /**
     * Subclasses must implement this to create an advised object based on the given target. In the case of AspectJ, the
     * advised object will already have been created, as there's no distinction between target and proxy. In the case of
     * Spring's own AOP framework, a proxy must be created using a suitably configured transaction interceptor
     * 
     * @param target
     *            the target if there's a distinct target. If not (AspectJ), return target.
