...
public abstract class SchedulerAccessor implements ResourceLoaderAware {
    ...
    /**
     * Set whether any jobs defined on this SchedulerFactoryBean should overwrite existing job definitions. Default is
     * "false", to not overwrite already registered jobs that have been read in from a persistent job store.
     */
     ...
     * Can be specified to automatically register jobs that are defined in such a file, possibly in addition to jobs
    ...
    /**
     * Set the locations of Quartz job definition XML files that follow the "job_scheduling_data_1_5" XSD or better. Can
     * be specified to automatically register jobs that are defined in such files, possibly in addition to jobs defined
    ...
    /**
     * Register a list of JobDetail objects with the Scheduler that this FactoryBean creates, to be referenced by
     * Triggers.
     * <p>
     * This is not necessary when a Trigger determines the JobDetail itself: In this case, the JobDetail will be
     * implicitly registered in combination with the Trigger.
     * 
     * @see #setTriggers
    ...
    /**
     * Register a list of Quartz Calendar objects with the Scheduler that this FactoryBean creates, to be referenced by
     * Triggers.
     * 
     * @param calendars
     *            a Map with calendar names as keys as Calendar objects as values
     ...
     * <p>
     * If the Trigger determines the corresponding JobDetail itself, the job will be automatically registered with the
     * Scheduler. Else, the respective JobDetail needs to be registered via the "jobDetails" property of this
     * FactoryBean.
     * 
     * @see #setJobDetails
     * @see org.quartz.JobDetail
    ...
    /**
     * Specify global Quartz JobListeners to be registered with the Scheduler. Such JobListeners will apply to all Jobs
     * in the Scheduler.
     */
    ...
    /**
     * Specify global Quartz TriggerListeners to be registered with the Scheduler. Such TriggerListeners will apply to
     * all Triggers in the Scheduler.
     */
    ...
    /**
     * Set the transaction manager to be used for registering jobs and triggers that are defined by this
     * SchedulerFactoryBean. Default is none; setting this only makes sense when specifying a DataSource for the
     * Scheduler.
     */
    ...
    /**
     * Add the given job to the Scheduler, if it doesn't already exist. Overwrites the job in any case if
     * "overwriteExistingJobs" is set.
     * 
     * @param jobDetail
     *            the job to add
    ...
    /**
     * Add the given trigger to the Scheduler, if it doesn't already exist. Overwrites the trigger in any case if
     * "overwriteExistingJobs" is set.
     * 
     * @param trigger
     *            the trigger to add
