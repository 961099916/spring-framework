...
/**
 * A Spring {@link FactoryBean} for creating a Quartz {@link org.quartz.SimpleTrigger} instance, supporting bean-style
 * usage for trigger configuration.
 *
 * <p>
 * {@code SimpleTrigger(Impl)} itself is already a JavaBean but lacks sensible defaults. This class uses the Spring bean
 * name as job name, the Quartz default group ("DEFAULT") as job group, the current time as start time, and indefinite
 * repetition, if not specified.
 *
 * <p>
 * This class will also register the trigger with the job name and group of a given {@link org.quartz.JobDetail}. This
 * allows {@link SchedulerFactoryBean} to automatically register a trigger for the corresponding JobDetail, instead of
 * registering the JobDetail separately.
 *
...
public class SimpleTriggerFactoryBean implements FactoryBean<SimpleTrigger>, BeanNameAware, InitializingBean {
     ...
     * <p>
     * Note that a dynamically computed {@link #setStartDelay} specification overrides a static timestamp set here.
     */
     ...
     * <p>
     * The start delay is added to the current system time (when the bean starts) to control the start time of the
     * trigger.
     * 
     * @see #setStartTime
     */
    ...
    /**
     * Set the misfire instruction via the name of the corresponding constant in the {@link org.quartz.SimpleTrigger}
     * class. Default is {@code MISFIRE_INSTRUCTION_SMART_POLICY}.
     * 
