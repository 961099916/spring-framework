...
/**
 * Base class for classes that are accessing a CommonJ {@link commonj.timers.TimerManager} Defines common configuration
...
public abstract class TimerManagerAccessor extends JndiLocatorSupport
    ...
    /**
     * Specify whether the TimerManager obtained by this FactoryBean is a shared instance ("true") or an independent
     * instance ("false"). The lifecycle of the former is supposed to be managed by the application server, while the
     * lifecycle of the latter is up to the application.
     * <p>
     * Default is "false", i.e. managing an independent TimerManager instance. This is what the CommonJ specification
     * suggests that application servers are supposed to offer via JNDI lookups, typically declared as a
     * {@code resource-ref} of type {@code commonj.timers.TimerManager} in {@code web.xml}, with
     * {@code res-sharing-scope} set to 'Unshareable'.
     * <p>
     * Switch this flag to "true" if you are obtaining a shared TimerManager, typically through specifying the JNDI
     * location of a TimerManager that has been explicitly declared as 'Shareable'. Note that WebLogic's cluster-aware
     * Job Scheduler is a shared TimerManager too.
     * <p>
     * The sole difference between this FactoryBean being in shared or non-shared mode is that it will only attempt to
     * suspend / resume / stop the underlying TimerManager in case of an independent (non-shared) instance. This only
     * affects the {@link org.springframework.context.Lifecycle} support as well as application context shutdown.
     *
     * @see #stop()
     * @see #start()
     * @see #destroy()
     ...
     *             in case of no TimerManager set
     * @since 5.0
     */
