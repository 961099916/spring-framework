...
/**
 * {@link FactoryBean} that creates a named EhCache {@link net.sf.ehcache.Cache} instance (or a decorator that
 * implements the {@link net.sf.ehcache.Ehcache} interface), representing a cache region within an EhCache
 * {@link net.sf.ehcache.CacheManager}.
 *
 * <p>
 * If the specified named cache is not configured in the cache configuration descriptor, this FactoryBean will construct
 * an instance of a Cache with the provided name and the specified cache properties and add it to the CacheManager for
 * later retrieval. If some or all properties are not set at configuration time, this FactoryBean will use defaults.
 *
 * <p>
 * Note: If the named Cache instance is found, the properties will be ignored and the Cache instance will be retrieved
...
public class EhCacheFactoryBean extends CacheConfiguration
    ...
    /**
     * Set a CacheManager from which to retrieve a named Cache instance. By default, {@code CacheManager.getInstance()}
     * will be called.
     * <p>
     * Note that in particular for persistent caches, it is advisable to properly handle the shutdown of the
     * CacheManager: Set up a separate EhCacheManagerFactoryBean and pass a reference to this bean property.
     * <p>
     * A separate EhCacheManagerFactoryBean is also necessary for loading EhCache configuration from a non-default
     * config location.
     * 
     * @see EhCacheManagerFactoryBean
     ...
     * <p>
     * If you intend to build a self-populating blocking cache, consider specifying a {@link #setCacheEntryFactory
     * CacheEntryFactory}.
     * 
    ...
    /**
     * Set an EhCache {@link net.sf.ehcache.constructs.blocking.CacheEntryFactory} to use for a self-populating cache.
     * If such a factory is specified, the cache will be decorated with EhCache's
     * {@link net.sf.ehcache.constructs.blocking.SelfPopulatingCache}.
     * <p>
     * The specified factory can be of type {@link net.sf.ehcache.constructs.blocking.UpdatingCacheEntryFactory}, which
     * will lead to the use of an {@link net.sf.ehcache.constructs.blocking.UpdatingSelfPopulatingCache}.
     * <p>
    ...
    /**
     * Specify EhCache {@link net.sf.ehcache.event.CacheEventListener cache event listeners} to registered with this
     * cache.
     */
    ...
    /**
     * Predict the particular {@code Ehcache} implementation that will be returned from {@link #getObject()} based on
     * logic in {@link #createCache()} and {@link #decorateCache(Ehcache)} as orchestrated by
