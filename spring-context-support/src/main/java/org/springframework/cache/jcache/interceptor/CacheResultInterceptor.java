...
class CacheResultInterceptor extends AbstractKeyCacheInterceptor<CacheResultOperation, CacheResult> {
    ...
    /**
     * Rewrite the call stack of the specified {@code exception} so that it matches the current call stack up to
     * (included) the specified method invocation.
     * <p>
     * Clone the specified exception. If the exception is not {@code serializable}, the original exception is returned.
     * If no common ancestor can be found, returns the original exception.
     * <p>
     * Used to make sure that a cached exception has a valid invocation context.
     *
     * @param exception
     *            the exception to merge with the current call stack
     * @param className
     *            the class name of the common ancestor
     * @param methodName
     *            the method name of the common ancestor
     * @return a clone exception with a rewritten call stack composed of the current call stack up to (included) the
     *         common ancestor specified by the {@code className} and {@code methodName} arguments, followed by stack
     *         trace elements of the specified {@code exception} after the common ancestor.
     */
