...
/**
 * Production implementation of the {@link JavaMailSender} interface, supporting both JavaMail {@link MimeMessage
 * MimeMessages} and Spring {@link SimpleMailMessage SimpleMailMessages}. Can also be used as a plain
 * {@link org.springframework.mail.MailSender} implementation.
 *
 * <p>
 * Allows for defining all settings locally as bean properties. Alternatively, a pre-configured JavaMail
 * {@link javax.mail.Session} can be specified, possibly pulled from an application server's JNDI environment.
 *
 * <p>
 * Non-default properties in this object will always override the settings in the JavaMail {@code Session}. Note that if
 * overriding all values locally, there is no added value in setting a pre-configured {@code Session}.
 *
...
public class JavaMailSenderImpl implements JavaMailSender {
     ...
     * <p>
     * Initializes the {@link #setDefaultFileTypeMap "defaultFileTypeMap"} property with a default
     ...
     * <p>
     * A new {@code Session} will be created with those properties. Use either this method or {@link #setSession}, but
     * not both.
     * <p>
     ...
     * <p>
     * Note that the underlying JavaMail {@code Session} has to be configured with the property {@code "mail.smtp.auth"}
     * set to {@code true}, else the specified username will not be sent to the mail server by the JavaMail runtime. If
     * you are not explicitly passing in a {@code Session} to use, simply specify this setting via
     ...
     * <p>
     * Note that the underlying JavaMail {@code Session} has to be configured with the property {@code "mail.smtp.auth"}
     * set to {@code true}, else the specified password will not be sent to the mail server by the JavaMail runtime. If
     * you are not explicitly passing in a {@code Session} to use, simply specify this setting via
     * {@link #setJavaMailProperties}.
     *
     * @see #setSession
    ...
    /**
     * Return the default Java Activation {@link FileTypeMap} for {@link MimeMessage MimeMessages}, or {@code null} if
     * none.
     */
    @Nullable
     ...
     * This special defaults-carrying message will be autodetected by {@link MimeMessageHelper}, which will use the
     ...
     *            corresponding original message objects that the MimeMessages have been created from (with same array
    ...
    /**
     * Obtain and connect a Transport from the underlying JavaMail Session, passing in the specified host, port,
