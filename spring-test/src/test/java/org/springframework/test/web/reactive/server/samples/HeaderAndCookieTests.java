...
public class HeaderAndCookieTests {
    ...
    public void requestResponseHeaderPair() throws Exception {
        this.client.get().uri("/header-echo").header("h1", "in").exchange().expectStatus().isOk().expectHeader()
            .valueEquals("h1", "in-out");
    }

    @Test
    public void headerMultipleValues() throws Exception {
        this.client.get().uri("/header-multi-value").exchange().expectStatus().isOk().expectHeader().valueEquals("h1",
            "v1", "v2", "v3");
    }

    @Test
    public void setCookies() {
        this.client.get().uri("/cookie-echo").cookies(cookies -> cookies.add("k1", "v1")).exchange().expectHeader()
