...
public class RequestContextHolderTests {
    ...
    public void requestScopedController() throws Exception {
        assertThat(AopUtils.isCglibProxy(this.requestScopedController))
            .as("request-scoped controller must be a CGLIB proxy").isTrue();
    ...
    public void requestScopedService() throws Exception {
        assertThat(AopUtils.isCglibProxy(this.requestScopedService)).as("request-scoped service must be a CGLIB proxy")
            .isTrue();
    ...
    public void sessionScopedService() throws Exception {
        assertThat(AopUtils.isCglibProxy(this.sessionScopedService)).as("session-scoped service must be a CGLIB proxy")
            .isTrue();
    ...
    static class RequestAttributesFilter extends GenericFilterBean {
        ...
        public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
            throws IOException, ServletException {
            RequestContextHolder.getRequestAttributes().setAttribute(FROM_REQUEST_ATTRIBUTES_FILTER,
                FROM_REQUEST_ATTRIBUTES_FILTER, RequestAttributes.SCOPE_REQUEST);
            chain.doFilter(request, response);
        }
    }
