...
public class MockMvcResultMatchersTests {
    ...
    public void redirectWithUrlTemplate() throws Exception {
        assertThatCode(() -> redirectedUrlTemplate("/orders/{orderId}/items/{itemId}", 1, 2)
            .match(redirectedUrlStub("/orders/1/items/2"))).doesNotThrowAnyException();
    }
    ...
    public void forwardWithQueryString() throws Exception {
        assertThatCode(
            () -> forwardedUrl("/api/resource/1?arg=value").match(forwardedUrlStub("/api/resource/1?arg=value")))
                .doesNotThrowAnyException();
    }

    @Test
    public void forwardWithUrlTemplate() throws Exception {
        assertThatCode(() -> forwardedUrlTemplate("/orders/{orderId}/items/{itemId}", 1, 2)
            .match(forwardedUrlStub("/orders/1/items/2"))).doesNotThrowAnyException();
    }
