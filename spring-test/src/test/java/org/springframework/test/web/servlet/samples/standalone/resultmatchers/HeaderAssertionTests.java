...
public class HeaderAssertionTests {
    ...
    public void stringWithMissingResponseHeader() throws Exception {
        this.mockMvc.perform(get("/persons/1").header(IF_MODIFIED_SINCE, now)).andExpect(status().isNotModified())
    ...
    public void existsFail() throws Exception {
        assertThatExceptionOfType(AssertionError.class)
            .isThrownBy(() -> this.mockMvc.perform(get("/persons/1")).andExpect(header().exists("X-Custom-Header")));
    }
    ...
    public void doesNotExistFail() throws Exception {
        assertThatExceptionOfType(AssertionError.class)
            .isThrownBy(() -> this.mockMvc.perform(get("/persons/1")).andExpect(header().doesNotExist(LAST_MODIFIED)));
    }
    ...
    private void assertMessageContains(AssertionError error, String expected) {
        assertThat(error.getMessage().contains(expected))
            .as("Failure message should contain [" + expected + "], actual is [" + error.getMessage() + "]").isTrue();
    }
    ...
    private static class PersonController {
        ...
        public ResponseEntity<Person> showEntity(@PathVariable long id, WebRequest request) {
            return ResponseEntity.ok().lastModified(this.timestamp).header("X-Rate-Limiting", "42")
                .header("Vary", "foo", "bar").body(new Person("Jason"));
        }
    }
