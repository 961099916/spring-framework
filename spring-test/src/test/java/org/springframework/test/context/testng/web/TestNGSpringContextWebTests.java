...
/**
 * TestNG-based integration tests that verify support for loading a {@link WebApplicationContext} when extending
...
public class TestNGSpringContextWebTests extends AbstractTestNGSpringContextTests implements ServletContextAware {
    ...
    void basicWacFeatures() throws Exception {
        ...
        assertThat(wac.getServletContext()).as("ServletContext instances must be the same object.")
            .isSameAs(mockServletContext);
        assertThat(request.getServletContext()).as("ServletContext in the WAC and in the mock request")
            .isSameAs(mockServletContext);
