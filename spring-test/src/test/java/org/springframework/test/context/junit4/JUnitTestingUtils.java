...
public class JUnitTestingUtils {
    ...
    /**
     * Run the tests in the supplied {@code testClass}, using the {@link Runner} configured via {@link RunWith @RunWith}
     * or the default JUnit runner, and assert the expectations of the test execution.
     *
     * @param testClass
     *            the test class to run with JUnit
     * @param expectedStartedCount
     *            the expected number of tests that started
     * @param expectedFailedCount
     *            the expected number of tests that failed
     * @param expectedFinishedCount
     *            the expected number of tests that finished
     * @param expectedIgnoredCount
     *            the expected number of tests that were ignored
     * @param expectedAssumptionFailedCount
     *            the expected number of tests that resulted in a failed assumption
     */
    ...
    /**
     * Run the tests in the supplied {@code testClass}, using the specified {@link Runner}, and assert the expectations
     * of the test execution.
     *
     * <p>
     * If the specified {@code runnerClass} is {@code null}, the tests will be run with the runner that the test class
     * is configured with (i.e., via {@link RunWith @RunWith}) or the default JUnit runner.
     *
     * @param runnerClass
     *            the explicit runner class to use or {@code null} if the default JUnit runner should be used
     * @param testClass
     *            the test class to run with JUnit
     * @param expectedStartedCount
     *            the expected number of tests that started
    ...
    /**
     * Run all tests in the supplied test classes according to the policies of the supplied {@link Computer}, using the
     * {@link Runner} configured via {@link RunWith @RunWith} or the default JUnit runner, and assert the expectations
     * of the test execution.
     *
     * <p>
     * To have all tests executed in parallel, supply {@link ParallelComputer#methods()} as the {@code Computer}. To
     * have all tests executed serially, supply {@link Computer#serial()} as the {@code Computer}.
     *
     * @param computer
     *            the JUnit {@code Computer} to use
     * @param expectedStartedCount
     *            the expected number of tests that started
     * @param expectedFailedCount
     *            the expected number of tests that failed
     ...
     *            the expected number of tests that resulted in a failed assumption
     * @param testClasses
     *            one or more test classes to run
     */
    public static void runTestsAndAssertCounters(Computer computer, int expectedStartedCount, int expectedFailedCount,
        int expectedFinishedCount, int expectedIgnoredCount, int expectedAssumptionFailedCount, Class<?>... testClasses)
        throws Exception {
        ...
        assertSoftly(softly -> {
            ...
            softly.assertThat(listener.getTestAssumptionFailureCount()).as("failed assumptions]")
                .isEqualTo(expectedAssumptionFailedCount);
        });
    }
