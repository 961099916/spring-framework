...
/**
 * Integration test which verifies proper transactional behavior when the default rollback flag is explicitly set to
 * {@code true} via {@link Rollback @Rollback}.
 *
 * <p>
 * Also tests configuration of the transaction manager qualifier configured via {@link Transactional @Transactional}.
 *
 * @author Sam Brannen
 * @since 4.2
 * @see Rollback
...
public class DefaultRollbackTrueRollbackAnnotationTransactionalTests extends AbstractTransactionalSpringRunnerTests {
    ...
    public static void verifyFinalTestData() {
        assertThat(countRowsInPersonTable(jdbcTemplate))
            .as("Verifying the final number of rows in the person table after all tests.").isEqualTo(originalNumRows);
    }
    ...
    public void modifyTestDataWithinTransaction() {
        ...
        assertThat(countRowsInPersonTable(jdbcTemplate))
            .as("Verifying the number of rows in the person table within a transaction.").isEqualTo(3);
    }
