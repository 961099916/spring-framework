...
/**
 * JUnit based integration test which verifies correct {@linkplain ContextCache application context caching} in
 * conjunction with Spring's TestNG support and {@link DirtiesContext @DirtiesContext} at the class level.
 *
 * <p>
 * This class is a direct copy of {@link ClassLevelDirtiesContextTests}, modified to verify behavior in conjunction with
 * TestNG.
 *
 * @author Sam Brannen
 * @since 4.2
...
class ClassLevelDirtiesContextTestNGTests {
    ...
    void verifyDirtiesContextBehavior() throws Exception {
        ...
        assertContextCacheStatistics("after class-level @DirtiesContext with clean test method and AFTER_CLASS mode", 0,
            cacheHits.incrementAndGet(), cacheMisses.get());
        assertBehaviorForCleanTestCase();
    ...
    private void runTestClassAndAssertStats(Class<?> testClass, int expectedTestCount) {
        ...
        assertThat(listener.testFailureCount).as("Failures for test class [" + testClass + "].")
            .isEqualTo(expectedTestFailureCount);
        assertThat(listener.testStartCount).as("Tests started for test class [" + testClass + "].")
            .isEqualTo(expectedTestStartedCount);
        assertThat(listener.testSuccessCount).as("Successful tests for test class [" + testClass + "].")
