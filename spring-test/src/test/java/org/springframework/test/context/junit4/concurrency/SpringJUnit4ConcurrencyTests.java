...
/**
 * Concurrency tests for the {@link SpringRunner}, {@link SpringClassRule}, and {@link SpringMethodRule} that use JUnit
 * 4's experimental {@link ParallelComputer} to execute tests in parallel.
 *
 * <p>
 * The tests executed by this test class come from a hand-picked collection of test classes within the test suite that
 * is intended to cover most categories of tests that are currently supported by the TestContext Framework on JUnit 4.
 *
 * <p>
 * The chosen test classes intentionally do <em>not</em> include any classes that fall under the following categories.
 *
 * <ul>
 ...
 *
 * <p>
 * <strong>NOTE</strong>: these tests only run if the {@link TestGroup#LONG_RUNNING LONG_RUNNING} test group is enabled.
 *
 * @author Sam Brannen
 * @since 5.0
...
public class SpringJUnit4ConcurrencyTests {
    ...
    private final Class<?>[] testClasses = new Class<?>[] {
        // Basics
        SpringJUnit4ClassRunnerAppCtxTests.class, InheritedConfigSpringJUnit4ClassRunnerAppCtxTests.class,
        SpringJUnit47ClassRunnerRuleTests.class, BaseAppCtxRuleTests.class,
        // Transactional
        MethodLevelTransactionalSpringRunnerTests.class, TimedTransactionalSpringRunnerTests.class,
        // Web and Scopes
    ...
    private int countAnnotatedMethods(Class<? extends Annotation> annotationType) {
        return (int)Arrays.stream(this.testClasses).map(ReflectionUtils::getUniqueDeclaredMethods)
            .flatMap(Arrays::stream).filter(method -> hasAnnotation(method, annotationType)).count();
    }
