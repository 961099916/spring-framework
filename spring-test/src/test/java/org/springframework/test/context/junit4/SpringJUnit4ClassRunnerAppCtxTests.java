...
/**
 * SpringJUnit4ClassRunnerAppCtxTests serves as a <em>proof of concept</em> JUnit 4 based test class, which verifies the
 * expected functionality of {@link SpringRunner} in conjunction with the following:
 *
 * <ul>
 ...
 *
 * <p>
 * Since no application context resource {@link ContextConfiguration#locations() locations} are explicitly declared and
 * since the {@link ContextConfiguration#loader() ContextLoader} is left set to the default value of
 * {@link GenericXmlContextLoader}, this test class's dependencies will be injected via {@link Autowired @Autowired},
 * {@link Inject @Inject}, and {@link Resource @Resource} from beans defined in the {@link ApplicationContext} loaded
 * from the default classpath resource: {@value #DEFAULT_CONTEXT_RESOURCE_PATH}.
 *
 * @author Sam Brannen
 * @since 2.5
...
public class SpringJUnit4ClassRunnerAppCtxTests implements ApplicationContextAware, BeanNameAware, InitializingBean {
    ...
    /**
     * Default resource path for the application context configuration for {@link SpringJUnit4ClassRunnerAppCtxTests}:
    ...
    public void verifyBeanNameSet() {
        assertThat(this.beanName.startsWith(getClass().getName()))
            .as("The bean name of this test instance should have been set due to BeanNameAware semantics.").isTrue();
    }

    @Test
    public void verifyApplicationContextSet() {
        assertThat(this.applicationContext)
            .as("The application context should have been set due to ApplicationContextAware semantics.").isNotNull();
    }

    @Test
    public void verifyBeanInitialized() {
        assertThat(this.beanInitialized)
            .as("This test bean should have been initialized due to InitializingBean semantics.").isTrue();
    }
    ...
    public void verifyAnnotationAutowiredAndInjectedFields() {
        ...
        assertThat(this.quux).as("The quux field should have been autowired via @Autowired and @Qualifier.")
            .isEqualTo("Quux");
        assertThat(this.namedQuux).as("The namedFoo field should have been injected via @Inject and @Named.")
            .isEqualTo("Quux");
        assertThat(this.namedQuux).as("@Autowired/@Qualifier and @Inject/@Named quux should be the same object.")
            .isSameAs(this.quux);
        ...
        assertThat(this.injectedPet).as("@Autowired and @Inject pet should be the same object.")
    ...
    public void verifyAutowiredAtValueMethods() {
        assertThat(this.literalParameterValue).as("Literal @Value method parameter should have been autowired.")
            .isNotNull();
