...
/**
 * Integration tests which verify that '<i>before</i>' and '<i>after</i>' methods of {@link TestExecutionListener
 * TestExecutionListeners} as well as {@code @BeforeTransaction} and {@code @AfterTransaction} methods can fail tests
 * run via the {@link SpringExtension} in a JUnit Jupiter environment.
 *
 * <p>
 * See: <a href="https://jira.spring.io/browse/SPR-3960" target="_blank">SPR-3960</a> and
 * <a href="https://jira.spring.io/browse/SPR-4365" target="_blank">SPR-4365</a>.
 *
 * <p>
 * Indirectly, this class also verifies that all {@code TestExecutionListener} lifecycle callbacks are called.
 *
 * <p>
 * To run these tests in an IDE that does not have built-in support for the JUnit Platform, simply run
 * {@link SpringJUnitJupiterTestSuite} as a JUnit 4 test.
 *
 * @author Sam Brannen
 * @since 5.0
 */
class FailingBeforeAndAfterMethodsSpringExtensionTests {
    ...
    @ParameterizedTest
    @ValueSource(classes = {AlwaysFailingBeforeTestClassTestCase.class, AlwaysFailingAfterTestClassTestCase.class,
        AlwaysFailingPrepareTestInstanceTestCase.class, AlwaysFailingBeforeTestMethodTestCase.class,
        AlwaysFailingBeforeTestExecutionTestCase.class, AlwaysFailingAfterTestExecutionTestCase.class,
        AlwaysFailingAfterTestMethodTestCase.class, FailingBeforeTransactionTestCase.class,
        FailingAfterTransactionTestCase.class})
    void failingBeforeAndAfterCallbacks(Class<?> testClass) {
        Events events = EngineTestKit.engine("junit-jupiter").selectors(selectClass(testClass)).execute().testEvents()
            .assertStatistics(stats -> stats.skipped(0).aborted(0).started(getExpectedStartedCount(testClass))
                .succeeded(getExpectedSucceededCount(testClass)).failed(getExpectedFailedCount(testClass)));
        ...
        if (getExpectedFailedCount(testClass) > 0) {
            events.assertThatEvents().haveExactly(1, event(test("testNothing"),
                finishedWithFailure(instanceOf(AssertionError.class), message(msg -> msg.contains("always failing")))));
        }
    }
