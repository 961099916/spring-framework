...
/**
 * Integration tests which demonstrate how to set up a {@link MockMvc} instance in an {@link BeforeEach @BeforeEach}
 * method with the {@link SpringExtension} (registered via a custom {@link SpringJUnitWebConfig @SpringJUnitWebConfig}
 * composed annotation).
 *
 * <p>
...
class MultipleWebRequestsSpringExtensionTests {
    ...
    void setUpMockMvc(WebApplicationContext wac) {
        this.mockMvc = webAppContextSetup(wac).alwaysExpect(status().isOk())
            .alwaysExpect(content().contentTypeCompatibleWith(APPLICATION_JSON)).build();
    }
