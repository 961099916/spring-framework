...
/**
 * Integration tests that verify support for {@link org.springframework.context.annotation.Configuration @Configuration}
 * classes with transactional TestNG-based tests.
 *
 * <p>
 * Configuration will be loaded from {@link AnnotationConfigTransactionalTestNGSpringContextTests.ContextConfiguration}.
 *
 * @author Sam Brannen
 * @since 3.1
 */
...
public class AnnotationConfigTransactionalTestNGSpringContextTests
    ...
    private void assertNumRowsInPersonTable(int expectedNumRows, String testState) {
        assertThat(countRowsInTable("person")).as("the number of rows in the person table (" + testState + ").")
    ...
    void afterClass() {
        ...
        assertThat(numSetUpCallsInTransaction).as("number of calls to setUp() within a transaction.")
            .isEqualTo(NUM_TX_TESTS);
        assertThat(numTearDownCalls).as("number of calls to tearDown().").isEqualTo(NUM_TESTS);
        assertThat(numTearDownCallsInTransaction).as("number of calls to tearDown() within a transaction.")
            .isEqualTo(NUM_TX_TESTS);
    }
