...
/**
 * JUnit Jupiter based integration tests which verify support for Spring's {@link Transactional @Transactional}
 * annotation in conjunction with JUnit Jupiter's {@link Timeout @Timeout}.
 *
 * @author Sam Brannen
 * @since 5.2
...
class TimedTransactionalSpringExtensionTests {
    ...
    void springTransactionsWorkWithJUnitJupiterTimeouts() {
        Events events = EngineTestKit.engine("junit-jupiter").selectors(selectClass(TestCase.class)).execute()
            .testEvents().assertStatistics(stats -> stats.started(4).succeeded(2).failed(2));

        events.failed().assertThatEvents().haveExactly(2,
            event(test("WithExceededJUnitJupiterTimeout"), finishedWithFailure(instanceOf(TimeoutException.class),
