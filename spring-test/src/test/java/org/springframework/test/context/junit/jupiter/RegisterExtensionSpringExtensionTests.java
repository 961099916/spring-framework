...
/**
 * Integration tests which demonstrate that the Spring TestContext Framework can be used with JUnit Jupiter by
 * registering the {@link SpringExtension} via a static field. Note, however, that this is not the recommended way to
 * register the {@code SpringExtension}.
 *
 * <p>
...
class RegisterExtensionSpringExtensionTests {
    ...
    /**
     * NOTE: Test code must be compiled with "-g" (debug symbols) or "-parameters" in order for the parameter name to be
     * used as the qualifier; otherwise, use {@code @Qualifier("wally")}.
     */
    @Test
    ...
    void valueParameterFromSpelExpression(@Value("#{@dilbert.name}") String name) {
        assertThat(name).as("Dilbert's name should have been injected via SpEL expression in @Value by Spring")
            .isNotNull();
