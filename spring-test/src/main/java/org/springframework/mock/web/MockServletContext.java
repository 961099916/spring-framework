 ...
 * {@link #setMajorVersion}/{@link #setMinorVersion}; default is 3.1. Note that Servlet 3.1 support is limited: servlet,
 * filter and listener registration methods are not supported; neither is JSP configuration. We generally do not
 * recommend to unit test your ServletContainerInitializers and WebApplicationInitializers which is where those
 * registration methods would be used.
 *
 * <p>
 * For setting up a full {@code WebApplicationContext} in a test environment, you can use
 * {@code AnnotationConfigWebApplicationContext}, {@code XmlWebApplicationContext}, or
 * {@code GenericWebApplicationContext}, passing in a corresponding {@code MockServletContext} instance. Consider
 * configuring your {@code MockServletContext} with a {@code FileSystemResourceLoader} in order to interpret resource
 * paths as relative filesystem locations.
 *
 * @author Rod Johnson
...
public class MockServletContext implements ServletContext {
    ...
    /**
     * Create a new {@code MockServletContext}, using no base path and a {@link DefaultResourceLoader} (i.e. the
     * classpath root as WAR root).
     * 
    ...
    public Set<String> getResourcePaths(String path) {
        ...
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("Could not get resource paths for " + (resource != null ? resource : resourceLocation),
                    ex);
            }
            return null;
        }
    ...
    public URL getResource(String path) throws MalformedURLException {
        ...
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("Could not get URL for resource " + (resource != null ? resource : resourceLocation), ex);
            }
            return null;
        }
    }
    ...
    public InputStream getResourceAsStream(String path) {
        ...
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                    "Could not open InputStream for resource " + (resource != null ? resource : resourceLocation), ex);
            }
            return null;
        }
    }
    ...
    /**
     * Register a {@link RequestDispatcher} (typically a {@link MockRequestDispatcher}) that acts as a wrapper for the
     * named Servlet.
     * 
     * @param name
     ...
     * <p>
     * Also {@link #unregisterNamedDispatcher unregisters} the current default {@link RequestDispatcher} and
     * {@link #registerNamedDispatcher replaces} it with a {@link MockRequestDispatcher} for the provided
    ...
    public String getRealPath(String path) {
        ...
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                    "Could not determine real path of resource " + (resource != null ? resource : resourceLocation),
                    ex);
            }
            return null;
        }
