...
/**
 * Result handler that prints {@link MvcResult} details to a given output stream &mdash; for example:
 * {@code System.out}, {@code System.err}, a custom {@code java.io.PrintWriter}, etc.
 *
 * <p>
 * An instance of this class is typically accessed via one of the {@link MockMvcResultHandlers#print print} or
 * {@link MockMvcResultHandlers#log log} methods in {@link MockMvcResultHandlers}.
 *
...
public class PrintingResultHandler implements ResultHandler {
     ...
     * 
     * @param printer
     *            a {@link ResultValuePrinter} to do the actual writing
     */
    ...
    protected void printRequest(MockHttpServletRequest request) throws Exception {
        String body =
            (request.getCharacterEncoding() != null ? request.getContentAsString() : MISSING_CHARACTER_ENCODING);
    ...
    protected final Map<String, Object> getSessionAttributes(MockHttpServletRequest request) {
        ...
        if (session != null) {
            ...
            if (attrNames != null) {
                return Collections.list(attrNames).stream().collect(Collectors.toMap(n -> n, session::getAttribute));
            }
        }
    ...
    protected void printResponse(MockHttpServletResponse response) throws Exception {
        String body =
            (response.getCharacterEncoding() != null ? response.getContentAsString() : MISSING_CHARACTER_ENCODING);
    ...
    /**
     * Print the supplied cookies in a human-readable form, assuming the {@link Cookie} implementation does not provide
     * its own {@code toString()}.
     * 
     * @since 4.2
     */
    private void printCookies(Cookie[] cookies) {
        ...
        for (int i = 0; i < cookies.length; i++) {
            ...
            cookieStrings[i] = new ToStringCreator(cookie).append("name", cookie.getName())
                .append("value", cookie.getValue()).append("comment", cookie.getComment())
                .append("domain", cookie.getDomain()).append("maxAge", cookie.getMaxAge())
                .append("path", cookie.getPath()).append("secure", cookie.getSecure())
