...
/**
 * <strong>Main entry point for client-side REST testing</strong>. Used for tests that involve direct or indirect use of
 * the {@link RestTemplate}. Provides a way to set up expected requests that will be performed through the
 * {@code RestTemplate} as well as mock responses to send back thus removing the need for an actual server.
 *
 * <p>
 * Below is an example that assumes static imports from {@code MockRestRequestMatchers},
 * {@code MockRestResponseCreators}, and {@code ExpectedCount}:
 *
 * <pre class="code">
 ...
 *
 * <p>
 * Note that as an alternative to the above you can also set the {@link MockMvcClientHttpRequestFactory} on a
 * {@code RestTemplate} which allows executing requests against an instance of
...
public final class MockRestServiceServer {
    ...
    /**
     * Private constructor with {@code RequestExpectationManager}. See static builder methods and {@code createServer}
     * shortcut methods.
     */
     ...
     */
    @Deprecated
    public static MockRestServiceServerBuilder
        bindTo(org.springframework.web.client.AsyncRestTemplate asyncRestTemplate) {
     ...
     *
     * @param restTemplate
     *            the RestTemplate to set up for mock testing
     ...
     *            the AsyncRestTemplate to set up for mock testing
     * @return the created mock server
     * @deprecated see deprecation notice on {@link org.springframework.web.client.AsyncRestTemplate} itself
     */
    @Deprecated
    public static MockRestServiceServer
        createServer(org.springframework.web.client.AsyncRestTemplate asyncRestTemplate) {
     ...
     *
     * @param restGateway
     *            the REST gateway to set up for mock testing
     * @return the created mock server
     */
    ...
    /**
     * Set up an expectation for a single HTTP request. The returned {@link ResponseActions} can be used to set up
     * further expectations as well as to define the response.
     * <p>
     * This method may be invoked any number times before starting to make request through the underlying
     * {@code RestTemplate} in order to set up all expected requests.
     *
     * @param matcher
     *            request matcher
    ...
    /**
     * An alternative to {@link #expect(RequestMatcher)} that also indicates how many times the request is expected to
     * be executed.
     * <p>
     * When request expectations have an expected count greater than one, only the first execution is expected to match
     * the order of declaration. Subsequent request executions may be inserted anywhere thereafter.
     *
     * @param count
     *            the expected count
     * @param matcher
    ...
    public interface MockRestServiceServerBuilder {
        ...
        /**
         * Whether to allow expected requests to be executed in any order not necessarily matching the order of
         * declaration.
         * <p>
         ...
         * 
         * @param ignoreExpectOrder
         *            whether to ignore the order of expectations
         */
        MockRestServiceServerBuilder ignoreExpectOrder(boolean ignoreExpectOrder);

        /**
         * Use the {@link BufferingClientHttpRequestFactory} wrapper to buffer the input and output streams, and for
         * example, allow multiple reads of the response body.
         * 
         * @since 5.0.5
         */
    ...
    private class MockClientHttpRequestFactory
        ...
        @Override
        public org.springframework.http.client.AsyncClientHttpRequest createAsyncRequest(URI uri,
            HttpMethod httpMethod) {

            return createRequestInternal(uri, httpMethod);
        }

        private org.springframework.mock.http.client.MockAsyncClientHttpRequest createRequestInternal(URI uri,
            HttpMethod httpMethod) {
