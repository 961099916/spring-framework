...
/**
 * Default builder for {@link MockHttpServletRequest} required as input to perform requests in {@link MockMvc}.
 *
 * <p>
 * Application tests will typically access this builder through the static factory methods in
 * {@link MockMvcRequestBuilders}.
 *
 * <p>
 * This class is not open for extension. To apply custom initialization to the created {@code MockHttpServletRequest},
 * please use the {@link #with(RequestPostProcessor)} extension point.
 *
...
public class MockHttpServletRequestBuilder
    ...
    /**
     * Package private constructor. To get an instance, use static factory methods in {@link MockMvcRequestBuilders}.
     * <p>
     * Although this class cannot be extended, additional ways to initialize the {@code MockHttpServletRequest} can be
     * plugged in via {@link #with(RequestPostProcessor)}.
     * 
     * @param httpMethod
     *            the HTTP method (GET, POST, etc)
     * @param url
     *            a URL template; the resulting URL will be encoded
     * @param vars
    ...
    /**
     * Specify the portion of the requestURI that represents the context path. The context path, if specified, must
     * match to the start of the request URI.
     * <p>
     * In most cases, tests can be written by omitting the context path from the requestURI. This is because most
     * applications don't actually depend on the name under which they're deployed. If specified here, the context path
    ...
    /**
     * Specify the portion of the requestURI that represents the path to which the Servlet is mapped. This is typically
     * a portion of the requestURI after the context path.
     * <p>
     * In most cases, tests can be written by omitting the servlet path from the requestURI. This is because most
     * applications don't actually depend on the prefix to which a servlet is mapped. For example if a Servlet is mapped
     * to {@code "/main/*"}, tests can be written with the requestURI {@code "/accounts/1"} as opposed to
     * {@code "/main/accounts/1"}. If specified here, the servletPath must start with a "/" and must not end with a "/".
     *
     ...
     * <p>
     * If left unspecified (recommended), the pathInfo will be automatically derived by removing the contextPath and the
     * servletPath from the requestURI and using any remaining part. If specified here, the pathInfo must start with a
     * "/".
     * <p>
     ...
     * <p>
     * If content is provided and {@link #contentType(MediaType)} is set to {@code application/x-www-form-urlencoded},
     * the content will be parsed and used to populate the {@link #param(String, String...) request parameters} map.
     *
     * @param content
     *            the body content
     */
     ...
     * <p>
     * If content is provided and {@code contentType} is set to {@code application/x-www-form-urlencoded}, the content
     * will be parsed and used to populate the {@link #param(String, String...) request parameters} map.
     *
     * @param contentType
     *            the content type
     */
    ...
    /**
     * Set the 'Content-Type' header of the request as a raw String value, possibly not even well formed (for testing
     * purposes).
     *
     * @param contentType
     *            the content type
     ...
     * {@code application/x-www-form-urlencoded} request. This method simply adds to the request parameter map. You may
     * also use add Servlet request parameters by specifying the query or form data through one of the following:
     * <ul>
     * <li>Supply a URL with a query to {@link MockMvcRequestBuilders}.
     * <li>Add query params via {@link #queryParam} or {@link #queryParams}.
     * <li>Provide {@link #content} with {@link #contentType} {@code application/x-www-form-urlencoded}.
     * </ul>
     *
     * @param name
     *            the parameter name
    ...
    /**
     * Append to the query string and also add to the {@link #param(String, String...) request parameters} map. The
     * parameter name and value are encoded when they are added to the query string.
     *
     * @param name
     *            the parameter name
     * @param values
    ...
    /**
     * Append to the query string and also add to the {@link #params(MultiValueMap)} request parameters} map. The
     * parameter name and value are encoded when they are added to the query string.
     *
     * @param params
     *            the parameters to add
     ...
     *
     * @param locale
     *            the locale, or {@code null} to reset it
     * @see #locale(Locale...)
     */
     ...
     * <p>
     * Individual attributes provided via {@link #sessionAttr(String, Object)} override the content of the session
     * provided here.
     *
     * @param session
     *            the HTTP session
    ...
    /**
     * An extension point for further initialization of {@link MockHttpServletRequest} in ways not built directly into
     * the {@code MockHttpServletRequestBuilder}. Implementation of this interface can have builder-style methods
     * themselves and be made accessible through static factory methods.
     *
     * @param postProcessor
     ...
     *
     * @param parent
     *            the parent {@code RequestBuilder} to inherit properties from
    ...
    public final MockHttpServletRequest buildRequest(ServletContext servletContext) {
        ...
        if (!ObjectUtils.isEmpty(this.content) && !this.headers.containsKey(HttpHeaders.CONTENT_LENGTH)
