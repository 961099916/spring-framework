...
/**
 * Factory methods for request content {@code RequestMatcher} implementations that use an XPath expression.
 *
 * <p>
...
public class XpathRequestMatchers {
     ...
     * @param namespaces
     *            the XML namespaces referenced in the XPath expression, or {@code null}
     * @param args
     *            arguments to parameterize the XPath expression with, using the formatting specifiers defined in
    ...
    public RequestMatcher node(Matcher<? super Node> matcher) {
        return (XpathRequestMatcher)request -> this.xpathHelper.assertNode(request.getBodyAsBytes(), DEFAULT_ENCODING,
            matcher);
    }
    ...
    public RequestMatcher exists() {
        return (XpathRequestMatcher)request -> this.xpathHelper.exists(request.getBodyAsBytes(), DEFAULT_ENCODING);
    }
    ...
    public RequestMatcher doesNotExist() {
        return (XpathRequestMatcher)request -> this.xpathHelper.doesNotExist(request.getBodyAsBytes(),
            DEFAULT_ENCODING);
    }
    ...
    public RequestMatcher nodeCount(Matcher<Integer> matcher) {
        return (XpathRequestMatcher)request -> this.xpathHelper.assertNodeCount(request.getBodyAsBytes(),
            DEFAULT_ENCODING, matcher);
    }
    ...
    public RequestMatcher nodeCount(int expectedCount) {
        return (XpathRequestMatcher)request -> this.xpathHelper.assertNodeCount(request.getBodyAsBytes(),
            DEFAULT_ENCODING, expectedCount);
    }
    ...
    public RequestMatcher string(Matcher<? super String> matcher) {
        return (XpathRequestMatcher)request -> this.xpathHelper.assertString(request.getBodyAsBytes(), DEFAULT_ENCODING,
            matcher);
    }
    ...
    public RequestMatcher number(Matcher<? super Double> matcher) {
        return (XpathRequestMatcher)request -> this.xpathHelper.assertNumber(request.getBodyAsBytes(), DEFAULT_ENCODING,
            matcher);
    }
    ...
    public RequestMatcher booleanValue(Boolean value) {
        return (XpathRequestMatcher)request -> this.xpathHelper.assertBoolean(request.getBodyAsBytes(),
