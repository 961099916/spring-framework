...
public interface ConfigurableMockMvcBuilder<B extends ConfigurableMockMvcBuilder<B>> extends MockMvcBuilder {
     ...
     *            the filter to add
     * @param urlPatterns
     *            the URL patterns to map to; if empty, "/*" is used by default
     */
    <T extends B> T addFilter(Filter filter, String... urlPatterns);

    /**
     * Define default request properties that should be merged into all performed requests. In effect this provides a
     * mechanism for defining common initialization for all requests such as the content type, request parameters,
     * session attributes, and any other request property.
     *
     * <p>
    ...
    /**
     * Define a global expectation that should <em>always</em> be applied to every response. For example, status code
     * 200 (OK), content type {@code "application/json"}, etc.
     * 
     * @param resultMatcher
    ...
    /**
     * Define a global action that should <em>always</em> be applied to every response. For example, writing detailed
     * information about the performed request and resulting response to {@code System.out}.
     * 
     * @param resultHandler
     ...
     * {@link org.springframework.web.servlet.DispatcherServlet#setDispatchOptionsRequest dispatchOptionsRequest} which
     * allows processing of HTTP OPTIONS requests.
     */
    <T extends B> T dispatchOptions(boolean dispatchOptions);

    /**
     * Add a {@code MockMvcConfigurer} that automates MockMvc setup and configures it for some specific purpose (e.g.
     * security).
     * <p>
     * There is a built-in {@link SharedHttpSessionConfigurer} that can be used to re-use the HTTP session across
     * requests. 3rd party frameworks like Spring Security also use this mechanism to provide configuration shortcuts.
     * 
