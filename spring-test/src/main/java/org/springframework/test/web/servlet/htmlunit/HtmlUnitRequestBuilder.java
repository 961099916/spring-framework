...
/**
 * Internal class used to transform a {@link WebRequest} into a {@link MockHttpServletRequest} using Spring MVC Test's
 * {@link RequestBuilder}.
 *
 * <p>
 * By default the first path segment of the URL is used as the context path. To override this default see
...
final class HtmlUnitRequestBuilder implements RequestBuilder, Mergeable {
     ...
     * 
     * @param sessions
     *            a {@link Map} from session {@linkplain HttpSession#getId() IDs} to currently managed
     *            {@link HttpSession} objects; never {@code null}
     * @param webClient
     *            the WebClient for retrieving cookies
     * @param webRequest
     *            the {@link WebRequest} to transform into a {@link MockHttpServletRequest}; never {@code null}
     */
     ...
     * <p>
     * The value may be null in which case the first path segment of the URL is turned into the contextPath. Otherwise
     * it must conform to {@link HttpServletRequest#getContextPath()} which states it can be an empty string, or it must
     * start with a "/" and not end with a "/".
     * 
     * @param contextPath
     *            a valid contextPath
     * @throws IllegalArgumentException
     *             if the contextPath is not a valid {@link HttpServletRequest#getContextPath()}
     */
    ...
    private void contextPath(MockHttpServletRequest request, UriComponents uriComponents) {
        if (this.contextPath == null) {
            ...
            String path = uriComponents.getPath();
            Assert.isTrue(path != null && path.startsWith(this.contextPath), () -> "\"" + uriComponents.getPath()
