 ...
 *
 * <p>
 * An instance of this class is typically accessed via {@link MockMvcResultMatchers#xpath}.
 *
...
public class XpathResultMatchers {
    ...
    /**
     * Protected constructor, not for direct instantiation. Use {@link MockMvcResultMatchers#xpath(String, Object...)}
     * or {@link MockMvcResultMatchers#xpath(String, Map, Object...)}.
     * 
     * @param expression
    ...
    public ResultMatcher nodeCount(int expectedCount) {
        return result -> {
            MockHttpServletResponse response = result.getResponse();
            this.xpathHelper.assertNodeCount(response.getContentAsByteArray(), getDefinedEncoding(response),
                expectedCount);
        };
    }
    ...
    public ResultMatcher string(String expectedValue) {
        return result -> {
            MockHttpServletResponse response = result.getResponse();
            this.xpathHelper.assertString(response.getContentAsByteArray(), getDefinedEncoding(response),
                expectedValue);
        };
    }
    ...
    public ResultMatcher number(Double expectedValue) {
        return result -> {
            MockHttpServletResponse response = result.getResponse();
            this.xpathHelper.assertNumber(response.getContentAsByteArray(), getDefinedEncoding(response),
                expectedValue);
        };
    }
