...
public abstract class MockRestRequestMatchers {
     ...
     * 
     * @param matcher
     *            the String matcher for the expected URI
     * @return the request matcher
     */
    ...
    /**
     * Variant of {@link #requestTo(URI)} that prepares the URI from a URI template plus optional variables via
     * {@link UriComponentsBuilder} including encoding.
     * 
     * @param expectedUri
     *            the expected URI template
     * @param uriVars
     *            zero or more URI variables to populate the expected URI
     * @return the request matcher
     */
    ...
    public static RequestMatcher headerDoesNotExist(String name) {
        return request -> {
            ...
            if (headerValues != null) {
                fail("Expected header <" + name + "> not to exist, but it exists with values: " + headerValues);
            }
        };
    }
    ...
    /**
     * Access to request body matchers using a <a href="https://github.com/jayway/JsonPath">JsonPath</a> expression to
     * inspect a specific subset of the body. The JSON path expression can be a parameterized string using formatting
     * specifiers as defined in {@link String#format(String, Object...)}.
     * 
     * @param expression
     *            the JSON path optionally parameterized with arguments
     * @param args
    ...
    /**
     * Access to request body matchers using a <a href="https://github.com/jayway/JsonPath">JsonPath</a> expression to
     * inspect a specific subset of the body and a Hamcrest match for asserting the value found at the JSON path.
     * 
     * @param expression
    ...
    /**
     * Access to request body matchers using an XPath to inspect a specific subset of the body. The XPath expression can
     * be a parameterized string using formatting specifiers as defined in {@link String#format(String, Object...)}.
     * 
     * @param expression
    ...
    /**
     * Access to response body matchers using an XPath to inspect a specific subset of the body. The XPath expression
     * can be a parameterized string using formatting specifiers as defined in {@link String#format(String, Object...)}.
     * 
     * @param expression
     *            the XPath optionally parameterized with arguments
     * @param namespaces
     *            the namespaces referenced in the XPath expression
     * @param args
     *            arguments to parameterize the XPath expression with
     */
