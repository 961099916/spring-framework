...
/**
 * Base class for {@code RequestExpectationManager} implementations responsible for storing expectations and actual
 * requests, and checking for unsatisfied expectations at the end.
 *
 * <p>
 * Subclasses are responsible for validating each request by matching it to to expectations following the order of
 * declaration or not.
 *
...
public abstract class AbstractRequestExpectationManager implements RequestExpectationManager {
    ...
    /**
     * Invoked at the time of the first actual request, which effectively means the expectations declaration phase is
     * over.
     */
     ...
     *             only the matched expectation, leaving the call to create the response as a separate step (to be
     ...
     * {@link #validateRequestInternal(ClientHttpRequest)} in order to match the request to an expectation, leaving the
     * call to create the response as a separate step (to be invoked by this class).
     * 
     * @param request
     *            the current request
    ...
    protected static class RequestExpectationGroup {
         ...
         * <p>
         * The count of the given expectation is incremented, then it is either stored if remainingCount > 0 or removed
         * otherwise.
         */
