...
/**
 * Connector that handles requests by invoking an {@link HttpHandler} rather than making actual requests to a network
 * socket.
 *
 * <p>
...
public class HttpHandlerConnector implements ClientHttpConnector {
    ...
    public Mono<ClientHttpResponse> connect(HttpMethod httpMethod, URI uri,
        ...
        return Mono.defer(() -> doConnect(httpMethod, uri, requestCallback)).subscribeOn(Schedulers.parallel());
    }

    private Mono<ClientHttpResponse> doConnect(HttpMethod httpMethod, URI uri,
        Function<? super ClientHttpRequest, Mono<Void>> requestCallback) {
        ...
        return Mono.when(requestWriteCompletion, handlerCompletion).onErrorMap(ex -> {
            ...
            () -> savedResponse[0] != null ? savedResponse[0] : adaptResponse(mockServerResponse, Flux.empty())));
    }
     ...
     * {@link ServerHttpResponse#writeWith} or {@link ServerHttpResponse#setComplete()}, and can no longer be changed.
     * This exception wraps the error and also provides {@link #getCompletedResponse() access} to the response.
     * <p>
     * What happens on an actual running server depends on when the server commits the response and the error may or may
     * not change the response. Therefore in tests without a server the exception is wrapped and allowed to propagate so
     * the application is alerted.
     * 
     * @since 5.2.2
     */
