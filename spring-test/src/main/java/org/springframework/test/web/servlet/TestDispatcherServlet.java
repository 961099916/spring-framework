...
/**
 * A sub-class of {@code DispatcherServlet} that saves the result in an {@link MvcResult}. The {@code MvcResult}
 * instance is expected to be available as the request attribute {@link MockMvc#MVC_RESULT_ATTRIBUTE}.
 *
...
final class TestDispatcherServlet extends DispatcherServlet {
            ...
            if (request.getAsyncContext() instanceof MockAsyncContext) {
                ...
                Assert.notNull(asyncContext,
                    () -> "Outer request wrapper " + request.getClass().getName() + " has an AsyncContext,"
                        + "but it is not a MockAsyncContext, while the nested " + mockRequest.getClass().getName()
    ...
    private void registerAsyncResultInterceptors(HttpServletRequest request) {
        ...
        WebAsyncUtils.getAsyncManager(request).registerCallableInterceptor(KEY, new CallableProcessingInterceptor() {
            @Override
