...
/**
 * Decorate another {@link ClientHttpConnector} with the purpose of intercepting, capturing, and exposing actual request
 * and response data transmitted to and received from the server.
 *
...
class WiretapConnector implements ClientHttpConnector {
    ...
    final static class WiretapRecorder {
        ...
        public WiretapRecorder(@Nullable Publisher<? extends DataBuffer> publisher,
            ...
            this.publisher = publisher != null ? Flux.from(publisher).doOnSubscribe(s -> this.hasContentConsumer = true)
                .doOnNext(this.buffer::write).doOnError(this::handleOnError).doOnCancel(this::handleOnComplete)
                .doOnComplete(this::handleOnComplete) : null;

            this.publisherNested =
                publisherNested != null ? Flux.from(publisherNested).doOnSubscribe(s -> this.hasContentConsumer = true)
                    .map(p -> Flux.from(p).doOnNext(this.buffer::write).doOnError(this::handleOnError))
                    .doOnError(this::handleOnError).doOnCancel(this::handleOnComplete)
                    .doOnComplete(this::handleOnComplete) : null;
            ...
            return Mono.defer(() -> {
                ...
                if (!this.hasContentConsumer) {
                    ...
                    (this.publisher != null ? this.publisher : this.publisherNested)
                        .onErrorMap(ex -> new IllegalStateException("Content has not been consumed, and "
