...
/**
 * {@code MockMvcHtmlUnitDriverBuilder} simplifies the building of an {@link HtmlUnitDriver} that delegates to
 * {@link MockMvc} and optionally delegates to an actual connection for specific requests.
 *
 * <p>
 * By default, the driver will delegate to {@code MockMvc} to handle requests to {@code localhost} and to a
 * {@link WebClient} to handle any other URL (i.e. to perform an actual HTTP request).
 *
 * @author Rob Winch
 * @author Sam Brannen
 * @since 4.2
...
public class MockMvcHtmlUnitDriverBuilder extends MockMvcWebConnectionBuilderSupport<MockMvcHtmlUnitDriverBuilder> {
    ...
    /**
     * Create a new {@code MockMvcHtmlUnitDriverBuilder} based on the supplied {@link WebApplicationContext}.
     * 
     * @param context
     *            the {@code WebApplicationContext} to create a {@link MockMvc} instance from (never {@code null})
     * @return the MockMvcHtmlUnitDriverBuilder to customize
     */
    ...
    /**
     * Create a new {@code MockMvcHtmlUnitDriverBuilder} based on the supplied {@link WebApplicationContext} and
     * {@link MockMvcConfigurer}.
     * 
     * @param context
     *            the {@code WebApplicationContext} to create a {@link MockMvc} instance from (never {@code null})
     * @param configurer
     *            the {@code MockMvcConfigurer} to apply (never {@code null})
     * @return the MockMvcHtmlUnitDriverBuilder to customize
     */
    public static MockMvcHtmlUnitDriverBuilder webAppContextSetup(WebApplicationContext context,
     ...
     *            {@code true} if JavaScript should be enabled
    ...
    /**
     * Supply the {@code WebConnectionHtmlUnitDriver} that the driver {@linkplain #build built} by this builder should
     * delegate to when processing non-{@linkplain WebRequestMatcher matching} requests.
     * 
     * @param driver
     *            the {@code WebConnectionHtmlUnitDriver} to delegate to for requests that do not match (never
     *            {@code null})
     * @return this builder for further customizations
     * @see #build()
     */
     ...
     * <p>
     * The returned driver will use the configured {@link MockMvc} instance for processing any
     * {@linkplain WebRequestMatcher matching} requests and a delegate {@code HtmlUnitDriver} for all other requests.
     * <p>
     * If a {@linkplain #withDelegate delegate} has been explicitly configured, it will be used; otherwise, a default
     * {@code WebConnectionHtmlUnitDriver} with the {@link BrowserVersion} set to {@link BrowserVersion#CHROME CHROME}
