 ...
 *
 * <p>
 * An instance of this class is typically accessed via {@link MockMvcResultMatchers#content}.
 *
...
public class ContentResultMatchers {
    ...
    /**
     * Assert the ServletResponse content type. The given content type must fully match including type, sub-type, and
     * parameters. For checking only the type and sub-type see {@link #contentTypeCompatibleWith(String)}.
     */
    ...
    /**
     * Assert the ServletResponse content type after parsing it as a MediaType. The given content type must fully match
     * including type, sub-type, and parameters. For checking only the type and sub-type see
    ...
    public ResultMatcher bytes(byte[] expectedContent) {
        return result -> assertEquals("Response content", expectedContent,
            result.getResponse().getContentAsByteArray());
    }

    /**
     * Parse the response content and the given string as XML and assert the two are "similar" - i.e. they contain the
     * same elements and attributes regardless of order.
     * <p>
     * Use of this matcher requires the <a href="http://xmlunit.sourceforge.net/">XMLUnit</a> library.
     * 
     * @param xmlContent
    ...
    /**
     * Parse the expected and actual strings as JSON and assert the two are "similar" - i.e. they contain the same
     * attribute-value pairs regardless of formatting with a lenient checking (extensible, and non-strict array
     * ordering).
     * 
     * @param jsonContent
    ...
    /**
     * Parse the response content and the given string as JSON and assert the two are "similar" - i.e. they contain the
     * same attribute-value pairs regardless of formatting.
     * <p>
     ...
     * </ul>
     * <p>
     * Use of this matcher requires the <a href="https://jsonassert.skyscreamer.org/">JSONassert</a> library.
     * 
     * @param jsonContent
     *            the expected JSON content
     * @param strict
