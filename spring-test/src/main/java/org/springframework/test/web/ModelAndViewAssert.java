 ...
 *
 * <p>
 * Intended for use with JUnit 4 and TestNG. All {@code assert*()} methods throw {@link AssertionError AssertionErrors}.
 *
 * @author Sam Brannen
...
public abstract class ModelAndViewAssert {
    ...
    /**
     * Checks whether the model value under the given {@code modelName} exists and checks it type, based on the
     * {@code expectedType}. If the model entry exists and the type matches, the model value is returned.
     * 
     * @param mav
     *            the ModelAndView to test against (never {@code null})
     * @param modelName
    ...
    /**
     * Compare a given {@code expectedValue} to the value from the model bound under the given {@code modelName}.
     * 
     * @param mav
     *            the ModelAndView to test against (never {@code null})
     * @param modelName
     ...
     *            the expected list
     * @param comparator
     *            the comparator to use (may be {@code null}). If not specifying the comparator, both lists will be
     *            sorted not using any comparator.
     */
