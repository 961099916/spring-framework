 ...
 *
 * <p>
 * As of 5.0 this class also implements {@link org.springframework.http.client.AsyncClientHttpRequestFactory
 * AsyncClientHttpRequestFactory}. However note that {@link org.springframework.web.client.AsyncRestTemplate} and
 * related classes have been deprecated at the same time.
 *
...
public class MockMvcClientHttpRequestFactory
    ...
    private ClientHttpResponse getClientHttpResponse(HttpMethod httpMethod, URI uri, HttpHeaders requestHeaders,
        byte[] requestBody) {

        try {
            MockHttpServletResponse servletResponse =
                this.mockMvc.perform(request(httpMethod, uri).content(requestBody).headers(requestHeaders)).andReturn()
                    .getResponse();
