...
/**
 * Utility methods for working with {@link ActiveProfiles @ActiveProfiles} and {@link ActiveProfilesResolver
 * ActiveProfilesResolvers}.
 *
 * <p>
 * Although {@code ActiveProfilesUtils} was first introduced in Spring Framework 4.1, the initial implementations of
 * methods in this class were based on the existing code base in {@code ContextLoaderUtils}.
 *
 * @author Sam Brannen
...
abstract class ActiveProfilesUtils {
     ...
     * {@code inheritProfiles} flag is set to {@code true}, profiles defined in the test class will be merged with those
     * defined in superclasses.
     * 
     * @param testClass
     *            the class for which to resolve the active profiles (must not be {@code null})
     * @return the set of active profiles for the specified class, including active profiles from superclasses if
    ...
    static String[] resolveActiveProfiles(Class<?> testClass) {
        ...
        if (descriptor == null && logger.isDebugEnabled()) {
            logger.debug(
                String.format("Could not find an 'annotation declaring class' for annotation type [%s] and class [%s]",
        ...
        while (descriptor != null) {
            ...
            try {
                ...
                String msg =
                    String.format("Could not instantiate ActiveProfilesResolver of type [%s] " + "for test class [%s]",
            ...
            descriptor = (annotation.inheritProfiles()
                ? MetaAnnotationUtils.findAnnotationDescriptor(rootDeclaringClass.getSuperclass(), annotationType)
                : null);
        }
