...
/**
 * {@code MergedContextConfiguration} encapsulates the <em>merged</em> context configuration declared on a test class
 * and all of its superclasses via {@link ContextConfiguration @ContextConfiguration},
 * {@link ActiveProfiles @ActiveProfiles}, and {@link TestPropertySource @TestPropertySource}.
 *
 * <p>
 * Merged context resource locations, annotated classes, active profiles, property resource locations, and in-lined
 * properties represent all declared values in the test class hierarchy taking into consideration the semantics of the
 * {@link ContextConfiguration#inheritLocations}, {@link ActiveProfiles#inheritProfiles},
 * {@link TestPropertySource#inheritLocations}, and {@link TestPropertySource#inheritProperties} flags.
 *
 * <p>
 ...
 *
 * <p>
 * {@code MergedContextConfiguration} is also used by the {@link org.springframework.test.context.cache.ContextCache
 * ContextCache} as the key for caching an {@link org.springframework.context.ApplicationContext ApplicationContext}
...
public class MergedContextConfiguration implements Serializable {
     ...
     * 
     * @param testClass
     *            the test class for which the configuration was merged
     * @param locations
     *            the merged context resource locations
     * @param classes
     *            the merged annotated classes
     * @param activeProfiles
     *            the merged active bean definition profiles
     * @param contextLoader
     *            the resolved {@code ContextLoader}
     */
     ...
     * @param classes
     *            the merged annotated classes
     * @param contextInitializerClasses
     ...
     *            a cache-aware context loader delegate with which to retrieve the parent context
     * @param parent
     *            the parent configuration or {@code null} if there is no parent
     * @since 3.2.2
     */
    public MergedContextConfiguration(Class<?> testClass, String[] locations, Class<?>[] classes,
        @Nullable Set<Class<? extends ApplicationContextInitializer<?>>> contextInitializerClasses,
    ...
    /**
     * Create a new {@code MergedContextConfiguration} instance by copying all fields from the supplied
    ...
    public MergedContextConfiguration(MergedContextConfiguration mergedConfig) {
        this(mergedConfig.testClass, mergedConfig.locations, mergedConfig.classes,
            ...
            mergedConfig.propertySourceProperties, mergedConfig.contextCustomizers, mergedConfig.contextLoader,
     ...
     * Create a new {@code MergedContextConfiguration} instance for the supplied parameters.
     * <p>
     * If a {@code null} value is supplied for {@code locations}, {@code classes}, {@code activeProfiles},
     * {@code propertySourceLocations}, or {@code propertySourceProperties} an empty array will be stored instead. If a
     * {@code null} value is supplied for the {@code contextInitializerClasses} an empty set will be stored instead.
     ...
     *            the merged {@code PropertySource} locations
     * @param propertySourceProperties
     *            the merged {@code PropertySource} properties
     * @param contextLoader
     *            the resolved {@code ContextLoader}
     * @param cacheAwareContextLoaderDelegate
     *            a cache-aware context loader delegate with which to retrieve the parent context
    ...
    public MergedContextConfiguration(Class<?> testClass, @Nullable String[] locations, @Nullable Class<?>[] classes,
        ...
        this(testClass, locations, classes, contextInitializerClasses, activeProfiles, propertySourceLocations,
            propertySourceProperties, EMPTY_CONTEXT_CUSTOMIZERS, contextLoader, cacheAwareContextLoaderDelegate,
            parent);
    }
     ...
     * If a {@code null} value is supplied for {@code locations}, {@code classes}, {@code activeProfiles},
     * {@code propertySourceLocations}, or {@code propertySourceProperties} an empty array will be stored instead. If a
     * {@code null} value is supplied for {@code contextInitializerClasses} or {@code contextCustomizers}, an empty set
     * will be stored instead. Furthermore, active profiles will be sorted, and duplicate profiles will be removed.
     * 
     * @param testClass
     *            the test class for which the configuration was merged
     * @param locations
    ...
    private static Set<ContextCustomizer>
        processContextCustomizers(@Nullable Set<ContextCustomizer> contextCustomizers) {

        return (contextCustomizers != null ? Collections.unmodifiableSet(contextCustomizers)
    ...
    /**
     * Generate a null-safe {@link String} representation of the supplied {@link ContextLoader} based solely on the
     * fully qualified name of the loader or &quot;null&quot; if the supplied loader is {@code null}.
     */
    ...
    /**
     * Determine if this {@code MergedContextConfiguration} instance has either path-based context resource locations or
     * class-based resources.
     *
     * @return {@code true} if either the {@link #getLocations() locations} or the {@link #getClasses() classes} array
     *         is not empty
     * @since 4.0.4
     * @see #hasLocations()
     * @see #hasClasses()
    ...
    /**
     * Get the merged resource locations for test {@code PropertySources} for the {@linkplain #getTestClass() test
     * class}.
     *
    ...
    /**
     * Get the merged test {@code PropertySource} properties for the {@linkplain #getTestClass() test class}.
     * <p>
     * Properties will be loaded into the {@code Environment}'s set of {@code PropertySources}.
     *
    ...
    /**
     * Get the merged {@link ContextCustomizer ContextCustomizers} that will be applied when the application context is
     * loaded.
     */
    ...
    /**
     * Get the parent {@link ApplicationContext} for the context defined by this {@code MergedContextConfiguration} from
     * the context cache.
     * <p>
    ...
    /**
     * Determine if the supplied object is equal to this {@code MergedContextConfiguration} instance by comparing both
     * object's {@linkplain #getLocations() locations}, {@linkplain #getClasses() annotated classes},
     * {@linkplain #getContextInitializerClasses() context initializer classes}, {@linkplain #getActiveProfiles() active
     * profiles}, {@linkplain #getPropertySourceLocations() property source locations},
     * {@linkplain #getPropertySourceProperties() property source properties}, {@linkplain #getParent() parents}, and
     * the fully qualified names of their {@link #getContextLoader() ContextLoaders}.
     */
    @Override
    ...
    /**
     * Generate a unique hash code for all properties of this {@code MergedContextConfiguration} excluding the
     * {@linkplain #getTestClass() test class}.
     */
    @Override
    public int hashCode() {
    ...
    /**
     * Provide a String representation of the {@linkplain #getTestClass() test class}, {@linkplain #getLocations()
     * locations}, {@linkplain #getClasses() annotated classes}, {@linkplain #getContextInitializerClasses() context
     * initializer classes}, {@linkplain #getActiveProfiles() active profiles},
     * {@linkplain #getPropertySourceLocations() property source locations}, {@linkplain #getPropertySourceProperties()
     * property source properties}, {@linkplain #getContextCustomizers() context customizers}, the name of the
     * {@link #getContextLoader() ContextLoader}, and the {@linkplain #getParent() parent configuration}.
     */
    @Override
    public String toString() {
        return new ToStringCreator(this).append("testClass", this.testClass)
            ...
            .append("contextLoader", nullSafeClassName(this.contextLoader)).append("parent", this.parent).toString();
    }
