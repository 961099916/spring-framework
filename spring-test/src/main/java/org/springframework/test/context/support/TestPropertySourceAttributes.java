...
/**
 * {@code TestPropertySourceAttributes} encapsulates attributes declared via
 * {@link TestPropertySource @TestPropertySource} annotations.
 *
 * <p>
 * In addition to encapsulating declared attributes, {@code TestPropertySourceAttributes} also enforces configuration
 * rules.
 *
 * @author Sam Brannen
 * @author Phillip Webb
...
class TestPropertySourceAttributes {
    ...
    /**
     * Determine if the annotation represented by this {@code TestPropertySourceAttributes} instance can be merged with
     * the supplied {@code annotation}.
     * <p>
     * This method effectively checks that two annotations are declared at the same level in the type hierarchy (i.e.,
     * have the same {@linkplain MergedAnnotation#getAggregateIndex() aggregate index}).
     *
     * @since 5.2
    ...
    /**
     * Merge this {@code TestPropertySourceAttributes} instance with the supplied {@code annotation}, asserting that the
     * two sets of test property source attributes have identical values for the
     * {@link TestPropertySource#inheritLocations} and {@link TestPropertySource#inheritProperties} flags and that the
     * two underlying annotations were declared on the same class.
     * <p>
     * This method should only be invoked if {@link #canMergeWith(MergedAnnotation)} returns {@code true}.
     *
     * @since 5.2
    ...
    void mergeWith(MergedAnnotation<TestPropertySource> annotation) {
        ...
        Assert.state(source == this.declaringClass,
            () -> "Detected @TestPropertySource declarations within an aggregate index " + "with different sources: "
                + this.declaringClass.getName() + " and " + source.getName());
        logger.trace(
            LogMessage.format("Retrieved %s for declaring class [%s].", annotation, this.declaringClass.getName()));
    ...
    private void assertSameBooleanAttribute(boolean expected, MergedAnnotation<TestPropertySource> annotation,
        ...
        Assert.isTrue(expected == annotation.getBoolean(attribute),
            () -> String.format(
                ...
                annotation.getRoot().getType().getSimpleName(), declaringClass(annotation).getSimpleName(), attribute));
    }
     ...
     * <p>
     * If the {@code prepend} flag is {@code false}, the elements will appended to the list.
     *
     * @param prepend
     *            whether the elements should be prepended to the list
     * @param list
     *            the list to which to add the elements
     * @param elements
     ...
     * <p>
     * Note: The returned value may represent a <em>detected default</em> or merged locations that do not match the
     * original value declared via a single {@code @TestPropertySource} annotation.
     *
     ...
     * <p>
     * Note: The returned value may represent merged properties that do not match the original value declared via a
    ...
    /**
     * Provide a String representation of the {@code @TestPropertySource} attributes and declaring class.
     */
    @Override
    public String toString() {
        return new ToStringCreator(this).append("declaringClass", this.declaringClass.getName())
            .append("locations", this.locations).append("inheritLocations", this.inheritLocations)
            .append("properties", this.properties).append("inheritProperties", this.inheritProperties).toString();
    }
