...
/**
 * Abstract implementation of the {@link TestContextBootstrapper} interface which provides most of the behavior required
 * by a bootstrapper.
 *
 * <p>
 ...
 *
 * <p>
 * To plug in custom {@link org.springframework.test.context.cache.ContextCache ContextCache} support, override
...
public abstract class AbstractTestContextBootstrapper implements TestContextBootstrapper {
    ...
    /**
     * Build a new {@link DefaultTestContext} using the {@linkplain Class test class} in the {@link BootstrapContext}
     * associated with this bootstrapper and by delegating to {@link #buildMergedContextConfiguration()} and
     * {@link #getCacheAwareContextLoaderDelegate()}.
     * <p>
     * Concrete subclasses may choose to override this method to return a custom {@link TestContext} implementation.
     *
     * @since 4.2
     */
    @Override
        ...
        if (descriptor == null) {
            ...
            while (descriptor != null) {
                ...
                AnnotationDescriptor<TestExecutionListeners> superDescriptor = MetaAnnotationUtils
                    .findAnnotationDescriptor(descriptor.getRootDeclaringClass().getSuperclass(), annotationType);
    ...
    private List<TestExecutionListener>
        instantiateListeners(Collection<Class<? extends TestExecutionListener>> classes) {
     ...
     * <p>
     * This method is invoked by {@link #getTestExecutionListeners()} and delegates to
     * {@link #getDefaultTestExecutionListenerClassNames()} to retrieve the class names.
     * <p>
     * If a particular class cannot be loaded, a {@code DEBUG} message will be logged, but the associated exception will
     * not be rethrown.
     */
     ...
     * <p>
     * The default implementation looks up all {@code org.springframework.test.context.TestExecutionListener} entries
    ...
    private MergedContextConfiguration buildDefaultMergedContextConfiguration(Class<?> testClass,
        ...
        if (logger.isInfoEnabled()) {
            logger.info(
                String.format("Neither @ContextConfiguration nor @ContextHierarchy found for test class [%s], using %s",
    ...
    /**
     * Build the {@link MergedContextConfiguration merged context configuration} for the supplied {@link Class
     * testClass}, context configuration attributes, and parent context configuration.
     *
     * @param testClass
     *            the test class for which the {@code MergedContextConfiguration} should be built (must not be
     *            {@code null})
     * @param configAttributesList
     *            the list of context configuration attributes for the specified test class, ordered <em>bottom-up</em>
     *            (i.e., as if we were traversing up the class hierarchy); never {@code null} or empty
     * @param parentConfig
     *            the merged context configuration for the parent application context in a context hierarchy, or
     *            {@code null} if there is no parent
     * @param cacheAwareContextLoaderDelegate
     *            the cache-aware context loader delegate to be passed to the {@code MergedContextConfiguration}
     *            constructor
     * @param requireLocationsClassesOrInitializers
     *            whether locations, classes, or initializers are required; typically {@code true} but may be set to
     *            {@code false} if the configured loader supports empty configuration
     ...
     * <p>
     * The default implementation uses the {@link SpringFactoriesLoader} mechanism for loading factories configured in
     * all {@code META-INF/spring.factories} files on the classpath.
     *
     * @since 4.3
     ...
     * {@link ContextConfigurationAttributes} and then instantiate and return that {@code ContextLoader}.
     * <p>
     * If the user has not explicitly declared which loader to use, the value returned from
     * {@link #getDefaultContextLoaderClass} will be used as the default context loader class. For details on the class
     * resolution process, see {@link #resolveExplicitContextLoaderClass} and {@link #getDefaultContextLoaderClass}.
     *
     * @param testClass
     *            the test class for which the {@code ContextLoader} should be resolved; must not be {@code null}
     * @param configAttributesList
     *            the list of configuration attributes to process; must not be {@code null}; must be ordered
     *            <em>bottom-up</em> (i.e., as if we were traversing up the class hierarchy)
     * @return the resolved {@code ContextLoader} for the supplied {@code testClass} (never {@code null})
     * @throws IllegalStateException
     *             if {@link #getDefaultContextLoaderClass(Class)} returns {@code null}
     */
     ...
     * <ol>
     * <li>If the {@link ContextConfigurationAttributes#getContextLoaderClass() contextLoaderClass} property of
     * {@link ContextConfigurationAttributes} is configured with an explicit class, that class will be returned.</li>
     * <li>If an explicit {@code ContextLoader} class is not specified at the current level in the hierarchy, traverse
     * to the next level in the hierarchy and return to step #1.</li>
     * </ol>
     *
     * @param configAttributesList
     *            the list of configuration attributes to process; must not be {@code null}; must be ordered
     *            <em>bottom-up</em> (i.e., as if we were traversing up the class hierarchy)
     * @return the {@code ContextLoader} class to use for the supplied configuration attributes, or {@code null} if no
     *         explicit loader is found
     * @throws IllegalArgumentException
     *             if supplied configuration attributes are {@code null} or <em>empty</em>
     */
    @Nullable
    protected Class<? extends ContextLoader>
        ...
        for (ContextConfigurationAttributes configAttributes : configAttributesList) {
            ...
            if (ContextLoader.class != contextLoaderClass) {
                if (logger.isDebugEnabled()) {
                    logger.debug(
                        String.format("Found explicit ContextLoader class [%s] for context configuration attributes %s",
    ...
    /**
     * Get the {@link CacheAwareContextLoaderDelegate} to use for transparent interaction with the {@code ContextCache}.
     * <p>
     * The default implementation simply delegates to
     * {@code getBootstrapContext().getCacheAwareContextLoaderDelegate()}.
     * <p>
     * Concrete subclasses may choose to override this method to return a custom {@code CacheAwareContextLoaderDelegate}
     * implementation with custom {@link org.springframework.test.context.cache.ContextCache ContextCache} support.
     *
    ...
    /**
     * Determine the default {@link ContextLoader} {@linkplain Class class} to use for the supplied test class.
     * <p>
     * The class returned by this method will only be used if a {@code ContextLoader} class has not been explicitly
     * declared via {@link ContextConfiguration#loader}.
     *
     * @param testClass
     *            the test class for which to retrieve the default {@code ContextLoader} class
     * @return the default {@code ContextLoader} class for the supplied test class (never {@code null})
     */
     ...
     * <p>
     * The returned {@link MergedContextConfiguration} instance may be a wrapper around or a replacement for the
     * original.
     * <p>
     * The default implementation simply returns the supplied instance unmodified.
     * <p>
     * Concrete subclasses may choose to return a specialized subclass of {@link MergedContextConfiguration} based on
     * properties in the supplied instance.
     *
     * @param mergedConfig
     *            the {@code MergedContextConfiguration} to process; never {@code null}
