 ...
 *
 * <p>
 * Uses a synchronized {@link Map} configured with a maximum size and a <em>least recently used</em> (LRU) eviction
 * policy to cache {@link ApplicationContext} instances.
 *
 * <p>
 * The maximum size may be supplied as a {@linkplain #DefaultContextCache(int) constructor argument} or set via a system
 * property or Spring property named {@code spring.test.context.cache.maxSize}.
 *
 * @author Sam Brannen
...
public class DefaultContextCache implements ContextCache {
    ...
    /**
     * Map of parent keys to sets of children keys, representing a top-down <em>tree</em> of context hierarchies. This
     * information is used for determining which subtrees need to be recursively removed and closed when removing a
     * context that is a parent of other contexts.
     */
    ...
    /**
     * Create a new {@code DefaultContextCache} using the maximum cache size obtained via
     * {@link ContextCacheUtils#retrieveMaxCacheSize()}.
     * 
     * @since 4.3
     ...
     *             if the supplied {@code maxSize} value is not positive
     * @since 4.3
    ...
    public void remove(MergedContextConfiguration key, @Nullable HierarchyMode hierarchyMode) {
        ...
        for (Map.Entry<MergedContextConfiguration, Set<MergedContextConfiguration>> entry : this.hierarchyMap
            .entrySet()) {
     ...
     * <p>
     * The string returned by this method contains all information required for compliance with the contract for
     * {@link #logStatistics()}.
     * 
    ...
    /**
     * Simple cache implementation based on {@link LinkedHashMap} with a maximum size and a <em>least recently used</em>
     * (LRU) eviction policy that properly closes application contexts.
     * 
     * @since 4.3
     */
