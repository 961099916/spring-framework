 ...
 * {@link ContextConfiguration @ContextConfiguration} and {@link ContextHierarchy @ContextHierarchy} annotations for use
 * with {@link SmartContextLoader SmartContextLoaders}.
 *
 * @author Sam Brannen
 * @since 3.1
...
abstract class ContextLoaderUtils {
    ...
    /**
     * Resolve the list of lists of {@linkplain ContextConfigurationAttributes context configuration attributes} for the
     * supplied {@linkplain Class test class} and its superclasses, taking into account context hierarchies declared via
     * {@link ContextHierarchy @ContextHierarchy} and {@link ContextConfiguration @ContextConfiguration}.
     * <p>
     * The outer list represents a top-down ordering of context configuration attributes, where each element in the list
     * represents the context configuration declared on a given test class in the class hierarchy. Each nested list
     * contains the context configuration attributes declared either via a single instance of
     * {@code @ContextConfiguration} on the particular class or via multiple instances of {@code @ContextConfiguration}
     * declared within a single {@code @ContextHierarchy} instance on the particular class. Furthermore, each nested
     * list maintains the order in which {@code @ContextConfiguration} instances are declared.
     * <p>
     ...
     * {@link ContextConfiguration @ContextConfiguration} will <strong>not</strong> be taken into consideration. If
     * these flags need to be honored, that must be handled manually when traversing the nested lists returned by this
     * method.
     * 
     * @param testClass
     *            the class for which to resolve the context hierarchy attributes (must not be {@code null})
     * @return the list of lists of configuration attributes for the specified class; never {@code null}
     * @throws IllegalArgumentException
     *             if the supplied class is {@code null}; or if neither {@code @ContextConfiguration} nor
     *             {@code @ContextHierarchy} is <em>present</em> on the supplied class
     * @throws IllegalStateException
     *             if a test class or composed annotation in the class hierarchy declares both
    ...
    /**
     * Build a <em>context hierarchy map</em> for the supplied {@linkplain Class test class} and its superclasses,
     * taking into account context hierarchies declared via {@link ContextHierarchy @ContextHierarchy} and
     * {@link ContextConfiguration @ContextConfiguration}.
     * <p>
     * Each value in the map represents the consolidated list of {@linkplain ContextConfigurationAttributes context
     * configuration attributes} for a given level in the context hierarchy (potentially across the test class
     * hierarchy), keyed by the {@link ContextConfiguration#name() name} of the context hierarchy level.
     * <p>
     * If a given level in the context hierarchy does not have an explicit name (i.e., configured via
     * {@link ContextConfiguration#name}), a name will be generated for that hierarchy level by appending the numerical
     * level to the {@link #GENERATED_CONTEXT_HIERARCHY_LEVEL_PREFIX}.
     * 
     * @param testClass
     *            the class for which to resolve the context hierarchy map (must not be {@code null})
     * @return a map of context configuration attributes for the context hierarchy, keyed by context hierarchy level
     *         name; never {@code null}
     * @throws IllegalArgumentException
     *             if the lists of context configuration attributes for each level in the {@code @ContextHierarchy} do
     *             not define unique context configuration within the overall hierarchy.
     * @since 3.2.2
    ...
    /**
     * Resolve the list of {@linkplain ContextConfigurationAttributes context configuration attributes} for the supplied
     * {@linkplain Class test class} and its superclasses.
     * <p>
     * Note that the {@link ContextConfiguration#inheritLocations inheritLocations} and
     * {@link ContextConfiguration#inheritInitializers() inheritInitializers} flags of
     * {@link ContextConfiguration @ContextConfiguration} will <strong>not</strong> be taken into consideration. If
     * these flags need to be honored, that must be handled manually when traversing the list returned by this method.
     * 
     * @param testClass
     *            the class for which to resolve the configuration attributes (must not be {@code null})
     * @return the list of configuration attributes for the specified class, ordered <em>bottom-up</em> (i.e., as if we
     *         were traversing up the class hierarchy); never {@code null}
     * @throws IllegalArgumentException
     *             if the supplied class is {@code null} or if {@code @ContextConfiguration} is not <em>present</em> on
     *             the supplied class
     */
    ...
    /**
     * Convenience method for creating a {@link ContextConfigurationAttributes} instance from the supplied
     * {@link ContextConfiguration} annotation and declaring class and then adding the attributes to the supplied list.
     */
