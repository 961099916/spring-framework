...
/**
 * {@code BootstrapUtils} is a collection of utility methods to assist with bootstrapping the <em>Spring TestContext
 * Framework</em>.
 *
 * @author Sam Brannen
 * @author Phillip Webb
...
abstract class BootstrapUtils {
     ...
     * <p>
     * Uses reflection to create a {@link org.springframework.test.context.support.DefaultBootstrapContext} that uses a
     * {@link org.springframework.test.context.cache.DefaultCacheAwareContextLoaderDelegate}.
     * 
     * @param testClass
     *            the test class for which the bootstrap context should be created
    ...
    private static CacheAwareContextLoaderDelegate createCacheAwareContextLoaderDelegate() {
        ...
        try {
            clazz = (Class<? extends CacheAwareContextLoaderDelegate>)ClassUtils
                .forName(DEFAULT_CACHE_AWARE_CONTEXT_LOADER_DELEGATE_CLASS_NAME, BootstrapUtils.class.getClassLoader());
    ...
    /**
     * Resolve the {@link TestContextBootstrapper} type for the test class in the supplied {@link BootstrapContext},
     * instantiate it, and provide it a reference to the {@link BootstrapContext}.
     * <p>
     * If the {@link BootstrapWith @BootstrapWith} annotation is present on the test class, either directly or as a
     * meta-annotation, then its {@link BootstrapWith#value value} will be used as the bootstrapper type. Otherwise,
     * either the {@link org.springframework.test.context.support.DefaultTestContextBootstrapper
     * DefaultTestContextBootstrapper} or the {@link org.springframework.test.context.web.WebTestContextBootstrapper
    ...
    static TestContextBootstrapper resolveTestContextBootstrapper(BootstrapContext bootstrapContext) {
        ...
        try {
            ...
            throw new IllegalStateException(
                "Could not load TestContextBootstrapper [" + clazz
    ...
    private static Class<?> resolveExplicitTestContextBootstrapper(Class<?> testClass) {
        ...
        throw new IllegalStateException(
            String.format("Configuration error: found multiple declarations of @BootstrapWith for test class [%s]: %s",
