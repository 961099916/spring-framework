...
/**
 * Utility methods for working with transactions and data access related beans within the <em>Spring TestContext
 * Framework</em>.
 *
 * <p>
...
public abstract class TestContextTransactionUtils {
     ...
     * <p>
     * The following algorithm is used to retrieve the {@code DataSource} from the
     * {@link org.springframework.context.ApplicationContext ApplicationContext} of the supplied test context:
     * <ol>
     * <li>Look up the {@code DataSource} by type and name, if the supplied {@code name} is non-empty, throwing a
     * {@link BeansException} if the named {@code DataSource} does not exist.
     * <li>Attempt to look up the single {@code DataSource} by type.
     * <li>Attempt to look up the <em>primary</em> {@code DataSource} by type.
     * <li>Attempt to look up the {@code DataSource} by type and the {@linkplain #DEFAULT_DATA_SOURCE_NAME default data
     * source name}.
     * </ol>
     * 
     * @param testContext
     *            the test context for which the {@code DataSource} should be retrieved; never {@code null}
     * @param name
     *            the name of the {@code DataSource} to retrieve (may be {@code null} or <em>empty</em>)
     * @return the {@code DataSource} to use, or {@code null} if not found
     * @throws BeansException
     *             if an error occurs while retrieving an explicitly named {@code DataSource}
     */
    @Nullable
    ...
    /**
     * Retrieve the {@linkplain PlatformTransactionManager transaction manager} to use for the supplied
     * {@linkplain TestContext test context}.
     * <p>
     * The following algorithm is used to retrieve the transaction manager from the
     * {@link org.springframework.context.ApplicationContext ApplicationContext} of the supplied test context:
     * <ol>
     * <li>Look up the transaction manager by type and explicit name, if the supplied {@code name} is non-empty,
     * throwing a {@link BeansException} if the named transaction manager does not exist.
     ...
     * <li>Attempt to look up the transaction manager via a {@link TransactionManagementConfigurer}, if present.
     * <li>Attempt to look up the transaction manager by type and the {@linkplain #DEFAULT_TRANSACTION_MANAGER_NAME
     * default transaction manager name}.
     * </ol>
     * 
     * @param testContext
     *            the test context for which the transaction manager should be retrieved; never {@code null}
     * @param name
     *            the name of the transaction manager to retrieve (may be {@code null} or <em>empty</em>)
    ...
    /**
     * Create a delegating {@link TransactionAttribute} for the supplied target {@link TransactionAttribute} and
     * {@link TestContext}, using the names of the test class and test method to build the name of the transaction.
     * 
     * @param testContext
     *            the {@code TestContext} upon which to base the name
     * @param targetAttribute
     *            the {@code TransactionAttribute} to delegate to
