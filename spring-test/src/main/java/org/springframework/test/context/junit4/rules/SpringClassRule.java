...
/**
 * {@code SpringClassRule} is a custom JUnit {@link TestRule} that supports <em>class-level</em> features of the
 * <em>Spring TestContext Framework</em> in standard JUnit tests by means of the {@link TestContextManager} and
 ...
 *
 * <p>
 * In order to achieve the same functionality as the {@code SpringJUnit4ClassRunner}, however, a {@code SpringClassRule}
 * must be combined with a {@link SpringMethodRule}, since {@code SpringClassRule} only supports the class-level
 ...
 *
 * <p>
 * The following list constitutes all annotations currently supported directly or indirectly by {@code SpringClassRule}.
...
public class SpringClassRule implements TestRule {
     ...
     *
     * @param testClass
     *            the test class to be managed; never {@code null}
     */
    ...
    /**
     * Apply <em>class-level</em> features of the <em>Spring TestContext Framework</em> to the supplied {@code base}
     * statement.
     * <p>
     * Specifically, this method retrieves the {@link TestContextManager} used by this rule and its associated
     * {@link SpringMethodRule} and invokes the {@link TestContextManager#beforeTestClass() beforeTestClass()} and
     * {@link TestContextManager#afterTestClass() afterTestClass()} methods on the {@code TestContextManager}.
     * <p>
     * In addition, this method checks whether the test is enabled in the current execution environment. This prevents
     * classes with a non-matching {@code @IfProfileValue} annotation from running altogether, even skipping the
     * execution of {@code beforeTestClass()} methods in {@code TestExecutionListeners}.
     *
     * @param base
     *            the base {@code Statement} that this rule should be applied to
     * @param description
     *            a {@code Description} of the current test execution
     * @return a statement that wraps the supplied {@code base} with class-level features of the Spring TestContext
     *         Framework
