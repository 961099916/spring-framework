...
/**
 * {@code MergedSqlConfig} encapsulates the <em>merged</em> {@link SqlConfig @SqlConfig} attributes declared locally via
 * {@link Sql#config} and globally as a class-level annotation.
 *
 * <p>
...
class MergedSqlConfig {
    ...
    /**
     * Construct a {@code MergedSqlConfig} instance by merging the configuration from the supplied local (potentially
     * method-level) {@code @SqlConfig} annotation with class-level configuration discovered on the supplied
     * {@code testClass}.
     * <p>
     * Local configuration overrides class-level configuration.
     * <p>
     * If the test class is not annotated with {@code @SqlConfig}, no merging takes place and the local configuration is
     * used "as is".
     */
    MergedSqlConfig(SqlConfig localSqlConfig, Class<?> testClass) {
        ...
        this.transactionMode =
            getEnum(mergedAttributes, "transactionMode", TransactionMode.DEFAULT, TransactionMode.INFERRED);
        ...
        this.blockCommentEndDelimiter =
            getString(mergedAttributes, "blockCommentEndDelimiter", ScriptUtils.DEFAULT_BLOCK_COMMENT_END_DELIMITER);
    ...
    private static boolean isExplicitValue(@Nullable Object value) {
        return !(isEmptyString(value) || isEmptyArray(value) || value == TransactionMode.DEFAULT
    ...
    private AnnotationAttributes mergeAttributes(SqlConfig localSqlConfig, Class<?> testClass) {
        ...
        AnnotationAttributes globalAttributes =
            AnnotatedElementUtils.findMergedAnnotationAttributes(testClass, SqlConfig.class.getName(), false, false);
    ...
    public String toString() {
        return new ToStringCreator(this).append("dataSource", this.dataSource)
            .append("transactionManager", this.transactionManager).append("transactionMode", this.transactionMode)
            .append("encoding", this.encoding).append("separator", this.separator)
            .append("commentPrefixes", this.commentPrefixes)
            .append("blockCommentStartDelimiter", this.blockCommentStartDelimiter)
            .append("blockCommentEndDelimiter", this.blockCommentEndDelimiter).append("errorMode", this.errorMode)
            .toString();
    }
