...
/**
 * {@code TestExecutionListener} that provides support for executing SQL {@link Sql#scripts scripts} and inlined
 * {@link Sql#statements statements} configured via the {@link Sql @Sql} annotation.
 *
 * <p>
 * Scripts and inlined statements will be executed {@linkplain #beforeTestMethod(TestContext) before} or
 * {@linkplain #afterTestMethod(TestContext) after} execution of the corresponding {@linkplain java.lang.reflect.Method
 * test method}, depending on the configured value of the {@link Sql#executionPhase executionPhase} flag.
 *
 * <p>
 * Scripts and inlined statements will be executed without a transaction, within an existing Spring-managed transaction,
 * or within an isolated transaction, depending on the configured value of {@link SqlConfig#transactionMode} and the
 ...
 * <p>
 * A {@link PlatformTransactionManager} <em>and</em> a {@link DataSource}, just a {@link PlatformTransactionManager}, or
 * just a {@link DataSource} must be defined as beans in the Spring {@link ApplicationContext} for the corresponding
 * test. Consult the javadocs for {@link SqlConfig#transactionMode}, {@link SqlConfig#transactionManager},
 * {@link SqlConfig#dataSource}, {@link TestContextTransactionUtils#retrieveDataSource}, and
 * {@link TestContextTransactionUtils#retrieveTransactionManager} for details on permissible configuration
...
public class SqlScriptsTestExecutionListener extends AbstractTestExecutionListener {
    ...
    /**
     * Execute SQL scripts configured via {@link Sql @Sql} for the supplied {@link TestContext} <em>before</em> the
     * current test method.
     */
    @Override
    ...
    /**
     * Execute SQL scripts configured via {@link Sql @Sql} for the supplied {@link TestContext} <em>after</em> the
     * current test method.
     */
    @Override
    ...
    /**
     * Determine if method-level {@code @Sql} annotations should be merged with class-level {@code @Sql} annotations.
     */
     ...
     */
    private void executeSqlScripts(Set<Sql> sqlAnnotations, TestContext testContext, ExecutionPhase executionPhase,
        boolean classLevel) {
     ...
     * <p>
     * Special care must be taken in order to properly support the configured {@link SqlConfig#transactionMode}.
     * 
     * @param sql
     *            the {@code @Sql} annotation to parse
     * @param executionPhase
     *            the current execution phase
     * @param testContext
     *            the current {@code TestContext}
     * @param classLevel
     *            {@code true} if {@link Sql @Sql} was declared at the class level
     */
    private void executeSqlScripts(Sql sql, ExecutionPhase executionPhase, TestContext testContext,
        ...
        List<Resource> scriptResources =
            TestContextResourceUtils.convertToResourceList(testContext.getApplicationContext(), scripts);
        ...
        if (txMgr == null) {
            ...
            TransactionAttribute txAttr = TestContextTransactionUtils.createDelegatingTransactionAttribute(testContext,
