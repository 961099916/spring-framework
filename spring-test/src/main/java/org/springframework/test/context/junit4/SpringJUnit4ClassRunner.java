...
/**
 * {@code SpringJUnit4ClassRunner} is a custom extension of JUnit's {@link BlockJUnit4ClassRunner} which provides
 * functionality of the <em>Spring TestContext Framework</em> to standard JUnit tests by means of the
 * {@link TestContextManager} and associated support classes and annotations.
 *
 * <p>
 * To use this class, simply annotate a JUnit 4 based test class with {@code @RunWith(SpringJUnit4ClassRunner.class)} or
 * {@code @RunWith(SpringRunner.class)}.
 *
 * <p>
 * The following list constitutes all annotations currently supported directly or indirectly by
 * {@code SpringJUnit4ClassRunner}. <em>(Note that additional annotations may be supported by various
 * {@link org.springframework.test.context.TestExecutionListener TestExecutionListener} or
 * {@link org.springframework.test.context.TestContextBootstrapper TestContextBootstrapper} implementations.)</em>
 *
 * <ul>
...
public class SpringJUnit4ClassRunner extends BlockJUnit4ClassRunner {
    ...
    static {
        Assert.state(
            ClassUtils.isPresent("org.junit.internal.Throwables", SpringJUnit4ClassRunner.class.getClassLoader()),
            "SpringJUnit4ClassRunner requires JUnit 4.12 or higher.");

        Method method = ReflectionUtils.findMethod(SpringJUnit4ClassRunner.class, "withRules", FrameworkMethod.class,
    ...
    /**
     * Construct a new {@code SpringJUnit4ClassRunner} and initialize a {@link TestContextManager} to provide Spring
    ...
    private static void ensureSpringRulesAreNotPresent(Class<?> testClass) {
        for (Field field : testClass.getFields()) {
            Assert
                .state(!SpringClassRule.class.isAssignableFrom(field.getType()),
                    () -> String.format(
     ...
     * <p>
     * This prevents classes with a non-matching {@code @IfProfileValue} annotation from running altogether, even
     * skipping the execution of {@code prepareTestInstance()} methods in {@code TestExecutionListeners}.
     * 
     * @see ProfileValueUtils#isTestEnabledInThisEnvironment(Class)
     ...
     * {@link #getTestContextManager() TestContextManager} to prepare the test instance before returning it.
     * 
    ...
    /**
     * Augment the default JUnit behavior {@linkplain #withPotentialRepeat with potential repeats} of the entire
     * execution chain.
     * <p>
     * Furthermore, support for timeouts has been moved down the execution chain in order to include execution of
     * {@link org.junit.Before @Before} and {@link org.junit.After @After} methods within the timed execution. Note that
     * this differs from the default JUnit behavior of executing {@code @Before} and {@code @After} methods in the main
     * thread while executing the actual test method in a separate thread. Thus, the net effect is that {@code @Before}
     * and {@code @After} methods will be executed in the same thread as the test method. As a consequence,
     * JUnit-specified timeouts will work fine in combination with Spring transactions. However, JUnit-specific timeouts
     * still differ from Spring-specific timeouts in that the former execute in a separate thread while the latter
     * simply execute in the main thread (like regular tests).
     * 
     ...
     * {@link BlockJUnit4ClassRunner#possiblyExpectingExceptions(FrameworkMethod, Object, Statement)} except that the
     * <em>expected exception</em> is retrieved using {@link #getExpectedException(FrameworkMethod)}.
     */
    @Override
    ...
    /**
     * Get the {@code exception} that the supplied {@linkplain FrameworkMethod test method} is expected to throw.
     * <p>
    ...
    /**
     * Perform the same logic as {@link BlockJUnit4ClassRunner#withPotentialTimeout(FrameworkMethod, Object, Statement)}
     * but with additional support for Spring's {@code @Timed} annotation.
     * <p>
     * Supports both Spring's {@link org.springframework.test.annotation.Timed @Timed} and JUnit's
     * {@link Test#timeout() @Test(timeout=...)} annotations, but not both simultaneously.
     * 
     * @return either a {@link SpringFailOnTimeout}, a {@link FailOnTimeout}, or the supplied {@link Statement} as
     *         appropriate
    ...
    /**
     * Retrieve the configured JUnit {@code timeout} from the {@link Test @Test} annotation on the supplied
     ...
     * {@link org.springframework.test.annotation.Timed @Timed} annotation on the supplied {@linkplain FrameworkMethod
     * test method}.
     * 
     * @return the timeout, or {@code 0} if none was specified
     ...
     * <p>
     * Supports Spring's {@link org.springframework.test.annotation.Repeat @Repeat} annotation.
     * 
