...
public class XmlExpectationsHelper {
    ...
    /**
     * Parse the expected and actual content strings as XML and assert that the two are "similar" -- i.e. they contain
     * the same elements and attributes regardless of order.
     * <p>
     * Use of this method assumes the <a href="https://github.com/xmlunit/xmlunit">XMLUnit</a> library is available.
     * 
     * @param expected
    ...
    private static class XmlUnitDiff {
        ...
        XmlUnitDiff(String expected, String actual) {
            this.diff = DiffBuilder.compare(expected).withTest(actual)
                .withNodeMatcher(new DefaultNodeMatcher(ElementSelectors.byNameAndText)).ignoreWhitespace()
