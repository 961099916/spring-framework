 ...
 * an annotation is declared, either directly or indirectly via a <em>composed annotation</em>. This additional
 * information is encapsulated in an {@link AnnotationDescriptor}.
 *
 * <p>
 * The additional information provided by an {@code AnnotationDescriptor} is required by the <em>Spring TestContext
 * Framework</em> in order to be able to support class hierarchy traversals for annotations such as
...
public abstract class MetaAnnotationUtils {
     ...
     * with the current annotation, interface, or superclass as the class to look for annotations on.
     * 
     * @param clazz
     ...
     */
    @Nullable
    private static <T extends Annotation> AnnotationDescriptor<T> findAnnotationDescriptor(@Nullable Class<?> clazz,
        ...
        for (Annotation composedAnn : clazz.getDeclaredAnnotations()) {
            ...
            if (!AnnotationUtils.isInJavaLangAnnotationPackage(composedType.getName()) && visited.add(composedAnn)) {
                ...
                if (descriptor != null) {
                    return new AnnotationDescriptor<>(clazz, descriptor.getDeclaringClass(), composedAnn,
     ...
     * <p>
     * This method traverses the annotations, interfaces, and superclasses of the specified {@code clazz} if no
     ...
     *            the types of annotations to look for
        ...
        for (Annotation composedAnnotation : clazz.getDeclaredAnnotations()) {
            if (!AnnotationUtils.isInJavaLangAnnotationPackage(composedAnnotation) && visited.add(composedAnnotation)) {
                UntypedAnnotationDescriptor descriptor =
                    findAnnotationDescriptorForTypes(composedAnnotation.annotationType(), visited, annotationTypes);
                if (descriptor != null) {
    ...
    public static class AnnotationDescriptor<T extends Annotation> {
        ...
        public AnnotationDescriptor(Class<?> rootDeclaringClass, Class<?> declaringClass,
            ...
            AnnotationAttributes attributes = AnnotatedElementUtils.findMergedAnnotationAttributes(rootDeclaringClass,
        ...
        /**
         * Synthesize the merged {@link #getAnnotationAttributes AnnotationAttributes} in this descriptor back into an
         * annotation of the target {@linkplain #getAnnotationType annotation type}.
         * 
         * @since 4.2
        ...
        public String toString() {
            return new ToStringCreator(this).append("rootDeclaringClass", this.rootDeclaringClass)
                .append("declaringClass", this.declaringClass).append("composedAnnotation", this.composedAnnotation)
                .append("annotation", this.annotation).toString();
        }
    }
    ...
    public static class UntypedAnnotationDescriptor extends AnnotationDescriptor<Annotation> {
        ...
        /**
         * Throws an {@link UnsupportedOperationException} since the type of annotation represented by the
         * {@link #getAnnotationAttributes AnnotationAttributes} in an {@code UntypedAnnotationDescriptor} is unknown.
         * 
         * @since 4.2
         */
        @Override
