 ...
 *
 * <p>
 * Based on the <a href="https://github.com/jayway/JsonPath">JsonPath</a> project: requiring version 0.9+, with 1.1+
...
public class JsonPathExpectationsHelper {
     ...
     * 
     * @param expression
     *            the {@link JsonPath} expression; never {@code null} or empty
     * @param args
     *            arguments to parameterize the {@code JsonPath} expression with, using formatting specifiers defined in
    ...
    /**
     * Evaluate the JSON path expression against the supplied {@code content} and assert the resulting value with the
     * given {@code Matcher}.
     * 
     * @param content
     *            the JSON content
    ...
    /**
     * An overloaded variant of {@link #assertValue(String, Matcher)} that also accepts a target type for the resulting
     * value. This can be useful for matching numbers reliably for example coercing an integer into a double.
     * 
     * @param content
     *            the JSON content
     * @param matcher
    ...
    /**
     * Evaluate the JSON path expression against the supplied {@code content} and assert that the result is equal to the
     * expected value.
     * 
     * @param content
     *            the JSON content
        ...
        if ((actualValue instanceof List) && !(expectedValue instanceof List)) {
            ...
            if (actualValueList.size() != 1) {
                AssertionErrors.fail(
                    "Got a list of values " + actualValue + " instead of the expected single value " + expectedValue);
            }
            ...
            try {
                ...
                String message =
                    String.format("At JSON path \"%s\", value <%s> of type <%s> cannot be converted to type <%s>",
    ...
    /**
     * Evaluate the JSON path expression against the supplied {@code content} and assert that the resulting value is a
     * {@link String}.
     * 
     * @param content
     *            the JSON content
    ...
    /**
     * Evaluate the JSON path expression against the supplied {@code content} and assert that the resulting value is an
     * array.
     * 
     * @param content
     *            the JSON content
    ...
    /**
     * Evaluate the JSON path expression against the supplied {@code content} and assert that a non-null value, possibly
     * an empty array or map, exists at the given path.
     * <p>
     * Note that if the JSON path expression is not {@linkplain JsonPath#isDefinite() definite}, this method asserts
    ...
    /**
     * Evaluate the JSON path expression against the supplied {@code content} and assert that a non-null value does not
    ...
    /**
     * Evaluate the JSON path expression against the supplied {@code content} and assert that an empty value exists at
     * the given path.
     * <p>
     * For the semantics of <em>empty</em>, consult the Javadoc for {@link ObjectUtils#isEmpty(Object)}.
     * 
     * @param content
     *            the JSON content
     */
    ...
    /**
     * Evaluate the JSON path expression against the supplied {@code content} and assert that a non-empty value exists
     * at the given path.
     * <p>
     * For the semantics of <em>empty</em>, consult the Javadoc for {@link ObjectUtils#isEmpty(Object)}.
     * 
    ...
    /**
     * Evaluate the JSON path expression against the supplied {@code content} and assert that a value, possibly
     * {@code null}, exists.
     * <p>
     * If the JSON path expression is not {@linkplain JsonPath#isDefinite() definite}, this method asserts that the list
     * of values at the given path is not <em>empty</em>.
     * 
     * @param content
     *            the JSON content
     * @since 5.0.3
    ...
    /**
     * Evaluate the JSON path expression against the supplied {@code content} and assert that a value, including
     * {@code null} values, does not exist at the given path.
     * <p>
     * If the JSON path expression is not {@linkplain JsonPath#isDefinite() definite}, this method asserts that the list
     * of values at the given path is <em>empty</em>.
     * 
     * @param content
     *            the JSON content
     * @since 5.0.3
     ...
     * 
     * @param content
     *            the content to evaluate against
