...
/**
 * Servlet Filter that binds a JPA EntityManager to the thread for the entire processing of the request. Intended for
 * the "Open EntityManager in View" pattern, i.e. to allow for lazy loading in web views despite the original
 * transactions already being completed.
 *
 * <p>
 * This filter makes JPA EntityManagers available via the current thread, which will be autodetected by transaction
 * managers. It is suitable for service layer transactions via {@link org.springframework.orm.jpa.JpaTransactionManager}
 * or {@link org.springframework.transaction.jta.JtaTransactionManager} as well as for non-transactional read-only
 * execution.
 *
 * <p>
 * Looks up the EntityManagerFactory in Spring's root web application context. Supports an
 * "entityManagerFactoryBeanName" filter init-param in {@code web.xml}; the default bean name is "entityManagerFactory".
 * As an alternative, the "persistenceUnitName" init-param allows for retrieval by logical unit name (as specified in
...
public class OpenEntityManagerInViewFilter extends OncePerRequestFilter {
    ...
    /**
     * Default EntityManagerFactory bean name: "entityManagerFactory". Only applies when no "persistenceUnitName" param
     * has been specified.
     * 
     ...
     * <p>
     * Default is "entityManagerFactory". Note that this default only applies when no "persistenceUnitName" param has
     * been specified.
     *
     * @see #setPersistenceUnitName
     ...
     * <p>
     * This is an alternative to specifying the EntityManagerFactory by bean name, resolving it by its persistence unit
     * name instead. If no bean name and no persistence unit name have been specified, we'll check whether a bean exists
     * for the default bean name "entityManagerFactory"; if not, a default EntityManagerFactory will be retrieved
    ...
    /**
     * Returns "false" so that the filter may re-bind the opened {@code EntityManager} to each asynchronously dispatched
     * thread and postpone closing it until the very last asynchronous dispatch.
     */
    @Override
    ...
    /**
     * Returns "false" so that the filter may provide an {@code EntityManager} to each error dispatches.
     */
    @Override
     ...
     * <p>
     * The default implementation delegates to the {@code lookupEntityManagerFactory} without arguments, caching the
