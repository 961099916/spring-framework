 ...
 * {@link javax.persistence.EntityManagerFactory} and {@link javax.persistence.EntityManager}. Any such annotated fields
 * or methods in any Spring-managed object will automatically be injected.
 *
 * <p>
 * This post-processor will inject sub-interfaces of {@code EntityManagerFactory} and {@code EntityManager} if the
 * annotated fields or methods are declared as such. The actual type will be verified early, with the exception of a
 * shared ("transactional") {@code EntityManager} reference, where type mismatches might be detected as late as on the
 * first actual invocation.
 *
 * <p>
 * Note: In the present implementation, PersistenceAnnotationBeanPostProcessor only supports {@code @PersistenceUnit}
 * and {@code @PersistenceContext} with the "unitName" attribute, or no attribute at all (for the default unit). If
 * those annotations are present with the "name" attribute at the class level, they will simply be ignored, since those
 * only serve as deployment hint (as per the Java EE specification).
 *
 * <p>
 * This post-processor can either obtain EntityManagerFactory beans defined in the Spring application context (the
 * default), or obtain EntityManagerFactory references from JNDI ("persistence unit references"). In the bean case, the
 * persistence unit name will be matched against the actual deployed unit, with the bean name used as fallback unit name
 * if no deployed name found. Typically, Spring's
 * {@link org.springframework.orm.jpa.LocalContainerEntityManagerFactoryBean} will be used for setting up such
 * EntityManagerFactory beans. Alternatively, such beans may also be obtained from JNDI, e.g. using the
 * {@code jee:jndi-lookup} XML configuration element (with the bean name matching the requested unit name). In both
 * cases, the post-processor definition will look as simple as this:
 *
 * <pre class="code">
 * &lt;bean class="org.springframework.orm.jpa.support.PersistenceAnnotationBeanPostProcessor"/&gt;
 * </pre>
 *
 * In the JNDI case, specify the corresponding JNDI names in this post-processor's {@link #setPersistenceUnits
 * "persistenceUnits" map}, typically with matching {@code persistence-unit-ref} entries in the Java EE deployment
 * descriptor. By default, those names are considered as resource references (according to the Java EE resource-ref
 * convention), located underneath the "java:comp/env/" namespace. For example:
 *
 * <pre class="code">
 * &lt;bean class="org.springframework.orm.jpa.support.PersistenceAnnotationBeanPostProcessor"&gt;
 ...
 * </pre>
 *
 * In this case, the specified persistence units will always be resolved in JNDI rather than as Spring-defined beans.
 * The entire persistence unit deployment, including the weaving of persistent classes, is then up to the Java EE
 * server. Persistence contexts (i.e. EntityManager references) will be built based on those server-provided
 * EntityManagerFactory references, using Spring's own transaction synchronization facilities for transactional
 * EntityManager handling (typically with Spring's {@code @Transactional} annotation for demarcation and
 ...
 * {@link #setPersistenceContexts "persistenceContexts" map} (or {@link #setExtendedPersistenceContexts
 * "extendedPersistenceContexts" map}, typically with matching {@code persistence-context-ref} entries in the Java EE
 ...
 * </pre>
 *
 * If the application only obtains EntityManager references in the first place, this is all you need to specify. If you
 * need EntityManagerFactory references as well, specify entries for both "persistenceUnits" and "persistenceContexts",
 * pointing to matching JNDI locations.
 *
 * <p>
 * <b>NOTE: In general, do not inject EXTENDED EntityManagers into STATELESS beans, i.e. do not use
 * {@code @PersistenceContext} with type {@code EXTENDED} in Spring beans defined with scope 'singleton' (Spring's
 * default scope).</b> Extended EntityManagers are <i>not</i> thread-safe, hence they must not be used in concurrently
 * accessed beans (which Spring-managed singletons usually are).
 *
 * <p>
 * Note: A default PersistenceAnnotationBeanPostProcessor will be registered by the "context:annotation-config" and
 * "context:component-scan" XML tags. Remove or turn off the default annotation configuration there if you intend to
 * specify a custom PersistenceAnnotationBeanPostProcessor bean definition.
 *
 * @author Rod Johnson
...
public class PersistenceAnnotationBeanPostProcessor
    ...
    /**
     * Set whether the lookup occurs in a Java EE container, i.e. if the prefix "java:comp/env/" needs to be added if
     * the JNDI name doesn't already contain it. PersistenceAnnotationBeanPostProcessor's default is "true".
     * 
    ...
    /**
     * Specify the persistence units for EntityManagerFactory lookups, as a Map from persistence unit name to
     * persistence unit JNDI name (which needs to resolve to an EntityManagerFactory instance).
     * <p>
     * JNDI names specified here should refer to {@code persistence-unit-ref} entries in the Java EE deployment
     * descriptor, matching the target persistence unit.
     * <p>
     * In case of no unit name specified in the annotation, the specified value for the
     * {@link #setDefaultPersistenceUnitName default persistence unit} will be taken (by default, the value mapped to
     * the empty String), or simply the single persistence unit if there is only one.
     * <p>
     * This is mainly intended for use in a Java EE environment, with all lookup driven by the standard JPA annotations,
     * and all EntityManagerFactory references obtained from JNDI. No separate EntityManagerFactory bean definitions are
     * necessary in such a scenario.
     * <p>
     * If no corresponding "persistenceContexts"/"extendedPersistenceContexts" are specified,
     * {@code @PersistenceContext} will be resolved to EntityManagers built on top of the EntityManagerFactory defined
     * here. Note that those will be Spring-managed EntityManagers, which implement transaction synchronization based on
     * Spring's facilities. If you prefer the Java EE server's own EntityManager handling, specify corresponding
    ...
    /**
     * Specify the <i>transactional</i> persistence contexts for EntityManager lookups, as a Map from persistence unit
     * name to persistence context JNDI name (which needs to resolve to an EntityManager instance).
     * <p>
     * JNDI names specified here should refer to {@code persistence-context-ref} entries in the Java EE deployment
     * descriptors, matching the target persistence unit and being set up with persistence context type
     * {@code Transaction}.
     * <p>
     * In case of no unit name specified in the annotation, the specified value for the
     * {@link #setDefaultPersistenceUnitName default persistence unit} will be taken (by default, the value mapped to
     ...
     * <p>
     * This is mainly intended for use in a Java EE environment, with all lookup driven by the standard JPA annotations,
     * and all EntityManager references obtained from JNDI. No separate EntityManagerFactory bean definitions are
     * necessary in such a scenario, and all EntityManager handling is done by the Java EE server itself.
     */
    ...
    /**
     * Specify the <i>extended</i> persistence contexts for EntityManager lookups, as a Map from persistence unit name
     * to persistence context JNDI name (which needs to resolve to an EntityManager instance).
     * <p>
     * JNDI names specified here should refer to {@code persistence-context-ref} entries in the Java EE deployment
     * descriptors, matching the target persistence unit and being set up with persistence context type
     * {@code Extended}.
     ...
     * <p>
     * This is mainly intended for lookups in the application context, indicating the target persistence unit name
     * (typically matching the bean name), but also applies to lookups in the {@link #setPersistenceUnits
     * "persistenceUnits"} / {@link #setPersistenceContexts "persistenceContexts"} /
     * {@link #setExtendedPersistenceContexts "extendedPersistenceContexts"} map, avoiding the need for duplicated
     * mappings for the empty String there.
     * <p>
     * Default is to check for a single EntityManagerFactory bean in the Spring application context, if any. If there
     * are multiple such factories, either specify this default persistence unit name or explicitly refer to named
     * persistence units in your annotations.
     */
    ...
    /**
     * Return a specified persistence context for the given unit name, as defined through the "persistenceContexts" (or
     * "extendedPersistenceContexts") map.
     * 
     * @param unitName
     *            the name of the persistence unit
     * @param extended
    ...
    /**
     * Find an EntityManagerFactory with the given name in the current Spring application context, falling back to a
     * single default EntityManagerFactory (if any) in case of no unit name specified.
     * 
     * @param unitName
     ...
     * <p>
     * Called for EntityManagerFactory and EntityManager lookup when JNDI names are mapped for specific persistence
     * units.
     * 
     * @param jndiName
    ...
    /**
     * Separate inner class to isolate the JNDI API dependency (for compatibility with Google App Engine's API white
     * list).
     */
