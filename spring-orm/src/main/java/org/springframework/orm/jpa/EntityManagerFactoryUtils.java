...
/**
 * Helper class featuring methods for JPA EntityManager handling, allowing for reuse of EntityManager instances within
...
public abstract class EntityManagerFactoryUtils {
     ...
     * DataSourceUtils.CONNECTION_SYNCHRONIZATION_ORDER - 100 to execute EntityManager cleanup before JDBC Connection
     * cleanup, if any.
     * 
    ...
    /**
     * Find an EntityManagerFactory with the given name in the given Spring application context (represented as
     * ListableBeanFactory).
     * <p>
     * The specified unit name will be matched against the configured persistence unit, provided that a discovered
     * EntityManagerFactory implements the {@link EntityManagerFactoryInfo} interface. If not, the persistence unit name
     * will be matched against the Spring bean name, assuming that the EntityManagerFactory bean names follow that
     * convention.
     * <p>
     * If no unit name has been given, this method will search for a default EntityManagerFactory through
     * {@link ListableBeanFactory#getBean(Class)}.
     * 
     * @param beanFactory
     *            the ListableBeanFactory to search
     * @param unitName
     *            the name of the persistence unit (may be {@code null} or empty, in which case a single bean of type
     *            EntityManagerFactory will be searched for)
    ...
    /**
     * Obtain a JPA EntityManager from the given factory. Is aware of a corresponding EntityManager bound to the current
     * thread, e.g. when using JpaTransactionManager.
     * <p>
     ...
     *             if the EntityManager couldn't be obtained
     ...
     * 
     * @param em
     *            the EntityManager to prepare
     * @param emf
     *            the EntityManagerFactory that the EntityManager has been created with
     ...
     *            arbitrary object that holds transaction data, if any (as returned by prepareTransaction)
     * @param emf
     *            the EntityManagerFactory that the EntityManager has been created with
     ...
     * hierarchy. Return null if no translation is appropriate: any other exception may have resulted from user code,
     * and should not be translated.
     * <p>
     * The most important cases like object not found or optimistic locking failure are covered here. For more
     * fine-granular conversion, JpaTransactionManager etc support sophisticated translation of exceptions via a
     * JpaDialect.
     * 
     * @param ex
     ...
     * 
     * @param em
     *            the JPA EntityManager to close (may be {@code null})
    ...
    private static class TransactionalEntityManagerSynchronization
        ...
        public TransactionalEntityManagerSynchronization(EntityManagerHolder emHolder, EntityManagerFactory emf,
            ...
            this.jpaDialect =
                (emf instanceof EntityManagerFactoryInfo ? ((EntityManagerFactoryInfo)emf).getJpaDialect() : null);
