...
/**
 * Default implementation of the {@link PersistenceUnitManager} interface. Used as internal default by
 * {@link org.springframework.orm.jpa.LocalContainerEntityManagerFactoryBean}.
 *
 * <p>
 * Supports standard JPA scanning for {@code persistence.xml} files, with configurable file locations, JDBC DataSource
 * lookup and load-time weaving.
 *
 * <p>
 * The default XML file location is {@code classpath*:META-INF/persistence.xml}, scanning for all matching files in the
 * classpath (as defined in the JPA specification). DataSource names are by default interpreted as JNDI names, and no
 * load time weaving is available (which requires weaving to be turned off in the persistence provider).
 *
 * <p>
...
public class DefaultPersistenceUnitManager
    ...
    /**
     * Specify the location of the {@code persistence.xml} files to load. These can be specified as Spring resource
     * locations and/or location patterns.
     * <p>
    ...
    /**
     * Specify multiple locations of {@code persistence.xml} files to load. These can be specified as Spring resource
     * locations and/or location patterns.
     * <p>
     * Default is "classpath*:META-INF/persistence.xml".
     * 
     * @param persistenceXmlLocations
     *            an array of Spring resource Strings identifying the location of the {@code persistence.xml} files to
     *            read
     */
    ...
    /**
     * Set the default persistence unit root location, to be applied if no unit-specific persistence unit root could be
     * determined.
     * <p>
     * Default is "classpath:", that is, the root of the current classpath (nearest root directory). To be overridden if
     * unit-specific resolution does not work and the classpath root is not appropriate either.
     */
     ...
     * <p>
     * Primarily applied to a scanned persistence unit without {@code persistence.xml}. Also applicable to selecting a
     * default unit from several persistence units available.
     * 
     ...
     * <p>
     * Such package scanning defines a "default persistence unit" in Spring, which may live next to regularly defined
     * units originating from {@code persistence.xml}. Its name is determined by {@link #setDefaultPersistenceUnitName}:
     * by default, it's simply "default".
     * <p>
     * <b>Note: There may be limitations in comparison to regular JPA scanning.</b> In particular, JPA providers may
     * pick up annotated packages for provider-specific annotations only when driven by {@code persistence.xml}. As of
     * 4.1, Spring's scan can detect annotated packages as well if supported by the given
     * {@link org.springframework.orm.jpa.JpaVendorAdapter} (e.g. for Hibernate).
     * <p>
     * If no explicit {@link #setMappingResources mapping resources} have been specified in addition to these packages,
     * this manager looks for a default {@code META-INF/orm.xml} file in the classpath, registering it as a mapping
     * resource for the default unit if the mapping file is not co-located with a {@code persistence.xml} file (in which
     * case we assume it is only meant to be used with the persistence units defined there, like in standard JPA).
     * 
    ...
    /**
     * Specify one or more mapping resources (equivalent to {@code <mapping-file>} entries in {@code persistence.xml})
     * for the default persistence unit. Can be used on its own or in combination with entity scanning in the classpath,
     ...
     * <p>
     * If no explicit mapping resources have been specified next to {@link #setPackagesToScan packages to scan}, this
     * manager looks for a default {@code META-INF/orm.xml} file in the classpath, registering it as a mapping resource
     * for the default unit if the mapping file is not co-located with a {@code persistence.xml} file (in which case we
     * assume it is only meant to be used with the persistence units defined there, like in standard JPA).
     * <p>
     * Note that specifying an empty array/list here suppresses the default {@code META-INF/orm.xml} check. On the other
     * hand, explicitly specifying {@code META-INF/orm.xml} here will register that file even if it happens to be
    ...
    /**
     * Specify the JDBC DataSources that the JPA persistence provider is supposed to use for accessing the database,
     * resolving data source names in {@code persistence.xml} against Spring-managed DataSources.
     * <p>
     * The specified Map needs to define data source names for specific DataSource objects, matching the data source
     * names used in {@code persistence.xml}. If not specified, data source names will be resolved as JNDI names instead
    ...
    /**
     * Return the JDBC DataSourceLookup that provides DataSources for the persistence provider, resolving data source
     * names in {@code persistence.xml} against Spring-managed DataSource instances.
     */
    @Nullable
    ...
    /**
     * Specify the JDBC DataSourceLookup that provides DataSources for the persistence provider, resolving data source
     * names in {@code persistence.xml} against Spring-managed DataSource instances.
     * <p>
     * Default is JndiDataSourceLookup, which resolves DataSource names as JNDI names (as defined by standard JPA).
     * Specify a BeanFactoryDataSourceLookup instance if you want DataSource names to be resolved against Spring bean
     * names.
     * <p>
     * Alternatively, consider passing in a map from names to DataSource instances via the "dataSources" property. If
     * the {@code persistence.xml} file does not define DataSource names at all, specify a default DataSource via the
    ...
    /**
     * Return the JDBC DataSource that the JPA persistence provider is supposed to use for accessing the database if
     * none has been specified in {@code persistence.xml}.
     */
    @Nullable
    ...
    /**
     * Specify the JDBC DataSource that the JPA persistence provider is supposed to use for accessing the database if
     * none has been specified in {@code persistence.xml}. This variant indicates no special transaction setup, i.e.
     * typical resource-local.
     * <p>
     * In JPA speak, a DataSource passed in here will be uses as "nonJtaDataSource" on the PersistenceUnitInfo passed to
     * the PersistenceProvider, provided that none has been registered before.
     *
    ...
    /**
     * Return the JTA-aware DataSource that the JPA persistence provider is supposed to use for accessing the database
     * if none has been specified in {@code persistence.xml}.
     */
    @Nullable
    ...
    /**
     * Specify the JDBC DataSource that the JPA persistence provider is supposed to use for accessing the database if
     * none has been specified in {@code persistence.xml}. This variant indicates that JTA is supposed to be used as
     * transaction type.
     * <p>
     * In JPA speak, a DataSource passed in here will be uses as "jtaDataSource" on the PersistenceUnitInfo passed to
     * the PersistenceProvider, provided that none has been registered before.
     *
    ...
    /**
     * Return the PersistenceUnitPostProcessors to be applied to each PersistenceUnitInfo that has been parsed by this
     * manager.
     */
    @Nullable
    ...
    /**
     * Set the PersistenceUnitPostProcessors to be applied to each PersistenceUnitInfo that has been parsed by this
     * manager.
     * <p>
    ...
    /**
     * Return the Spring LoadTimeWeaver to use for class instrumentation according to the JPA class transformer
     * contract.
     */
    @Nullable
    ...
    /**
     * Specify the Spring LoadTimeWeaver to use for class instrumentation according to the JPA class transformer
     * contract.
     * <p>
     * It is not required to specify a LoadTimeWeaver: Most providers will be able to provide a subset of their
     * functionality without class instrumentation as well, or operate with their own VM agent specified on JVM startup.
     * Furthermore, DefaultPersistenceUnitManager falls back to an InstrumentationLoadTimeWeaver if Spring's agent-based
     * instrumentation is available at runtime.
     * <p>
     * In terms of Spring-provided weaving options, the most important ones are InstrumentationLoadTimeWeaver, which
     * requires a Spring-specific (but very general) VM agent specified on JVM startup, and ReflectiveLoadTimeWeaver,
     * which interacts with an underlying ClassLoader based on specific extended methods being available on it (for
     * example, interacting with Spring's TomcatInstrumentableClassLoader). Consider using the
     * {@code context:load-time-weaver} XML tag for creating such a shared LoadTimeWeaver (autodetecting the environment
     * by default).
     *
     ...
     * {@code persistence.xml} files, parsing all matching files, configuring and post-processing them.
     * <p>
     ...
     * <p>
     * Checks whether a "META-INF/orm.xml" file exists in the classpath and uses it if it is not co-located with a
    ...
    /**
     * Return the specified PersistenceUnitInfo from this manager's cache of processed persistence units, keeping it in
     * the cache (i.e. not 'obtaining' it for use but rather just accessing it for post-processing).
     * <p>
     * This can be used in {@link #postProcessPersistenceUnitInfo} implementations, detecting existing persistence units
     * of the same name and potentially merging them.
     * 
     ...
     * <p>
     * The default implementation delegates to all registered PersistenceUnitPostProcessors. It is usually preferable to
     * register further entity classes, jar files etc there rather than in a subclass of this manager, to be able to
     * reuse the post-processors.
     * 
     * @param pui
     *            the chosen PersistenceUnitInfo, as read from {@code persistence.xml}. Passed in as
