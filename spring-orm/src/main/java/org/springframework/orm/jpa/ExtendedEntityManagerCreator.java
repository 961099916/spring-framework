...
/**
 * Delegate for creating a variety of {@link javax.persistence.EntityManager} proxies that follow the JPA spec's
 * semantics for "extended" EntityManagers.
 *
 * <p>
 * Supports several different variants of "extended" EntityManagers: in particular, an "application-managed extended
 * EntityManager", as defined by {@link javax.persistence.EntityManagerFactory#createEntityManager()}, as well as a
 * "container-managed extended EntityManager", as defined by {@link javax.persistence.PersistenceContextType#EXTENDED}.
 *
 * <p>
 * The original difference between "application-managed" and "container-managed" was the need for explicit joining of an
 * externally managed transaction through the {@link EntityManager#joinTransaction()} method in the "application" case
 * versus the automatic joining on each user-level EntityManager operation in the "container" case. As of JPA 2.1, both
 * join modes are available with both kinds of EntityManagers, so the difference between "application-" and
 * "container-managed" is now primarily in the join mode default and in the restricted lifecycle of a container-managed
 * EntityManager (i.e. tied to the object that it is injected into).
 *
...
public abstract class ExtendedEntityManagerCreator {
     ...
     * 
     * @param rawEntityManager
     *            the raw EntityManager to decorate
     * @param emfInfo
     *            the EntityManagerFactoryInfo to obtain the JpaDialect and PersistenceUnitInfo from
     * @return an application-managed EntityManager that can join transactions but does not participate in them
     *         automatically
     */
     ...
     *            whether to automatically join ongoing transactions (according to the JPA 2.1 SynchronizationType
     *            rules)
     * @return an application-managed EntityManager that can join transactions but does not participate in them
     *         automatically
     * @since 4.0
     ...
     * 
     * @param emf
     *            the EntityManagerFactory to create the EntityManager with. If this implements the
     *            EntityManagerFactoryInfo interface, the corresponding JpaDialect and PersistenceUnitInfo will be
     ...
     *            detected accordingly.
     * @param properties
     *            the properties to be passed into the {@code createEntityManager} call (may be {@code null})
     * @return a container-managed EntityManager that will automatically participate in any managed transaction
     ...
     *            whether to automatically join ongoing transactions (according to the JPA 2.1 SynchronizationType
     *            rules)
     * @return a container-managed EntityManager that expects container-driven lifecycle management but may opt out of
     ...
     *            whether to follow container-managed EntityManager or application-managed EntityManager semantics
     * @param synchronizedWithTransaction
     *            whether to automatically join ongoing transactions (according to the JPA 2.1 SynchronizationType
     *            rules)
     ...
     *            raw EntityManager
     * @param emIfc
     *            the (potentially vendor-specific) EntityManager interface to proxy, or {@code null} for default
     *            detection of all interfaces
     * @param cl
     *            the ClassLoader to use for proxy creation (maybe {@code null})
     * @param exceptionTranslator
     *            the PersistenceException translator to use
     * @param jta
     *            whether to create a JTA-aware EntityManager (or {@code null} if not known in advance)
     * @param containerManaged
     *            whether to follow container-managed EntityManager or application-managed EntityManager semantics
     * @param synchronizedWithTransaction
     *            whether to automatically join ongoing transactions (according to the JPA 2.1 SynchronizationType
    ...
    private static EntityManager createProxy(EntityManager rawEm, @Nullable Class<? extends EntityManager> emIfc,
        @Nullable ClassLoader cl, @Nullable PersistenceExceptionTranslator exceptionTranslator, @Nullable Boolean jta,
        boolean containerManaged, boolean synchronizedWithTransaction) {
    ...
    private static final class ExtendedEntityManagerInvocationHandler implements InvocationHandler, Serializable {
        ...
        public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
            ...
            if (method.getName().equals("equals")) {
                ...
                ExtendedEntityManagerSynchronization synch =
                    (ExtendedEntityManagerSynchronization)TransactionSynchronizationManager.getResource(this.target);
                if (synch != null) {
         ...
         * 
         * @param enforce
         *            whether to enforce the transaction (i.e. whether failure to join is considered fatal)
         */
    ...
    private static class ExtendedEntityManagerSynchronization
        ...
        private RuntimeException convertException(RuntimeException ex) {
            DataAccessException dae =
                (this.exceptionTranslator != null) ? this.exceptionTranslator.translateExceptionIfPossible(ex)
