...
/**
 * {@link org.springframework.orm.jpa.JpaDialect} implementation for Hibernate EntityManager. Developed against
 * Hibernate 5.1/5.2/5.3/5.4.
 *
...
public class HibernateJpaDialect extends DefaultJpaDialect {
     ...
     * Connection.
     * <p>
     * Default is "true". If you turn this flag off, JPA transaction management will not support per-transaction
     * isolation levels anymore. It will not call {@code Connection.setReadOnly(true)} for read-only transactions
     * anymore either. If this flag is turned off, no cleanup of a JDBC Connection is required after a transaction,
     * since no Connection settings will get modified.
     * <p>
     * <b>NOTE:</b> The default behavior in terms of read-only handling changed in Spring 4.1, propagating the read-only
     * status to the JDBC Connection now, analogous to other Spring transaction managers. This may have the effect that
     * you're running into read-only enforcement now where previously write access has accidentally been tolerated:
     * Please revise your transaction declarations accordingly, removing invalid read-only markers if necessary.
     * 
     * @since 4.1
     ...
     * <p>
     * Applied to any detected {@link java.sql.SQLException} root cause of a Hibernate {@link JDBCException}, overriding
     * Hibernate's own {@code SQLException} translation (which is based on a Hibernate Dialect for a specific target
     * database).
     * 
     * @since 5.1
    ...
    protected DataAccessException convertHibernateAccessException(HibernateException ex) {
        if (this.jdbcExceptionTranslator != null && ex instanceof JDBCException) {
            ...
            DataAccessException dae = this.jdbcExceptionTranslator
                .translate("Hibernate operation: " + jdbcEx.getMessage(), jdbcEx.getSQL(), jdbcEx.getSQLException());
            if (dae != null) {
                throw dae;
            }
        }
