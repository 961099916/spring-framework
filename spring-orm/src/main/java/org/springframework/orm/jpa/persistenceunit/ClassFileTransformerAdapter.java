...
/**
 * Simple adapter that implements the {@code java.lang.instrument.ClassFileTransformer} interface based on a JPA
 * {@code ClassTransformer} which a JPA PersistenceProvider asks the {@code PersistenceUnitInfo} to install in the
 * current runtime.
 *
 * @author Rod Johnson
            ...
            try {
                ...
                if (transformed != null && logger.isDebugEnabled()) {
                    logger.debug("Transformer of class [" + this.classTransformer.getClass().getName()
                        + "] transformed class [" + className + "]; bytes in=" + classfileBuffer.length + "; bytes out="
                        + transformed.length);
                }
                return transformed;
