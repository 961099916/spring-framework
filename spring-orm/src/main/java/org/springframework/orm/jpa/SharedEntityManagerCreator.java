...
/**
 * Delegate for creating a shareable JPA {@link javax.persistence.EntityManager} reference for a given
 ...
 * as defined by the JPA specification. It will delegate all calls to the current transactional EntityManager, if any;
 ...
 * {@link javax.persistence.PersistenceContextType#TRANSACTION} and its discussion in the JPA spec document. This is
 * also the default being used for the annotation-based {@link javax.persistence.PersistenceContext#type()}.
 *
...
public abstract class SharedEntityManagerCreator {
     ...
     * 
     * @param emf
     *            the EntityManagerFactory to obtain EntityManagers from as needed
     * @param properties
     *            the properties to be passed into the {@code createEntityManager} call (may be {@code null})
     * @param entityManagerInterfaces
     *            the interfaces to be implemented by the EntityManager. Allows the addition or specification of
    ...
    /**
     * Invocation handler that delegates all calls to the current transactional EntityManager, if any; else, it will
     * fall back to a newly created EntityManager per operation.
     */
    ...
    private static class SharedEntityManagerInvocationHandler implements InvocationHandler, Serializable {
        ...
        public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
            ...
            if (target == null) {
                ...
                target =
                    (!CollectionUtils.isEmpty(this.properties) ? this.targetFactory.createEntityManager(this.properties)
    ...
    /**
     * Invocation handler that handles deferred Query objects created by non-transactional createQuery invocations on a
     * shared EntityManager.
     * <p>
     * Includes deferred output parameter access for JPA 2.1 StoredProcedureQuery, retrieving the corresponding values
     * for all registered parameters on query termination and returning the locally cached values for subsequent access.
     */
