 ...
 * {@link javax.persistence.EntityManagerFactory} instance within a Spring application context.
 *
 * <p>
 * Encapsulates the common functionality between the different JPA bootstrap contracts (standalone as well as
 * container).
 *
 * <p>
 ...
 *
 * <p>
 * This class also implements the {@link org.springframework.dao.support.PersistenceExceptionTranslator} interface, as
 * autodetected by Spring's {@link org.springframework.dao.annotation.PersistenceExceptionTranslationPostProcessor}, for
 * AOP-based translation of native exceptions to Spring DataAccessExceptions. Hence, the presence of e.g.
 * LocalEntityManagerFactoryBean automatically enables a PersistenceExceptionTranslationPostProcessor to translate JPA
 * exceptions.
 *
...
public abstract class AbstractEntityManagerFactoryBean
    ...
    /**
     * Set the PersistenceProvider implementation class to use for creating the EntityManagerFactory. If not specified,
     * the persistence provider will be taken from the JpaVendorAdapter (if any) or retrieved through scanning (as far
     * as possible).
     * 
     ...
     * <p>
     * {@code EntityManagerFactory} initialization will then switch into background bootstrap mode, with a
     * {@code EntityManagerFactory} proxy immediately returned for injection purposes instead of waiting for the JPA
     * provider's bootstrapping to complete. However, note that the first actual call to a {@code EntityManagerFactory}
     * method will then block until the JPA provider's bootstrapping completed, if not ready by then. For maximum
     * benefit, make sure to avoid early {@code EntityManagerFactory} calls in init methods of related beans, even for
    ...
    public void afterPropertiesSet() throws PersistenceException {
        ...
        if (jpaVendorAdapter != null) {
            ...
            Map<String, ?> vendorPropertyMap =
                (pui != null ? jpaVendorAdapter.getJpaPropertyMap(pui) : jpaVendorAdapter.getJpaPropertyMap());
    ...
    /**
     * Create a proxy for the given {@link EntityManagerFactory}. We do this to be able to return a transaction-aware
     * proxy for an application-managed {@link EntityManager}.
     * 
     * @param emf
     *            the EntityManagerFactory as returned by the persistence provider, if initialized already
        ...
        try {
            ...
            if (entityManagerFactoryInterface != null) {
                throw new IllegalStateException(
                    "EntityManagerFactory interface [" + entityManagerFactoryInterface
    ...
    /**
     * Delegate an incoming invocation from the proxy, dispatching to EntityManagerFactoryInfo or the native
     * EntityManagerFactory accordingly.
     */
    Object invokeProxyMethod(Method method, @Nullable Object[] args) throws Throwable {
        if (method.getDeclaringClass().isAssignableFrom(EntityManagerFactoryInfo.class)) {
            ...
            EntityManager rawEntityManager =
                (args.length > 1 ? getNativeEntityManagerFactory().createEntityManager((Map<?, ?>)args[1])
    ...
    /**
     * Minimal bean reference to the surrounding AbstractEntityManagerFactoryBean. Resolved to the actual
     * AbstractEntityManagerFactoryBean instance on deserialization.
     */
    @SuppressWarnings("serial")
    ...
    /**
     * Dynamic proxy invocation handler for an {@link EntityManagerFactory}, returning a proxy {@link EntityManager} (if
     * necessary) from {@code createEntityManager} methods.
     */
    @SuppressWarnings("serial")
