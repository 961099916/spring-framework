...
/**
 * Interface that specifies a common set of Hibernate operations as well as a general {@link #execute} method for
 * Session-based lambda expressions. Implemented by {@link HibernateTemplate}. Not often used, but a useful option to
 * enhance testability, as it can easily be mocked or stubbed.
 *
 * <p>
 * Defines {@code HibernateTemplate}'s data access methods that mirror various {@link org.hibernate.Session} methods.
 * Users are strongly encouraged to read the Hibernate {@code Session} javadocs for details on the semantics of those
 * methods.
 *
 * <p>
 * <b>A deprecation note:</b> While {@link HibernateTemplate} and this operations interface are being kept around for
 * backwards compatibility in terms of the data access implementation style in Spring applications, we strongly
 * recommend the use of native {@link org.hibernate.Session} access code for non-trivial interactions. This in
 * particular affects parameterized queries where - on Java 8+ - a custom {@link HibernateCallback} lambda code block
 * with {@code createQuery} and several {@code setParameter} calls on the {@link org.hibernate.query.Query} interface is
 * an elegant solution, to be executed via the general {@link #execute} method. All such operations which benefit from a
 * lambda variant have been marked as {@code deprecated} on this interface.
 *
 * <p>
 * <b>A Hibernate compatibility note:</b> {@link HibernateTemplate} and the operations on this interface generally aim
 * to be applicable across all Hibernate versions. In terms of binary compatibility, Spring ships a variant for each
 * major generation of Hibernate (in the present case: Hibernate ORM 5.x). However, due to refactorings and removals in
 * Hibernate ORM 5.3, some variants - in particular legacy positional parameters starting from index 0 - do not work
 * anymore. All affected operations are marked as deprecated; please replace them with the general {@link #execute}
 * method and custom lambda blocks creating the queries, ideally setting named parameters through
 * {@link org.hibernate.query.Query}. <b>Please be aware that deprecated operations are known to work with Hibernate ORM
 * 5.0-5.2 but may not work with Hibernate ORM 5.3 and higher anymore.</b>
 *
...
public interface HibernateOperations {
     ...
     * Hibernate exceptions are transformed into appropriate DAO ones. Allows for returning a result object, that is a
     * domain object or a collection of domain objects.
     * <p>
     * Note: Callback code is not supposed to handle transactions itself! Use an appropriate transaction manager like
     * {@link HibernateTransactionManager}. Generally, callback code must not touch any {@code Session} lifecycle
     * methods, like close, disconnect, or reconnect, to let the template do its work.
     * 
     * @param action
     ...
     * <p>
     * This method is a thin wrapper around {@link org.hibernate.Session#get(Class, Serializable)} for convenience. For
     * an explanation of the exact semantics of this method, please do refer to the Hibernate API documentation in the
     * first instance.
     * 
     * @param entityClass
     *            a persistent class
     ...
     * <p>
     * This method is a thin wrapper around {@link org.hibernate.Session#get(Class, Serializable, LockMode)} for
     ...
     * Return the persistent instance of the given entity class with the given identifier, or {@code null} if not found.
     * <p>
     * This method is a thin wrapper around {@link org.hibernate.Session#get(String, Serializable)} for convenience. For
     * an explanation of the exact semantics of this method, please do refer to the Hibernate API documentation in the
     * first instance.
     * 
     * @param entityName
     ...
     * Obtains the specified lock mode if the instance exists.
     * <p>
     * This method is a thin wrapper around {@link org.hibernate.Session#get(String, Serializable, LockMode)} for
     * convenience. For an explanation of the exact semantics of this method, please do refer to the Hibernate API
     * documentation in the first instance.
     * 
     * @param entityName
     ...
     * found.
     * <p>
     * This method is a thin wrapper around {@link org.hibernate.Session#load(Class, Serializable)} for convenience. For
     * an explanation of the exact semantics of this method, please do refer to the Hibernate API documentation in the
     * first instance.
     * 
     * @param entityClass
    ...
    /**
     * Return the persistent instance of the given entity class with the given identifier, throwing an exception if not
     * found. Obtains the specified lock mode if the instance exists.
     * <p>
     * This method is a thin wrapper around {@link org.hibernate.Session#load(Class, Serializable, LockMode)} for
     * convenience. For an explanation of the exact semantics of this method, please do refer to the Hibernate API
     * documentation in the first instance.
     * 
     * @param entityClass
     ...
     * Obtains the specified lock mode if the instance exists.
     * <p>
     * This method is a thin wrapper around {@link org.hibernate.Session#load(String, Serializable, LockMode)} for
     * convenience. For an explanation of the exact semantics of this method, please do refer to the Hibernate API
     * documentation in the first instance.
     * 
     * @param entityName
    ...
    /**
     * Return all persistent instances of the given entity class. Note: Use queries or criteria for retrieving a
     * specific subset.
     * 
     * @param entityClass
     *            a persistent class
     ...
     * @throws DataAccessException
     *             if we can't initialize the proxy, for example because it is not associated with an active Session
     * @see org.hibernate.Hibernate#initialize
     */
    void initialize(Object proxy) throws DataAccessException;

    /**
     * Return an enabled Hibernate {@link Filter} for the given filter name. The returned {@code Filter} instance can be
     ...
     *             if we are not running within a transactional Session (in which case this operation does not make
     *             sense)
     */
    ...
    /**
     * Obtain the specified lock level upon the given object, implicitly checking whether the corresponding database
     * entry still exists.
     * 
     * @param entity
     ...
     * Update the given persistent instance, associating it with the current Hibernate {@link org.hibernate.Session}.
     * <p>
     * Obtains the specified lock mode if the instance exists, implicitly checking whether the corresponding database
     * entry still exists.
     * 
     * @param entity
     *            the persistent instance to update
     ...
     * 
     * @param entity
     *            the persistent instance to save or update (to be associated with the Hibernate {@code Session})
     * @throws DataAccessException
     *             in case of Hibernate errors
     ...
     * semantics.
     * <p>
     * Similar to {@code saveOrUpdate}, but never associates the given object with the current Hibernate Session. In
     * case of a new entity, the state will be copied over as well.
     * <p>
     * Note that {@code merge} will <i>not</i> update the identifiers in the passed-in object graph (in contrast to
     * TopLink)! Consider registering Spring's {@code IdTransferringMergeEventListener} if you would like to have newly
     ...
     * semantics.
     * <p>
     * Similar to {@code saveOrUpdate}, but never associates the given object with the current Hibernate
     * {@link org.hibernate.Session}. In the case of a new entity, the state will be copied over as well.
     * <p>
     * Note that {@code merge} will <i>not</i> update the identifiers in the passed-in object graph (in contrast to
     * TopLink)! Consider registering Spring's {@code IdTransferringMergeEventListener} if you would like to have newly
     * assigned ids transferred to the original object graph too.
     ...
     * same transaction. Else, it is preferable to rely on auto-flushing at transaction completion.
     * 
     * @throws DataAccessException
     *             in case of Hibernate errors
    ...
    /**
     * Remove all objects from the {@link org.hibernate.Session} cache, and cancel all pending saves, updates and
     * deletes.
     * 
     * @throws DataAccessException
     *             in case of Hibernate errors
     ...
     * 
     * @param criteria
     *            the detached Hibernate criteria object. <b>Note: Do not reuse criteria objects! They need to recreated
     *            per execution, due to the suboptimal design of Hibernate's criteria facility.</b>
     * @return a {@link List} containing 0 or more persistent instances
     * @throws DataAccessException
     *             in case of Hibernate errors
     ...
     * 
     * @param exampleEntity
     *            an instance of the desired entity, serving as example for "query-by-example"
     * @return a {@link List} containing 0 or more persistent instances
     * @throws DataAccessException
     *             in case of Hibernate errors
     ...
     * @deprecated as of 5.0.4, in favor of a custom {@link HibernateCallback} lambda code block passed to the general
     *             {@link #execute} method
     */
    @Deprecated
     ...
     * <p>
     * Returns the results as an {@link Iterator}. Entities returned are initialized on demand. See the Hibernate API
    ...
    /**
     * Immediately close an {@link Iterator} created by any of the various {@code iterate(..)} operations, instead of
     * waiting until the session is closed or disconnected.
     * 
     * @param it
    ...
    /**
     * Update/delete all objects according to the given query, binding a number of values to "?" parameters in the query
     * string.
     * 
     * @param queryString
