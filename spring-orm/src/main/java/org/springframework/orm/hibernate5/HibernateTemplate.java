...
/**
 * Helper class that simplifies Hibernate data access code. Automatically converts HibernateExceptions into
 * DataAccessExceptions, following the {@code org.springframework.dao} exception hierarchy.
 *
 * <p>
 * The central method is {@code execute}, supporting Hibernate access code implementing the {@link HibernateCallback}
 * interface. It provides Hibernate Session handling such that neither the HibernateCallback implementation nor the
 * calling code needs to explicitly care about retrieving/closing Hibernate Sessions, or handling Session lifecycle
 * exceptions. For typical single step actions, there are various convenience methods (find, load, saveOrUpdate,
 * delete).
 *
 * <p>
 * Can be used within a service implementation via direct instantiation with a SessionFactory reference, or get prepared
 * in an application context and given to services as bean reference. Note: The SessionFactory should always be
 * configured as bean in the application context, in the first case given to the service directly, in the second case to
 * the prepared template.
 *
 * <p>
 * <b>NOTE: Hibernate access code can also be coded against the native Hibernate {@link Session}. Hence, for newly
 * started projects, consider adopting the standard Hibernate style of coding against
 * {@link SessionFactory#getCurrentSession()}. Alternatively, use {@link #execute(HibernateCallback)} with Java 8 lambda
 * code blocks against the callback-provided {@code Session} which results in elegant code as well, decoupled from the
 * Hibernate Session lifecycle. The remaining operations on this HibernateTemplate are deprecated in the meantime and
 * primarily exist as a migration helper for older Hibernate 3.x/4.x data access code in existing applications.</b>
 *
...
public class HibernateTemplate implements HibernateOperations, InitializingBean {
     ...
     * <p>
     * Each of those filters will be enabled at the beginning of each operation and correspondingly disabled at the end
     * of the operation. This will work for newly opened Sessions as well as for existing Sessions (for example, within
     * a transaction).
     *
     ...
     * <p>
     * Default is "false": a Session proxy will be returned, suppressing {@code close} calls and automatically applying
    ...
    /**
     * Return whether to check that the Hibernate Session is not in read-only mode in case of write operations
     * (save/update/delete).
     */
    ...
    /**
     * Set whether to check that the Hibernate Session is not in read-only mode in case of write operations
     * (save/update/delete).
     * <p>
     * Default is "true", for fail-fast behavior when attempting write operations within a read-only transaction. Turn
     * this off to allow save/update/delete on a Session with flush mode MANUAL.
     *
     ...
     * <p>
     * This execute variant overrides the template-wide {@link #isExposeNativeSession() "exposeNativeSession"} setting.
     *
     * @param action
    ...
    /**
     * Create a close-suppressing proxy for the given Hibernate Session. The proxy also prepares returned Query and
     * Criteria objects.
     *
     * @param session
    ...
    public List<?> find(final String queryString, @Nullable final Object... values) throws DataAccessException {
        return nonNull(executeWithNativeSession((HibernateCallback<List<?>>)session -> {
            org.hibernate.Query queryObject =
                queryObject(ReflectionUtils.invokeMethod(createQueryMethod, session, queryString));
    ...
    public List<?> findByNamedQuery(final String queryName, @Nullable final Object... values)
        ...
        return nonNull(executeWithNativeSession((HibernateCallback<List<?>>)session -> {
            org.hibernate.Query queryObject =
                queryObject(ReflectionUtils.invokeMethod(getNamedQueryMethod, session, queryName));
            prepareQuery(queryObject);
            if (values != null) {
                for (int i = 0; i < values.length; i++) {
                    queryObject.setParameter(i, values[i]);
    ...
    public List<?> findByNamedQueryAndNamedParam(final String queryName, @Nullable final String[] paramNames,
        ...
        return nonNull(executeWithNativeSession((HibernateCallback<List<?>>)session -> {
            org.hibernate.Query queryObject =
                (org.hibernate.Query)nonNull(ReflectionUtils.invokeMethod(getNamedQueryMethod, session, queryName));
            prepareQuery(queryObject);
            if (values != null) {
                for (int i = 0; i < values.length; i++) {
                    applyNamedParameterToQuery(queryObject, paramNames[i], values[i]);
     ...
     * <p>
     * Default implementation throws an InvalidDataAccessApiUsageException in case of {@code FlushMode.MANUAL}. Can be
    ...
    /**
     * Invocation handler that suppresses close calls on Hibernate Sessions. Also prepares returned Query and Criteria
     * objects.
     * 
     * @see Session#close
     */
