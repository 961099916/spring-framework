...
/**
 * A Spring-provided extension of the standard Hibernate {@link Configuration} class, adding
 * {@link SpringSessionContext} as a default and providing convenient ways to specify a JDBC {@link DataSource} and an
 * application class loader.
 *
 * <p>
 * This is designed for programmatic use, e.g. in {@code @Bean} factory methods; consider using
 * {@link LocalSessionFactoryBean} for XML bean definition files. Typically combined with
 * {@link HibernateTransactionManager} for declarative transactions against the {@code SessionFactory} and its JDBC
 * {@code DataSource}.
 *
 * <p>
 * Compatible with Hibernate 5.0/5.1 as well as 5.2/5.3/5.4, as of Spring 5.2. Set up with Hibernate 5.2+, this builder
 * is also a convenient way to set up a JPA {@code EntityManagerFactory} since the Hibernate {@code SessionFactory}
 * natively exposes the JPA {@code EntityManagerFactory} interface as well now.
 *
 * <p>
 * This builder supports Hibernate 5.3/5.4 {@code BeanContainer} integration, {@link MetadataSources} from custom
 * {@link BootstrapServiceRegistryBuilder} setup, as well as other advanced Hibernate configuration options beyond the
...
public class LocalSessionFactoryBuilder extends Configuration {
    ...
    private static final TypeFilter[] DEFAULT_ENTITY_TYPE_FILTERS =
        new TypeFilter[] {new AnnotationTypeFilter(Entity.class, false),
     ...
     * 
     * @param dataSource
     *            the JDBC DataSource that the resulting Hibernate SessionFactory should be using (may be {@code null})
     */
     ...
     *            the Hibernate MetadataSources service to use (e.g. reusing an existing one)
     * @since 4.3
     */
    public LocalSessionFactoryBuilder(@Nullable DataSource dataSource, ResourceLoader resourceLoader,
    ...
    /**
     * Set the Spring {@link JtaTransactionManager} or the JTA {@link TransactionManager} to be used with Hibernate, if
     * any. Allows for using a Spring-managed transaction manager for Hibernate 5's session and cache synchronization,
     * with the "hibernate.transaction.jta.platform" automatically set to it.
     * <p>
     * A passed-in Spring {@link JtaTransactionManager} needs to contain a JTA {@link TransactionManager} reference to
     * be usable here, except for the WebSphere case where we'll automatically set {@code WebSphereExtendedJtaPlatform}
     * accordingly.
     * <p>
     * Note: If this is set, the Hibernate settings should not contain a JTA platform setting to avoid meaningless
    ...
    /**
     * Set a Hibernate {@link org.hibernate.resource.beans.container.spi.BeanContainer} for the given Spring
     * {@link ConfigurableListableBeanFactory}.
     * <p>
     * Note: Bean container integration requires Hibernate 5.3 or higher. It enables autowiring of Hibernate attribute
     * converters and entity listeners.
     * 
     * @since 5.1
    ...
    /**
     * Build the Hibernate {@code SessionFactory} through background bootstrapping, using the given executor for a
     * parallel initialization phase (e.g. a {@link org.springframework.core.task.SimpleAsyncTaskExecutor}).
     * <p>
    ...
    /**
     * Proxy invocation handler for background bootstrapping, only enforcing a fully initialized target
     * {@code SessionFactory} when actually needed.
     * 
     * @since 4.3
     */
