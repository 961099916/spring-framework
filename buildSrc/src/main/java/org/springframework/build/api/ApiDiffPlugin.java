...
/**
 * {@link Plugin} that applies the {@code "japicmp-gradle-plugin"} and create tasks for all subprojects, diffing the
 * public API one by one and creating the reports in {@code "build/reports/api-diff/$OLDVERSION_to_$NEWVERSION/"}.
 * <p>
 * {@code "./gradlew apiDiff -PbaselineVersion=5.1.0.RELEASE"} will output the reports for the API diff between the
 * baseline version and the current one for all modules. You can limit the report to a single module with
...
public class ApiDiffPlugin implements Plugin<Project> {
    ...
    public void apply(Project project) {
        if (project.hasProperty(BASELINE_VERSION_PROPERTY) && project.equals(project.getRootProject())) {
            ...
            project.getPlugins().withType(JapicmpPlugin.class, plugin -> applyApiDiffConventions(project));
        }
    }
    ...
    private File getOutputFile(String baseLineVersion, Project project) {
        Path outDir = Paths.get(project.getRootProject().getBuildDir().getAbsolutePath(), "reports", "api-diff",
