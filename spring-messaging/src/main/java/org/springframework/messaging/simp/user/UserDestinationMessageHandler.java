 ...
 *
 * <p>
 * Listens for messages with "user" destinations, translates their destination to actual target destinations unique to
 * the active session(s) of a user, and then sends the resolved messages to the broker channel to be delivered.
 *
...
public class UserDestinationMessageHandler implements MessageHandler, SmartLifecycle {
    ...
    /**
     * Create an instance with the given client and broker channels subscribing to handle messages from each and then
     * sending any resolved messages to the broker channel.
     * 
    ...
    /**
     * Set a destination to broadcast messages to that remain unresolved because the user is not connected. In a
     * multi-application server scenario this gives other application servers a chance to try.
     * <p>
    ...
    public void setBroadcastDestination(@Nullable String destination) {
        this.broadcastHandler =
            (StringUtils.hasText(destination) ? new BroadcastHandler(this.messagingTemplate, destination) : null);
    }
    ...
    /**
     * Configure a custom {@link MessageHeaderInitializer} to initialize the headers of resolved target messages.
     * <p>
     * By default this is not set.
     */
    ...
    /**
     * A handler that broadcasts locally unresolved messages to the broker and also handles similar broadcasts received
     * from the broker.
     */
