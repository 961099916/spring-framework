...
/**
 * A registration class for customizing the configuration for a {@link org.springframework.messaging.MessageChannel}.
 *
...
public class ChannelRegistration {
     ...
     * 
     * @param taskExecutor
     *            the executor to use (or {@code null} for a default executor)
     */
    public TaskExecutorRegistration taskExecutor(@Nullable ThreadPoolTaskExecutor taskExecutor) {
        if (this.registration == null) {
            this.registration =
                (taskExecutor != null ? new TaskExecutorRegistration(taskExecutor) : new TaskExecutorRegistration());
        }
        return this.registration;
    }

    /**
     * Configure the given interceptors for this message channel, adding them to the channel's current list of
     * interceptors.
     * 
     * @since 4.3.12
     */
