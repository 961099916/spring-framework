...
/**
 * {@code SimpUserRegistry} that looks up users in a "local" user registry as well as a set of "remote" user registries.
 * The local registry is provided as a constructor argument while remote registries are updated via broadcasts handled
 * by {@link UserRegistryMessageHandler} which in turn notifies this registry when updates are received.
 *
...
public class MultiServerUserRegistry implements SimpUserRegistry, SmartApplicationListener {
    ...
    /**
     * Holds a copy of a SimpUserRegistry for the purpose of broadcasting to and receiving broadcasts from other
     * application servers.
     */
