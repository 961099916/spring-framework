...
/**
 * Contains the result from parsing a "user" destination from a source message and translating it to target destinations
...
public class UserDestinationResult {
    ...
    /**
     * The "user" destination from the source message. This may look like "/user/queue/position-updates" when
     * subscribing or "/user/{username}/queue/position-updates" when sending a message.
     * 
     ...
     * 
     * @return the target destinations, never {@code null} but possibly an empty set if there are no active sessions for
     *         the user.
     */
     ...
     * 
     * @return the user name or {@code null} if we have a session id only such as when the user is not authenticated; in
     *         such cases it is possible to use sessionId in place of a user name thus removing the need for a
     *         user-to-session lookup via {@link SimpUserRegistry}.
     */
    @Nullable
