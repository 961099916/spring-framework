...
/**
 * A "simple" message broker that recognizes the message types defined in {@link SimpMessageType}, keeps track of
 * subscriptions with the help of a {@link SubscriptionRegistry} and sends messages to subscribers.
 *
...
public class SimpleBrokerMessageHandler extends AbstractBrokerMessageHandler {
     ...
     * <p>
     * <strong>Note</strong> that when a custom PathMatcher is configured via {@link #setPathMatcher}, if the custom
     * registry is not an instance of {@link DefaultSubscriptionRegistry}, the provided PathMatcher is not used and must
    ...
    /**
     * When configured, the given PathMatcher is passed down to the underlying SubscriptionRegistry to use for matching
    ...
    /**
     * When configured, the specified cache limit is passed down to the underlying SubscriptionRegistry, overriding any
     * default there.
     * <p>
    ...
    /**
     * Configure the name of a header that a subscription message can have for the purpose of filtering messages matched
     * to the subscription. The header value is expected to be a Spring EL boolean expression to be applied to the
     * headers of messages matched to the subscription.
     * <p>
     * For example:
     * 
     * <pre>
     ...
     * </pre>
     * <p>
     * By default this is set to "selector". You can set it to a different name, or to {@code null} to turn off support
     * for a selector header.
     * 
     * @param selectorHeaderName
    ...
    /**
     * Configure the {@link org.springframework.scheduling.TaskScheduler} to use for providing heartbeat support.
     * Setting this property also sets the {@link #setHeartbeatValue heartbeatValue} to "10000, 10000".
     * <p>
    ...
    /**
     * Configure the value for the heart-beat settings. The first number represents how often the server will write or
     * send a heartbeat. The second is how often the client should write. 0 means no heartbeats.
     * <p>
     * By default this is set to "0, 0" unless the {@link #setTaskScheduler taskScheduler} in which case the default
     * becomes "10000,10000" (in milliseconds).
     *
     * @since 4.2
     */
