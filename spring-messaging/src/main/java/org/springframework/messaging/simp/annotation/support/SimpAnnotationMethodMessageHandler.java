...
public class SimpAnnotationMethodMessageHandler extends AbstractMethodMessageHandler<SimpMessageMappingInfo>
    ...
    private static final boolean reactorPresent =
        ClassUtils.isPresent("reactor.core.publisher.Flux", SimpAnnotationMethodMessageHandler.class.getClassLoader());
    ...
    /**
     * Create an instance of SimpAnnotationMethodMessageHandler with the given message channels and broker messaging
     * template.
     * 
     ...
     *            the channel for messages to clients (e.g. WebSocket clients)
     * @param brokerTemplate
     ...
     * {@inheritDoc}
     * <p>
     * Destination prefixes are expected to be slash-separated Strings and therefore a slash is automatically appended
     * where missing to ensure a proper prefix-based match (i.e. matching complete segments).
     * <p>
     * Note however that the remaining portion of a destination after the prefix may use a different separator (e.g.
     * commonly "." in messaging) depending on the configured {@code PathMatcher}.
     */
    @Override
    ...
    /**
     * Configure a {@link MessageConverter} to use to convert the payload of a message from its serialized form with a
     * specific MIME type to an Object matching the target method parameter. The converter is also used when sending a
    ...
    /**
     * Configure a {@link ConversionService} to use when resolving method arguments, for example message header values.
     * <p>
    ...
    /**
     * Configure a {@link MessageHeaderInitializer} to pass on to {@link HandlerMethodReturnValueHandler
     * HandlerMethodReturnValueHandlers} that send messages from controller return values.
     * <p>
