...
/**
 * Abstract base class for {@link SmartMessageConverter} implementations including support for common properties and a
 * partial implementation of the conversion methods, mainly to check if the converter supports the conversion based on
 * the payload class and MIME type.
 *
...
public abstract class AbstractMessageConverter implements SmartMessageConverter {
    ...
    /**
     * Configure the {@link ContentTypeResolver} to use to resolve the content type of an input message.
     * <p>
     * Note that if no resolver is configured, then {@link #setStrictContentTypeMatch(boolean) strictContentTypeMatch}
     * should be left as {@code false} (the default) or otherwise this converter will ignore all messages.
     * <p>
     ...
     * <p>
     * A converter can configured to be strict only when a {@link #setContentTypeResolver contentTypeResolver} is
     * configured and the list of {@link #getSupportedMimeTypes() supportedMimeTypes} is not be empty.
     * <p>
     * When this flag is set to {@code true}, {@link #supportsMimeType(MessageHeaders)} will return {@code false} if the
     * {@link #setContentTypeResolver contentTypeResolver} is not defined or if no content-type header is present.
     */
    ...
    /**
     * Return the default content type for the payload. Called when {@link #toMessage(Object, MessageHeaders)} is
     * invoked without message headers or without a content type header.
     * <p>
     * By default, this returns the first element of the {@link #getSupportedMimeTypes() supportedMimeTypes}, if any.
     * Can be overridden in subclasses.
     * 
     * @param payload
     *            the payload being converted to a message
     ...
     *            an extra object passed to the {@link MessageConverter}, e.g. the associated {@code MethodParameter}
     *            (may be {@code null}}
     ...
     * @param headers
     *            optional headers for the message (may be {@code null})
     * @param conversionHint
     *            an extra object passed to the {@link MessageConverter}, e.g. the associated {@code MethodParameter}
     *            (may be {@code null}}
     * @return the resulting payload for the message, or {@code null} if the converter cannot perform the conversion
     * @since 4.2
     */
    @Nullable
