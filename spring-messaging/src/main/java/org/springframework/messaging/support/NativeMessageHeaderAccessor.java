...
/**
 * {@link MessageHeaderAccessor} sub-class that supports storage and access of headers from an external source such as a
 * message broker. Headers from the external source are kept separate from other headers, in a sub-map under the key
 * {@link #NATIVE_HEADERS}. This allows separating processing headers from headers that need to be sent to or received
 * from the external source.
 *
 * <p>
 * This class is likely to be used through indirectly through a protocol specific sub-class that also provide factory
 * methods to translate message headers to an from an external messaging source.
 *
...
public class NativeMessageHeaderAccessor extends MessageHeaderAccessor {
     ...
     * 
     * @param nativeHeaders
     *            native headers to create the message with (may be {@code null})
     */
     ...
     * <p>
     * In order for this to work, the accessor must be {@link #isMutable() mutable}. See {@link MessageHeaderAccessor}
     * for details.
     */
