...
/**
 * Wrapper around {@link MessageHeaders} that provides extra features such as strongly typed accessors for specific
 * headers, the ability to leave headers in a {@link Message} mutable, and the option to suppress automatic generation
 * of {@link MessageHeaders#ID id} and {@link MessageHeaders#TIMESTAMP timesteamp} headers. Sub-classes such as
 * {@link NativeMessageHeaderAccessor} and others provide support for managing processing vs external source headers as
 * well as protocol specific headers.
 *
 * <p>
 * Below is a workflow to initialize headers via {@code MessageHeaderAccessor}, or one of its sub-classes, then create a
 * {@link Message}, and then re-obtain the accessor possibly from a different component:
 * 
 * <pre class="code">
 ...
 *
 * <p>
 * In order for the above to work, all participating components must use {@code MessageHeaders} to create, access, or
 * modify headers, or otherwise {@link MessageHeaderAccessor#getAccessor(Message, Class)} will return null. Below is a
 * workflow that shows how headers are created and left mutable, then modified possibly by a different component, and
 * finally made immutable perhaps before the possibility of being accessed on a different thread:
 * 
 * <pre class="code">
...
public class MessageHeaderAccessor {
    ...
    private static final MimeType[] READABLE_MIME_TYPES =
        new MimeType[] {MimeTypeUtils.APPLICATION_JSON, MimeTypeUtils.APPLICATION_XML, new MimeType("text", "*"),
            new MimeType("application", "*+json"), new MimeType("application", "*+xml")};
     ...
     * 
     * @param message
     *            a message to copy the headers from, or {@code null} if none
     */
    ...
    /**
     * Return the original {@code MessageHeaderAccessor} used to create the headers of the given {@code Message}, or
     * {@code null} if that's not available or if its type does not match the required type.
     * <p>
     * This is for cases where the existence of an accessor is strongly expected (followed up with an assertion) or
     * where an accessor will be created otherwise.
     *
     * @param message
     *            the message to get an accessor for
     ...
     *            the message to get an accessor for
     * @param requiredType
     *            the required accessor type (or {@code null} for any)
     * @return an accessor instance of the specified type, or {@code null} if none
     * @since 4.1
     */
    @Nullable
    ...
    /**
     * A variation of {@link #getAccessor(org.springframework.messaging.Message, Class)} with a {@code MessageHeaders}
     * instance instead of a {@code Message}.
     * <p>
     * This is for cases when a full message may not have been created yet.
     *
     * @param messageHeaders
     *            the message headers to get an accessor for
     * @param requiredType
     *            the required accessor type (or {@code null} for any)
     * @return an accessor instance of the specified type, or {@code null} if none
     * @since 4.1
    ...
    /**
     * Return a mutable {@code MessageHeaderAccessor} for the given message attempting to match the type of accessor
     * used to create the message headers, or otherwise wrapping the message with a {@code MessageHeaderAccessor}
     * instance.
     * <p>
     * This is for cases where a header needs to be updated in generic code while preserving the accessor type for
     ...
     *
     * @param message
     *            the message to build a new accessor for
    ...
    /**
     * By default when {@link #getMessageHeaders()} is called, {@code "this"} {@code MessageHeaderAccessor} instance can
     * no longer be used to modify the underlying message headers and the returned {@code MessageHeaders} is immutable.
     * <p>
     * However when this is set to {@code true}, the returned (underlying) {@code MessageHeaders} instance remains
     * mutable. To make further modifications continue to use the same accessor instance or re-obtain it via:<br>
     * {@link MessageHeaderAccessor#getAccessor(Message, Class) MessageHeaderAccessor.getAccessor(Message, Class)}
     * <p>
     * When modifications are complete use {@link #setImmutable()} to prevent further changes. The intended use case for
     * this mechanism is initialization of a Message within a single thread.
     * <p>
    ...
    /**
     * By default when {@link #getMessageHeaders()} is called, {@code "this"} {@code MessageHeaderAccessor} instance can
     * no longer be used to modify the underlying message headers. However if {@link #setLeaveMutable(boolean)} is used,
     * this method is necessary to indicate explicitly when the {@code MessageHeaders} instance should no longer be
     * modified.
     *
     * @since 4.1
     */
     ...
     *
     * @param modified
     *            typically {@code true}, or {@code false} to reset the flag
     * @since 4.1
     */
     ...
     * <p>
     * Unless {@link #setLeaveMutable(boolean)} was set to {@code true}, after this call, the headers are immutable and
     * this accessor can no longer modify them.
     * <p>
     * This method always returns the same {@code MessageHeaders} instance if invoked multiples times. To obtain a copy
     * of the underlying headers, use {@link #toMessageHeaders()} or {@link #toMap()} instead.
     *
     * @since 4.1
     */
     ...
     * <p>
     * This method can be invoked many times, with modifications in between where each new call returns a fresh copy of
     * the current header values.
     *
     * @since 4.1
     */
     ...
     * <p>
     * As the name suggests, array may contain simple matching patterns for header names. Supported pattern styles are:
     ...
     * <p>
     * This operation will overwrite any existing values. Use {@link #copyHeadersIfAbsent(Map)} to avoid overwriting
     * values.
     */
     ...
     *
     * @param payload
     *            the payload that corresponds to the headers.
     * @return the message
     */
    ...
    /**
     * Extension of {@link MessageHeaders} that helps to preserve the link to the outer {@link MessageHeaderAccessor}
     * instance that created it as well as keeps track of whether headers are still mutable.
     */
