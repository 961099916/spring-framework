...
public class ExecutorSubscribableChannel extends AbstractSubscribableChannel {
    ...
    /**
     * Create a new {@link ExecutorSubscribableChannel} instance where messages will be sent via the specified executor.
     * 
     * @param executor
     *            the executor used to send the message, or {@code null} to execute in the callers thread.
     */
    ...
    private class SendTask implements MessageHandlingRunnable {
        ...
        private Message<?> applyBeforeHandle(Message<?> message) {
            ...
            for (ExecutorChannelInterceptor interceptor : executorInterceptors) {
                messageToUse =
                    interceptor.beforeHandle(messageToUse, ExecutorSubscribableChannel.this, this.messageHandler);
