...
/**
 * A condition to assist with mapping onto handler methods based on the RSocket frame type. This helps to separate the
 * handling of connection-level frame types, i.e. {@code SETUP} and {@code METADATA_PUSH}, from the handling of stream
 * requests.
 *
...
public class RSocketFrameTypeMessageCondition extends AbstractMessageCondition<RSocketFrameTypeMessageCondition> {
    ...
    /**
     * Condition to match "REQUEST_FNF", "REQUEST_RESPONSE", "REQUEST_STREAM", and "REQUEST_CHANNEL".
     * 
     * @deprecated as of 5.2.2 because matching to all interaction types is too flexible. Please use one of the other
     *             constants in this class that match to specific frames.
     */
    @Deprecated
    ...
    /**
     * Return a condition for matching the RSocket request interaction type with that is selected based on the delcared
     * request and response cardinality of some handler method.
     * <p>
     ...
     *
     * @param cardinalityIn
     *            -- the request cardinality: 1 for a single payload, 2 for many payloads, and 0 if input is not
     *            handled.
     * @param cardinalityOut
     *            -- the response cardinality: 0 for no output payloads, 1 for a single payload, and 2 for many
     *            payloads.
