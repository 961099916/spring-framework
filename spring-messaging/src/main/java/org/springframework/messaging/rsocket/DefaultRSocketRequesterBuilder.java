...
final class DefaultRSocketRequesterBuilder implements RSocketRequester.Builder {
    ...
    private final static boolean rsocketConnectorPresent =
        ClassUtils.isPresent("io.rsocket.core.RSocketConnector", DefaultRSocketRequesterBuilder.class.getClassLoader());
    ...
    public Mono<RSocketRequester> connect(ClientTransport transport) {
        ...
        if (rsocketConnectorPresent) {
            connectFunction = payload -> new RSocketConnectorHelper().getRSocketMono(this.rsocketConnectorConfigurers,
                this.rsocketFactoryConfigurers, metaMimeType, dataMimeType, setupPayload, rsocketStrategies, transport,
                payload);
        } else {
