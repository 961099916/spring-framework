...
public abstract class PayloadUtils {
    ...
    /**
     * Use this method to slice, retain and wrap the data portion of the {@code Payload}, and also to release the
     * {@code Payload}. This assumes the Payload metadata has been read by now and ensures downstream code need only be
     * aware of {@code DataBuffer}s.
     * 
     * @param payload
     ...
     * <p>
     * If at least one is {@link NettyDataBuffer} then {@link ByteBufPayload} is created with either obtaining the
     * underlying native {@link ByteBuf} or using {@link Unpooled#wrappedBuffer(ByteBuffer...)} if necessary. Otherwise,
     * if both are {@link DefaultDataBuffer}, then {@link DefaultPayload} is created.
     * 
     * @param data
     *            the data part for the payload
     * @param metadata
     *            the metadata part for the payload
