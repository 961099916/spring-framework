...
public class ReactorNettyTcpClient<P> implements TcpOperations<P> {
     ...
     * <p>
     * This constructor manages the lifecycle of the {@link TcpClient} and underlying resources such as
     * {@link ConnectionProvider}, {@link LoopResources}, and {@link ChannelGroup}.
     * <p>
     ...
     * @param codec
     *            for encoding and decoding the input/output byte streams
    ...
    /**
     * A variant of {@link #ReactorNettyTcpClient(String, int, ReactorNettyCodec)} that still manages the lifecycle of
     * the {@link TcpClient} and underlying resources, but allows for direct configuration of other properties of the
     * client through a {@code Function<TcpClient, TcpClient>}.
     * 
     * @param clientConfigurer
     *            the configurer function
     * @param codec
     *            for encoding and decoding the input/output byte streams
     * @since 5.1.3
     * @see org.springframework.messaging.simp.stomp.StompReactorNettyCodec
    ...
    /**
     * Constructor with an externally created {@link TcpClient} instance whose lifecycle is expected to be managed
     * externally.
     * 
     * @param tcpClient
     *            the TcpClient instance to use
     * @param codec
     *            for encoding and decoding the input/output byte streams
     * @see org.springframework.messaging.simp.stomp.StompReactorNettyCodec
     */
    ...
    public ListenableFuture<Void> connect(TcpConnectionHandler<P> handler, ReconnectStrategy strategy) {
        ...
        this.tcpClient.handle(new ReactorNettyHandler(handler)).connect().doOnNext(updateConnectMono(connectMono))
            .doOnError(updateConnectMono(connectMono)).doOnError(handler::afterConnectFailure) // report all connect
                                                                                               // failures to the
                                                                                               // handler
