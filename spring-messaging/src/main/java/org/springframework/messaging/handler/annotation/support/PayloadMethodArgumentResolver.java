...
/**
 * A resolver to extract and convert the payload of a message using a {@link MessageConverter}. It also validates the
 * payload using a {@link Validator} if the argument is annotated with a Validation annotation.
 *
 * <p>
 * This {@link HandlerMethodArgumentResolver} should be ordered last as it supports all types and does not require the
 * {@link Payload} annotation.
 *
...
public class PayloadMethodArgumentResolver implements HandlerMethodArgumentResolver {
     ...
     *            if "true" (the default) this resolver supports all parameters; if "false" then only arguments with the
     *            {@code @Payload} annotation are supported.
     */
    public PayloadMethodArgumentResolver(MessageConverter messageConverter, @Nullable Validator validator,
     ...
     * <p>
     * By default this is simply {@link MethodParameter#getParameterType()} but that can be overridden to select a more
     * specific target type after also taking into account the "Content-Type", e.g. return {@code String} if target type
     * is {@code Object} and {@code "Content-Type:text/**"}.
     * 
     * @param parameter
