...
/**
 * {@code HandlerMethodArgumentResolver} for {@link Message} method arguments. Validates that the generic type of the
 * payload matches to the message value or otherwise applies {@link MessageConverter} to convert to the expected payload
 * type.
 *
...
public class MessageMethodArgumentResolver implements HandlerMethodArgumentResolver {
     ...
     * 
     * @param converter
     *            the MessageConverter to use (may be {@code null})
     * @since 4.3
     */
    ...
    public Object resolveArgument(MethodParameter parameter, Message<?> message) throws Exception {
        ...
        if (!targetMessageType.isAssignableFrom(message.getClass())) {
            throw new MethodArgumentTypeMismatchException(message, parameter,
                "Actual message type '" + ClassUtils.getDescriptiveType(message) + "' does not match expected type '"
     ...
     * <p>
     * By default this is the generic type declared in the {@code Message} method parameter but that can be overridden
     * to select a more specific target type after also taking into account the "Content-Type", e.g. return
     * {@code String} if target type is {@code Object} and {@code "Content-Type:text/**"}.
     * 
     * @param parameter
    ...
    private Object convertPayload(Message<?> message, MethodParameter parameter, Class<?> targetPayloadType) {
        ...
        if (result == null) {
            throw new MessageConversionException(message,
                "No converter found from actual payload type '" + ClassUtils.getDescriptiveType(message.getPayload())
                    + "' to expected payload type '" + ClassUtils.getQualifiedName(targetPayloadType) + "'");
        }
        return result;
    }
