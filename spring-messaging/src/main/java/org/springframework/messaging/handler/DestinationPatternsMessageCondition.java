...
/**
 * {@link MessageCondition} to match the destination header of a Message against one or more patterns through a
 * {@link RouteMatcher}.
 *
...
public class DestinationPatternsMessageCondition extends AbstractMessageCondition<DestinationPatternsMessageCondition> {
    ...
    /**
     * Constructor with patterns only. Creates and uses an instance of {@link AntPathMatcher} with default settings.
     * <p>
    ...
    /**
     * Check if any of the patterns match the given Message destination and return an instance that is guaranteed to
     * contain matching patterns, sorted via {@link org.springframework.util.PathMatcher#getPatternComparator(String)}.
     * 
     * @param message
     *            the message to match to
     * @return the same instance if the condition contains no patterns; or a new condition with sorted matching
     *         patterns; or {@code null} either if a destination can not be extracted or there is no match
     */
    @Override
    @Nullable
    ...
    /**
     * Compare the two conditions based on the destination patterns they contain. Patterns are compared one at a time,
     * from top to bottom via {@link org.springframework.util.PathMatcher#getPatternComparator(String)}. If all compared
     * patterns match equally, but one instance has more patterns, it is considered a closer match.
     * <p>
     * It is assumed that both instances have been obtained via {@link #getMatchingCondition(Message)} to ensure they
     * contain only patterns that match the request and are sorted with the best matches on top.
     */
    @Override
