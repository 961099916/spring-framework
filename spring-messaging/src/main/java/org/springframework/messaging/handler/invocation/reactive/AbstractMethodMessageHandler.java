...
/**
 * Abstract base class for reactive HandlerMethod-based message handling. Provides most of the logic required to
 * discover handler methods at startup, find a matching handler method at runtime for a given message and invoke it.
 *
 * <p>
...
public abstract class AbstractMethodMessageHandler<T>
     ...
     * <p>
     * This is not set by default. However sub-classes may initialize it to some default strategy (e.g.
    ...
    /**
     * Manually configure the handlers to check for the presence of message handling methods, which also disables
     * auto-detection via a {@link #setHandlerPredicate(Predicate) handlerPredicate}. If you do not want to disable
     * auto-detection, then call this method first, and then set the handler predicate.
     * 
     * @param handlers
     ...
     * <p>
     * By default this is an instance of {@link ReactiveAdapterRegistry} with default settings.
     */
    ...
    /**
     * Return the argument resolvers initialized during {@link #afterPropertiesSet()}. Primarily for internal use in
     * sub-classes.
     * 
     * @since 5.2.2
     */
     ...
     * mapping information.
     * <p>
     * <strong>Note:</strong> This method is protected and can be invoked by subclasses, but this should be done on
     * startup only as documented in {@link #registerHandlerMethod}.
     * 
     * @param handler
     ...
     * <p>
     * <strong>Note:</strong> This method is protected and can be invoked by subclasses. Keep in mind however that the
     * registration is not protected for concurrent use, and is expected to be done on startup.
     * 
     * @param handler
    ...
    /**
     * This method is invoked just before mappings are added. It allows sub-classes to update the mapping with the
     * {@link HandlerMethod} in mind. This can be useful when the method signature is used to refine the mapping, e.g.
     * based on the cardinality of input and output.
     * <p>
     ...
     * @param handlerMethod
     *            the target handler for the mapping
    ...
    /**
     * Return String-based destinations for the given mapping, if any, that can be used to find matches with a direct
     * lookup (i.e. non-patterns).
     * <p>
     * <strong>Note:</strong> This is completely optional. The mapping metadata for a subclass may support neither
     * direct lookups, nor String based destinations.
     */
    ...
    /**
     * Create a concrete instance of {@link AbstractExceptionHandlerMethodResolver} that finds exception handling
     * methods based on some criteria, e.g. based on the presence of {@code @MessageExceptionHandler}.
     * 
     * @param beanType
    ...
    /**
     * Container for matched mapping and HandlerMethod. Used for best match comparison and for access to mapping
     * information.
     */
