...
/**
 * Base class for a return value handler that encodes return values to {@code Flux<DataBuffer>} through the configured
 * {@link Encoder}s.
 *
 * <p>
 * Subclasses must implement the abstract method {@link #handleEncodedContent} to handle the resulting encoded content.
 *
 * <p>
 * This handler should be ordered last since its {@link #supportsReturnType} returns {@code true} for any method
 * parameter type.
 *
...
public abstract class AbstractEncoderMethodReturnValueHandler implements HandlerMethodReturnValueHandler {
    ...
    private Flux<DataBuffer> encodeContent(@Nullable Object content, MethodParameter returnType,
        ...
        if (adapter != null) {
            ...
            boolean isUnwrapped =
                KotlinDetector.isKotlinReflectPresent() && KotlinDetector.isKotlinType(returnType.getContainingClass())
     ...
     *            the encoded content; each {@code DataBuffer} represents the fully-aggregated, encoded content for one
     *            value (i.e. payload) returned from the HandlerMethod.
     * @param returnType
     *            return type of the handler method that produced the data
     * @param message
     *            the input message handled by the handler method
    ...
    /**
     * Invoked for a {@code null} return value, which could mean a void method or method returning an async type
     * parameterized by void.
     * 
     * @param returnType
     *            return type of the handler method that produced the data
     * @param message
