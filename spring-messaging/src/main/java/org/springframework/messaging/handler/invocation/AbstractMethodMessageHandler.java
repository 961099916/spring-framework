...
/**
 * Abstract base class for HandlerMethod-based message handling. Provides most of the logic required to discover handler
 * methods at startup, find a matching handler method at runtime for a given message and invoke it.
 *
 * <p>
 * Also supports discovering and invoking exception handling methods to process exceptions raised during message
 * handling.
 *
...
public abstract class AbstractMethodMessageHandler<T>
     ...
     * <p>
     * We're not checking the autowire-candidate status here, which is how the proxy target filtering problem is being
     * handled at the autowiring level, since autowire-candidate may have been turned to {@code false} for other
     * reasons, while still expecting the bean to be eligible for handler methods.
     * <p>
     * Originally defined in {@link org.springframework.aop.scope.ScopedProxyUtils} but duplicated here to avoid a hard
     * dependency on the spring-aop module.
     */
    ...
    /**
     * When this property is configured only messages to destinations matching one of the configured prefixes are
     * eligible for handling. When there is a match the prefix is removed and only the remaining part of the destination
    ...
    /**
     * Sets the list of custom {@code HandlerMethodArgumentResolver}s that will be used after resolvers for supported
     * argument type.
     */
    ...
    /**
     * Set the list of custom {@code HandlerMethodReturnValueHandler}s that will be used after return value handlers for
     * known types.
     */
    public void
        setCustomReturnValueHandlers(@Nullable List<HandlerMethodReturnValueHandler> customReturnValueHandlers) {
     ...
     * This is an advanced option; for most use cases it should be sufficient to use
    ...
    /**
     * Configure the complete list of supported return value types, effectively overriding the ones configured by
     * default. This is an advanced option; for most use cases it should be sufficient to use
    ...
    /**
     * Return the list of argument resolvers to use. Invoked only if the resolvers have not already been set via
     * {@link #setArgumentResolvers}.
     * <p>
    ...
    /**
     * Return the list of return value handlers to use. Invoked only if the return value handlers have not already been
     * set via {@link #setReturnValueHandlers}.
     * <p>
    ...
    /**
     * Detect if the given handler has any methods that can handle messages and if so register it with the extracted
     * mapping information.
     * 
     * @param handler
     ...
     * @param handlerType
     *            the handler type, possibly a sub-type of the method's declaring class
    ...
    /**
     * Create a HandlerMethod instance from an Object handler that is either a handler instance or a String-based bean
     * name.
     */
    ...
    /**
     * Subclasses can invoke this method to populate the MessagingAdviceBean cache (e.g. to support "global"
    ...
    /**
     * Check whether the given destination (of an incoming message) matches to one of the configured destination
     * prefixes and if so return the remaining portion of the destination after the matched prefix.
     * <p>
    ...
    /**
     * Check if a mapping matches the current message and return a possibly new mapping with conditions relevant to the
     * current request.
     * 
     * @param mapping
    ...
    /**
     * Find an {@code @MessageExceptionHandler} method for the given exception. The default implementation searches
     * methods in the class hierarchy of the HandlerMethod first and if not found, it continues searching for additional
     * {@code @MessageExceptionHandler} methods among the configured
     * {@linkplain org.springframework.messaging.handler.MessagingAdviceBean MessagingAdviceBean}, if any.
     * 
     * @param handlerMethod
    ...
    protected InvocableHandlerMethod getExceptionHandlerMethod(HandlerMethod handlerMethod, Exception exception) {
        ...
        for (Map.Entry<MessagingAdviceBean,
            AbstractExceptionHandlerMethodResolver> entry : this.exceptionHandlerAdviceCache.entrySet()) {
    ...
    /**
     * A thin wrapper around a matched HandlerMethod and its matched mapping for the purpose of comparing the best match
     * with a comparator in the context of a message.
     */
    private class Match {
