...
/**
 * Extension of {@link AbstractMethodMessageHandler} for reactive, non-blocking handling of messages via
 * {@link MessageMapping @MessageMapping} methods. By default such methods are detected in {@code @Controller} Spring
 * beans but that can be changed via {@link #setHandlerPredicate(Predicate)}.
 *
 * <p>
 * Payloads for incoming messages are decoded through the configured {@link #setDecoders(List)} decoders, with the help
 * of {@link PayloadMethodArgumentResolver}.
 *
 * <p>
 * There is no default handling for return values but {@link #setReturnValueHandlerConfigurer} can be used to configure
 * custom return value handlers. Sub-classes may also override {@link #initReturnValueHandlers()} to set up default
...
public class MessageMappingMessageHandler extends AbstractMethodMessageHandler<CompositeMessageCondition>
    ...
    /**
     * Return the {@code RouteMatcher} used to map messages to handlers. May be {@code null} before the component is
     * initialized.
     */
    @Nullable
    ...
    /**
     * Set the {@code RouteMatcher} to use for mapping messages to handlers based on the route patterns they're
     * configured with.
     * <p>
     * By default, {@link SimpleRouteMatcher} is used, backed by {@link AntPathMatcher} with "." as separator. For
     * greater efficiency consider using the {@code PathPatternRouteMatcher} from {@code spring-web} instead.
     */
     ...
     *             in case of no RouteMatcher set
     * @since 5.0
     */
    ...
    /**
     * Configure a {@link ConversionService} to use for type conversion of String based values, e.g. in destination
    ...
    protected List<? extends HandlerMethodArgumentResolver> initArgumentResolvers() {
        ...
        // Catch-all
        resolvers
            .add(new PayloadMethodArgumentResolver(getDecoders(), this.validator, getReactiveAdapterRegistry(), true));
    ...
    protected String[] processDestinations(String[] destinations) {
        if (this.valueResolver != null) {
            destinations =
                Arrays.stream(destinations).map(s -> this.valueResolver.resolveStringValue(s)).toArray(String[]::new);
        }
        return destinations;
    }
