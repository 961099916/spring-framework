...
public class GenericMessagingTemplate extends AbstractDestinationResolvingMessagingTemplate<MessageChannel>
    ...
    /**
     * Configure the default timeout value to use for receive operations. May be overridden for individual messages when
     * using sendAndReceive operations.
     *
    ...
    /**
     * Set the name of the header used to determine the send timeout (if present). Default
     * {@value #DEFAULT_RECEIVE_TIMEOUT_HEADER}. The header is removed before sending the message to avoid propagation.
     *
     * @since 5.0
     */
    ...
    /**
     * Whether the thread sending a reply should have an exception raised if the receiving thread isn't going to receive
     * the reply either because it timed out, or because it already received a reply, or because it got an exception
     * while sending the request message.
     * <p>
     * The default value is {@code false} in which case only a WARN message is logged. If set to {@code true} a
     * {@link MessageDeliveryException} is raised in addition to the log message.
     * 
     * @param throwExceptionOnLateReply
    ...
    protected final Message<?> doReceive(MessageChannel channel, long timeout) {
        ...
        Message<?> message =
            (timeout >= 0 ? ((PollableChannel)channel).receive(timeout) : ((PollableChannel)channel).receive());
