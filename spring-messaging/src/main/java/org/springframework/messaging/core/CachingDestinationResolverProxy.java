 ...
 * {@link #resolveDestination} results. Such caching is particularly useful if the destination resolving process is
 * expensive (e.g. the destination has to be resolved through an external system) and the resolution results are stable
 * anyway.
 *
 * @author Agim Emruli
...
public class CachingDestinationResolverProxy<D> implements DestinationResolver<D>, InitializingBean {
     ...
     * {@link #setTargetDestinationResolver} bean property.
     */
    public CachingDestinationResolverProxy() {}

    /**
     * Create a new CachingDestinationResolverProxy using the given target DestinationResolver to actually resolve
     * destinations.
     * 
     * @param targetDestinationResolver
     ...
     * 
     * @param name
     *            the destination name to be resolved
