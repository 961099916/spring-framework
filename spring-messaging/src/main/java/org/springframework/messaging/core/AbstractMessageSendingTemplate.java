...
public abstract class AbstractMessageSendingTemplate<D> implements MessageSendingOperations<D> {
    ...
    /**
     * Name of the header that can be set to provide further information (e.g. a {@code MethodParameter} instance) about
     * the origin of the payload, to be taken into account as a conversion hint.
     * 
     * @since 4.2
     */
    ...
    /**
     * Configure the default destination to use in send methods that don't have a destination argument. If a default
     * destination is not configured, send methods without a destination argument will raise an exception if invoked.
     */
    ...
    /**
     * Convert the given Object to serialized form, possibly using a {@link MessageConverter}, wrap it as a message with
     * the given headers and apply the given post processor.
     * 
     * @param payload
    ...
    /**
     * Provides access to the map of input headers before a send operation. Subclasses can modify the headers and then
     * return the same or a different map.
     * <p>
     * This default implementation in this class returns the input map.
     * 
     * @param headers
     *            the headers to send (or {@code null} if none)
