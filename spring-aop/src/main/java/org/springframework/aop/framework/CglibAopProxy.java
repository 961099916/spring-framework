 ...
 *
 * <p>
 * Objects of this type should be obtained through proxy factories, configured by an {@link AdvisedSupport} object. This
 * class is internal to Spring's AOP framework and need not be used directly by client code.
 *
 * <p>
 * {@link DefaultAopProxyFactory} will automatically create CGLIB-based proxies if necessary, for example in case of
 * proxying a target class (see the {@link DefaultAopProxyFactory attendant javadoc} for details).
 *
 * <p>
...
class CglibAopProxy implements AopProxy, Serializable {
     ...
     *             if the config is invalid. We try to throw an informative exception in this case, rather than let a
     *             mysterious failure happen later.
     */
    ...
    /**
     * Process a return value. Wraps a return of {@code this} if necessary to be the {@code proxy} and also verifies
     * that {@code null} is not returned as a primitive.
     */
    @Nullable
    ...
    /**
     * Creates the CGLIB {@link Enhancer}. Subclasses may wish to override this to return a custom {@link Enhancer}
     * implementation.
     */
    ...
    /**
     * Checks for final methods on the given {@code Class}, as well as package-visible methods across ClassLoaders, and
     * writes warnings to the log for each one found.
     */
     ...
     * (We know there was no advice for this method.)
     */
    ...
    /**
     * Dispatcher for a static target. Dispatcher is much faster than interceptor. This will be used whenever it can be
     * determined that a method definitely does not return "this"
     */
    ...
    private static class CglibMethodInvocation extends ReflectiveMethodInvocation {
        ...
        /**
         * Gives a marginal performance improvement versus using reflection to invoke the target when invoking public
         * methods.
         */
        @Override
    ...
    private static class ProxyCallbackFilter implements CallbackFilter {
         ...
         * <p>
         * The callbacks for each proxy are built up of a set of fixed callbacks for general use and then a set of
         * callbacks that are specific to a method for use on static targets with a fixed advice chain.
         * <p>
         * The callback used is determined thus:
         * <dl>
         * <dt>For exposed proxies</dt>
         * <dd>Exposing the proxy requires code to execute before and after the method/chain invocation. This means we
         * must use DynamicAdvisedInterceptor, since all other interceptors can avoid the need for a try/catch
         * block</dd>
         * <dt>For Object.finalize():</dt>
         * <dd>No override for this method is used.</dd>
         * <dt>For equals():</dt>
         * <dd>The EqualsInterceptor is used to redirect equals() calls to a special handler to this proxy.</dd>
         * <dt>For methods on the Advised class:</dt>
         * <dd>the AdvisedDispatcher is used to dispatch the call directly to the target</dd>
         * <dt>For advised methods:</dt>
         * <dd>If the target is static and the advice chain is frozen then a FixedChainStaticTargetInterceptor specific
         * to the method is used to invoke the advice chain. Otherwise a DynamicAdvisedInterceptor is used.</dd>
         * <dt>For non-advised methods:</dt>
         * <dd>Where it can be determined that the method will not return {@code this} or when
         * {@code ProxyFactory.getExposeProxy()} returns {@code false}, then a Dispatcher is used. For static targets,
         * the StaticDispatcher is used; and for dynamic targets, a DynamicUnadvisedInterceptor is used. If it possible
         * for the method to return {@code this} then a StaticUnadvisedInterceptor is used for static targets - the
