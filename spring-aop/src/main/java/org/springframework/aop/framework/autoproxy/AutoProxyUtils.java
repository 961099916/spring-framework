...
public abstract class AutoProxyUtils {
    ...
    /**
     * Bean definition attribute that may indicate whether a given bean is supposed to be proxied with its target class
     * (in case of it getting proxied in the first place). The value is {@code Boolean.TRUE} or {@code Boolean.FALSE}.
     * <p>
     * Proxy factories can set this attribute if they built a target class proxy for a specific bean, and want to
     * enforce that bean can always be cast to its target class (even if AOP advices get applied through auto-proxying).
     * 
    ...
    /**
     * Bean definition attribute that indicates the original target class of an auto-proxied bean, e.g. to be used for
     * the introspection of annotations on the target class behind an interface-based proxy.
     * 
     * @since 4.2.3
     ...
     * {@link #PRESERVE_TARGET_CLASS_ATTRIBUTE "preserveTargetClass" attribute} of the corresponding bean definition.
     * 
     * @param beanFactory
     *            the containing ConfigurableListableBeanFactory
     * @param beanName
     ...
     * {@link AutowireCapableBeanFactory#ORIGINAL_INSTANCE_SUFFIX}, skipping any proxy attempts for it.
     * 
     * @param beanName
     *            the name of the bean
     * @param beanClass
