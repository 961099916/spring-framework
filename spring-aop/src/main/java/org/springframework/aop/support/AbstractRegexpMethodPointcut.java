 ...
 * <ul>
 * <li>pattern: regular expression for the fully-qualified method names to match. The exact regexp syntax will depend on
 * the subclass (e.g. Perl5 regular expressions)
 * <li>patterns: alternative property taking a String array of patterns. The result will be the union of these patterns.
 * </ul>
 *
 * <p>
 * Note: the regular expressions must be a match. For example, {@code .*get.*} will match com.mycom.Foo.getBar().
 * {@code get.*} will not.
 *
 * <p>
...
public abstract class AbstractRegexpMethodPointcut extends StaticMethodMatcherPointcut implements Serializable {
    ...
    /**
     * Convenience method when we have only a single pattern. Use either this method or {@link #setPatterns}, not both.
     * 
     * @see #setPatterns
     */
    ...
    /**
     * Set the regular expressions defining methods to match. Matching will be the union of all these; if any match, the
     * pointcut matches.
     *
     * @see #setPattern
     */
    ...
    /**
     * Set the regular expressions defining methods to match for exclusion. Matching will be the union of all these; if
    ...
    /**
     * Try to match the regular expression against the fully qualified name of the target class as well as against the
     * method's declaring class, plus the name of the method.
     */
    @Override
