...
/**
 * Performance monitor interceptor that uses <b>JAMon</b> library to perform the performance measurement on the
 * intercepted method and output the stats. In addition, it tracks/counts exceptions thrown by the intercepted method.
...
public class JamonPerformanceMonitorInterceptor extends AbstractMonitoringInterceptor {
     ...
     *            whether to track all invocations that go through this interceptor, or just invocations with trace
     *            logging enabled
     * @see #setUseDynamicLogger
     */
    ...
    /**
     * Set whether to track all invocations that go through this interceptor, or just invocations with trace logging
     * enabled.
     * <p>
     * Default is "false": Only invocations with trace logging enabled will be monitored. Specify "true" to let JAMon
    ...
    /**
     * Always applies the interceptor if the "trackAllInvocations" flag has been set; else just kicks in if the log is
     * enabled.
     * 
    ...
    /**
     * Count the thrown exception and put the stack trace in the details portion of the key. This will allow the stack
     * trace to be viewed in the JAMon web application.
     */
