 ...
 * <ul>
 * <li>If {@code A} and {@code B} are defined in different aspects, then the advice in the aspect with the lowest order
 * value has the highest precedence.</li>
 * <li>If {@code A} and {@code B} are defined in the same aspect, if one of {@code A} or {@code B} is a form of
 * <em>after</em> advice, then the advice declared last in the aspect has the highest precedence. If neither {@code A}
 * nor {@code B} is a form of <em>after</em> advice, then the advice declared first in the aspect has the highest
 * precedence.</li>
 * </ul>
 *
 * <p>
 * Important: This comparator is used with AspectJ's {@link org.aspectj.util.PartialOrder PartialOrder} sorting utility.
 * Thus, unlike a normal {@link Comparator}, a return value of {@code 0} from this comparator means we don't care about
 * the ordering, not that the two elements must be sorted identically.
 *
...
class AspectJPrecedenceComparator implements Comparator<Advisor> {
    ...
    /**
     * Create an {@code AspectJPrecedenceComparator}, using the given {@link Comparator} for comparing
     * {@link org.springframework.aop.Advisor} instances.
     * 
     * @param advisorComparator
     *            the {@code Comparator} to use for advisors
     */
