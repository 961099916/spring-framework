...
/**
 * Spring {@link org.springframework.aop.Pointcut} implementation that uses the AspectJ weaver to evaluate a pointcut
 * expression.
 *
 * <p>
 * The pointcut expression value is an AspectJ expression. This can reference other pointcuts and use composition and
 * other operations.
 *
 * <p>
 * Naturally, as this is to be processed by Spring AOP's proxy-based model, only method execution pointcuts are
 * supported.
 *
 * @author Rob Harrop
...
public class AspectJExpressionPointcut extends AbstractExpressionPointcut
     ...
     *            the declaration scope for the pointcut
     * @param paramNames
    ...
    /**
     * If a pointcut expression has been specified in XML, the user cannot write {@code and} as "&&" (though &amp;&amp;
     * will work). We also allow {@code and} between two pointcut sub-expressions.
     * <p>
     ...
     * <p>
     * This handler must be added to each pointcut object that needs to handle the {@code bean()} PCD. Matching context
     * is obtained automatically by examining a thread local variable and therefore a matching context need not be set
     * on the pointcut.
     */
     ...
     * <p>
     * Dynamic match tests for this matcher always return true, since the matching decision is made at the proxy
     * creation time. For static match tests, this matcher abstains to allow the overall pointcut to match even when
