...
/**
 * Abstract {@link org.springframework.aop.TargetSource} implementation that wraps a refreshable target object.
 * Subclasses can determine whether a refresh is required, and need to provide fresh target objects.
 *
 * <p>
...
public abstract class AbstractRefreshableTargetSource implements TargetSource, Refreshable {
     ...
     * <p>
     * Note that an actual refresh will only happen when {@link #requiresRefresh()} returns {@code true}.
     */
    ...
    /**
     * Determine whether a refresh is required. Invoked for each refresh check, after the refresh check delay has
     * elapsed.
     * <p>
     * The default implementation always returns {@code true}, triggering a refresh every time the delay has elapsed. To
     * be overridden by subclasses with an appropriate check of the underlying target resource.
     * 
     ...
     * <p>
     * Only invoked if a refresh check has found that a refresh is required (that is, {@link #requiresRefresh()} has
     * returned {@code true}).
     * 
