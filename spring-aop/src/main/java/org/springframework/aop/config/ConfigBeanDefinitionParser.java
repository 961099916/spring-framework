...
class ConfigBeanDefinitionParser implements BeanDefinitionParser {
    ...
    /**
     * Configures the auto proxy creator needed to support the {@link BeanDefinition BeanDefinitions} created by the
     * '{@code <aop:config/>}' tag. Will force class proxying if the '{@code proxy-target-class}' attribute is set to
     * '{@code true}'.
     * 
     ...
     * and any resulting {@link org.springframework.aop.Pointcut} with the supplied {@link BeanDefinitionRegistry}.
     */
    ...
    /**
     * Create a {@link RootBeanDefinition} for the advisor described in the supplied. Does <strong>not</strong> parse
     * any associated '{@code pointcut}' or '{@code pointcut-ref}' attributes.
     */
    ...
    /**
     * Return {@code true} if the supplied node describes an advice type. May be one of: '{@code before}',
     * '{@code after}', '{@code after-returning}', '{@code after-throwing}' or '{@code around}'.
     */
    ...
    /**
     * Parse a '{@code declare-parents}' element and register the appropriate DeclareParentsAdvisor with the
     * BeanDefinitionRegistry encapsulated in the supplied ParserContext.
     */
    ...
    /**
     * Parses one of '{@code before}', '{@code after}', '{@code after-returning}', '{@code after-throwing}' or
     * '{@code around}' and registers the resulting BeanDefinition with the supplied BeanDefinitionRegistry.
     * 
    ...
    /**
     * Creates the RootBeanDefinition for a POJO advice bean. Also causes pointcut parsing to occur so that the pointcut
     * may be associate with the advice bean. This same pointcut is also configured as the pointcut for the enclosing
    ...
    /**
     * Parses the supplied {@code <pointcut>} and registers the resulting Pointcut with the BeanDefinitionRegistry.
     */
    private AbstractBeanDefinition parsePointcut(Element pointcutElement, ParserContext parserContext) {
        ...
        try {
            ...
            parserContext
                .registerComponent(new PointcutComponentDefinition(pointcutBeanName, pointcutDefinition, expression));
        } finally {
            this.parseState.pop();
        }
    ...
    /**
     * Parses the {@code pointcut} or {@code pointcut-ref} attributes of the supplied {@link Element} and add a
     * {@code pointcut} property as appropriate. Generates a
     * {@link org.springframework.beans.factory.config.BeanDefinition} for the pointcut if necessary and returns its
     * bean name, otherwise returns the bean name of the referred pointcut.
     */
    @Nullable
    ...
    /**
     * Creates a {@link BeanDefinition} for the {@link AspectJExpressionPointcut} class using the supplied pointcut
     * expression.
     */
