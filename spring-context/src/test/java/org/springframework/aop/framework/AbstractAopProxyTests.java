...
public abstract class AbstractAopProxyTests {
    ...
    /**
     * Make a clean target source available if code wants to use it. The target must be set. Verification will be
     * automatic in tearDown to ensure that it was used appropriately by code.
     */
    @BeforeEach
    public void setUp() {
    ...
    /**
     * This is primarily a test for the efficiency of our usage of CGLIB. If we create too many classes with CGLIB this
     * will be slow or will run out of memory.
     */
    @Test
    ...
    /**
     * Check that the two MethodInvocations necessary are independent and don't conflict. Check also proxy exposure.
     */
    @Test
    ...
    /**
     * An interceptor throws a checked exception not on the method signature. For efficiency, we don't bother unifying
     * java.lang.reflect and org.springframework.cglib UndeclaredThrowableException
     */
    @Test
    ...
    /**
     * Check that although a method is eligible for advice chain optimization and direct reflective invocation, it
     * doesn't happen if we've asked to see the proxy, so as to guarantee a consistent programming model.
     */
    @Test
    ...
    /**
     * Check that the introduction advice isn't allowed to introduce interfaces that are unsupported by the
    ...
    /**
     * Note that an introduction can't throw an unexpected checked exception, as it's constrained by the interface.
     */
    @Test
    ...
    /**
     * There are times when we want to call proceed() twice. We can do this if we clone the invocation.
     */
    @Test
...
MethodInterceptor() {
    ...
    /**
     * Same thing for a proxy. Only works when exposeProxy is set to true. Checks that the proxy is the same on the way
     * in and out.
     */
    ...
    /**
     * Note that trapping the Invocation as in previous version of this test isn't safe, as invocations may be reused
     * and hence cleared at the end of each invocation. So we trap only the target.
     */
