...
public class ConfigurationClassPostProcessorTests {
    ...
    /**
     * Enhanced {@link Configuration} classes are only necessary for respecting certain bean semantics, like
     * singleton-scoping, scoped proxies, etc.
     * <p>
     * Technically, {@link ConfigurationClassPostProcessor} could fail to enhance the registered Configuration classes
     * and many use cases would still work. Certain cases, however, like inter-bean singleton references would not. We
     * test for such a case below, and in doing so prove that enhancement is working.
     */
    @Test
    ...
    /**
     * Tests the fix for SPR-5655, a special workaround that prefers reflection over ASM if a bean class is already
     * loaded.
     */
    @Test
    ...
    public void postProcessorWorksWithExtendedConfigurationWithAttributeOverrideForExcludesFilterUsingAsm() {
        RootBeanDefinition beanDefinition =
            new RootBeanDefinition(ExtendedConfigurationWithAttributeOverrideForExcludeFilter.class.getName());
        assertSupportForComposedAnnotationWithExclude(beanDefinition);
    }
    ...
    public void processingAllowedOnlyOncePerProcessorRegistryPair() {
        ...
        assertThatIllegalStateException().isThrownBy(() -> pp.postProcessBeanFactory(bf1)); // second invocation for bf1
                                                                                            // -- should throw
        pp.postProcessBeanFactory(bf2); // first invocation for bf2 -- should succeed
        assertThatIllegalStateException().isThrownBy(() -> pp.postProcessBeanFactory(bf2)); // second invocation for bf2
                                                                                            // -- should throw
    }
    ...
    public void genericsBasedInjectionWithEarlyGenericsMatching() {
        ...
        beanNames =
            beanFactory.getBeanNamesForType(ResolvableType.forClassWithGenerics(Repository.class, String.class));
    ...
    public void testCollectionArgumentOnBeanMethod() {
        ApplicationContext ctx =
            new AnnotationConfigApplicationContext(CollectionArgumentConfiguration.class, TestBean.class);
