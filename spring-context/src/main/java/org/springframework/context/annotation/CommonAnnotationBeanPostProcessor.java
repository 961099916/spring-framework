...
/**
 * {@link org.springframework.beans.factory.config.BeanPostProcessor} implementation that supports common Java
 * annotations out of the box, in particular the JSR-250 annotations in the {@code javax.annotation} package. These
 * common Java annotations are supported in many Java EE 5 technologies (e.g. JSF 1.2), as well as in Java 6's JAX-WS.
 *
 * <p>
 * This post-processor includes support for the {@link javax.annotation.PostConstruct} and
 * {@link javax.annotation.PreDestroy} annotations - as init annotation and destroy annotation, respectively - through
 * inheriting from {@link InitDestroyAnnotationBeanPostProcessor} with pre-configured annotation types.
 *
 * <p>
 * The central element is the {@link javax.annotation.Resource} annotation for annotation-driven injection of named
 * beans, by default from the containing Spring BeanFactory, with only {@code mappedName} references resolved in JNDI.
 * The {@link #setAlwaysUseJndiLookup "alwaysUseJndiLookup" flag} enforces JNDI lookups equivalent to standard Java EE 5
 * resource injection for {@code name} references and default names as well. The target beans can be simple POJOs, with
 * no special requirements other than the type having to match.
 *
 * <p>
 ...
 *
 * <p>
 * The common annotations supported by this post-processor are available in Java 6 (JDK 1.6) as well as in Java EE 5/6
 ...
 * </pre>
 *
 * {@code mappedName} references will always be resolved in JNDI, allowing for global JNDI names (including "java:"
 * prefix) as well. The "alwaysUseJndiLookup" flag just affects {@code name} references and default names (inferred from
 ...
 * <p>
 * <b>NOTE:</b> Annotation injection will be performed <i>before</i> XML injection; thus the latter configuration will
...
public class CommonAnnotationBeanPostProcessor extends InitDestroyAnnotationBeanPostProcessor
     ...
     * {@link javax.annotation.PostConstruct} and {@link javax.annotation.PreDestroy}, respectively.
     */
     ...
     * <p>
     * By default, the {@code javax.xml.ws.WebServiceContext} interface will be ignored, since it will be resolved by
     * the JAX-WS runtime.
     *
     * @param resourceType
    ...
    /**
     * Set whether to allow a fallback to a type match if no explicit name has been specified. The default name (i.e.
     * the field name or bean property name) will still be checked first; if a bean of that name exists, it will be
     * taken. However, if no bean of that name exists, a by-type resolution of the dependency will be attempted if this
     * flag is "true".
     * <p>
     * Default is "true". Switch this flag to "false" in order to enforce a by-name lookup in all cases, throwing an
     * exception in case of no name match.
     *
    ...
    /**
     * Set whether to always use JNDI lookups equivalent to standard Java EE 5 resource injection, <b>even for
     ...
     * {@code mappedName} attributes point directly into JNDI. Switch this flag to "true" for enforcing Java EE style
     * JNDI lookups in any case, even for {@code name} attributes and default names.
     *
     * @see #setJndiFactory
     * @see #setResourceFactory
     */
    ...
    /**
     * Specify the factory for objects to be injected into {@code @Resource} / {@code @WebServiceRef} / {@code @EJB}
     * annotated fields and setter methods, <b>for {@code mappedName} attributes that point directly into JNDI</b>. This
     * factory will also be used if "alwaysUseJndiLookup" is set to "true" in order to enforce JNDI lookups even for
     * {@code name} attributes and default names.
     * <p>
     * The default is a {@link org.springframework.jndi.support.SimpleJndiBeanFactory} for JNDI lookup behavior
     * equivalent to standard Java EE 5 resource injection.
     *
     * @see #setResourceFactory
     * @see #setAlwaysUseJndiLookup
     */
    ...
    /**
     * Specify the factory for objects to be injected into {@code @Resource} / {@code @WebServiceRef} / {@code @EJB}
     * annotated fields and setter methods, <b>for {@code name} attributes and default names</b>.
     * <p>
     * The default is the BeanFactory that this post-processor is defined in, if any, looking up resource names as
     * Spring bean names. Specify the resource factory explicitly for programmatic usage of this post-processor.
     * <p>
     * Specifying Spring's {@link org.springframework.jndi.support.SimpleJndiBeanFactory} leads to JNDI lookup behavior
     * equivalent to standard Java EE 5 resource injection, even for {@code name} attributes and default names. This is
     * the same behavior that the "alwaysUseJndiLookup" flag enables.
     *
     * @see #setAlwaysUseJndiLookup
     */
    ...
    /**
     * Obtain a lazily resolving resource proxy for the given name and type, delegating to {@link #getResource} on
     * demand once a method call comes in.
     *
     * @param element
     ...
     *
     * @param factory
     *            the factory to autowire against
     * @param element
     *            the descriptor for the annotated field/method
     * @param requestingBeanName
     *            the name of the requesting bean
