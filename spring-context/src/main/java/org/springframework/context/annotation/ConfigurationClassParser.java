...
/**
 * Parses a {@link Configuration} class definition, populating a collection of {@link ConfigurationClass} objects
 * (parsing a single Configuration class may result in any number of ConfigurationClass objects because one
 * Configuration class may import another using the {@link Import} annotation).
 *
 * <p>
 * This class helps separate the concern of parsing the structure of a Configuration class from the concern of
 * registering BeanDefinition objects based on the content of that model (with the exception of {@code @ComponentScan}
 * annotations which need to be registered immediately).
 *
 * <p>
 * This ASM-based implementation avoids reflection and eager class loading in order to interoperate effectively with
 * lazy class loading in a Spring ApplicationContext.
 *
 * @author Chris Beams
...
class ConfigurationClassParser {
    ...
    /**
     * Create a new {@link ConfigurationClassParser} instance that will be used to populate the set of configuration
     * classes.
     */
    public ConfigurationClassParser(MetadataReaderFactory metadataReaderFactory, ProblemReporter problemReporter,
        ...
        this.componentScanParser =
            new ComponentScanAnnotationParser(environment, resourceLoader, componentScanBeanNameGenerator, registry);
    ...
    /**
     * Apply processing and build a complete {@link ConfigurationClass} by reading the annotations, members and methods
     * from the source class. This method can be called multiple times as relevant sources are discovered.
     * 
     * @param configClass
     *            the configuration class being build
     * @param sourceClass
     *            a source class
     ...
     *            metadata for the <code>@PropertySource</code> annotation found
     * @throws IOException
     *             if loading a property source failed
     */
    ...
    /**
     * Recursively collect all declared {@code @Import} values. Unlike most meta-annotations it is valid to have several
     * {@code @Import}s declared with different values; the usual process of returning values from the first
     * meta-annotation on a class is not sufficient.
     * <p>
     * For example, it is common for a {@code @Configuration} class to declare direct {@code @Import}s in addition to
     * meta-imports originating from an {@code @Enable} annotation.
     * 
     * @param sourceClass
     *            the class to search
     * @param imports
     *            the imports collected so far
     * @param visited
     *            used to track visited classes to prevent infinite recursion
     * @throws IOException
     *             if there is any problem reading metadata from the named class
     */
    ...
    private class DeferredImportSelectorHandler {
        ...
        /**
         * Handle the specified {@link DeferredImportSelector}. If deferred import selectors are being collected, this
         * registers this instance to the list. If they are being processed, the {@link DeferredImportSelector} is also
         * processed immediately according to its {@link DeferredImportSelector.Group}.
         *
         * @param configClass
         *            the source configuration class
         * @param importSelector
         *            the selector to handle
         */
    ...
    private class DeferredImportSelectorGroupingHandler {
        ...
        public void register(DeferredImportSelectorHolder deferredImport) {
            ...
            DeferredImportSelectorGrouping grouping =
                this.groupings.computeIfAbsent((group != null ? group : deferredImport),
    ...
    /**
     * Simple wrapper that allows annotated source classes to be dealt with in a uniform manner, regardless of how they
     * are loaded.
     */
