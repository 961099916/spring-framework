...
/**
 * Abstract implementation of the {@link HierarchicalMessageSource} interface, implementing common handling of message
 * variants, making it easy to implement a specific strategy for a concrete MessageSource.
 *
 * <p>
 * Subclasses must implement the abstract {@link #resolveCode} method. For efficient resolution of messages without
 * arguments, the {@link #resolveCodeWithoutArguments} method should be overridden as well, resolving messages without a
 * MessageFormat being involved.
 *
 * <p>
 * <b>Note:</b> By default, message texts are only parsed through MessageFormat if arguments have been passed in for the
 * message. In case of no arguments, message texts will be returned as-is. As a consequence, you should only use
 * MessageFormat escaping for messages with actual arguments, and keep all other messages unescaped. If you prefer to
 * escape all messages, set the "alwaysUseMessageFormat" flag to "true".
 *
 * <p>
 * Supports not only MessageSourceResolvables as primary messages but also resolution of message arguments that are in
 * turn MessageSourceResolvables themselves.
 *
 * <p>
 * This class does not implement caching of messages per code, thus subclasses can dynamically change messages over
 * time. Subclasses are encouraged to cache their messages in a modification-aware fashion, allowing for hot deployment
 * of updated messages.
 *
...
public abstract class AbstractMessageSource extends MessageSourceSupport implements HierarchicalMessageSource {
    ...
    /**
     * Specify locale-independent common messages, with the message code as key and the full message String (may contain
     * argument placeholders) as value.
     * <p>
    ...
    /**
     * Return whether to use the message code as default message instead of throwing a NoSuchMessageException. Useful
     * for development and debugging. Default is "false".
     * <p>
     * Alternatively, consider overriding the {@link #getDefaultMessage} method to return a custom fallback message for
     * an unresolvable code.
     *
     ...
     * <p>
     * Note: In case of a MessageSourceResolvable with multiple codes (like a FieldError) and a MessageSource that has a
     * parent MessageSource, do <i>not</i> activate "useCodeAsDefaultMessage" in the <i>parent</i>: Else, you'll get the
     * first code returned as message by the parent, without attempts to check further codes.
     * <p>
     * To be able to work with "useCodeAsDefaultMessage" turned on in the parent, AbstractMessageSource and
     * AbstractApplicationContext contain special checks to delegate to the internal {@link #getMessageInternal} method
     * if available. In general, it is recommended to just use "useCodeAsDefaultMessage" during development and not rely
     * on it in production in the first place, though.
     *
     ...
     * <p>
     * This implementation fully renders the default message if available, or just returns the plain default message
     * {@code String} if the primary message code is being used as a default message.
     * 
     * @param resolvable
     ...
     * case of no fallback, the caller will usually receive a {@code NoSuchMessageException} from {@code getMessage}.
     * 
     * @param code
     ...
     * <p>
     * Returns a MessageFormat instance rather than a message String, to allow for appropriate caching of MessageFormats
     * in subclasses.
     * <p>
     * <b>Subclasses are encouraged to provide optimized resolution for messages without arguments, not involving
     * MessageFormat.</b> See the {@link #resolveCodeWithoutArguments} javadoc for details.
     * 
     * @param code
     *            the code of the message to resolve
     * @param locale
