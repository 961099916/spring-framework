 ...
 * {@link GenericApplicationContext} and implements {@link GroovyObject} such that beans can be retrieved with the dot
 * de-reference syntax instead of using {@link #getBean}.
 *
 * <p>
 * Consider this as the equivalent of {@link GenericXmlApplicationContext} for Groovy bean definitions, or even an
 * upgrade thereof since it seamlessly understands XML bean definition files as well. The main difference is that,
 * within a Groovy script, the context can be used with an inline bean definition closure as follows:
 *
 * <pre class="code">
 ...
 *
 * <p>
 * Alternatively, load a Groovy bean definition script like the following from an external resource (e.g. an
 * "applicationContext.groovy" file):
 *
 * <pre class="code">
 * import org.hibernate.SessionFactory
 * import org.apache.commons.dbcp.BasicDataSource
 ...
 *
 * <p>
 * With the following Java code creating the {@code GenericGroovyApplicationContext} (potentially using Ant-style
 * '*'/'**' location patterns):
 *
 * <pre class="code">
 ...
 *
 * <p>
 * <b>This application context also understands XML bean definition files, allowing for seamless mixing and matching
 * with Groovy bean definition files.</b> ".xml" files will be parsed as XML content; all other kinds of resources will
...
public class GenericGroovyApplicationContext extends GenericApplicationContext implements GroovyObject {
    ...
    /**
     * Create a new GenericGroovyApplicationContext that needs to be {@link #load loaded} and then manually
     * {@link #refresh refreshed}.
     */
    public GenericGroovyApplicationContext() {}

    /**
     * Create a new GenericGroovyApplicationContext, loading bean definitions from the given resources and automatically
    ...
    /**
     * Create a new GenericGroovyApplicationContext, loading bean definitions from the given resource locations and
     ...
     * 
     * @param relativeClass
     *            class whose package will be used as a prefix when loading each specified resource name
     * @param resourceNames
    ...
    /**
     * Exposes the underlying {@link GroovyBeanDefinitionReader} for convenient access to the {@code loadBeanDefinition}
    ...
    /**
     * Delegates the given environment to underlying {@link GroovyBeanDefinitionReader}. Should be called before any
     * call to {@code #load}.
     */
    @Override
     ...
     * <p>
     * Note that ".xml" files will be parsed as XML content; all other kinds of resources will be parsed as Groovy
     * scripts.
     * 
     * @param resources
