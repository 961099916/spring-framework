 ...
 * {@code ApplicationContext} (with a local {@code LiveBeansView} bean definition) or all registered ApplicationContexts
...
public class LiveBeansView implements LiveBeansViewMBean, ApplicationContextAware {
     ...
     * {@link #findApplicationContexts()}, then delegating to {@link #generateJson(java.util.Set)}.
     */
    @Override
     ...
     * <p>
     * This implementation doesn't use any JSON parsing libraries in order to avoid third-party library dependencies. It
     * produces an array of context description objects, each containing a context and parent attribute as well as a
     * beans attribute with nested bean description objects. Each bean object contains a bean, scope, type and resource
     * attribute, as well as a dependencies attribute with a nested array of bean names that the present bean depends
     * on.
     * 
     * @param contexts
    ...
    /**
     * Determine a resource description for the given bean definition and apply basic JSON escaping (backslashes, double
     * quotes) to it.
     * 
     * @param bd
