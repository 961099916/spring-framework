...
/**
 * {@link GenericApplicationListener} adapter that delegates the processing of an event to an {@link EventListener}
 * annotated method.
 *
 * <p>
 * Delegates to {@link #processEvent(ApplicationEvent)} to give subclasses a chance to deviate from the default. Unwraps
 * the content of a {@link PayloadApplicationEvent} if necessary to allow a method declaration to define any arbitrary
 * event type. If a condition is defined, it is evaluated prior to invoking the underlying method.
 *
...
public class ApplicationListenerMethodAdapter implements GenericApplicationListener {
    ...
    private static final boolean reactiveStreamsPresent =
        ClassUtils.isPresent("org.reactivestreams.Publisher", ApplicationListenerMethodAdapter.class.getClassLoader());
    ...
    /**
     * Process the specified {@link ApplicationEvent}, checking if the condition matches and handling a non-null result,
     * if any.
     */
     ...
     * <p>
     * These arguments will be used to invoke the method handled by this instance. Can return {@code null} to indicate
     * that no suitable arguments could be resolved and therefore the method should not be invoked at all for the
     * specified event.
     */
    @Nullable
     ...
     * <p>
     * Matches the {@code condition} attribute of the {@link EventListener} annotation or any matching attribute on a
     * composed annotation that is meta-annotated with {@code @EventListener}.
     */
    @Nullable
    ...
    /**
     * Assert that the target bean class is an instance of the class where the given method is declared. In some cases
     * the actual bean instance at event- processing time may be a JDK dynamic proxy (lazy initialization, prototype
     * beans, and others). Event listener beans that require proxying should prefer class-based proxy mechanisms.
     */
