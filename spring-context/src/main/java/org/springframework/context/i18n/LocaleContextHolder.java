...
/**
 * Simple holder class that associates a LocaleContext instance with the current thread. The LocaleContext will be
 ...
 * DispatcherServlet automatically exposes its current Locale here. Other applications can expose theirs too, to make
 * classes like MessageSourceAccessor automatically use that Locale.
 *
...
public final class LocaleContextHolder {
     ...
     *
     * @param localeContext
     *            the current LocaleContext, or {@code null} to reset the thread-bound context
     * @param inheritable
     *            whether to expose the LocaleContext as inheritable for child threads (using an
    ...
    /**
     * Associate the given LocaleContext with the current thread, <i>not</i> exposing it as inheritable for child
     * threads.
     * <p>
     * The given LocaleContext may be a {@link TimeZoneAwareLocaleContext}, containing a locale with associated time
     * zone information.
     ...
     *
     * @param locale
     *            the current Locale, or {@code null} to reset the locale part of thread-bound context
     * @param inheritable
     *            whether to expose the LocaleContext as inheritable for child threads (using an
     *            {@link InheritableThreadLocal})
     ...
     * <p>
     * <b>NOTE:</b> This can be useful to set an application-level default locale which differs from the JVM-wide
     * default locale. However, this requires each such application to operate against locally deployed Spring Framework
     * jars. Do not deploy Spring as a shared library at the server level in such a scenario!
     *
     * @param locale
     *            the default locale (or {@code null} for none, letting lookups fall back to
    ...
    /**
     * Return the Locale associated with the current thread, if any, or the system default Locale otherwise. This is
     * effectively a replacement for {@link java.util.Locale#getDefault()}, able to optionally respect a user-level
     * Locale setting.
     * <p>
     * Note: This method has a fallback to the shared default Locale, either at the framework level or at the JVM-wide
     * system level. If you'd like to check for the raw LocaleContext content (which may indicate no specific locale
     * through {@code null}, use {@link #getLocaleContext()} and call {@link LocaleContext#getLocale()}
     *
     ...
     * Associate the given Locale with the current thread, preserving any TimeZone that may have been set already.
     * <p>
     * Will implicitly create a LocaleContext for the given Locale, <i>not</i> exposing it as inheritable for child
     * threads.
     *
     * @param locale
     *            the current Locale, or {@code null} to reset the locale part of thread-bound context
    ...
    /**
     * Return the Locale associated with the given user context, if any, or the system default Locale otherwise. This is
     * effectively a replacement for {@link java.util.Locale#getDefault()}, able to optionally respect a user-level
     * Locale setting.
     * 
     * @param localeContext
     ...
     *
     * @param timeZone
     *            the current TimeZone, or {@code null} to reset the time zone part of the thread-bound context
     * @param inheritable
     *            whether to expose the LocaleContext as inheritable for child threads (using an
     *            {@link InheritableThreadLocal})
     ...
     * <p>
     * <b>NOTE:</b> This can be useful to set an application-level default time zone which differs from the JVM-wide
     * default time zone. However, this requires each such application to operate against locally deployed Spring
     * Framework jars. Do not deploy Spring as a shared library at the server level in such a scenario!
     *
     * @param timeZone
     *            the default time zone (or {@code null} for none, letting lookups fall back to
    ...
    /**
     * Return the TimeZone associated with the current thread, if any, or the system default TimeZone otherwise. This is
     * effectively a replacement for {@link java.util.TimeZone#getDefault()}, able to optionally respect a user-level
     * TimeZone setting.
     * <p>
     * Note: This method has a fallback to the shared default TimeZone, either at the framework level or at the JVM-wide
     * system level. If you'd like to check for the raw LocaleContext content (which may indicate no specific time zone
     * through {@code null}, use {@link #getLocaleContext()} and call {@link TimeZoneAwareLocaleContext#getTimeZone()}
     ...
     * This is effectively a replacement for {@link java.util.TimeZone#getDefault()}, able to optionally respect a
