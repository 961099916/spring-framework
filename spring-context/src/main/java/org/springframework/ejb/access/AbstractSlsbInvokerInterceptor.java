...
/**
 * Base class for AOP interceptors invoking local or remote Stateless Session Beans. Designed for EJB 2.x, but works for
 * EJB 3 Session Beans as well.
 *
 * <p>
 * Such an interceptor must be the last interceptor in the advice chain. In this case, there is no direct target object:
 * The call is handled in a special way, getting executed on an EJB instance retrieved via an EJB home.
 *
 * @author Rod Johnson
...
public abstract class AbstractSlsbInvokerInterceptor extends JndiObjectLocator implements MethodInterceptor {
    ...
    /**
     * The EJB's home object, potentially cached. The type must be Object as it could be either EJBHome or EJBLocalHome.
     */
    @Nullable
     ...
     * <p>
     * Can be turned off to allow for late start of the EJB server. In this case, the EJB home object will be fetched on
     * first access.
     * 
     * @see #setCacheHome
     */
     ...
     * <p>
     * Can be turned off to allow for hot restart of the EJB server. In this case, the EJB home object will be fetched
     * for each invocation.
     * 
    ...
    /**
     * Set whether to expose the JNDI environment context for all access to the target EJB, i.e. for all method
     ...
     * to expose the JNDI environment (including the authorization context) for each EJB invocation, as needed by
     * WebLogic for EJBs with authorization requirements.
     */
     ...
     * <p>
     * Default implementation returns the home created on initialization, if any; else, it invokes lookup to get a new
     * proxy for each invocation.
     * <p>
     * Can be overridden in subclasses, for example to cache a home object for a given amount of time before recreating
     * it, or to test the home object whether it is still alive.
     * 
     * @return the EJB home object to use for an invocation
     * @throws NamingException
     *             if proxy creation failed
     * @see #lookup
     * @see #getCreateMethod
     */
