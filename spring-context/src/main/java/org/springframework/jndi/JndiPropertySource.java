 ...
 * {@link JndiLocatorDelegate#setResourceRef(boolean) "resourceRef"} property set to {@code true}, meaning that names
 * looked up will automatically be prefixed with "java:comp/env/" in alignment with published
 * <a href="https://download.oracle.com/javase/jndi/tutorial/beyond/misc/policy.html">JNDI naming conventions</a>. To
 * override this setting or to change the prefix, manually configure a {@code JndiLocatorDelegate} and provide it to one
 * of the constructors here that accepts it. The same applies when providing custom JNDI properties. These should be
 * specified using {@link JndiLocatorDelegate#setJndiEnvironment(java.util.Properties)} prior to construction of the
 * {@code JndiPropertySource}.
 *
 * <p>
 * Note that {@link org.springframework.web.context.support.StandardServletEnvironment StandardServletEnvironment}
 * includes a {@code JndiPropertySource} by default, and any customization of the underlying {@link JndiLocatorDelegate}
 * may be performed within an {@link org.springframework.context.ApplicationContextInitializer
...
public class JndiPropertySource extends PropertySource<JndiLocatorDelegate> {
    ...
    /**
     * Create a new {@code JndiPropertySource} with the given name and a {@link JndiLocatorDelegate} configured to
     * prefix any names with "java:comp/env/".
     */
     ...
     * {@link JndiLocatorDelegate}. If a {@link NamingException} is thrown during the call to
     * {@link JndiLocatorDelegate#lookup(String)}, returns {@code null} and issues a DEBUG-level log statement with the
     * exception message.
     */
    @Override
    @Nullable
