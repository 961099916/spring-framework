 ...
 *
 * <p>
 * Exposes a {@link #setJndiName "jndiName"} property. This may or may not include the "java:comp/env/" prefix expected
 * by Java EE applications when accessing a locally mapped (Environmental Naming Context) resource. If it doesn't, the
 ...
 *
 * <p>
 * Subclasses may invoke the {@link #lookup()} method whenever it is appropriate. Some classes might do this on
 * initialization, while others might do it on demand. The latter strategy is more flexible in that it allows for
...
public abstract class JndiObjectLocator extends JndiLocatorSupport implements InitializingBean {
    ...
    /**
     * Specify the JNDI name to look up. If it doesn't begin with "java:comp/env/" this prefix is added automatically if
     ...
     *             if the JNDI lookup failed or if the located JNDI object is not assignable to the expected type
     * @see #setJndiName
     * @see #setExpectedType
