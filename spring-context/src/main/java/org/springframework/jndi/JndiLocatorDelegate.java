...
public class JndiLocatorDelegate extends JndiLocatorSupport {
    ...
    /**
     * System property that instructs Spring to ignore a default JNDI environment, i.e. to always return {@code false}
     * from {@link #isDefaultJndiEnvironmentAvailable()}.
     * <p>
     * The default is "false", allowing for regular default JNDI access e.g. in {@link JndiPropertySource}. Switching
     * this flag to {@code true} is an optimization for scenarios where nothing is ever to be found for such JNDI
     * fallback searches to begin with, avoiding the repeated JNDI lookup overhead.
     * <p>
     * Note that this flag just affects JNDI fallback searches, not explicitly configured JNDI lookups such as for a
     * {@code DataSource} or some other environment resource. The flag literally just affects code which attempts JNDI
     * searches based on the {@code JndiLocatorDelegate.isDefaultJndiEnvironmentAvailable()} check: in particular,
    ...
    /**
     * Configure a {@code JndiLocatorDelegate} with its "resourceRef" property set to {@code true}, meaning that all
     * names will be prefixed with "java:comp/env/".
     *
     * @see #setResourceRef
     */
