...
/**
 * AOP {@link org.springframework.aop.TargetSource} that provides configurable JNDI lookups for {@code getTarget()}
 * calls.
 *
 * <p>
 * Can be used as alternative to {@link JndiObjectFactoryBean}, to allow for relocating a JNDI object lazily or for each
 * operation (see "lookupOnStartup" and "cache" properties). This is particularly useful during development, as it
 ...
 * </pre>
 *
 * A {@code createQueueConnection} call on the "queueConnectionFactory" proxy will cause a lazy JNDI lookup for
 * "JmsQueueConnectionFactory" and a subsequent delegating call to the retrieved QueueConnectionFactory's
 * {@code createQueueConnection}.
 *
 * <p>
 * <b>Alternatively, use a {@link JndiObjectFactoryBean} with a "proxyInterface".</b> "lookupOnStartup" and "cache" can
 * then be specified on the JndiObjectFactoryBean, creating a JndiObjectTargetSource underneath (instead of defining
 * separate ProxyFactoryBean and JndiObjectTargetSource beans).
 *
