...
/**
 * BeanDefinitionParser implementation for the '{@code <lang:groovy/>}', '{@code <lang:std/>}' and '{@code <lang:bsh/>}'
 * tags. Allows for objects written using dynamic languages to be easily exposed with the
 * {@link org.springframework.beans.factory.BeanFactory}.
 *
 * <p>
 * The script for each object can be specified either as a reference to the resource containing it (using the
 * '{@code script-source}' attribute) or inline in the XML configuration itself (using the '{@code inline-script}'
 * attribute.
 *
 * <p>
 * By default, dynamic objects created with these tags are <strong>not</strong> refreshable. To enable refreshing,
 * specify the refresh check delay for each object (in milliseconds) using the '{@code refresh-check-delay}' attribute.
 *
 * @author Rob Harrop
 * @author Rod Johnson
...
class ScriptBeanDefinitionParser extends AbstractBeanDefinitionParser {
    ...
    /**
     * The {@link org.springframework.scripting.ScriptFactory} class that this parser instance will create bean
     * definitions for.
     */
    ...
    /**
     * Resolves the script source from either the '{@code script-source}' attribute or the '{@code inline-script}'
     * element. Logs and {@link XmlReaderContext#error} and returns {@code null} if neither or both of these values are
     * specified.
     */
    @Nullable
