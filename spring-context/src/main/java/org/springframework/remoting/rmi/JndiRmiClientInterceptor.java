...
/**
 * {@link org.aopalliance.intercept.MethodInterceptor} for accessing RMI services from JNDI. Typically used for RMI-IIOP
 * but can also be used for EJB home objects (for example, a Stateful Session Bean home). In contrast to a plain JNDI
 * lookup, this accessor also performs narrowing through PortableRemoteObject.
 *
 * <p>
 ...
 *
 * <p>
 * The JNDI environment can be specified as "jndiEnvironment" property, or be configured in a {@code jndi.properties}
...
public class JndiRmiClientInterceptor extends JndiObjectLocator implements MethodInterceptor, InitializingBean {
    ...
    /**
     * Set whether to expose the JNDI environment context for all access to the target RMI stub, i.e. for all method
     ...
     * to expose the JNDI environment (including the authorization context) for each RMI invocation, as needed by
     * WebLogic for RMI stubs with authorization requirements.
     */
        ...
        if (this.lookupStubOnStartup) {
            ...
            if (logger.isDebugEnabled()) {
                if (remoteObj instanceof RmiInvocationHandler) {
                    ...
                    logger
                        .debug("Using service interface [" + getServiceInterface().getName() + "] for JNDI RMI object ["
    ...
    /**
     * Fetches an RMI stub and delegates to {@link #doInvoke}. If configured to refresh on connect failure, it will call
     ...
     * @throws Throwable
     *             an exception raised by the new invocation, if failed too.
     */
    ...
    /**
     * Convert the given RMI RemoteException that happened during remote access to Spring's RemoteAccessException if the
     * method signature does not declare RemoteException. Else, return the original RemoteException.
     * 
     * @param method
     *            the invoked method
     * @param ex
