...
/**
 * Implementation of the {@link MBeanInfoAssembler} interface that reads the management interface information from
 * source level metadata.
 *
 * <p>
 * Uses the {@link JmxAttributeSource} strategy interface, so that metadata can be read using any supported
 * implementation. Out of the box, Spring provides an implementation based on annotations:
...
public class MetadataMBeanInfoAssembler extends AbstractReflectiveMBeanInfoAssembler
    ...
    /**
     * Throws an IllegalArgumentException if it encounters a JDK dynamic proxy. Metadata can only be read from target
     * classes and CGLIB proxies!
     */
    @Override
    ...
    /**
     * Reads managed resource description from the source level metadata. Returns an empty {@code String} if no
    ...
    /**
     * Creates a description for the attribute corresponding to this property descriptor. Attempts to create the
     * description using metadata from either the getter or setter attributes, otherwise uses the property name.
     */
    @Override
    ...
    /**
     * Retrieves the description for the supplied {@code Method} from the metadata. Uses the method name is no
     * description is present in the metadata.
     */
    @Override
    ...
    /**
     * Reads {@code MBeanParameterInfo} from the {@code ManagedOperationParameter} attributes attached to a method.
     * Returns an empty array of {@code MBeanParameterInfo} if no attributes are found.
     */
    @Override
    ...
    /**
     * Reads the {@link ManagedNotification} metadata from the {@code Class} of the managed resource and generates and
     * returns the corresponding {@link ModelMBeanNotificationInfo} metadata.
     */
    @Override
    ...
    /**
     * Adds descriptor fields from the {@code ManagedResource} attribute to the MBean descriptor. Specifically, adds the
     * {@code currencyTimeLimit}, {@code persistPolicy}, {@code persistPeriod}, {@code persistLocation} and
    ...
    /**
     * Adds descriptor fields from the {@code ManagedAttribute} attribute or the {@code ManagedMetric} attribute to the
    ...
    /**
     * Determines which of two {@code int} values should be used as the value for an attribute descriptor. In general,
     * only the getter or the setter will be have a non-negative value so we use that value. In the event that both
     * values are non-negative, we use the greater of the two. This method can be used to resolve any {@code int} valued
     * descriptor where there are two possible values.
     * 
     * @param getter
     *            the int value associated with the getter for this attribute
     * @param setter
     *            the int associated with the setter for this attribute
     */
    ...
    /**
     * Locates the value of a descriptor based on values attached to both the getter and setter methods. If both have
     * values supplied then the value attached to the getter is preferred.
     * 
     * @param getter
     *            the Object value associated with the get method
     * @param setter
     *            the Object value associated with the set method
    ...
    /**
     * Locates the value of a descriptor based on values attached to both the getter and setter methods. If both have
     * values supplied then the value attached to the getter is preferred. The supplied default value is used to check
     * to see if the value associated with the getter has changed from the default.
     * 
     * @param getter
     *            the String value associated with the get method
     * @param setter
     *            the String value associated with the set method
