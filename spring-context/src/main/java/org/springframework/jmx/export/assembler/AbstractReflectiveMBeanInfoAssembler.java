...
/**
 * Builds on the {@link AbstractMBeanInfoAssembler} superclass to add a basic algorithm for building metadata based on
 * the reflective metadata of the MBean class.
 *
 * <p>
 * The logic for creating MBean metadata from the reflective metadata is contained in this class, but this class makes
 * no decisions as to which methods and properties are to be exposed. Instead it gives subclasses a chance to 'vote' on
 * each property or method through the {@code includeXXX} methods.
 *
 * <p>
 * Subclasses are also given the opportunity to populate attribute and operation metadata with additional descriptors
 * once the metadata is assembled through the {@code populateXXXDescriptor} methods.
 *
 * @author Rob Harrop
...
public abstract class AbstractReflectiveMBeanInfoAssembler extends AbstractMBeanInfoAssembler {
    ...
    /**
     * Set the default for the JMX field "currencyTimeLimit". The default will usually indicate to never cache attribute
     * values.
     * <p>
     * Default is none, not explicitly setting that field, as recommended by the JMX 1.2 specification. This should
     * result in "never cache" behavior, always reading attribute values freshly (which corresponds to a
     * "currencyTimeLimit" of {@code -1} in JMX 1.2).
     * <p>
     * However, some JMX implementations (that do not follow the JMX 1.2 spec in that respect) might require an explicit
     * value to be set here to get "never cache" behavior: for example, JBoss 3.2.x.
     * <p>
     * Note that the "currencyTimeLimit" value can also be specified on a managed attribute or operation. The default
     * value will apply if not overridden with a "currencyTimeLimit" value {@code >= 0} there: a metadata
     * "currencyTimeLimit" value of {@code -1} indicates to use the default; a value of {@code 0} indicates to "always
     * cache" and will be translated to {@code Integer.MAX_VALUE}; a positive value indicates the number of cache
     * seconds.
     *
     ...
     * <p>
     * When using strict casing, a JavaBean property with a getter such as {@code getFoo()} translates to an attribute
     * called {@code Foo}. With strict casing disabled, {@code getFoo()} would translate to just {@code foo}.
     */
    ...
    /**
     * Set whether to expose the JMX descriptor field "class" for managed operations. Default is "false", letting the
     * JMX implementation determine the actual class through reflection.
     * <p>
     * Set this property to {@code true} for JMX implementations that require the "class" field to be specified, for
     * example WebLogic's. In that case, Spring will expose the target class name there, in case of a plain bean
     * instance or a CGLIB proxy. When encountering a JDK dynamic proxy, the <b>first</b> interface implemented by the
     * proxy will be specified.
     * <p>
     * <b>WARNING:</b> Review your proxy definitions when exposing a JDK dynamic proxy through JMX, in particular with
     * this property turned to {@code true}: the specified interface list should start with your management interface in
     * this case, with all other interfaces following. In general, consider exposing your target bean directly or a
     * CGLIB proxy for it instead.
     *
    ...
    /**
     * Return the ParameterNameDiscoverer to use for resolving method parameter names if needed (may be {@code null} in
     * order to skip parameter detection).
     */
    @Nullable
    ...
    /**
     * Set the ParameterNameDiscoverer to use for resolving method parameter names if needed (e.g. for parameter names
     * of MBean operation methods).
     * <p>
    ...
    /**
     * Iterate through all properties on the MBean class and gives subclasses the chance to vote on the inclusion of
     * both the accessor and mutator. If a particular accessor or mutator is voted for inclusion, the appropriate
     * metadata is assembled and passed to the subclass for descriptor population.
     * 
     * @param managedBean
     *            the bean instance (might be an AOP proxy)
     * @param beanKey
     *            the key associated with the MBean in the beans map of the {@code MBeanExporter}
    ...
    /**
     * Iterate through all methods on the MBean class and gives subclasses the chance to vote on their inclusion. If a
     * particular method corresponds to the accessor or mutator of an attribute that is included in the management
     * interface, then the corresponding operation is exposed with the &quot;role&quot; descriptor field set to the
     * appropriate value.
     * 
     * @param managedBean
     *            the bean instance (might be an AOP proxy)
    ...
    /**
     * Creates an instance of {@code ModelMBeanOperationInfo} for the given method. Populates the parameter info for the
     * operation.
     * 
     * @param method
     *            the {@code Method} to create a {@code ModelMBeanOperationInfo} for
     * @param name
     *            the logical name for the operation (method name or property name); not used by the default
     *            implementation but possibly by subclasses
     * @param beanKey
     *            the key associated with the MBean in the beans map of the {@code MBeanExporter}
    ...
    protected ModelMBeanOperationInfo createModelMBeanOperationInfo(Method method, String name, String beanKey) {
        ...
        if (params.length == 0) {
            ...
            return new ModelMBeanOperationInfo(method.getName(), getOperationDescription(method, beanKey),
                getOperationParameters(method, beanKey), method.getReturnType().getName(), MBeanOperationInfo.UNKNOWN);
        }
    }

    /**
     * Return the class to be used for the JMX descriptor field "class". Only applied when the "exposeClassDescriptor"
     * property is "true".
     * <p>
     ...
     * 
     * @param method
     *            the accessor {@code Method}
     * @param beanKey
     *            the key associated with the MBean in the beans map of the {@code MBeanExporter}
     * @return {@code true} if the accessor should be included in the management interface, otherwise {@code false}
     */
     ...
     * 
     * @param method
     *            the mutator {@code Method}.
     * @param beanKey
     *            the key associated with the MBean in the beans map of the {@code MBeanExporter}
     * @return {@code true} if the mutator should be included in the management interface, otherwise {@code false}
     */
     ...
     * <p>
     * The default implementation returns a description for the operation that is the name of corresponding
     * {@code Method}.
     * 
     * @param propertyDescriptor
     *            the PropertyDescriptor for the attribute
     * @param beanKey
     *            the key associated with the MBean in the beans map of the {@code MBeanExporter}
     ...
     * 
     * @param method
     *            the {@code Method} to get the parameter information for
     * @param beanKey
     *            the key associated with the MBean in the beans map of the {@code MBeanExporter}
     ...
     * <p>
     * The default implementation sets the {@code currencyTimeLimit} field to the specified "defaultCurrencyTimeLimit",
     * if any (by default none).
     * 
     * @param descriptor
     *            the {@code Descriptor} for the MBean resource.
     * @param managedBean
     *            the bean instance (might be an AOP proxy)
     * @param beanKey
     *            the key associated with the MBean in the beans map of the {@code MBeanExporter}
     ...
     * @param getter
     *            the accessor method for the attribute
     * @param setter
     *            the mutator method for the attribute
     * @param beanKey
     *            the key associated with the MBean in the beans map of the {@code MBeanExporter}
     * @see #setDefaultCurrencyTimeLimit(Integer)
     * @see #applyDefaultCurrencyTimeLimit(javax.management.Descriptor)
     ...
     * 
     * @param desc
     *            the JMX attribute or operation descriptor
     * @see #setDefaultCurrencyTimeLimit(Integer)
     */
     ...
     * <p>
     * The default implementation sets a value {@code >0} as-is (as number of cache seconds), turns a value of {@code 0}
     * into {@code Integer.MAX_VALUE} ("always cache") and sets the "defaultCurrencyTimeLimit" (if any, indicating
     * "never cache") in case of a value {@code <0}. This follows the recommendation in the JMX 1.2 specification.
     * 
     * @param desc
     *            the JMX attribute or operation descriptor
     * @param currencyTimeLimit
     *            the "currencyTimeLimit" value to apply
     * @see #setDefaultCurrencyTimeLimit(Integer)
     * @see #applyDefaultCurrencyTimeLimit(javax.management.Descriptor)
     */
