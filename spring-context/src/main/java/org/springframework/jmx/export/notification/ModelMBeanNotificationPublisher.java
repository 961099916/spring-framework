...
/**
 * {@link NotificationPublisher} implementation that uses the infrastructure provided by the {@link ModelMBean}
 * interface to track {@link javax.management.NotificationListener javax.management.NotificationListeners} and send
 * {@link Notification Notifications} to those listeners.
 *
 * @author Rob Harrop
...
public class ModelMBeanNotificationPublisher implements NotificationPublisher {
    ...
    /**
     * The {@link ModelMBean} instance wrapping the managed resource into which this {@code NotificationPublisher} will
     * be injected.
     */
    ...
    /**
     * Create a new instance of the {@link ModelMBeanNotificationPublisher} class that will publish all
     * {@link javax.management.Notification Notifications} to the supplied {@link ModelMBean}.
     * 
     * @param modelMBean
     *            the target {@link ModelMBean}; must not be {@code null}
     * @param objectName
     *            the {@link ObjectName} of the source {@link ModelMBean}
     * @param managedResource
     *            the managed resource exposed by the supplied {@link ModelMBean}
     ...
     * 
     * @param notification
     *            the {@link Notification} to be sent
     * @throws IllegalArgumentException
     *             if the supplied {@code notification} is {@code null}
     * @throws UnableToSendNotificationException
     *             if the supplied {@code notification} could not be sent
     */
    @Override
    ...
    /**
     * Replaces the notification source if necessary to do so. From the {@link Notification javadoc}: <i>"It is strongly
     * recommended that notification senders use the object name rather than a reference to the MBean object as the
     * source."</i>
     * 
     * @param notification
     *            the {@link Notification} whose {@link javax.management.Notification#getSource()} might need massaging
     */
