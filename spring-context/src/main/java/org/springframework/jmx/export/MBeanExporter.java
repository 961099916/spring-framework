...
/**
 * JMX exporter that allows for exposing any <i>Spring-managed bean</i> to a JMX {@link javax.management.MBeanServer},
 * without the need to define any JMX-specific information in the bean classes.
 *
 * <p>
 * If a bean implements one of the JMX management interfaces, MBeanExporter can simply register the MBean with the
 * server through its autodetection process.
 *
 * <p>
 * If a bean does not implement one of the JMX management interfaces, MBeanExporter will create the management
 * information using the supplied {@link MBeanInfoAssembler}.
 *
 * <p>
 * A list of {@link MBeanExporterListener MBeanExporterListeners} can be registered via the
 * {@link #setListeners(MBeanExporterListener[]) listeners} property, allowing application code to be notified of MBean
...
public class MBeanExporter extends MBeanRegistrationSupport implements MBeanExportOperations, BeanClassLoaderAware,
    ...
    /**
     * Wildcard used to map a {@link javax.management.NotificationListener} to all MBeans registered by the
     * {@code MBeanExporter}.
     */
     ...
     * <p>
     * The String keys are the basis for the creation of JMX object names. By default, a JMX {@code ObjectName} will be
     * created straight from the given key. This can be customized through specifying a custom {@code NamingStrategy}.
     * <p>
     * Both bean instances and bean names are allowed as values. Bean instances are typically linked in through bean
     * references. Bean names will be resolved as beans in the current factory, respecting lazy-init markers (that is,
     * not triggering initialization of such beans).
     * 
     * @param beans
     *            a Map with JMX names as keys and bean instances or bean names as values
    ...
    /**
     * Set whether to autodetect MBeans in the bean factory that this exporter runs in. Will also ask an
     * {@code AutodetectCapableMBeanInfoAssembler} if available.
     * <p>
     * This feature is turned off by default. Explicitly specify {@code true} here to enable autodetection.
     * 
     * @see #setAssembler
     ...
     *             if the supplied value is not one of the {@code AUTODETECT_} constants
     ...
     * 
     * @throws IllegalArgumentException
     *             if the supplied value is not resolvable to one of the {@code AUTODETECT_} constants or is
     *             {@code null}
    ...
    /**
     * Specify whether to allow eager initialization of candidate beans when autodetecting MBeans in the Spring
     * application context.
     * <p>
     * Default is "false", respecting lazy-init flags on bean definitions. Switch this to "true" in order to search
     * lazy-init beans as well, including FactoryBean-produced objects that haven't been initialized yet.
     */
    ...
    /**
     * Set the implementation of the {@code MBeanInfoAssembler} interface to use for this exporter. Default is a
     * {@code SimpleReflectiveMBeanInfoAssembler}.
     * <p>
     * The passed-in assembler can optionally implement the {@code AutodetectCapableMBeanInfoAssembler} interface, which
     * enables it to participate in the exporter's MBean autodetection process.
     * 
    ...
    /**
     * Set the implementation of the {@code ObjectNamingStrategy} interface to use for this exporter. Default is a
     * {@code KeyNamingStrategy}.
     * 
     * @see org.springframework.jmx.export.naming.KeyNamingStrategy
    ...
    /**
     * Indicates whether Spring should ensure that {@link ObjectName ObjectNames} generated by the configured
     * {@link ObjectNamingStrategy} for runtime-registered MBeans ({@link #registerManagedResource}) should get
    ...
    /**
     * Indicates whether or not the managed resource should be exposed on the {@link Thread#getContextClassLoader()
     * thread context ClassLoader} before allowing any invocations on the MBean to occur.
     * <p>
     * The default value is {@code true}, exposing a {@link SpringModelMBean} which performs thread context ClassLoader
     * management. Switch this flag off to expose a standard JMX {@link javax.management.modelmbean.RequiredModelMBean}.
     */
     ...
     * {@link javax.management.NotificationListener NotificationListeners} that will be registered with the
     * {@link MBeanServer}.
     * 
     ...
     * {@link javax.management.ObjectName} or the bean name of the MBean the listener should be registered for.
     * Specifying an asterisk ({@code *}) for a key will cause the listener to be associated with all MBeans registered
     * by this class at startup time.
     * <p>
     * The value of each entry is the {@link javax.management.NotificationListener} to register. For more advanced
     * options such as registering {@link javax.management.NotificationFilter NotificationFilters} and handback objects
     * see {@link #setNotificationListeners(NotificationListenerBean[])}.
     */
    ...
    /**
     * This callback is only required for resolution of bean names in the {@link #setBeans(java.util.Map) "beans"}
     * {@link Map} and for autodetection of MBeans (in the latter case, a {@code ListableBeanFactory} is required).
     * 
     * @see #setBeans
     * @see #setAutodetect
     */
    ...
    /**
     * Unregisters all beans that this exported has exposed via JMX when the enclosing {@code ApplicationContext} is
     * destroyed.
     */
    @Override
    public void destroy() {
     ...
     * <p>
     * Each bean is exposed to the {@code MBeanServer} via a {@code ModelMBean}. The actual implementation of the
     * {@code ModelMBean} interface used depends on the implementation of the {@code ModelMBeanProvider} interface that
     * is configured. By default the {@code RequiredModelMBean} class that is supplied with all JMX implementations is
     * used.
     * <p>
     * The management interface produced for each bean is dependent on the {@code MBeanInfoAssembler} implementation
     * being used. The {@code ObjectName} given to each bean is dependent on the implementation of the
     * {@code ObjectNamingStrategy} interface being used.
     */
     ...
     * <p>
     * This method is responsible for deciding <strong>how</strong> a bean should be exposed to the {@code MBeanServer}.
     * Specifically, if the supplied {@code mapValue} is the name of a bean that is configured for lazy initialization,
     * then a proxy to the resource is registered with the {@code MBeanServer} so that the lazy load behavior is
     * honored. If the bean is already an MBean then it will be registered directly with the {@code MBeanServer} without
     * any intervention. For all other beans or bean names, the resource itself is registered with the
     * {@code MBeanServer} directly.
     * 
     * @param mapValue
     *            the value configured for this bean in the beans map; may be either the {@code String} name of a bean,
     *            or the bean itself
     * @param beanKey
    ...
    /**
     * Replace any bean names used as keys in the {@code NotificationListener} mappings with their corresponding
     * {@code ObjectName} values.
     * 
     * @param beanName
     *            the name of the bean to be registered
     * @param objectName
     *            the {@code ObjectName} under which the bean will be registered with the {@code MBeanServer}
     */
    ...
    /**
     * Register beans that are configured for lazy initialization with the {@code MBeanServer} indirectly through a
     * proxy.
     * 
     * @param beanName
     *            the name of the bean in the {@code BeanFactory}
     * @param beanKey
     *            the key associated with this bean in the beans map
     * @return the {@code ObjectName} under which the bean was registered with the {@code MBeanServer}
     */
     ...
     * <p>
     * If the bean implements the {@code SelfNaming} interface, then the {@code ObjectName} will be retrieved using
     * {@code SelfNaming.getObjectName()}. Otherwise, the configured {@code ObjectNamingStrategy} is used.
     * 
     * @param bean
     *            the name of the bean in the {@code BeanFactory}
     * @param beanKey
     *            the key associated with the bean in the beans map
     ...
     * {@link javax.management.DynamicMBean} classes as well as classes with corresponding "*MBean" interface (Standard
     * MBeans) or corresponding "*MXBean" interface (Java 6 MXBeans).
     * 
     * @param beanClass
     ...
     * <p>
     * The default implementation builds a JMX 1.2 StandardMBean for the target's MBean/MXBean interface in case of an
     * AOP proxy, delegating the interface's management operations to the proxy.
     * 
     * @param bean
    ...
    protected ModelMBean createAndConfigureMBean(Object managedResource, String beanKey) throws MBeanExportException {
        try {
            ...
            throw new MBeanExportException(
                "Could not create ModelMBean for managed resource [" + managedResource + "] with key '" + beanKey + "'",
                ex);
        }
    }
     ...
     * <p>
     * This method is called to obtain a {@code ModelMBean} instance to use when registering a bean. This method is
     * called once per bean during the registration phase and must return a new instance of {@code ModelMBean}
     * 
    ...
    /**
     * Performs the actual autodetection process, delegating to an {@code AutodetectCallback} instance to vote on the
     * inclusion of a given bean.
     * 
     * @param callback
     *            the {@code AutodetectCallback} to use when deciding whether to include a bean or not
     */
                ...
                try {
                    ...
                    if (beanClass != null && callback.include(beanClass, beanName)) {
                        ...
                        if (!ScopedProxyUtils.isScopedTarget(beanName) && !beans.containsValue(beanName)
                            ...
                            if (logger.isDebugEnabled()) {
                                logger
                                    .debug("Bean with name '" + beanName + "' has been autodetected for JMX exposure");
                            }
                        } else {
                            if (logger.isTraceEnabled()) {
                                logger
                                    .trace("Bean with name '" + beanName + "' is already registered for JMX exposure");
                            }
                        }
                    }
    ...
    private void injectNotificationPublisherIfNecessary(Object managedResource, @Nullable ModelMBean modelMBean,
        ...
        if (managedResource instanceof NotificationPublisherAware && modelMBean != null && objectName != null) {
            ((NotificationPublisherAware)managedResource)
                .setNotificationPublisher(new ModelMBeanNotificationPublisher(modelMBean, objectName, managedResource));
        }
    }
     ...
     * <p>
     * Please note that if an {@link MBeanExporterListener} throws a (runtime) exception when notified, this will
     * essentially interrupt the notification process and any remaining listeners that have yet to be notified will not
     * (obviously) receive the {@link MBeanExporterListener#mbeanRegistered(javax.management.ObjectName)} callback.
     * 
     * @param objectName
     *            the {@code ObjectName} of the registered MBean
     */
    @Override
     ...
     * Please note that if an {@link MBeanExporterListener} throws a (runtime) exception when notified, this will
     * essentially interrupt the notification process and any remaining listeners that have yet to be notified will not
     * (obviously) receive the {@link MBeanExporterListener#mbeanUnregistered(javax.management.ObjectName)} callback.
     * 
     * @param objectName
     *            the {@code ObjectName} of the unregistered MBean
     */
    @Override
    ...
    /**
     * Notifies all registered {@link MBeanExporterListener MBeanExporterListeners} of the registration of the MBean
     * identified by the supplied {@link ObjectName}.
     */
    ...
    /**
     * Notifies all registered {@link MBeanExporterListener MBeanExporterListeners} of the unregistration of the MBean
     * identified by the supplied {@link ObjectName}.
     */
     ...
     * {@link org.springframework.jmx.export.notification.NotificationPublisher} into the lazy resource as it is created
     * if required.
     */
