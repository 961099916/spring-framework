...
/**
 * {@link org.aopalliance.intercept.MethodInterceptor} that routes calls to an MBean running on the supplied
 * {@code MBeanServerConnection}. Works for both local and remote {@code MBeanServerConnection}s.
 *
 * <p>
 * By default, the {@code MBeanClientInterceptor} will connect to the {@code MBeanServer} and cache MBean metadata at
 * startup. This can be undesirable when running against a remote {@code MBeanServer} that may not be running when the
 * application starts. Through setting the {@link #setConnectOnStartup(boolean) connectOnStartup} property to "false",
 * you can defer this process until the first invocation against the proxy.
 *
 * <p>
 * This functionality is usually used through {@link MBeanProxyFactoryBean}. See the javadoc of that class for more
 * information.
 *
 * @author Rob Harrop
...
public class MBeanClientInterceptor
     ...
     * entries.
     * <p>
     * Useful for specifying entries directly, for example via "environment[myKey]". This is particularly useful for
     ...
     * <p>
     * Default is none. If specified, this will result in an attempt being made to locate the attendant MBeanServer,
     * unless the {@link #setServiceUrl "serviceUrl"} property has been set.
     * 
    ...
    /**
     * Set whether or not the proxy should connect to the {@code MBeanServer} at creation time ("true") or the first
     * time it is invoked ("false"). Default is "true".
     */
     ...
     * <p>
     * Can be turned on to allow for hot restart of the JMX server, automatically reconnecting and retrying in case of
     * an IOException.
     */
    ...
    /**
     * Set the {@code ObjectName} of the MBean which calls are routed to, as {@code ObjectName} instance or as
     * {@code String}.
     */
    ...
    /**
     * Set the management interface of the target MBean, exposing bean property setters and getters for MBean attributes
     * and conventional Java methods for MBean operations.
     */
    ...
    /**
     * Ensures that an {@code MBeanServerConnection} is configured and attempts to detect a local connection if one is
     * not supplied.
     */
    public void prepare() {
    ...
    /**
     * Loads the management interface info for the configured MBean into the caches. This information is used by the
     * proxy when determining whether an invocation matches a valid operation or attribute on the management interface
    ...
    /**
     * Return whether this client interceptor has already been prepared, i.e. has already looked up the server and
     * cached all metadata.
     */
     ...
     * 
     * @param invocation
     *            the {@code MethodInvocation} to re-route
    ...
    /**
     * Route the invocation to the configured managed resource. Correctly routes JavaBean property access to
     * {@code MBeanServerConnection.get/setAttribute} and method invocation to {@code MBeanServerConnection.invoke}.
     * 
     * @param invocation
     *            the {@code MethodInvocation} to re-route
     * @return the value returned as a result of the re-routed invocation
     ...
     * 
     * @param method
     *            the method corresponding to operation on the managed resource.
     * @param args
    ...
    /**
     * Convert the given result object (from attribute access or operation invocation) to the specified target class for
     * returning from the proxy method.
     * 
     * @param result
     *            the result object as returned by the {@code MBeanServer}
     * @param parameter
     *            the method parameter of the proxy method that's been invoked
