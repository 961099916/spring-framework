...
/**
 * {@link FactoryBean} that obtains a {@link javax.management.MBeanServer} reference through the standard JMX 1.2
 * {@link javax.management.MBeanServerFactory} API.
 *
 * <p>
 * Exposes the {@code MBeanServer} for bean references.
 *
 * <p>
 * By default, {@code MBeanServerFactoryBean} will always create a new {@code MBeanServer} even if one is already
 * running. To have the {@code MBeanServerFactoryBean} attempt to locate a running {@code MBeanServer} first, set the
 * value of the "locateExistingServerIfPossible" property to "true".
 *
 * @author Rob Harrop
...
public class MBeanServerFactoryBean implements FactoryBean<MBeanServer>, InitializingBean, DisposableBean {
    ...
    /**
     * Set whether or not the {@code MBeanServerFactoryBean} should attempt to locate a running {@code MBeanServer}
     * before creating one.
     * <p>
     ...
     * <p>
     * Default is none. If specified, this will result in an automatic attempt being made to locate the attendant
     * MBeanServer, and (importantly) if said MBeanServer cannot be located no attempt will be made to create a new
     * MBeanServer (and an MBeanServerNotFoundException will be thrown at resolution time).
     * <p>
     ...
     * {@code MBeanServerFactory.createMBeanServer()} or {@code MBeanServerFactory.findMBeanServer()}.
     * <p>
     * Default is none.
     * 
    ...
    /**
     * Set whether to register the {@code MBeanServer} with the {@code MBeanServerFactory}, making it available through
    ...
    /**
     * Attempt to locate an existing {@code MBeanServer}. Called if {@code locateExistingServerIfPossible} is set to
     * {@code true}.
     * <p>
     * The default implementation attempts to find an {@code MBeanServer} using a standard lookup. Subclasses may
     * override to add additional location logic.
     * 
     * @param agentId
     *            the agent identifier of the MBeanServer to retrieve. If this parameter is {@code null}, all registered
     *            MBeanServers are considered.
    ...
    /**
     * Create a new {@code MBeanServer} instance and register it with the {@code MBeanServerFactory}, if desired.
     * 
     * @param defaultDomain
     *            the default domain, or {@code null} if none
     * @param registerWithFactory
     *            whether to register the {@code MBeanServer} with the {@code MBeanServerFactory}
     * @see javax.management.MBeanServerFactory#createMBeanServer
     * @see javax.management.MBeanServerFactory#newMBeanServer
     */
