...
/**
 * {@link org.springframework.beans.factory.xml.BeanDefinitionParser} implementation that allows users to easily
 * configure all the infrastructure beans required to enable annotation-driven cache demarcation.
 *
 * <p>
 * By default, all proxies are created as JDK proxies. This may cause some problems if you are injecting objects as
 * concrete classes rather than interfaces. To overcome this restriction you can set the '{@code proxy-target-class}'
 * attribute to '{@code true}', which will result in class-based proxies being created.
 *
 * <p>
 * If the JSR-107 API and Spring's JCache implementation are present, the necessary infrastructure beans required to
 * handle methods annotated with {@code CacheResult}, {@code CachePut}, {@code CacheRemove} or {@code CacheRemoveAll}
 * are also registered.
 *
 * @author Costin Leau
...
class AnnotationDrivenCacheBeanDefinitionParser implements BeanDefinitionParser {
    ...
    /**
     * Parse the cache resolution strategy to use. If a 'cache-resolver' attribute is set, it is injected. Otherwise the
     * 'cache-manager' is set. If {@code setBoth} is {@code true}, both service are actually injected.
     */
     ...
     * {@link AopNamespaceUtils#registerAutoProxyCreatorIfNecessary register an AutoProxyCreator} with the container as
     * necessary.
     */
    @Override
    @Nullable
