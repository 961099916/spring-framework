 ...
 *
 * <p>
 * This enables the underlying Spring caching infrastructure to be used easily to implement an aspect for any aspect
 * system.
 *
 * <p>
 ...
 *
 * <p>
 * Uses the <b>Strategy</b> design pattern. A {@link CacheOperationSource} is used for determining caching operations, a
 * {@link KeyGenerator} will build the cache keys, and a {@link CacheResolver} will resolve the actual cache(s) to use.
 *
 * <p>
...
public abstract class CacheAspectSupport extends AbstractCacheInvoker
    ...
    /**
     * Configure this aspect with the given error handler, key generator and cache resolver/manager suppliers, applying
    ...
    public void configure(@Nullable Supplier<CacheErrorHandler> errorHandler,
        ...
        this.cacheResolver =
            new SingletonSupplier<>(cacheResolver, () -> SimpleCacheResolver.of(SupplierUtils.resolve(cacheManager)));
    }

    /**
     * Set one or more cache operation sources which are used to find the cache attributes. If more than one source is
     * provided, they will be aggregated using a {@link CompositeCacheOperationSource}.
     * 
    ...
    /**
     * Set the default {@link KeyGenerator} that this cache aspect should delegate to if no specific key generator has
    ...
    /**
     * Set the default {@link CacheResolver} that this cache aspect should delegate to if no specific cache resolver has
     * been set for the operation.
     * <p>
    ...
    /**
     * Set the {@link CacheManager} to use to create a default {@link CacheResolver}. Replace the current
     * {@link CacheResolver}, if any.
     * 
     ...
     *            the operation
     * @param method
     *            the method on which the operation is invoked
     * @param targetClass
     *            the target type
     ...
     * 
     * @param beanName
     *            the name of the bean, as defined by the operation
     * @param expectedType
     *            type for the bean
    ...
    /**
     * Execute the underlying operation (typically in case of cache miss) and return the result of the invocation. If an
     * exception occurs it will be wrapped in a {@link CacheOperationInvoker.ThrowableWrapper}: the exception can be
     * handled or modified but it <em>must</em> be wrapped in a {@link CacheOperationInvoker.ThrowableWrapper} as well.
     * 
     * @param invoker
    ...
    /**
     * Collect the {@link CachePutRequest} for all {@link CacheOperation} using the specified result item.
     * 
     * @param contexts
     *            the contexts to handle
     * @param result
     *            the result item (never {@code null})
     * @param putRequests
    ...
    /**
     * Metadata of a cache operation that does not depend on a particular invocation which makes it a good candidate for
     * caching.
     */
    protected static class CacheOperationMetadata {
        ...
        public CacheOperationMetadata(CacheOperation operation, Method method, Class<?> targetClass,
            ...
            this.targetMethod =
                (!Proxy.isProxyClass(targetClass) ? AopUtils.getMostSpecificMethod(method, targetClass) : this.method);
