...
/**
 * Adapter that implements WebSphere 7.0 ClassPreProcessPlugin interface, delegating to a standard JDK
 * {@link ClassFileTransformer} underneath.
 *
 * <p>
...
class WebSphereClassPreDefinePlugin implements InvocationHandler {
     ...
     * 
     * @param transformer
     *            the {@link ClassFileTransformer} to be adapted (must not be {@code null})
     */
    ...
    protected byte[] transform(String className, byte[] classfileBuffer, CodeSource codeSource, ClassLoader classLoader)
        ...
        byte[] result =
            this.transformer.transform(classLoader, className.replace('.', '/'), null, null, classfileBuffer);
