 ...
 *
 * <p>
 * Field names can be properties of the target object (e.g. "name" when binding to a customer object), or nested fields
 * in case of subobjects (e.g. "address.street"). Supports subtree navigation via {@link #setNestedPath(String)}: for
 * example, an {@code AddressValidator} validates "address", not being aware that this is a subobject of customer.
 *
 * <p>
...
public interface Errors {
     ...
     * <p>
     * "." = same as the {@link org.springframework.beans.PropertyAccessor#NESTED_PROPERTY_SEPARATOR nested property
     ...
     *
     * @param nestedPath
     *            nested path within this object, e.g. "address" (defaults to "", {@code null} is also acceptable). Can
     ...
     * <p>
     * A {@link #popNestedPath()} call will reset the original nested path before the corresponding
     ...
     *            error code, interpretable as a message key
     * @param errorArgs
     *            error arguments, for argument binding via MessageFormat (can be {@code null})
     * @param defaultMessage
     *            fallback default message
     */
    ...
    /**
     * Register a field error for the specified field of the current object (respecting the current nested path, if
     * any), using the given error description.
     * <p>
     * The field name may be {@code null} or empty String to indicate the current object itself rather than a field of
     * it. This may result in a corresponding field error within the nested object graph or a global error if the
     * current object is the top object.
     * 
     * @param field
     *            the field name (may be {@code null} or empty String)
     * @param errorCode
     *            error code, interpretable as a message key
     * @see #getNestedPath()
     */
     ...
     * <p>
     * This is a convenience method to avoid repeated {@code reject(..)} calls for merging an {@code Errors} instance
     * into another {@code Errors} instance.
     * <p>
     * Note that the passed-in {@code Errors} instance is supposed to refer to the same target object, or at least
     * contain compatible errors that apply to the target object of this {@code Errors} instance.
     * 
     * @param errors
    ...
    /**
     * Return the current value of the given field, either the current bean property value or a rejected update from the
     * last binding.
     * <p>
     ...
     * <p>
     * Implementations should be able to determine the type even when the field value is {@code null}, for example from
     * some associated descriptor.
     * 
     * @param field
     *            the field name
