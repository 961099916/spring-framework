...
/**
 * This is the central class for {@code javax.validation} (JSR-303) setup in a Spring application context: It bootstraps
 * a {@code javax.validation.ValidationFactory} and exposes it through the Spring
 * {@link org.springframework.validation.Validator} interface as well as through the JSR-303
 * {@link javax.validation.Validator} interface and the {@link javax.validation.ValidatorFactory} interface itself.
 *
 * <p>
 * When talking to an instance of this bean through the Spring or JSR-303 Validator interfaces, you'll be talking to the
 * default Validator of the underlying ValidatorFactory. This is very convenient in that you don't have to perform yet
 * another call on the factory, assuming that you will almost always use the default Validator anyway. This can also be
 * injected directly into any target dependency of type {@link org.springframework.validation.Validator}!
 *
 * <p>
 * <b>As of Spring 5.0, this class requires Bean Validation 1.1+, with special support for Hibernate Validator 5.x</b>
 * (see {@link #setValidationMessageSource}). This class is also runtime-compatible with Bean Validation 2.0 and
 * Hibernate Validator 6.0, with one special note: If you'd like to call BV 2.0's {@code getClockProvider()} method,
 * obtain the native {@code ValidatorFactory} through {@code #unwrap(ValidatorFactory.class)} and call the
 * {@code getClockProvider()} method on the returned native reference there.
 *
 * <p>
 * This class is also being used by Spring's MVC configuration namespace, in case of the {@code javax.validation} API
 * being present but no explicit Validator having been configured.
 *
...
public class LocalValidatorFactoryBean extends SpringValidatorAdapter
    ...
    /**
     * Specify a JSR-303 {@link ValidationProviderResolver} for bootstrapping the provider of choice, as an alternative
     * to {@code META-INF} driven resolution.
     * 
     * @since 4.3
     */
    ...
    /**
     * Specify a custom Spring MessageSource for resolving validation messages, instead of relying on JSR-303's default
     * "ValidationMessages.properties" bundle in the classpath. This may refer to a Spring context's shared
     * "messageSource" bean, or to some special MessageSource setup for validation purposes only.
     * <p>
     * <b>NOTE:</b> This feature requires Hibernate Validator 4.3 or higher on the classpath. You may nevertheless use a
     * different validation provider but Hibernate Validator's {@link ResourceBundleMessageInterpolator} class must be
     * accessible during configuration.
     * <p>
     * Specify either this property or {@link #setMessageInterpolator "messageInterpolator"}, not both. If you would
     * like to build a custom MessageInterpolator, consider deriving from Hibernate Validator's
     * {@link ResourceBundleMessageInterpolator} and passing in a Spring-based {@code ResourceBundleLocator} when
     * constructing your interpolator.
     * <p>
     * In order for Hibernate's default validation messages to be resolved still, your {@link MessageSource} must be
     * configured for optional resolution (usually the default). In particular, the {@code MessageSource} instance
     * specified here should not apply
    ...
    /**
     * Set the ParameterNameDiscoverer to use for resolving method and constructor parameter names if needed for message
     * interpolation.
     * <p>
    ...
    /**
     * Allow Map access to the bean validation properties to be passed to the validation provider, with the option to
     ...
     * <p>
     * Default is a {@link SpringConstraintValidatorFactory}, delegating to the containing ApplicationContext for
     * creating autowired ConstraintValidator instances.
     */
