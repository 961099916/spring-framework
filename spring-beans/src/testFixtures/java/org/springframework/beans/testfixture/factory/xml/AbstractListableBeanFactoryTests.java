...
public abstract class AbstractListableBeanFactoryTests extends AbstractBeanFactoryTests {
    ...
    protected void assertTestBeanCount(int count) {
        ...
        assertThat(defNames.length == count)
            .as("We should have " + count
                + " beans for class org.springframework.beans.testfixture.beans.TestBean, not " + defNames.length)
            .isTrue();
        ...
        assertThat(names.length == countIncludingFactoryBeans)
            .as("We should have " + countIncludingFactoryBeans
    ...
    public void getCountForFactoryClass() {
        assertThat(getListableBeanFactory().getBeanNamesForType(FactoryBean.class).length == 2).as(
            "Should have 2 factories, not " + getListableBeanFactory().getBeanNamesForType(FactoryBean.class).length)
            .isTrue();
