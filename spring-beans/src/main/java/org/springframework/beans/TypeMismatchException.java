...
public class TypeMismatchException extends PropertyAccessException {
     ...
     * 
     * @param value
     *            the offending value that couldn't be converted (may be {@code null})
     * @param requiredType
     *            the required target type (or {@code null} if not known)
    ...
    public TypeMismatchException(@Nullable Object value, @Nullable Class<?> requiredType, @Nullable Throwable cause) {
        super(
            "Failed to convert value of type '" + ClassUtils.getDescriptiveType(value) + "'"
                + (requiredType != null ? " to required type '" + ClassUtils.getQualifiedName(requiredType) + "'" : ""),
            cause);
        this.value = value;
        this.requiredType = requiredType;
    ...
    /**
     * Initialize this exception's property name for exposure through {@link #getPropertyName()}, as an alternative to
     * having it initialized via a {@link PropertyChangeEvent}.
     * 
     * @param propertyName
     *            the property name to expose
     * @since 5.0.4
