 ...
 * </pre>
 *
 * The standard JavaBeans {@code Introspector} will discover the {@code getFoo} read method, but will bypass the
 * {@code #setFoo(Foo)} write method, because its non-void returning signature does not comply with the JavaBeans
 * specification. {@code ExtendedBeanInfo}, on the other hand, will recognize and include it. This is designed to allow
 * APIs with "builder" or method-chaining style setter signatures to be used within Spring {@code <beans>} XML.
 * {@link #getPropertyDescriptors()} returns all existing property descriptors from the wrapped {@code BeanInfo} as well
 * any added for non-void returning setters. Both standard ("non-indexed") and
 * <a href="https://docs.oracle.com/javase/tutorial/javabeans/writing/properties.html"> indexed properties</a> are fully
 * supported.
 *
 * @author Chris Beams
...
class ExtendedBeanInfo implements BeanInfo {
    ...
    /**
     * Wrap the given {@link BeanInfo} instance; copy all its existing property descriptors locally, wrapping each in a
     * custom {@link SimpleIndexedPropertyDescriptor indexed} or {@link SimplePropertyDescriptor non-indexed}
     * {@code PropertyDescriptor} variant that bypasses default JDK weak/soft reference management; then search through
     * its method descriptors to find any non-void returning write methods and update or create the corresponding
    ...
    /**
     * Return the set of {@link PropertyDescriptor PropertyDescriptors} from the wrapped {@link BeanInfo} object as well
     * as {@code PropertyDescriptors} for each non-void returning setter method found during construction.
     * 
     * @see #ExtendedBeanInfo(BeanInfo)
     */
    @Override
