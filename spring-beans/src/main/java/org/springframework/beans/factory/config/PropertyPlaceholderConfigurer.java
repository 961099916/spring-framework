...
/**
 * {@link PlaceholderConfigurerSupport} subclass that resolves ${...} placeholders against {@link #setLocation local}
 * {@link #setProperties properties} and/or system properties and environment variables.
 *
 * <p>
 * {@link PropertyPlaceholderConfigurer} is still appropriate for use when:
 * <ul>
 * <li>the {@code spring-context} module is not available (i.e., one is using Spring's {@code BeanFactory} API as
 * opposed to {@code ApplicationContext}).
 * <li>existing configuration makes use of the {@link #setSystemPropertiesMode(int) "systemPropertiesMode"} and/or
 * {@link #setSystemPropertiesModeName(String) "systemPropertiesModeName"} properties. Users are encouraged to move away
 * from using these settings, and rather configure property source search order through the container's
 * {@code Environment}; however, exact preservation of functionality may be maintained by continuing to use
 * {@code PropertyPlaceholderConfigurer}.
 * </ul>
 *
 ...
 * @deprecated as of 5.2; use {@code org.springframework.context.support.PropertySourcesPlaceholderConfigurer} instead
 *             which is more flexible through taking advantage of the {@link org.springframework.core.env.Environment}
