...
/**
 * A {@link FactoryBean} implementation that takes an interface which must have one or more methods with the signatures
 * {@code MyType xxx()} or {@code MyType xxx(MyIdType id)} (typically, {@code MyService getService()} or
 * {@code MyService getService(String id)}) and creates a dynamic proxy which implements that interface, delegating to
 * an underlying {@link org.springframework.beans.factory.BeanFactory}.
 *
 * <p>
 * Such service locators permit the decoupling of calling code from the
 * {@link org.springframework.beans.factory.BeanFactory} API, by using an appropriate custom locator interface. They
 * will typically be used for <b>prototype beans</b>, i.e. for factory methods that are supposed to return a new
 * instance for each call. The client receives a reference to the service locator via setter or constructor injection,
 * to be able to invoke the locator's factory methods on demand. <b>For singleton beans, direct setter or constructor
 * injection of the target bean is preferable.</b>
 *
 * <p>
 * On invocation of the no-arg factory method, or the single-arg factory method with a String id of {@code null} or
 * empty String, if exactly <b>one</b> bean in the factory matches the return type of the factory method, that bean is
 * returned, otherwise a {@link org.springframework.beans.factory.NoSuchBeanDefinitionException} is thrown.
 *
 * <p>
 * On invocation of the single-arg factory method with a non-null (and non-empty) argument, the proxy returns the result
 * of a {@link org.springframework.beans.factory.BeanFactory#getBean(String)} call, using a stringified version of the
 * passed-in id as bean name.
 *
 * <p>
 * A factory method argument will usually be a String, but can also be an int or a custom enumeration type, for example,
 * stringified via {@code toString}. The resulting String can be used as bean name as-is, provided that corresponding
 * beans are defined in the bean factory. Alternatively, {@linkplain #setServiceMappings(java.util.Properties) a custom
 * mapping} between service IDs and bean names can be defined.
 *
 * <p>
 * By way of an example, consider the following service locator interface. Note that this interface is not dependent on
 * any Spring APIs.
 *
 * <pre class="code">
 * package a.b.c;
 ...
 *
 * <p>
 * By way of an example that looks up a bean <b>by name</b>, consider the following service locator interface. Again,
...
public class ServiceLocatorFactoryBean implements FactoryBean<Object>, BeanFactoryAware, InitializingBean {
     ...
     * {@code MyType xxx()} or {@code MyType xxx(MyIdType id)} (typically, {@code MyService getService()} or
     * {@code MyService getService(String id)}). See the {@link ServiceLocatorFactoryBean class-level Javadoc} for
    ...
    /**
     * Set the exception class that the service locator should throw if service lookup failed. The specified exception
     * class must have a constructor with one of the following parameter types: {@code (String, Throwable)} or
     * {@code (Throwable)} or {@code (String)}.
     * <p>
     * If not specified, subclasses of Spring's BeansException will be thrown, for example
     * NoSuchBeanDefinitionException. As those are unchecked, the caller does not need to handle them, so it might be
     * acceptable that Spring exceptions get thrown as long as they are just handled generically.
     * 
    ...
    /**
     * Set mappings between service ids (passed into the service locator) and bean names (in the bean factory). Service
     * ids that are not defined here will be treated as bean names as-is.
     * <p>
     * The empty string as service id key defines the mapping for {@code null} and empty string, and for factory methods
     * without parameter. If not defined, a single matching bean will be retrieved from the bean factory.
     * 
    ...
    /**
     * Determine the constructor to use for the given service locator exception class. Only called in case of a custom
     * service locator exception.
     * <p>
     * The default implementation looks for a constructor with one of the following parameter types:
    ...
    /**
     * Create a service locator exception for the given cause. Only called in case of a custom service locator
     * exception.
     * <p>
