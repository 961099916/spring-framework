 ...
 * instantiation but before explicit properties are set or autowiring occurs.
 *
 * <p>
 * Typically used to suppress default instantiation for specific target beans, for example to create proxies with
 * special TargetSources (pooling targets, lazily initializing targets, etc), or to implement additional injection
 * strategies such as field injection.
 *
 * <p>
 * <b>NOTE:</b> This interface is a special purpose interface, mainly for internal use within the framework. It is
 * recommended to implement the plain {@link BeanPostProcessor} interface as far as possible, or to derive from
...
public interface InstantiationAwareBeanPostProcessor extends BeanPostProcessor {
     ...
     * <p>
     * If a non-null object is returned by this method, the bean creation process will be short-circuited. The only
     * further processing applied is the {@link #postProcessAfterInitialization} callback from the configured
     ...
     * <p>
     * Post-processors may implement the extended {@link SmartInstantiationAwareBeanPostProcessor} interface in order to
     ...
     * <p>
     * This is the ideal callback for performing custom field injection on the given bean instance, right before
     * Spring's autowiring kicks in.
     * <p>
     * The default implementation returns {@code true}.
     * 
     * @param bean
     *            the bean instance created, with properties not having been set yet
     * @param beanName
     *            the name of the bean
    ...
    /**
     * Post-process the given property values before the factory applies them to the given bean, without any need for
     * property descriptors.
     * <p>
     * Implementations should return {@code null} (the default) if they provide a custom
     * {@link #postProcessPropertyValues} implementation, and {@code pvs} otherwise. In a future version of this
     * interface (with {@link #postProcessPropertyValues} removed), the default implementation will return the given
     * {@code pvs} as-is directly.
     * 
     * @param pvs
     *            the property values that the factory is about to apply (never {@code null})
     * @param bean
     *            the bean instance created, but whose properties have not yet been set
     * @param beanName
     *            the name of the bean
     * @return the actual property values to apply to the given bean (can be the passed-in PropertyValues instance), or
     *         {@code null} which proceeds with the existing properties but specifically continues with a call to
     *         {@link #postProcessPropertyValues} (requiring initialized {@code PropertyDescriptor}s for the current
     *         bean class)
     * @throws org.springframework.beans.BeansException
     *             in case of errors
     * @since 5.1
     ...
     *            the property values that the factory is about to apply (never {@code null})
     * @param pds
     *            the relevant property descriptors for the target bean (with ignored dependency types - which the
