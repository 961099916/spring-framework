...
/**
 * {@link org.springframework.beans.factory.config.BeanPostProcessor BeanPostProcessor} implementation that autowires
 * annotated fields, setter methods, and arbitrary config methods. Such members to be injected are detected through
 * annotations: by default, Spring's {@link Autowired @Autowired} and {@link Value @Value} annotations.
 *
 * <p>
 * Also supports JSR-330's {@link javax.inject.Inject @Inject} annotation, if available, as a direct alternative to
 * Spring's own {@code @Autowired}.
 *
 ...
 * 'required' attribute is set to {@code true}, only a single constructor may be annotated with {@code @Autowired}. If
 * multiple <i>non-required</i> constructors declare the annotation, they will be considered as candidates for
 * autowiring. The constructor with the greatest number of dependencies that can be satisfied by matching beans in the
 * Spring container will be chosen. If none of the candidates can be satisfied, then a primary/default constructor (if
 * present) will be used. If a class only declares a single constructor to begin with, it will always be used, even if
 ...
 * <p>
 * A default {@code AutowiredAnnotationBeanPostProcessor} will be registered by the "context:annotation-config" and
 * "context:component-scan" XML tags. Remove or turn off the default annotation configuration there if you intend to
 * specify a custom {@code AutowiredAnnotationBeanPostProcessor} bean definition.
 *
 * <p>
 ...
 * {@link Lookup @Lookup} annotation which identifies lookup methods to be replaced by the container at runtime. This is
 * essentially a type-safe version of {@code getBean(Class, args)} and {@code getBean(String, args)}. See
...
public class AutowiredAnnotationBeanPostProcessor extends InstantiationAwareBeanPostProcessorAdapter
    ...
    /**
     * Create a new {@code AutowiredAnnotationBeanPostProcessor} for Spring's standard {@link Autowired @Autowired} and
     ...
     * methods.
     * <p>
     * The default autowired annotation types are the Spring-provided {@link Autowired @Autowired} and
     * {@link Value @Value} annotations as well as JSR-330's {@link javax.inject.Inject @Inject} annotation, if
     * available.
     * <p>
     ...
     * <p>
     * For example if using 'required=true' (the default), this value should be {@code true}; but if using
                ...
                if (candidateConstructors == null) {
                    ...
                    try {
                        ...
                        throw new BeanCreationException(beanName, "Resolution of declared constructors on bean Class ["
                            + beanClass.getName() + "] from ClassLoader [" + beanClass.getClassLoader() + "] failed",
                            ex);
                    }
    ...
    /**
     * 'Native' processing method for direct calls with an arbitrary target instance, resolving all of its fields and
     * methods which are annotated with one of the configured 'autowired' annotation types.
     * 
     * @param bean
            ...
            ReflectionUtils.doWithLocalMethods(targetClass, method -> {
                ...
                if (ann != null && method.equals(ClassUtils.getMostSpecificMethod(method, clazz))) {
                    ...
                    if (method.getParameterCount() == 0) {
                        if (logger.isInfoEnabled()) {
                            logger
                                .info("Autowired annotation should only be used on methods with parameters: " + method);
                        }
                    }
                    boolean required = determineRequiredStatus(ann);
