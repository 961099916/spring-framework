 ...
 * style of configuration for property path access. (See also the appendix entitled 'XML Schema-based configuration' in
 * the Spring reference manual for more examples.)
 *
 * <pre class="code">
 *  &lt;!-- will result in 10, which is the value of property 'age' of bean 'tb' --&gt;
...
public class PropertyPathFactoryBean implements FactoryBean<Object>, BeanNameAware, BeanFactoryAware {
     ...
     * 
     * @param targetObject
     *            a target object, for example a bean reference or an inner bean
     * @see #setTargetBeanName
     */
     ...
     *            the bean name to be looked up in the containing bean factory (e.g. "testBean")
     * @see #setTargetObject
     */
     ...
     * 
     * @param propertyPath
     *            the property path, potentially nested (e.g. "age" or "spouse.age")
     */
     ...
     * <p>
     * Note: This is not necessary for directly specified target objects or singleton target beans, where the type can
     * be determined through introspection. Just specify this in case of a prototype target, provided that you need
     * matching by type (for example, for autowiring).
     * 
     * @param resultType
    ...
    /**
     * The bean name of this PropertyPathFactoryBean will be interpreted as "beanName.property" pattern, if neither
     * "targetObject" nor "targetBeanName" nor "propertyPath" have been specified. This allows for concise bean
     * definitions with just an id/name.
     */
    @Override
    ...
    /**
     * While this FactoryBean will often be used for singleton targets, the invoked getters for the property path might
     * return a new object for each call, so we have to assume that we're not returning the same object for each
     * {@link #getObject()} call.
     */
    @Override
