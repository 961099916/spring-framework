...
/**
 * Convenient base class for bean configurers that can perform Dependency Injection on objects (however they may be
 * created). Typically subclassed by AspectJ aspects.
 *
 * <p>
 * Subclasses may also need a custom metadata resolution strategy, in the {@link BeanWiringInfoResolver} interface. The
 * default implementation looks for a bean with the same name as the fully-qualified class name. (This is the default
...
public class BeanConfigurerSupport implements BeanFactoryAware, InitializingBean, DisposableBean {
     ...
     * <p>
     * The default behavior is to look for a bean with the same name as the class. As an alternative, consider using
     * annotation-driven bean wiring.
     * 
     * @see ClassNameBeanWiringInfoResolver
     ...
     * <p>
     * Subclasses can override this to provide custom configuration logic. Typically called by an aspect, for all bean
     * instances matched by a pointcut.
     * 
     * @param beanInstance
     *            the bean instance to configure (must <b>not</b> be {@code null})
     */
