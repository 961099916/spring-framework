 ...
 *
 * <p>
 * Supports values for a specific index in the constructor argument list as well as for generic argument matches by
 * type.
 *
...
public class ConstructorArgumentValues {
    ...
    /**
     * Copy all given argument values into this object, using separate holder instances to keep the values independent
     * from the original object.
     * <p>
     * Note: Identical ValueHolder instances will only be registered once, to allow for merging and re-merging of
     * argument value definitions. Distinct ValueHolder instances carrying the same content are of course allowed.
     */
    ...
    /**
     * Add an argument value for the given index in the constructor argument list, merging the new value (typically a
     * collection) with the current value if demanded: see {@link org.springframework.beans.Mergeable}.
     * 
     * @param key
     *            the index in the constructor argument list
     * @param newValue
     ...
     *            the type to match (can be {@code null} to match untyped values only)
     * @param requiredName
     *            the type to match (can be {@code null} to match unnamed values only, or empty String to match any
     *            name)
     * @return the ValueHolder for the argument, or {@code null} if none set
     */
    @Nullable
     ...
     *            the argument value in the form of a ValueHolder
     *            <p>
     *            Note: Identical ValueHolder instances will only be registered once, to allow for merging and
     *            re-merging of argument value definitions. Distinct ValueHolder instances carrying the same content are
     *            of course allowed.
     */
     ...
     * @param requiredName
     *            the name to match (can be {@code null} to not match argument values by name, or empty String to match
     *            any name)
     * @param usedValueHolders
     *            a Set of ValueHolder objects that have already been used in the current resolution process and should
     ...
     * @param requiredName
     *            the parameter name to match (can be {@code null} to find an unnamed argument value, or empty String to
     *            match any name)
     * @param usedValueHolders
     *            a Set of ValueHolder objects that have already been used in the current resolution process and should
     *            therefore not be returned again (allowing to return the next generic argument match in case of
     *            multiple generic argument values of the same type)
     * @return the ValueHolder for the argument, or {@code null} if none set
     */
    @Nullable
    ...
    public static class ValueHolder implements BeanMetadataElement {
        ...
        /**
         * Return whether this holder contains a converted value already ({@code true}), or whether the value still
         * needs to be converted ({@code false}).
         */
         ...
         * <p>
         * Note that ValueHolder does not implement {@code equals} directly, to allow for multiple ValueHolder instances
         * with the same content to reside in the same Set.
         */
         ...
         * <p>
         * Note that ValueHolder does not implement {@code hashCode} directly, to allow for multiple ValueHolder
         * instances with the same content to reside in the same Set.
         */
