 ...
 *
 * <p>
 * Mainly for internal use within the framework, but to some degree also useful for application classes. Consider
 * <a href="https://commons.apache.org/proper/commons-beanutils/">Apache Commons BeanUtils</a>,
 * <a href="https://hotelsdotcom.github.io/bull/">BULL - Bean Utils Light Library</a>, or similar third-party frameworks
...
public abstract class BeanUtils {
     ...
     * @return the new instance
     * @throws BeanInstantiationException
     *             if the bean cannot be instantiated. The cause may notably indicate a {@link NoSuchMethodException} if
     *             no primary/default constructor was found, a {@link NoClassDefFoundError} or other
     *             {@link LinkageError} in case of an unresolvable class definition (e.g. due to a missing dependency at
     ...
     * <p>
     * Useful in cases where the type of the class to instantiate (clazz) is not available, but the type desired
     * (assignableTo) is known.
     * <p>
     * Note that this method tries to set the constructor accessible if given a non-accessible (that is, non-public)
     * constructor.
     * 
    ...
    /**
     * Return the primary constructor of the provided class. For Kotlin classes, this returns the Java constructor
     * corresponding to the Kotlin primary constructor (as defined in the Kotlin specification). Otherwise, in
     ...
     * <p>
     * Checks {@code Class.getMethod} first, falling back to {@code findDeclaredMethod}. This allows to find public
     ...
     * too.
     * <p>
     * Checks {@code Class.getMethods} first, falling back to {@code findDeclaredMethodWithMinimalParameters}. This
     * allows for finding public methods without issues even in environments with restricted Java security settings.
     * 
     * @param clazz
     *            the class to check
     * @param methodName
     ...
     * {@code Class}.
     * <p>
     * When not supplying an argument list ({@code methodName}) the method whose name matches and has the least number
     * of parameters will be returned. When supplying an argument type list, only the method whose name and argument
     * types match will be returned.
     * <p>
     * Note then that {@code methodName} and {@code methodName()} are <strong>not</strong> resolved in the same way. The
     * signature {@code methodName} means the method called {@code methodName} with the least number of arguments,
     * whereas {@code methodName()} means the method called {@code methodName} with exactly 0 arguments.
     * <p>
    ...
    /**
     * Find a JavaBeans {@code PropertyDescriptor} for the given method, with the method either being the read method or
     * the write method for that bean property.
     * 
     * @param method
     *            the method to find a corresponding PropertyDescriptor for, introspecting its declaring class
     * @return the corresponding PropertyDescriptor, or {@code null} if none
     * @throws BeansException
     *             if PropertyDescriptor lookup fails
     */
     ...
     * isolated from the latter's registered default editors for primitive types.
     * 
     * @param targetType
     ...
     * <p>
     * See {@link #isSimpleValueType(Class)} for the definition of <em>simple value type</em>.
     * <p>
    ...
    /**
     * Check if the given type represents a "simple" value type: a primitive or primitive wrapper, an enum, a String or
     * other CharSequence, a Number, a Date, a Temporal, a URI, a URL, a Locale, or a Class.
     * <p>
    ...
    public static boolean isSimpleValueType(Class<?> type) {
        return (Void.class != type && void.class != type
            && (ClassUtils.isPrimitiveOrWrapper(type) || Enum.class.isAssignableFrom(type)
                ...
                || Date.class.isAssignableFrom(type) || Temporal.class.isAssignableFrom(type) || URI.class == type
    ...
    private static class KotlinDelegate {
         ...
         * @param args
         *            the constructor arguments to apply (use {@code null} for unspecified parameter if needed)
         */
