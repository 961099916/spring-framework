...
/**
 * Extension of the standard JavaBeans {@link PropertyDescriptor} class, overriding {@code getPropertyType()} such that
 * a generically declared type variable will be resolved against the containing bean class.
 *
            ...
            if (this.readMethod == null) {
                ...
                for (Method method : beanClass.getMethods()) {
                    if (method.getName().equals(writeMethodToUse.getName()) && !method.equals(writeMethodToUse)
                        && !method.isBridge() && method.getParameterCount() == writeMethodToUse.getParameterCount()) {
        ...
        if (ambiguousCandidates != null) {
            ...
            LogFactory.getLog(GenericTypeAwarePropertyDescriptor.class)
                .warn("Invalid JavaBean property '" + getName()
                    + "' being accessed! Ambiguous write methods found next to actually used [" + this.writeMethod
                    + "]: " + ambiguousCandidates);
        }
        return this.writeMethod;
    }
