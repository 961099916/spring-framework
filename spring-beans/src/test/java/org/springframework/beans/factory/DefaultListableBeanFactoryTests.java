...
class DefaultListableBeanFactoryTests {
    ...
    void autowireWithUnsatisfiedConstructorDependency() {
        ...
        lbf.registerBeanDefinition("rod", bd);
        assertThat(lbf.getBeanDefinitionCount()).isEqualTo(1);
        assertThatExceptionOfType(UnsatisfiedDependencyException.class).isThrownBy(() -> lbf
            .autowire(UnsatisfiedConstructorDependency.class, AutowireCapableBeanFactory.AUTOWIRE_CONSTRUCTOR, true));
    }
    ...
    void autowireBeanByName() {
        ...
        DependenciesBean bean =
            (DependenciesBean)lbf.autowire(DependenciesBean.class, AutowireCapableBeanFactory.AUTOWIRE_BY_NAME, true);
    ...
    void autowireBeanByNameWithDependencyCheck() {
        ...
        assertThatExceptionOfType(UnsatisfiedDependencyException.class)
            .isThrownBy(() -> lbf.autowire(DependenciesBean.class, AutowireCapableBeanFactory.AUTOWIRE_BY_NAME, true));
    }
    ...
    void autowireBeanByNameWithNoDependencyCheck() {
        ...
        lbf.registerBeanDefinition("spous", bd);
        DependenciesBean bean =
            (DependenciesBean)lbf.autowire(DependenciesBean.class, AutowireCapableBeanFactory.AUTOWIRE_BY_NAME, false);
    ...
    void mapInjectionWithPriority() {
        ...
        RootBeanDefinition bd4 =
            new RootBeanDefinition(TestBeanRecipient.class, RootBeanDefinition.AUTOWIRE_CONSTRUCTOR, false);
        lbf.registerBeanDefinition("bd1", bd1);
        lbf.registerBeanDefinition("bd2", bd2);
        lbf.registerBeanDefinition("bd3", bd3);
    ...
    void autowireBeanByType() {
        ...
        lbf.registerBeanDefinition("test", bd);
        DependenciesBean bean =
            (DependenciesBean)lbf.autowire(DependenciesBean.class, AutowireCapableBeanFactory.AUTOWIRE_BY_TYPE, true);
        TestBean test = (TestBean)lbf.getBean("test");
        assertThat(bean.getSpouse()).isEqualTo(test);
    }

    /**
     * Verifies that a dependency on a {@link FactoryBean} can be autowired <em>by type</em>, specifically addressing
     * the JIRA issue raised in
     * <a href="https://opensource.atlassian.com/projects/spring/browse/SPR-4040" target="_blank">SPR-4040</a>.
     */
    @Test
    ...
    /**
     * Verifies that a dependency on a {@link FactoryBean} can <strong>not</strong> be autowired <em>by name</em>, as
     * &amp; is an illegal character in Java method names. In other words, you can't name a method
    ...
    void autowireBeanByTypeWithNoDependencyCheck() {
        DependenciesBean bean =
            (DependenciesBean)lbf.autowire(DependenciesBean.class, AutowireCapableBeanFactory.AUTOWIRE_BY_TYPE, false);
        assertThat(bean.getSpouse()).isNull();
    }
    ...
    /**
     * @param singleton
     *            whether the bean created from the factory method on the bean instance should be a singleton or
     *            prototype. This flag is used to allow checking of the new ability in 1.2.4 to determine the type of a
     *            prototype created from invoking a factory method on a bean instance in the factory.
     */
    ...
    void initSecurityAwarePrototypeBean() {
        ...
        TestSecuredBean bean =
            (TestSecuredBean)Subject.doAsPrivileged(subject, (PrivilegedAction)() -> lbf.getBean("test"), null);
    ...
    /**
     * Test that by-type bean lookup caching is working effectively by searching for a bean of type B 10K times within a
     * container having 1K additional beans of type A. Prior to by-type caching, each bean lookup would traverse the
     * entire container (all 1001 beans), performing expensive assignability checks, etc. Now these operations are
     * necessary only once, providing a dramatic performance improvement. On load-free modern hardware (e.g. an 8-core
     * MPB), this method should complete well under the 1000 ms timeout, usually ~= 300ms. With caching removed and on
     * the same hardware the method will take ~13000 ms. See SPR-6870.
     */
    @Test
    @Timeout(1)
    @EnabledForTestGroups(TestGroup.PERFORMANCE)
