...
public class ExtendedBeanInfoTests {
    ...
    /**
     * Prior to SPR-10111 (a follow-up fix for SPR-9702), this method would throw an IntrospectionException regarding a
     * "type mismatch between indexed and non-indexed methods" intermittently (approximately one out of every four
     * times) under JDK 7 due to non-deterministic results from {@link Class#getDeclaredMethods()}. See
    ...
    /**
     * Ensures that an empty string is not passed into a PropertyDescriptor constructor. This could occur when handling
    ...
    /**
     * Corners the bug revealed by SPR-8522, in which an (apparently) indexed write method without a corresponding
     * indexed read method would fail to be processed correctly by ExtendedBeanInfo. The local class C below represents
     * the relevant methods from Google's GsonBuilder class. Interestingly, the setDateFormat(int, int) method was not
     * actually intended to serve as an indexed write method; it just appears that way.
     */
    @Test
    ...
    /**
     * {@link BeanInfo#getPropertyDescriptors()} returns alphanumerically sorted. Test that
     * {@link ExtendedBeanInfo#getPropertyDescriptors()} does the same.
     */
    @Test
