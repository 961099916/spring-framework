...
public class RouterFunctionBuilderTests {
    ...
    public void route() {
        RouterFunction<ServerResponse> route =
            RouterFunctions.route().GET("/foo", request -> ServerResponse.ok().build())
        ...
        Optional<Integer> responseStatus =
            route.route(getFooRequest).map(handlerFunction -> handle(handlerFunction, getFooRequest))
    ...
    public void resources() {
        ...
        Optional<Integer> responseStatus =
            route.route(resourceRequest).map(handlerFunction -> handle(handlerFunction, resourceRequest))
                .map(ServerResponse::statusCode).map(HttpStatus::value);
        assertThat(responseStatus.get().intValue()).isEqualTo(200);
    ...
    public void filters() {
        ...
        RouterFunction<ServerResponse> route =
            RouterFunctions.route().GET("/foo", request -> ServerResponse.ok().build()).GET("/bar", request -> {
        ...
        Optional<Integer> responseStatus =
            route.route(barRequest).map(handlerFunction -> handle(handlerFunction, barRequest))
                .map(ServerResponse::statusCode).map(HttpStatus::value);
