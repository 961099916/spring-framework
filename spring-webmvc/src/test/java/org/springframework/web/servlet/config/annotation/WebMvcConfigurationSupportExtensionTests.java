...
/**
 * A test fixture with a sub-class of {@link WebMvcConfigurationSupport} that also implements the various
 * {@link WebMvcConfigurer} extension points.
 *
 * The former doesn't implement the latter but the two must have compatible callback method signatures to support moving
 * from simple to advanced configuration -- i.e. dropping @EnableWebMvc + WebMvcConfigurer and extending directly from
...
public class WebMvcConfigurationSupportExtensionTests {
    ...
    public void handlerMappings() throws Exception {
        RequestMappingHandlerMapping rmHandlerMapping =
            this.config.requestMappingHandlerMapping(this.config.mvcContentNegotiationManager(),
        ...
        RequestMappingInfo info =
            map.entrySet().stream().filter(entry -> entry.getValue().getBeanType().equals(UserController.class))
                .findFirst().orElseThrow(() -> new AssertionError("UserController bean not found")).getKey();
        ...
        AbstractHandlerMapping handlerMapping =
            (AbstractHandlerMapping)this.config.viewControllerHandlerMapping(this.config.mvcPathMatcher(),
                this.config.mvcUrlPathHelper(), this.config.mvcConversionService(),
    ...
    public void requestMappingHandlerAdapter() throws Exception {
        RequestMappingHandlerAdapter adapter = this.config.requestMappingHandlerAdapter(
            this.config.mvcContentNegotiationManager(), this.config.mvcConversionService(), this.config.mvcValidator());
    ...
    public void contentNegotiation() throws Exception {
        ...
        SimpleUrlHandlerMapping handlerMapping =
            (SimpleUrlHandlerMapping)this.config.resourceHandlerMapping(this.config.mvcUrlPathHelper(),
                this.config.mvcPathMatcher(), this.config.mvcContentNegotiationManager(),
                this.config.mvcConversionService(), this.config.mvcResourceUrlProvider());
        handlerMapping.setApplicationContext(this.context);
    ...
    public void exceptionResolvers() throws Exception {
        List<HandlerExceptionResolver> resolvers =
            ((HandlerExceptionResolverComposite)this.config.handlerExceptionResolver(null)).getExceptionResolvers();
    ...
    public void viewResolvers() throws Exception {
        ViewResolverComposite viewResolver =
            (ViewResolverComposite)this.config.mvcViewResolver(this.config.mvcContentNegotiationManager());
    ...
    /**
     * Since WebMvcConfigurationSupport does not implement WebMvcConfigurer, the purpose of this test class is also to
     * ensure the two are in sync with each other. Effectively that ensures that application config classes that use the
     * combo {@code @EnableWebMvc} plus WebMvcConfigurer can switch to extending WebMvcConfigurationSupport directly for
