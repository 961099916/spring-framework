 ...
 * {@link org.springframework.web.servlet.mvc.WebContentInterceptor}. Can also be used for custom handlers that have
 * their own {@link org.springframework.web.servlet.HandlerAdapter}.
 *
 * <p>
 * Supports HTTP cache control options. The usage of corresponding HTTP headers can be controlled via the
 * {@link #setCacheSeconds "cacheSeconds"} and {@link #setCacheControl "cacheControl"} properties.
 *
 * <p>
 * <b>NOTE:</b> As of Spring 4.2, this generator's default behavior changed when using only {@link #setCacheSeconds},
 * sending HTTP response headers that are in line with current browsers and proxies implementations (i.e. no HTTP 1.0
 * headers anymore) Reverting to the previous behavior can be easily done by using one of the newly deprecated methods
...
public abstract class WebContentGenerator extends WebApplicationObjectSupport {
     ...
     *            {@code true} if this generator should support HTTP methods GET, HEAD and POST by default, or
     *            {@code false} if it should be unrestricted
     */
     ...
     * <p>
     * Default is GET, HEAD and POST for simple form controller types; unrestricted for general controllers and
     * interceptors.
     */
     ...
     * {@link #setSupportedMethods supported methods} also automatically adding "OPTIONS" to the list even if not
     * present as a supported method. This means subclasses don't have to explicitly list "OPTIONS" as a supported
     * method as long as HTTP OPTIONS requests are handled before making a call to
    ...
    /**
     * Get the {@link org.springframework.http.CacheControl} instance that builds the Cache-Control HTTP response
     * header.
     *
     * @since 4.2
     */
     ...
     * inform clients that the response is subject to content negotiation and variances based on the value of the given
     * request headers. The configured request header names are added only if not already present in the response "Vary"
     * header.
     *
     * @param varyByRequestHeaders
     *            one or more request header names
     * @since 4.3
     */
     ...
     * <p>
     * Note: Cache headers will only get applied if caching is enabled (or explicitly prevented) for the current
     * request.
     *
     * @deprecated as of 4.2, since going forward, the HTTP 1.1 cache-control header will be required, with the HTTP 1.0
    ...
    /**
     * An option to add 'must-revalidate' to every Cache-Control header. This may be useful with annotated controller
     * methods, which can programmatically do a last-modified calculation as described in
    ...
    /**
     * Prepare the given response according to the settings of this generator. Applies the number of cache seconds
     ...
     *            current HTTP response
     * @param cacheControl
     *            the pre-configured cache control settings
     * @since 4.2
     */
    ...
    /**
     * Apply the given cache seconds and generate corresponding HTTP headers, i.e. allow caching for the given number of
     * seconds in case of a positive value, prevent caching if given a 0 value, do nothing else. Does not tell the
     * browser to revalidate the resource.
     * 
     * @param response
     *            current HTTP response
     * @param cacheSeconds
     *            positive number of seconds into the future that the response should be cacheable for, 0 to prevent
     *            caching
     */
     ...
     * @deprecated as of 4.2, since the {@code lastModified} flag is effectively ignored, with a must-revalidate header
     *             only generated if explicitly configured
     */
    @Deprecated
     ...
     * <p>
     * That is, allow caching for the given number of seconds in the case of a positive value, prevent caching if given
     * a 0 value, else do nothing (i.e. leave caching to the client).
     * 
     * @param response
     *            the current HTTP response
     * @param cacheSeconds
     *            the (positive) number of seconds into the future that the response should be cacheable for; 0 to
     *            prevent caching; and a negative value to leave caching to the client.
     * @param mustRevalidate
     *            whether the client should revalidate the resource (typically only necessary for controllers with
    ...
    /**
     * Set HTTP headers to allow caching for the given number of seconds. Does not tell the browser to revalidate the
     * resource.
     * 
     * @param response
     *            current HTTP response
    ...
    /**
     * Set HTTP headers to allow caching for the given number of seconds. Tells the browser to revalidate the resource
     * if mustRevalidate is {@code true}.
     * 
     * @param response
     *            the current HTTP response
     * @param seconds
