...
/**
 * Provides supporting functionality to render a list of '{@code option}' tags based on some source object. This object
 * can be either an array, a {@link Collection}, or a {@link Map}.
 * <h3>Using an array or a {@link Collection}:</h3>
 * <p>
 * If you supply an array or {@link Collection} source object to render the inner '{@code option}' tags, you may
 * optionally specify the name of the property on the objects which corresponds to the <em>value</em> of the rendered
 * '{@code option}' (i.e., the {@code valueProperty}) and the name of the property that corresponds to the
 * <em>label</em> (i.e., the {@code labelProperty}). These properties are then used when rendering each element of the
 * array/{@link Collection} as an '{@code option}'. If either property name is omitted, the value of
 * {@link Object#toString()} of the corresponding array/{@link Collection} element is used instead. However, if the item
 * is an enum, {@link Enum#name()} is used as the default value.
 * </p>
 ...
 * </p>
 * <ul>
 * <li>the {@code key} of each {@link Map} entry will correspond to the <em>value</em> of the rendered '{@code option}',
 * and</li>
 * <li>the {@code value} of each {@link Map} entry will correspond to the <em>label</em> of the rendered
 * '{@code option}'.</li>
 * </ul>
 * <p>
 * If you <strong>supply</strong> property names for the <em>value</em> and <em>label</em>:
 * </p>
 * <ul>
 * <li>the <em>value</em> of the rendered '{@code option}' will be retrieved from the {@code valueProperty} on the
 * object corresponding to the {@code key} of each {@link Map} entry, and</li>
 * <li>the <em>label</em> of the rendered '{@code option}' will be retrieved from the {@code labelProperty} on the
 * object corresponding to the {@code value} of each {@link Map} entry.
 * </ul>
 ...
 * <li>An '{@code option}' is marked as 'selected' if its key {@link #isOptionSelected matches} the value that is bound
 * to the tag instance.</li>
 * </ul>
 *
 * @author Rob Harrop
...
class OptionWriter {
     ...
     * 
     * @param optionSource
     *            the source of the {@code options} (never {@code null})
     * @param bindStatus
     *            the {@link BindStatus} for the bound value (never {@code null})
     * @param valueProperty
     *            the name of the property used to render {@code option} values (optional)
     * @param labelProperty
     *            the name of the property used to render {@code option} labels (optional)
     */
    ...
    private void renderFromMap(TagWriter tagWriter) throws JspException {
        ...
        for (Map.Entry<?, ?> entry : optionMap.entrySet()) {
            ...
            Object renderValue = (this.valueProperty != null
                ? PropertyAccessorFactory.forBeanPropertyAccess(mapKey).getPropertyValue(this.valueProperty) : mapKey);
    ...
    /**
     * Render the inner '{@code option}' tags using the supplied {@link Collection} of objects as the source. The value
     * of the {@link #valueProperty} field is used when rendering the '{@code value}' of the '{@code option}' and the
     * value of the {@link #labelProperty} property is used when rendering the label.
     */
    ...
    /**
     * Render an HTML '{@code option}' with the supplied value and label. Marks the value as 'selected' if either the
     * item itself or its value match the bound value.
     */
