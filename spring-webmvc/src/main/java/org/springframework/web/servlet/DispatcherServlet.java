 ...
 *
 * <p>
 * This servlet is very flexible: It can be used with just about any workflow, with the installation of the appropriate
 * adapter classes. It offers the following functionality that distinguishes it from other request-driven web MVC
 * frameworks:
 *
 * <ul>
 ...
 * control the routing of requests to handler objects. Default is
 * {@link org.springframework.web.servlet.handler.BeanNameUrlHandlerMapping} and
 * {@link org.springframework.web.servlet.mvc.method.annotation.RequestMappingHandlerMapping}. HandlerMapping objects
 ...
 *
 * <li>It can use any {@link HandlerAdapter}; this allows for using any handler interface. Default adapters are
 * {@link org.springframework.web.servlet.mvc.HttpRequestHandlerAdapter},
 * {@link org.springframework.web.servlet.mvc.SimpleControllerHandlerAdapter}, for Spring's
 * {@link org.springframework.web.HttpRequestHandler} and {@link org.springframework.web.servlet.mvc.Controller}
 * interfaces, respectively. A default
 * {@link org.springframework.web.servlet.mvc.method.annotation.RequestMappingHandlerAdapter} will be registered as
 * well. HandlerAdapter objects can be added as beans in the application context, overriding the default
 * HandlerAdapters. Like HandlerMappings, HandlerAdapters can be given any bean name (they are tested by type).
 *
 ...
 * {@link org.springframework.web.servlet.mvc.support.DefaultHandlerExceptionResolver}. These HandlerExceptionResolvers
 * can be overridden through the application context. HandlerExceptionResolver can be given any bean name (they are
 * tested by type).
 *
 ...
 * {@link RequestToViewNameTranslator} will translate the current request into a view name. The corresponding bean name
 * is "viewNameTranslator"; the default is
 * {@link org.springframework.web.servlet.view.DefaultRequestToViewNameTranslator}.
 *
 * <li>The dispatcher's strategy for resolving multipart requests is determined by a
 * {@link org.springframework.web.multipart.MultipartResolver} implementation. Implementations for Apache Commons
 * FileUpload and Servlet 3 are included; the typical choice is
 * {@link org.springframework.web.multipart.commons.CommonsMultipartResolver}. The MultipartResolver bean name is
 ...
 * HTTP accept header, cookie, or session. The LocaleResolver bean name is "localeResolver"; default is
 ...
 * cookie and session storage are included. The ThemeResolver bean name is "themeResolver"; default is
 * {@link org.springframework.web.servlet.theme.FixedThemeResolver}.
 * </ul>
 *
 * <p>
 * <b>NOTE: The {@code @RequestMapping} annotation will only be processed if a corresponding {@code HandlerMapping} (for
 * type-level annotations) and/or {@code HandlerAdapter} (for method-level annotations) is present in the
 * dispatcher.</b> This is the case by default. However, if you are defining custom {@code HandlerMappings} or
 * {@code HandlerAdapters}, then you need to make sure that a corresponding custom {@code RequestMappingHandlerMapping}
 * and/or {@code RequestMappingHandlerAdapter} is defined as well - provided that you intend to use
 * {@code @RequestMapping}.
 *
 * <p>
 * <b>A web application can define any number of DispatcherServlets.</b> Each servlet will operate in its own namespace,
 ...
 *
 * <p>
 * As of Spring 3.1, {@code DispatcherServlet} may now be injected with a web application context, rather than creating
 * its own internally. This is useful in Servlet 3.0+ environments, which support programmatic registration of servlet
 * instances. See the {@link #DispatcherServlet(WebApplicationContext)} javadoc for details.
 *
 * @author Rod Johnson
...
public class DispatcherServlet extends FrameworkServlet {
    ...
    /**
     * Name of request attribute that holds a read-only {@code Map<String,?>} with "input" flash attributes saved by a
    ...
    /**
     * Name of request attribute that holds the "output" {@link FlashMap} with attributes to save for a subsequent
     * request.
     * 
    ...
    /**
     * Name of request attribute that exposes an Exception resolved with a {@link HandlerExceptionResolver} but where no
     * view was rendered (e.g. setting the status code).
     */
    ...
    /**
     * Name of the class path resource (relative to the DispatcherServlet class) that defines DispatcherServlet's
    ...
    /**
     * Create a new {@code DispatcherServlet} that will create its own internal web application context based on
     * defaults and values provided through servlet init-params. Typically used in Servlet 2.5 or earlier environments,
     * where the only option for servlet registration is through {@code web.xml} which requires the use of a no-arg
     * constructor.
     * <p>
     ...
     * {@code ApplicationContextInitializer} classes should be used to further configure the internal application
    ...
    /**
     * Create a new {@code DispatcherServlet} with the given web application context. This constructor is useful in
     ...
     * <li>{@link ConfigurableApplicationContext#refresh refresh()} will be called if the context implements
     * {@link ConfigurableApplicationContext}</li>
     * </ul>
    ...
    /**
     * Set whether to detect all HandlerAdapter beans in this servlet's context. Otherwise, just a single bean with name
    ...
    /**
     * Set whether to detect all HandlerExceptionResolver beans in this servlet's context. Otherwise, just a single bean
     * with name "handlerExceptionResolver" will be expected.
     * <p>
    ...
    /**
     * Set whether to detect all ViewResolver beans in this servlet's context. Otherwise, just a single bean with name
    ...
    /**
     * Set whether to throw a NoHandlerFoundException when no Handler was found for this request. This exception can
     * then be caught with a HandlerExceptionResolver or an {@code @ExceptionHandler} controller method.
     * <p>
     * Note that if {@link org.springframework.web.servlet.resource.DefaultServletHttpRequestHandler} is used, then
     * requests will always be forwarded to the default servlet and a NoHandlerFoundException would never be thrown in
     * that case.
     * <p>
     ...
     * original state of all request attributes after the DispatcherServlet has processed within an include request.
     * Otherwise, just the DispatcherServlet's own request attributes will be reset, but not model attributes for JSPs
     * or special attributes set by views (for example, JSTL's).
     * <p>
     ...
     *
     * @return the MultipartResolver used by this servlet, or {@code null} if none (indicating that no multipart support
     *         is available)
     */
    @Nullable
    ...
    /**
     * Return the configured {@link HandlerMapping} beans that were detected by type in the
     * {@link WebApplicationContext} or initialized based on the default set of strategies from
     * {@literal DispatcherServlet.properties}.
     * <p>
     ...
     * DispatcherServlet class) to determine the class names. It instantiates the strategy objects through the context's
     * BeanFactory.
     *
     * @param context
     *            the current WebApplicationContext
    ...
    /**
     * Handle the result of handler selection and handler invocation, which is either a ModelAndView or an Exception to
     * be resolved to a ModelAndView.
     */
