...
/**
 * Base class for all JSP form tags. Provides utility methods for null-safe EL evaluation and for accessing and working
 * with a {@link TagWriter}.
 *
 * <p>
 * Subclasses should implement the {@link #writeTagContent(TagWriter)} to perform actual tag rendering.
 *
 * <p>
 * Subclasses (or test classes) can override the {@link #createTagWriter()} method to redirect output to a
 * {@link java.io.Writer} other than the {@link javax.servlet.jsp.JspWriter} associated with the current
...
public abstract class AbstractFormTag extends HtmlEscapingAwareTag {
     ...
     * {@link ObjectUtils#getDisplayString String representation} is written as the attribute value. If the resultant
     * {@code String} representation is {@code null} or empty, no attribute is written.
     * 
    ...
    /**
     * Create the {@link TagWriter} which all output will be written to. By default, the {@link TagWriter} writes its
    ...
    /**
     * Provide a simple template method that calls {@link #createTagWriter()} and passes the created {@link TagWriter}
