 ...
 *
 * <p>
 * This class is aware of Spring MVC handler mappings used to serve static resources and uses the
 * {@code ResourceResolver} chains of the configured {@code ResourceHttpRequestHandler}s to make its decisions.
 *
...
public class ResourceUrlProvider implements ApplicationListener<ContextRefreshedEvent>, ApplicationContextAware {
     ...
     * {@link #getForRequestUrl(javax.servlet.http.HttpServletRequest, String)} in order to derive the lookup path for a
     ...
     * <p>
     * <strong>Note:</strong> by default resource mappings are auto-detected from the Spring {@code ApplicationContext}.
    ...
    /**
     * A variation on {@link #getForLookupPath(String)} that accepts a full request URL path (i.e. including context and
     * servlet path) and returns the full request URL path to expose for public use.
     * 
     * @param request
     *            the current request
     * @param requestUrl
     ...
     * {@code ResourceResolver} chain of the matched {@code ResourceHttpRequestHandler} to resolve the URL path to
     * expose for public use.
     * <p>
     * It is expected that the given path is what Spring MVC would use for request mapping purposes, i.e. excluding
     * context and servlet path portions.
     * <p>
