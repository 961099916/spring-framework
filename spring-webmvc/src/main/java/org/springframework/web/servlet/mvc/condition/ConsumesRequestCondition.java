...
/**
 * A logical disjunction (' || ') request condition to match a request's 'Content-Type' header to a list of media type
 * expressions. Two kinds of media type expressions are supported, which are described in
 * {@link RequestMapping#consumes()} and {@link RequestMapping#headers()} where the header name is 'Content-Type'.
...
public final class ConsumesRequestCondition extends AbstractRequestCondition<ConsumesRequestCondition> {
     ...
     * 
     * @param consumes
     *            expressions with the syntax described in {@link RequestMapping#consumes()}; if 0 expressions are
    ...
    /**
     * Creates a new instance with "consumes" and "header" expressions. "Header" expressions where the header name is
     * not 'Content-Type' or have no header value defined are ignored. If 0 expressions are provided in total, the
    ...
    /**
     * Private constructor for internal when creating matching conditions. Note the expressions List is neither sorted
     * nor deep copied.
     */
     ...
     * <p>
     * By default this is set to {@code true} in which case it is assumed a request body is required and this condition
     * matches to the "Content-Type" header or falls back on "Content-Type: application/octet-stream".
     * <p>
     * If set to {@code false}, and the request does not have a body, then this condition matches automatically, i.e.
     * without checking expressions.
     *
     * @param bodyRequired
     *            whether requests are expected to have a body
     * @since 5.2
     */
     ...
     * a method-level "consumes" overrides a type-level "consumes" condition.
     */
    @Override
     ...
     * @param request
     *            the current request
     * @return the same instance if the condition contains no expressions; or a new condition with matching expressions
     *         only; or {@code null} if no expressions match
     */
    @Override
    @Nullable
     ...
     * each instance contains the matching consumable media type expression only or is otherwise empty.
     */
    @Override
