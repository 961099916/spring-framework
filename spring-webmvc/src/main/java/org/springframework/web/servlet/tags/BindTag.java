...
/**
 * The {@code <bind>} tag supports evaluation of binding errors for a certain bean or bean property. Exposes a "status"
 * variable of type {@link org.springframework.web.servlet.support.BindStatus}, to both Java expressions and JSP EL
 * expressions.
 *
 * <p>
 * Can be used to bind to any bean or bean property in the model. The specified path determines whether the tag exposes
 * the status of the bean itself (showing object-level errors), a specific bean property (showing field errors), or a
 * matching set of bean properties (showing all corresponding field errors).
 *
 * <p>
 * The {@link org.springframework.validation.Errors} object that has been bound using this tag is exposed to
 * collaborating tags, as well as the bean property that this errors object applies to. Nested tags such as the
 * {@link TransformTag} can access those exposed properties.
 *
 * <table>
 ...
 * company.address.zipCode or just employee. The status object will exported to the page scope, specifically for this
...
public class BindTag extends HtmlEscapingAwareTag implements EditorAwareTag {
    ...
    /**
     * Set the path that this tag should apply. Can be a bean (e.g. "person") to get global errors, or a bean property
     * (e.g. "person.name") to get field errors (also supporting nested fields and "person.na*" mappings). "person.*"
     * will return all errors for the specified bean, both global and field errors.
     *
    ...
    /**
     * Retrieve the property that this tag is currently bound to, or {@code null} if bound to an object rather than a
     * specific property. Intended for cooperating nesting tags.
     * 
