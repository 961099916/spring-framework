...
/**
 * The {@code <url>} tag creates URLs. Modeled after the JSTL {@code c:url} tag with backwards compatibility in mind.
 *
 * <p>
 ...
 *
 * <p>
 * Template URI variables are indicated in the {@link #setValue(String) 'value'} attribute and marked by braces
 * '{variableName}'. The braces and attribute name are replaced by the URL encoded value of a parameter defined with the
 * spring:param tag in the body of the url tag. If no parameter is available the literal value is passed through. Params
 * matched to template variables will not be added to the query string.
 *
 * <p>
 * Use of the spring:param tag for URI template variables is strongly recommended over direct EL substitution as the
 * values are URL encoded. Failure to properly encode URL can leave an application vulnerable to XSS and other injection
 * attacks.
 *
 * <p>
 * URLs can be HTML/XML escaped by setting the {@link #setHtmlEscape(boolean) 'htmlEscape'} attribute to 'true'. Detects
 * an HTML escaping setting, either on this tag instance, the page level, or the {@code web.xml} level. The default is
 * 'false'. When setting the URL value into a variable, escaping is not recommended.
 *
 * <p>
 * Example usage:
 * 
 ...
 *
 * <p>
 * The above results in: {@code /currentApplicationContext/url/path/more%20than%20JSTL%20c%3Aurl}
 *
 * <table>
 ...
 * <td>true</td>
 * <td>true</td>
 * <td>The URL to build. This value can include template {placeholders} that are replaced with the URL encoded value of
 * the named parameter. Parameters must be defined using the param tag inside the body of this tag.</td>
 * </tr>
 * <tr>
 * <td>context</td>
 * <td>false</td>
 * <td>true</td>
 * <td>Specifies a remote application context path. The default is the current application context path.</td>
 * </tr>
 * <tr>
 * <td>var</td>
 * <td>false</td>
 * <td>true</td>
 * <td>The name of the variable to export the URL value to. If not specified the URL is written as output.</td>
 * </tr>
 * <tr>
 * <td>scope</td>
 * <td>false</td>
 * <td>true</td>
 * <td>The scope for the var. 'application', 'session', 'request' and 'page' scopes are supported. Defaults to page
 * scope. This attribute has no effect unless the var attribute is also defined.</td>
 * </tr>
 * <tr>
 * <td>htmlEscape</td>
 * <td>false</td>
 * <td>true</td>
 * <td>Set HTML escaping for this tag, as a boolean value. Overrides the default HTML escaping setting for the current
 * page.</td>
 * </tr>
 * <tr>
...
public class UrlTag extends HtmlEscapingAwareTag implements ParamAware {
     ...
     * @param usedParams
     *            set of parameter names that have been applied as template params
    ...
    /**
     * Replace template markers in the URL matching available parameters. The name of matched parameters are added to
     * the used parameters set.
     * <p>
    ...
    protected String replaceUriTemplateParams(String uri, List<Param> params, Set<String> usedParams)
        ...
        for (Param param : params) {
            ...
            if (uri.contains(template)) {
                ...
                try {
                    uri =
                        StringUtils.replace(uri, template, (value != null ? UriUtils.encodePath(value, encoding) : ""));
                } catch (UnsupportedCharsetException ex) {
                    throw new JspException(ex);
                }
            } else {
