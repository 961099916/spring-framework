...
/**
 * Assist with the configuration of a chain of {@link org.springframework.web.servlet.ViewResolver ViewResolver}
 * instances. This class is expected to be used via {@link WebMvcConfigurer#configureViewResolvers}.
 *
...
public class ViewResolverRegistry {
    ...
    /**
     * Enable use of a {@link ContentNegotiatingViewResolver} to front all other configured view resolvers and select
     * among all selected Views based on media types requested by the client (e.g. in the Accept header).
     * <p>
     * If invoked multiple times the provided default views will be added to any other default views that may have been
     * configured already.
     * 
     ...
     * <p>
     * When this method is invoked more than once, each call will register a new ViewResolver instance. Note that since
     * it's not easy to determine if a JSP exists without forwarding to it, using multiple JSP-based view resolvers only
     * makes sense in combination with the "viewNames" property on the resolver indicating which view names are handled
     * by which resolver.
     */
    ...
    /**
     * Register a {@link ViewResolver} bean instance. This may be useful to configure a custom (or 3rd party) resolver
     * implementation. It may also be used as an alternative to other registration methods in this class when they don't
     * expose some more advanced property that needs to be set.
     */
     ...
     * {@link org.springframework.web.servlet.view.ViewResolverComposite ViewResolverComposite} and follow the order of
     * registration. This property determines the order of the ViewResolverComposite itself relative to any additional
     * ViewResolver's (not registered here) present in the Spring configuration
     * <p>
     * By default this property is not set, which means the resolver is ordered at {@link Ordered#LOWEST_PRECEDENCE}
     * unless content negotiation is enabled in which case the order (if not set explicitly) is changed to
