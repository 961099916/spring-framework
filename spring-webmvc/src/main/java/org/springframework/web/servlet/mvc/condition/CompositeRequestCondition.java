...
/**
 * Implements the {@link RequestCondition} contract by delegating to multiple {@code RequestCondition} types and using a
 * logical conjunction (' && ') to ensure all conditions match a given request.
 *
 * <p>
 * When {@code CompositeRequestCondition} instances are combined or compared they are expected to (a) contain the same
 * number of conditions and (b) that conditions in the respective index are of the same type. It is acceptable to
...
public class CompositeRequestCondition extends AbstractRequestCondition<CompositeRequestCondition> {
     ...
     * {@code CompositeRequestCondition} instances with the same number of conditions so they may be compared and
     * combined. It is acceptable to provide {@code null} conditions.
     */
    ...
    /**
     * If one instance is empty, return the other. If both instances have conditions, combine the individual conditions
    ...
    /**
     * Delegate to <em>all</em> contained conditions to match the request and return the resulting "matching" condition
     * instances.
     * <p>
    ...
    /**
     * If one instance is empty, the other "wins". If both instances have conditions, compare them in the order in which
     * they were provided.
     */
    @Override
