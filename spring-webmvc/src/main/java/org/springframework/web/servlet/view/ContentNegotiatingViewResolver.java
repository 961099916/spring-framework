 ...
 *
 * <p>
 * The {@code ContentNegotiatingViewResolver} does not resolve views itself, but delegates to other {@link ViewResolver
 * ViewResolvers}. By default, these other view resolvers are picked up automatically from the application context,
 * though they can also be set explicitly by using the {@link #setViewResolvers viewResolvers} property.
 * <strong>Note</strong> that in order for this view resolver to work properly, the {@link #setOrder order} property
 * needs to be set to a higher precedence than the others (the default is {@link Ordered#HIGHEST_PRECEDENCE}).
 *
 * <p>
 * This view resolver uses the requested {@linkplain MediaType media type} to select a suitable {@link View} for a
 * request. The requested media type is determined through the configured {@link ContentNegotiationManager}. Once the
 * requested media type has been determined, this resolver queries each delegate view resolver for a {@link View} and
 * determines if the requested media type is {@linkplain MediaType#includes(MediaType) compatible} with the view's
 ...
 * override the views provided by the view resolvers. Note that these default views are offered as candidates, and still
 * need have the content type requested (via file extension, parameter, or {@code Accept} header, described above).
 *
 * <p>
 * For example, if the request path is {@code /view.html}, this view resolver will look for a view that has the
 * {@code text/html} content type (based on the {@code html} file extension). A request for {@code /view} with a
 * {@code text/html} request {@code Accept} header has the same result.
 *
...
public class ContentNegotiatingViewResolver extends WebApplicationObjectSupport
     ...
     * <p>
     * If not set, ContentNegotiationManager's default constructor will be used, applying a
    ...
    public View resolveViewName(String viewName, Locale locale) throws Exception {
        ...
        String mediaTypeInfo =
            logger.isDebugEnabled() && requestedMediaTypes != null ? " given " + requestedMediaTypes.toString() : "";
