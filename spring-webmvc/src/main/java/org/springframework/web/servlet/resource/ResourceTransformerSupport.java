...
/**
 * A base class for a {@code ResourceTransformer} with an optional helper method for resolving public links within a
...
public abstract class ResourceTransformerSupport implements ResourceTransformer {
    ...
    /**
     * Configure a {@link ResourceUrlProvider} to use when resolving the public URL of links in a transformed resource
     * (e.g. import links in a CSS file). This is required only for links expressed as full paths and not for relative
     * links.
     */
    ...
    /**
     * A transformer can use this method when a resource being transformed contains links to other resources. Such links
     * need to be replaced with the public facing link as determined by the resource resolver chain (e.g. the public URL
     * may have a version inserted).
     * 
     * @param resourcePath
     *            the path to a resource that needs to be re-written
     * @param request
     *            the current request
     * @param resource
    ...
    /**
     * Transform the given relative request path to an absolute path, taking the path of the given request as a point of
     * reference. The resulting path is also cleaned from sequences like "path/..".
     * 
     * @param path
