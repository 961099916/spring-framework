...
/**
 * A simple {@code ResourceResolver} that tries to find a resource under the given locations matching to the request
 * path.
 *
 * <p>
 * This resolver does not delegate to the {@code ResourceResolverChain} and is expected to be configured at the end in a
 * chain of resolvers.
 *
...
public class PathResourceResolver extends AbstractResourceResolver {
    ...
    /**
     * By default when a Resource is found, the path of the resolved resource is compared to ensure it's under the input
     * location where it was found. However sometimes that may not be the case, e.g. when
     * {@link org.springframework.web.servlet.resource.CssLinkResourceTransformer} resolves public URLs of links it
     * contains, the CSS file is the location and the resources being resolved are css files, images, fonts and others
     * located in adjacent or parent directories.
     * <p>
     * This property allows configuring a complete list of locations under which resources must be so that if a resource
     * is not under the location relative to which it was found, this list may be checked as well.
     * <p>
     * By default {@link ResourceHttpRequestHandler} initializes this property to match its list of locations.
     *
     * @param locations
     ...
     * {@link org.springframework.core.io.UrlResource URL resource} location the charset is used to encode the relative
     * path
     * <p>
     * <strong>Note:</strong> the charset is used only if the {@link #setUrlPathHelper urlPathHelper} property is also
     * configured and its {@code urlDecode} property is set to true.
     *
     * @since 4.3.13
     */
     ...
     * <p>
     * The default implementation checks if there is a readable {@code Resource} for the given path relative to the
     * location.
     * 
     * @param resourcePath
     ...
     * The default implementation also verifies the resource is either under the location relative to which it was found
     * or is under one of the {@link #setAllowedLocations allowed locations}.
     * 
     * @param resource
