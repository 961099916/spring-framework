...
public class PatternsRequestCondition extends AbstractRequestCondition<PatternsRequestCondition> {
    ...
    /**
     * Creates a new instance with the given URL patterns. Each pattern that is not empty and does not start with "/" is
     * prepended with "/".
     * 
     * @param patterns
    ...
    /**
     * Alternative constructor with additional, optional {@link UrlPathHelper}, {@link PathMatcher}, and whether to
     * automatically match trailing slashes.
     * 
     * @param patterns
     *            the URL patterns to use; if 0, the condition will match to every request.
     * @param urlPathHelper
     *            a {@link UrlPathHelper} for determining the lookup path for a request
     * @param pathMatcher
     ...
     *             {@link org.springframework.web.servlet.mvc.method.annotation.RequestMappingHandlerMapping} on the
     ...
     * <ul>
     * <li>If there are patterns in both instances, combine the patterns in "this" with the patterns in "other" using
    ...
    /**
     * Checks if any of the patterns match the given request and returns an instance that is guaranteed to contain
     * matching patterns, sorted via {@link PathMatcher#getPatternComparator(String)}.
     * <p>
     ...
     * @param request
     *            the current request
     * @return the same instance if the condition contains no patterns; or a new condition with sorted matching
     *         patterns; or {@code null} if no patterns match.
     */
    @Override
    @Nullable
    ...
    /**
     * Find the patterns matching the given lookup path. Invoking this method should yield results equivalent to those
     * of calling {@link #getMatchingCondition}. This method is provided as an alternative to be used if no request is
     * available (e.g. introspection, tooling, etc).
     * 
     * @param lookupPath
     *            the lookup path to match to existing patterns
    ...
    /**
     * Compare the two conditions based on the URL patterns they contain. Patterns are compared one at a time, from top
     * to bottom via {@link PathMatcher#getPatternComparator(String)}. If all compared patterns match equally, but one
     * instance has more patterns, it is considered a closer match.
     * <p>
     * It is assumed that both instances have been obtained via {@link #getMatchingCondition(HttpServletRequest)} to
     * ensure they contain only patterns that match the request and are sorted with the best matches on top.
     */
    @Override
