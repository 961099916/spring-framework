 ...
 *
 * <p>
 * Supports lookup of current WebApplicationContext, LocaleResolver, Locale, ThemeResolver, Theme, and
 * MultipartResolver.
 *
...
public abstract class RequestContextUtils {
     ...
     * and for the global context if none was found associated with the current request. The global context will be
     * found via the ServletContext or via ContextLoader's current context.
     * <p>
     * NOTE: This variant remains compatible with Servlet 2.5, explicitly checking a given ServletContext instead of
     ...
     * @return the request-specific WebApplicationContext, or the global one if no request-specific context has been
     *         found, or {@code null} if none
     * @since 4.2.1
    ...
    /**
     * Retrieve the current locale from the given request, using the LocaleResolver bound to the request by the
     ...
     * {@link javax.servlet.http.HttpServletRequest#getLocale()} method, falling back to the latter if no more specific
     * locale has been found.
     * <p>
     * Consider using {@link org.springframework.context.i18n.LocaleContextHolder#getLocale()} which will normally be
     * populated with the same Locale.
     * 
     * @param request
     *            current HTTP request
     * @return the current locale for the given request, either from the LocaleResolver or from the plain request itself
     ...
     * <p>
     * Note: This method returns {@code null} if no specific time zone can be resolved for the given request. This is in
     * contrast to {@link #getLocale} where there is always the request's accept-header locale to fall back to.
     * <p>
     * Consider using {@link org.springframework.context.i18n.LocaleContextHolder#getTimeZone()} which will normally be
     * populated with the same TimeZone: That method only differs in terms of its fallback to the system time zone if
     * the LocaleResolver hasn't provided a specific time zone (instead of this method's {@code null}).
     * 
     * @param request
     *            current HTTP request
     ...
     * @param request
     *            current request
     * @return a {@link FlashMap} instance, never {@code null} within a {@code DispatcherServlet}-handled request
     */
     ...
     * <p>
     * As of 5.0 the convenience method {@link #saveOutputFlashMap} may be used to save the "output" FlashMap.
     * 
     * @param request
     *            the current request
     * @return a {@link FlashMapManager} instance, never {@code null} within a {@code DispatcherServlet}-handled request
     */
    @Nullable
    ...
    /**
     * Convenience method that retrieves the {@link #getOutputFlashMap "output" FlashMap}, updates it with the path and
     * query params of the target URL, and then saves it using the {@link #getFlashMapManager FlashMapManager}.
     * 
     * @param location
