 ...
 * needed when redirecting from one URL to another -- e.g. the Post/Redirect/Get pattern. A FlashMap is saved before the
 * redirect (typically in the session) and is made available after the redirect and removed immediately.
 *
 * <p>
 * A FlashMap can be set up with a request path and request parameters to help identify the target request. Without this
 * information, a FlashMap is made available to the next request, which may or may not be the intended recipient. On a
 * redirect, the target URL is known and a FlashMap can be updated with that information. This is done automatically
 * when the {@code org.springframework.web.servlet.view.RedirectView} is used.
 *
 * <p>
 * Note: annotated controllers will usually not use FlashMap directly. See
 * {@code org.springframework.web.servlet.mvc.support.RedirectAttributes} for an overview of using flash attributes in
...
public final class FlashMap extends HashMap<String, Object> implements Comparable<FlashMap> {
     ...
     * 
     * @param name
     *            the expected parameter name (skipped if empty)
     * @param value
    ...
    /**
     * Set the expiration time for the FlashMap. This is provided for serialization purposes but can also be used
     * instead {@link #startExpirationPeriod(int)}.
     *
     * @since 4.2
     */
     ...
     * Before comparing FlashMap instances ensure that they match a given request.
     */
    @Override
