...
/**
 * Represents a function that evaluates on a given {@link ServerRequest}. Instances of this function that evaluate on
 * common request properties can be found in {@link RequestPredicates}.
 *
...
public interface RequestPredicate {
    ...
    /**
     * Return a composed request predicate that tests against both this predicate AND the {@code other} predicate. When
    ...
    /**
     * Return a composed request predicate that tests against both this predicate OR the {@code other} predicate. When
    ...
    /**
     * Transform the given request into a request used for a nested route. For instance, a path-based predicate can
     * return a {@code ServerRequest} with a the path remaining after a match.
     * <p>
     * The default implementation returns an {@code Optional} wrapping the given request if {@link #test(ServerRequest)}
     * evaluates to {@code true}; or {@link Optional#empty()} if it evaluates to {@code false}.
     * 
     * @param request
     *            the request to be nested
     ...
     * {@link RequestPredicates.Visitor#unknown(RequestPredicate)}; composed {@code RequestPredicate} implementations
     * are expected to call {@code accept} for all components that make up this request predicate.
     * 
     * @param visitor
