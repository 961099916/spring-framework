...
/**
 * Abstract base class for {@link HandlerMapping} implementations that define a mapping between a request and a
 * {@link HandlerMethod}.
 *
 * <p>
 * For each registered handler method, a unique mapping is maintained with subclasses defining the details of the
 * mapping type {@code <T>}.
 *
 ...
 * @since 3.1
 * @param <T>
 *            the mapping for a {@link HandlerMethod} containing the conditions needed to match the handler method to an
 *            incoming request.
 */
public abstract class AbstractHandlerMethodMapping<T> extends AbstractHandlerMapping implements InitializingBean {
     ...
     * <p>
     * Default is "false": Only beans in the current ApplicationContext are considered, i.e. only in the context that
     * this HandlerMapping itself is defined in (typically the current DispatcherServlet's context).
     * <p>
     * Switch this flag on to detect handler beans in ancestor contexts (typically the Spring root
     ...
     * <p>
     * The default naming strategy is based on the capital letters of the class name followed by "#" and then the method
     * name, e.g. "TC#getFoo" for a class named TestController with method getFoo.
     */
     ...
     * @param mappingName
     *            the mapping name
     * @return a list of matching HandlerMethod's or {@code null}; the returned list will never be modified and is safe
     *         to iterate.
    ...
    /**
     * Determine the type of the specified candidate bean and call {@link #detectHandlerMethods} if identified as a
     * handler type.
     * <p>
     * This implementation avoids bean creation through checking
     * {@link org.springframework.beans.factory.BeanFactory#getType} and calling {@link #detectHandlerMethods} with the
     * bean name.
     * 
     * @param beanName
    ...
    /**
     * Look up the best-matching handler method for the current request. If multiple matches are found, the best match
     * is selected.
     * 
     * @param lookupPath
    ...
    /**
     * Check if a mapping matches the current request and return a (potentially new) mapping with conditions relevant to
     * the current request.
     * 
     * @param mapping
    ...
    /**
     * A registry that maintains all mappings to handler methods, exposing methods to perform lookups and providing
     * concurrent access.
     * <p>
    ...
    /**
     * A thin wrapper around a matched HandlerMethod and its mapping, for the purpose of comparing the best match with a
     * comparator in the context of the current request.
     */
    private class Match {
