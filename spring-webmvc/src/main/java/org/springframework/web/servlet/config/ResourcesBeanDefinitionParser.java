...
/**
 * {@link org.springframework.beans.factory.xml.BeanDefinitionParser} that parses a {@code resources} element to
 * register a {@link ResourceHttpRequestHandler} and register a {@link SimpleUrlHandlerMapping} for mapping resource
 * requests, and a {@link HttpRequestHandlerAdapter}. Will also create a resource handling chain with
...
class ResourcesBeanDefinitionParser implements BeanDefinitionParser {
    ...
    private static final boolean webJarsPresent =
        ClassUtils.isPresent("org.webjars.WebJarAssetLocator", ResourcesBeanDefinitionParser.class.getClassLoader());
    ...
    private CacheControl parseCacheControl(Element element) {
        ...
        if (element.hasAttribute("stale-while-revalidate")) {
            cacheControl = cacheControl
                .staleWhileRevalidate(Long.parseLong(element.getAttribute("stale-while-revalidate")), TimeUnit.SECONDS);
        }
