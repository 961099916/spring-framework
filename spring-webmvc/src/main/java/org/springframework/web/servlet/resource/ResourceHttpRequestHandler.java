...
/**
 * {@code HttpRequestHandler} that serves static resources in an optimized way according to the guidelines of Page
 * Speed, YSlow, etc.
 *
 * <p>
 * The {@linkplain #setLocations "locations"} property takes a list of Spring {@link Resource} locations from which
 * static resources are allowed to be served by this handler. Resources could be served from a classpath location, e.g.
 * "classpath:/META-INF/public-web-resources/", allowing convenient packaging and serving of resources such as .js,
 * .css, and others in jar files.
 *
 * <p>
 * This request handler may also be configured with a {@link #setResourceResolvers(List) resourcesResolver} and
 * {@link #setResourceTransformers(List) resourceTransformer} chains to support arbitrary resolution and transformation
 * of resources being served. By default a {@link PathResourceResolver} simply finds resources based on the configured
 * "locations". An application can configure additional resolvers and transformers such as the
 * {@link VersionResourceResolver} which can resolve and prepare URLs for resources with a version in the URL.
 *
 * <p>
 * This handler also properly evaluates the {@code Last-Modified} header (if present) so that a {@code 304} status code
 * will be returned as appropriate, avoiding unnecessary overhead for resources that are already cached by the client.
 *
 * @author Keith Donald
...
public class ResourceHttpRequestHandler extends WebContentGenerator
    ...
    /**
     * An alternative to {@link #setLocations(List)} that accepts a list of String-based location values, with support
     * for {@link UrlResource}'s (e.g. files or HTTP URLs) with a special prefix to indicate the charset to use when
     * appending relative paths. For example {@code "[charset=Windows-31J]https://example.org/path"}.
     * 
     * @since 4.3.13
     */
     ...
     * <p>
     * Note that if {@link #setLocationValues(List) locationValues} are provided, instead of loaded Resource-based
     * locations, this method will return empty until after initialization via {@link #afterPropertiesSet()}.
     *
    ...
    /**
     * Set the {@code List} of {@code Resource} locations to use as sources for serving static resources.
     *
     ...
     * <p>
     * By default {@link PathResourceResolver} is configured. If using this property, it is recommended to add
     * {@link PathResourceResolver} as the last resolver.
     */
    ...
    /**
     * Configure a {@code ContentNegotiationManager} to help determine the media types for resources being served. If
     * the manager contains a path extension strategy it will be checked for registered file extension.
     *
     * @since 4.3
     * @deprecated as of 5.2.4 in favor of using {@link #setMediaTypes(Map)} with mappings possibly obtained from
    ...
    /**
     * Add mappings between file extensions, extracted from the filename of a static {@link Resource}, and corresponding
     * media type to set on the response.
     * <p>
     * Use of this method is typically not necessary since mappings are otherwise determined via
     * {@link javax.servlet.ServletContext#getMimeType(String)} or via {@link MediaTypeFactory#getMediaType(Resource)}.
     *
     * @param mediaTypes
     *            media type mappings
     * @since 5.2.4
    ...
    /**
     * Provide a reference to the {@link UrlPathHelper} used to map requests to static resources. This helps to derive
     * information about the lookup path such as whether it is decoded or not.
     *
     * @since 4.3.13
     */
     ...
     * {@code allowedLocations} property (if empty) to match the {@link #setLocations locations} configured on this
     * class.
     */
     ...
     * 
     * @deprecated as of 5.2.4 this method returns {@code null}, and if a sub-class returns an actual instance,the
     *             instance is used only as a source of media type mappings, if it contains any. Please, use
     *             {@link #setMediaTypes(Map)} instead, or if you need to change behavior, you can override
     ...
     * <p>
     * Checks for the existence of the requested resource in the configured list of locations. If the resource does not
     * exist, a {@code 404} response will be returned to the client. If the resource exists, the request will be checked
     * for the presence of the {@code Last-Modified} header, and its value will be compared against the last-modified
     * timestamp of the given resource, returning a {@code 304} status code if the {@code Last-Modified} value is
     * greater. If the resource is newer than the {@code Last-Modified} value, or the header is not present, the content
     * resource of the resource will be written to the response with caching headers set to expire one year in the
     * future.
     */
    @Override
     ...
     * <li>Any combination of leading slash and control characters (00-1F and 7F) with a single "/" or "". For example
     ...
     * <li>Paths that contain "../" after a call to {@link org.springframework.util.StringUtils#cleanPath}.
     * <li>Paths that represent a {@link org.springframework.util.ResourceUtils#isUrl valid URL} or would represent one
     * after the leading slash is removed.
     * </ul>
     * <p>
     * <strong>Note:</strong> this method assumes that leading, duplicate '/' or control characters (e.g. white space)
     * have been trimmed so that the path starts predictably with a single '/' or does not have one.
     * 
     * @param path
     *            the path to validate
     * @return {@code true} if the path is invalid, {@code false} otherwise
    ...
    /**
     * Determine the media type for the given request and the resource matched to it. This implementation tries to
     * determine the MediaType using one of the following lookups based on the resource filename and its path extension:
     * <ol>
     ...
     * @param resource
     *            the identified resource (never {@code null})
     * @param mediaType
     *            the resource's media type (never {@code null})
     * @throws IOException
