 ...
 * <ul>
 * <li><b>url</b>: the location of the FreeMarker template to be wrapped, relative to the FreeMarker template context
 * (directory).
 * <li><b>encoding</b> (optional, default is determined by FreeMarker configuration): the encoding of the FreeMarker
 * template file
 * </ul>
 *
 * <p>
 * Depends on a single {@link FreeMarkerConfig} object such as {@link FreeMarkerConfigurer} being accessible in the
 * current web application context, with any bean name. Alternatively, you can set the FreeMarker {@link Configuration}
 * object as a bean property. See {@link #setConfiguration} for more details on the impacts of this approach.
 *
 * <p>
...
public class FreeMarkerView extends AbstractTemplateView {
    ...
    /**
     * Set the encoding of the FreeMarker template file. Default is determined by the FreeMarker Configuration:
     * "ISO-8859-1" if not specified otherwise.
     * <p>
     * Specify the encoding in the FreeMarker Configuration rather than per template if all your templates share a
     * common encoding.
     */
     ...
     * <p>
     * If this is not set, the default lookup will occur: a single {@link FreeMarkerConfig} is expected in the current
     * web application context, with any bean name. <strong>Note:</strong> using this method will cause a new instance
     * of {@link TaglibFactory} to created for every single {@link FreeMarkerView} instance. This can be quite expensive
     * in terms of memory and initial CPU usage. In production it is recommended that you use a {@link FreeMarkerConfig}
     ...
     *             in case of no Configuration object set
     * @since 5.0
     */
     ...
     * <p>
     * Checks that the template for the default Locale can be found: FreeMarker will check non-Locale-specific templates
     * if a locale-specific one is not found.
     * 
    ...
    /**
     * Return the configured FreeMarker {@link ObjectWrapper}, or the {@link ObjectWrapper#DEFAULT_WRAPPER default
     ...
     * @throws Exception
     *             if there's a fatal error while we're adding information to the context
     * @see #renderMergedTemplateModel
     */
    protected void exposeHelpers(Map<String, Object> model, HttpServletRequest request) throws Exception {}

    /**
     * Render the FreeMarker view to the given response, using the given model map which contains the complete template
     * model to use.
     * <p>
     * The default implementation renders the template specified by the "url" bean property, retrieved via
     * {@code getTemplate}. It delegates to the {@code processTemplate} method to merge the template instance with the
     * given template model.
     * <p>
     * Adds the standard Freemarker hash models to the model: request parameters, request, session and application
     * (ServletContext), as well as the JSP tag library hash model.
     * <p>
    ...
    /**
     * Build a FreeMarker {@link HttpSessionHashModel} for the given request, detecting whether a session already exists
    ...
    /**
     * Retrieve the FreeMarker template specified by the given name, using the encoding specified by the "encoding" bean
     * property.
     * <p>
