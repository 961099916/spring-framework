 ...
 *
 * <p>
 * Suitable for exposition to views, and usage within JSP's "useBean" tag, JSP scriptlets, JSTL EL, etc. Necessary for
 * views that do not have access to the servlet request, like FreeMarker templates.
 *
 * <p>
...
public class RequestContext {
     ...
     * <p>
     * This only works with InternalResourceViews, as Errors instances are part of the model and not normally exposed as
     * request attributes. It will typically be used within JSPs or custom tags.
     * <p>
     * <b>Will only work within a DispatcherServlet request.</b> Pass in a ServletContext to be able to fallback to the
     ...
     * @see org.springframework.web.servlet.DispatcherServlet
     * @see #RequestContext(javax.servlet.http.HttpServletRequest, javax.servlet.http.HttpServletResponse,
     ...
     * request attributes. It will typically be used within JSPs or custom tags.
     * <p>
     * If a ServletContext is specified, the RequestContext will also work with the root WebApplicationContext (outside
     * a DispatcherServlet).
     * 
     * @param request
     *            current HTTP request
     * @param servletContext
     *            the servlet context of the web application (can be {@code null}; necessary for fallback to root
     ...
     *            current HTTP request
     * @param model
     *            the model attributes for the current view (can be {@code null}, using the request attributes for
     *            Errors retrieval)
     * @see org.springframework.web.servlet.DispatcherServlet
     * @see #RequestContext(javax.servlet.http.HttpServletRequest, javax.servlet.http.HttpServletResponse,
     *      javax.servlet.ServletContext, Map)
     ...
     * This works with all View implementations. It will typically be used by View implementations.
     * <p>
     * If a ServletContext is specified, the RequestContext will also work with a root WebApplicationContext (outside a
     * DispatcherServlet).
     * 
     * @param request
     *            current HTTP request
     ...
     * <p>
     * Typically coming from a DispatcherServlet's {@link LocaleResolver}. Also includes a fallback check for JSTL's
     * Locale attribute.
     * 
     ...
     * <p>
     * Typically coming from a DispatcherServlet's {@link LocaleContextResolver}. Also includes a fallback check for
     * JSTL's TimeZone attribute.
     * 
     ...
     * <p>
     * The default implementation checks for a JSTL locale attribute in request, session or application scope; if not
     * found, returns the {@code HttpServletRequest.getLocale()}.
     * 
     ...
     * <p>
     * The default implementation checks for a JSTL time zone attribute in request, session or application scope;
    ...
    /**
     * Change the current locale to the specified locale and time zone context, storing the new locale context through
     * the configured {@link LocaleResolver}.
     * 
     * @param locale
     *            the new locale
     * @param timeZone
    ...
    /**
     * Change the current theme to the specified one, storing the new theme name through the configured
     * {@link ThemeResolver}.
     * 
     * @param theme
     *            the new theme
    ...
    /**
     * Change the current theme to the specified theme by name, storing the new theme name through the configured
     * {@link ThemeResolver}.
     * 
     * @param themeName
    ...
    /**
     * Is HTML escaping using the response encoding by default? If enabled, only XML markup significant characters will
     * be escaped with UTF-* encodings.
     * <p>
    ...
    /**
     * Return the default setting about use of response encoding for HTML escape setting, differentiating between no
     * default specified and an explicit value.
     * 
     ...
     * {@code "requestDataValueProcessor"}. Or {@code null} if no matching bean was found.
     */
    @Nullable
    ...
    /**
     * Return a context-aware URl for the given relative URL with placeholders (named keys with braces {@code {}}). For
     * example, send in a relative URL {@code foo/{bar}?spam={spam}} and a parameter map {@code {bar=baz,spam=nuts}} and
    ...
    /**
     * Return the path to URL mappings within the current servlet including the context path and the servlet path of the
     * original request. This is useful for building links to other resources within the application where a servlet
     * mapping of the style {@code "/main/*"} is used.
     * <p>
     ...
     * useful as HTML form action target, possibly in combination with the original query string.
     * <p>
     * Delegates to the UrlPathHelper for decoding.
     * 
     * @see #getQueryString
     ...
     * useful for building an HTML form action target in combination with the original request URI.
     * <p>
     * Delegates to the UrlPathHelper for decoding.
     * 
     * @see #getRequestUri
     ...
     * <p>
     * Note that theme messages are never HTML-escaped, as they typically denote theme-specific resource paths and not
     ...
     * 
     * @param path
     *            the bean and property path for which values and errors will be resolved (e.g. "person.age")
