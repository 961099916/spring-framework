...
class DefaultServerRequest implements ServerRequest {
    ...
    static Optional<ServerResponse> checkNotModified(HttpServletRequest servletRequest, @Nullable Instant lastModified,
        ...
        if (webRequest.checkNotModified(etag, lastModifiedTimestamp)) {
            return Optional.of(
                ServerResponse.status(response.status).headers(headers -> headers.addAll(response.headers)).build());
        } else {
    ...
    public Map<String, String> pathVariables() {
        Map<String, String> pathVariables =
            (Map<String, String>)servletRequest().getAttribute(RouterFunctions.URI_TEMPLATE_VARIABLES_ATTRIBUTE);
     ...
     * {@link ServletWebRequest#checkNotModified(String, long)}. Throws an {@code UnsupportedOperationException} for
     * other methods.
     */
