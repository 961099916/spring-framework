...
public class ResourceHandlerRegistration {
     ...
     * directory. Multiple locations may be specified as a comma-separated list, and the locations will be checked for a
     * given resource in the order specified.
     * <p>
     * For example, {{@code "/"}, {@code "classpath:/META-INF/public-web-resources/"}} allows resources to be served
     ...
     * <p>
     * For {@link org.springframework.core.io.UrlResource URL-based resources} (e.g. files, HTTP URLs, etc) this method
     * supports a special prefix to indicate the charset associated with the URL so that relative paths appended to it
     * can be encoded correctly, e.g. {@code [charset=Windows-31J]https://example.org/path}.
     * 
    ...
    /**
     * Configure a chain of resource resolvers and transformers to use. This can be useful, for example, to apply a
     * version strategy to resource URLs.
     * <p>
     * If this method is not invoked, by default only a simple {@link PathResourceResolver} is used in order to match
     * URL paths to resources under the configured locations.
     * 
     * @param cacheResources
     *            whether to cache the result of resource resolution; setting this to "true" is recommended for
     *            production (and "false" for development, especially when applying a version strategy)
     * @return the same {@link ResourceHandlerRegistration} instance, for chained method invocation
     * @since 4.1
     */
     ...
     *            production (and "false" for development, especially when applying a version strategy
     * @param cache
     *            the cache to use for storing resolved and transformed resources; by default a
     *            {@link org.springframework.cache.concurrent.ConcurrentMapCache} is used. Since Resources aren't
     *            serializable and can be dependent on the application host, one should not use a distributed cache but
     *            rather an in-memory cache.
     * @return the same {@link ResourceHandlerRegistration} instance, for chained method invocation
     * @since 4.1
     */
