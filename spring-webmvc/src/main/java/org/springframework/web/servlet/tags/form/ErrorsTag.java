...
/**
 * The {@code <errors>} tag renders field errors in an HTML 'span' tag. Displays errors for either an object or a
 * particular field.
 *
 * <p>
...
public class ErrorsTag extends AbstractHtmlElementBodyTag implements BodyTag {
    ...
    /**
     * The key under which this tag exposes error messages in the {@link PageContext#PAGE_SCOPE page context scope}.
     */
     ...
     * <p>
     * Appends '{@code .errors}' to the value returned by {@link #getPropertyPath()} or to the model attribute name if
     * the {@code <form:errors/>} tag's '{@code path}' attribute has been omitted.
     * 
    ...
    protected String autogenerateId() throws JspException {
        ...
        if (!StringUtils.hasLength(path) || "*".equals(path)) {
            path =
                (String)this.pageContext.getAttribute(FormTag.MODEL_ATTRIBUTE_VARIABLE_NAME, PageContext.REQUEST_SCOPE);
        }
     ...
     * <p>
     * Simply returns {@code null} because the '{@code name}' attribute is not a validate attribute for the
     * '{@code span}' element.
     */
    @Override
    @Nullable
    ...
    /**
     * Removes any bind status error messages that were previously stored under {@link #MESSAGES_ATTRIBUTE this key} in
     * the {@link PageContext#PAGE_SCOPE}.
     * 
