...
/**
 * Extends {@link AbstractMessageConverterMethodArgumentResolver} with the ability to handle method return values by
 * writing to the response with {@link HttpMessageConverter HttpMessageConverters}.
 *
...
public abstract class AbstractMessageConverterMethodProcessor extends AbstractMessageConverterMethodArgumentResolver
    ...
    /**
     * Constructor with list of converters and ContentNegotiationManager as well as request/response body advice
     * instances.
     */
    protected AbstractMessageConverterMethodProcessor(List<HttpMessageConverter<?>> converters,
     ...
     * 
     * @param value
     *            the value to write to the output message
     * @param returnType
     *            the type of the value
     * @param inputMessage
     *            the input messages. Used to inspect the {@code Accept} header.
     * @param outputMessage
     *            the output message to write to
     * @throws IOException
     *             thrown in case of I/O errors
     * @throws HttpMediaTypeNotAcceptableException
     *             thrown when the conditions indicated by the {@code Accept} header on the request cannot be met by the
     *             message converters
     * @throws HttpMessageNotWritableException
     *             thrown if a given message cannot be written by a converter, or if the content-type chosen by the
     *             server has no compatible converter.
     */
    ...
    /**
     * Return the type of the value to be written to the response. Typically this is a simple check via getClass on the
     * value but if the value is null, then the return type needs to be examined possibly including generic type
     * determination (e.g. {@code ResponseEntity<T>}).
     */
    ...
    /**
     * Check if the path has a file extension and whether the extension is either on the list of {@link #SAFE_EXTENSIONS
     * safe extensions} or explicitly {@link ContentNegotiationManager#getAllFileExtensions() registered}. If not, and
     * the status is in the 2xx range, a 'Content-Disposition' header with a safe attachment file name ("f.txt") is
