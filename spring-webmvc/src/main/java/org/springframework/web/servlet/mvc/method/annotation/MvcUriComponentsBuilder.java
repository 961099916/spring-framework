...
/**
 * Creates instances of {@link org.springframework.web.util.UriComponentsBuilder} by pointing to {@code @RequestMapping}
 * methods on Spring MVC controllers.
 *
 * <p>
 * There are several groups of methods:
 * <ul>
 * <li>Static {@code fromXxx(...)} methods to prepare links using information from the current request as determined by
 * a call to {@link org.springframework.web.servlet.support.ServletUriComponentsBuilder#fromCurrentServletMapping()}.
 * <li>Static {@code fromXxx(UriComponentsBuilder,...)} methods can be given a baseUrl when operating outside the
 * context of a request.
 * <li>Instance-based {@code withXxx(...)} methods where an instance of MvcUriComponentsBuilder is created with a
 * baseUrl via {@link #relativeTo(org.springframework.web.util.UriComponentsBuilder)}.
 * </ul>
 *
 * <p>
 * <strong>Note:</strong> This class uses values from "Forwarded" (<a href="https://tools.ietf.org/html/rfc7239">RFC
 * 7239</a>), "X-Forwarded-Host", "X-Forwarded-Port", and "X-Forwarded-Proto" headers, if present, in order to reflect
 * the client-originated protocol and address. Consider using the {@code ForwardedHeaderFilter} in order to choose from
 * a central place whether to extract and use, or to discard such headers. See the Spring Framework reference for more
 * on this filter.
 *
...
public class MvcUriComponentsBuilder {
    ...
    /**
     * Create a {@link UriComponentsBuilder} from the mapping of a controller class and current request information
     * including Servlet mapping. If the controller contains multiple mappings, only the first one is used.
     * <p>
     * <strong>Note:</strong> This method extracts values from "Forwarded" and "X-Forwarded-*" headers if found. See
     * class-level docs.
     * 
    ...
    /**
     * An alternative to {@link #fromController(Class)} that accepts a {@code UriComponentsBuilder} representing the
     * base URL. This is useful when using MvcUriComponentsBuilder outside the context of processing a request or to
     * apply a custom baseUrl not matching the current request.
     * <p>
     * <strong>Note:</strong> This method extracts values from "Forwarded" and "X-Forwarded-*" headers if found. See
     * class-level docs.
     * 
     * @param builder
     *            the builder for the base URL; the builder will be cloned and therefore not modified and may be re-used
     *            for further calls.
     * @param controllerType
     *            the controller to build a URI for
     * @return a UriComponentsBuilder instance (never {@code null})
    ...
    /**
     * Create a {@link UriComponentsBuilder} from the mapping of a controller method and an array of method argument
     * values. This method delegates to {@link #fromMethod(Class, Method, Object...)}.
     * <p>
     * <strong>Note:</strong> This method extracts values from "Forwarded" and "X-Forwarded-*" headers if found. See
     * class-level docs.
     * 
     ...
     * representing the base URL. This is useful when using MvcUriComponentsBuilder outside the context of processing a
    ...
    /**
     * Create a {@link UriComponentsBuilder} from the mapping of a controller method and an array of method argument
     * values. The array of values must match the signature of the controller method. Values for {@code @RequestParam}
     * and {@code @PathVariable} are used for building the URI (via implementations of
     * {@link org.springframework.web.method.support.UriComponentsContributor UriComponentsContributor}) while remaining
     * argument values are ignored and can be {@code null}.
     * <p>
     * <strong>Note:</strong> This method extracts values from "Forwarded" and "X-Forwarded-*" headers if found. See
     * class-level docs.
     * 
    ...
    /**
     * Create a {@link UriComponentsBuilder} by invoking a "mock" controller method. The controller method and the
     * supplied argument values are then used to delegate to {@link #fromMethod(Class, Method, Object...)}.
     * <p>
     ...
     * 
     * @param info
     *            either the value returned from a "mock" controller invocation or the "mock" controller itself after an
     *            invocation
    ...
    /**
     * An alternative to {@link #fromMethodCall(Object)} that accepts a {@code UriComponentsBuilder} representing the
     * base URL. This is useful when using MvcUriComponentsBuilder outside the context of processing a request or to
     * apply a custom baseUrl not matching the current request.
     * <p>
     * <strong>Note:</strong> This method extracts values from "Forwarded" and "X-Forwarded-*" headers if found. See
    ...
    /**
     * Return a "mock" controller instance. When an {@code @RequestMapping} method on the controller is invoked, the
     * supplied argument values are remembered and the result can then be used to create a {@code UriComponentsBuilder}
     * via {@link #fromMethodCall(Object)}.
     * <p>
     * Note that this is a shorthand version of {@link #controller(Class)} intended for inline use (with a static
     * import), for example:
     * 
     * <pre class="code">
    ...
    /**
     * Return a "mock" controller instance. When an {@code @RequestMapping} method on the controller is invoked, the
     * supplied argument values are remembered and the result can then be used to create {@code UriComponentsBuilder}
     * via {@link #fromMethodCall(Object)}.
     * <p>
     * This is a longer version of {@link #on(Class)}. It is needed with controller methods returning void as well for
     ...
     * <p>
     * The configured {@link org.springframework.web.servlet.handler.HandlerMethodMappingNamingStrategy
     * HandlerMethodMappingNamingStrategy} determines the names of controller method request mappings at startup. By
     * default all mappings are assigned a name based on the capital letters of the class name, followed by "#" as
     * separator, and then the method name. For example "PC#getPerson" for a class named PersonController with method
     * getPerson. In case the naming convention does not produce unique results, an explicit name may be assigned
     * through the name attribute of the {@code @RequestMapping} annotation.
     * <p>
     * This is aimed primarily for use in view rendering technologies and EL expressions. The Spring URL tag library
     * registers this method as a function called "mvcUrl".
     * <p>
     * For example, given this controller:
     * 
     * <pre class="code">
     ...
     * @throws IllegalArgumentException
     *             if the mapping name is not found or if there is no unique match
     * @since 4.1
     */
    ...
    /**
     * An alternative to {@link #fromMappingName(String)} that accepts a {@code UriComponentsBuilder} representing the
     * base URL. This is useful when using MvcUriComponentsBuilder outside the context of processing a request or to
     * apply a custom baseUrl not matching the current request.
     * <p>
     * <strong>Note:</strong> This method extracts values from "Forwarded" and "X-Forwarded-*" headers if found. See
    ...
    private static Method getMethod(Class<?> controllerType, final String methodName, final Object... args) {
        ...
        if (methods.size() == 1) {
            ...
            throw new IllegalArgumentException(
                String.format("Found two methods named '%s' accepting arguments %s in controller %s: [%s]", methodName,
                    Arrays.asList(args), controllerType.getName(), methods));
        } else {
    ...
    private static UriComponentsBuilder applyContributors(UriComponentsBuilder builder, Method method, Object... args) {
        ...
        if (paramCount != argCount) {
            throw new IllegalArgumentException(
                "Number of method parameters " + paramCount + " does not match number of argument values " + argCount);
        }
    ...
    /**
     * An alternative to {@link #fromController(Class)} for use with an instance of this class created via a call to
     * {@link #relativeTo}.
     * <p>
     * <strong>Note:</strong> This method extracts values from "Forwarded" and "X-Forwarded-*" headers if found. See
     * class-level docs.
    ...
    /**
     * An alternative to {@link #fromMethodName(Class, String, Object...)}} for use with an instance of this class
    ...
    /**
     * An alternative to {@link #fromMethodCall(Object)} for use with an instance of this class created via
     * {@link #relativeTo}.
     * <p>
     * <strong>Note:</strong> This method extracts values from "Forwarded" and "X-Forwarded-*" headers if found. See
     * class-level docs.
    ...
    /**
     * An alternative to {@link #fromMappingName(String)} for use with an instance of this class created via
     * {@link #relativeTo}.
     * <p>
     * <strong>Note:</strong> This method extracts values from "Forwarded" and "X-Forwarded-*" headers if found. See
     * class-level docs.
    ...
    /**
     * An alternative to {@link #fromMethod(Class, Method, Object...)} for use with an instance of this class created
     * via {@link #relativeTo}.
     * <p>
     * <strong>Note:</strong> This method extracts values from "Forwarded" and "X-Forwarded-*" headers if found. See
     * class-level docs.
    ...
    public static class MethodArgumentBuilder {
        ...
        /**
         * Use this method only if you need to apply strong encoding to expanded URI variables by quoting all characters
