...
public class SimpleMappingExceptionResolver extends AbstractHandlerExceptionResolver {
    ...
    /**
     * Set one or more exceptions to be excluded from the exception mappings. Excluded exceptions are checked first and
     * if one of them equals the actual exception, the exception will remain unresolved.
     * 
    ...
    /**
     * Set the HTTP status code that this exception resolver will apply for a given resolved error view. Keys are view
     * names; values are status codes.
     * <p>
     * Note that this error code will only get applied in case of a top-level request. It will not be set for an include
    ...
    /**
     * Returns the HTTP status codes provided via {@link #setStatusCodes(Properties)}. Keys are view names; values are
     * status codes.
     */
    ...
    /**
     * Set the default HTTP status code that this exception resolver will apply if it resolves an error view and if
     * there is no status code mapping defined.
     * <p>
     * Note that this error code will only get applied in case of a top-level request. It will not be set for an include
     * request, since the HTTP status cannot be modified from within an include.
     * <p>
     * If not specified, no status code will be applied, either leaving this to the controller or view, or keeping the
     * servlet engine's default of 200 (OK).
     * 
     * @param defaultStatusCode
     *            the HTTP status code value, for example 500 ({@link HttpServletResponse#SC_INTERNAL_SERVER_ERROR}) or
     *            404 ({@link HttpServletResponse#SC_NOT_FOUND})
     ...
     * <p>
     * This can be either set to a different attribute name or to {@code null} for not exposing an exception attribute
     * at all.
     * 
     ...
     * {@link #setExcludedExceptions(Class[]) "excludedExecptions"}, then searching the {@link #setExceptionMappings
     * "exceptionMappings"}, and finally using the {@link #setDefaultErrorView "defaultErrorView"} as a fallback.
     * 
     * @param ex
     *            the exception that got thrown during handler execution
     * @param request
     ...
     * {@link #setStatusCodes(Properties) statusCodes} property), or falls back to the {@link #setDefaultStatusCode
     ...
     * @return the HTTP status code to use, or {@code null} for the servlet container's default (200 in case of a
     *         standard error view)
    ...
    /**
     * Apply the specified HTTP status code to the given response, if possible (that is, if not executing within an
     * include request).
     * 
     * @param request
     *            current HTTP request
