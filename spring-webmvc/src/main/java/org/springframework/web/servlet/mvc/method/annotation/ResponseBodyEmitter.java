...
public class ResponseBodyEmitter {
    ...
    /**
     * After an I/O error, we don't call {@link #completeWithError} directly but wait for the Servlet container to call
     * us via {@code AsyncListener#onError} on a container thread at which point we call completeWithError. This flag is
     * used to ignore further calls to complete or completeWithError that may come for example from an application
     * try-catch block on the thread of the I/O error.
     */
    ...
    /**
     * Invoked after the response is updated with the status code and headers, if the ResponseBodyEmitter is wrapped in
     * a ResponseEntity, but before the response is committed, i.e. before the response body has been written to.
     * <p>
     ...
     * <p>
     * If any exception occurs a dispatch is made back to the app server where Spring MVC will pass the exception
     * through its exception handling mechanism.
     * <p>
     * <strong>Note:</strong> if the send fails with an IOException, you do not need to call
     * {@link #completeWithError(Throwable)} in order to clean up. Instead the Servlet container creates a notification
     * that results in a dispatch where Spring MVC invokes exception resolvers and completes processing.
     * 
     * @param object
     *            the object to write
     * @throws IOException
    ...
    public synchronized void send(Object object, @Nullable MediaType mediaType) throws IOException {
        Assert.state(!this.complete,
            "ResponseBodyEmitter has already completed" + (this.failure != null ? " with error: " + this.failure : ""));
    ...
    /**
     * Complete request processing by performing a dispatch into the servlet container, where Spring MVC is invoked once
     * more, and completes the request processing lifecycle.
     * <p>
     * <strong>Note:</strong> this method should be called by the application to complete request processing. It should
     * not be used after container related events such as an error while {@link #send(Object) sending}.
     */
     ...
     * mechanism. Note however that at this stage of request processing, the response is committed and the response
     * status can no longer be changed.
     * <p>
     * <strong>Note:</strong> this method should be called by the application to complete request processing with an
     * error. It should not be used after container related events such as an error while {@link #send(Object) sending}.
     */
    ...
    /**
     * Register code to invoke when the async request times out. This method is called from a container thread when an
    ...
    /**
     * Register code to invoke for an error during async request processing. This method is called from a container
     * thread when an error occurred while processing an async request.
     * 
     * @since 5.0
     */
    ...
    /**
     * Register code to invoke when the async request completes. This method is called from a container thread when an
     * async request completed for any reason including timeout and network error. This method is useful for detecting
    ...
    /**
     * Contract to handle the sending of event data, the completion of event sending, and the registration of callbacks
     * to be invoked in case of timeout, error, and completion for any reason (including from the container side).
     */
    interface Handler {
