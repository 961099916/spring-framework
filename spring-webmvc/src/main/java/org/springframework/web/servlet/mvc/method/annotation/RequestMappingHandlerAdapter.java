...
/**
 * Extension of {@link AbstractHandlerMethodAdapter} that supports {@link RequestMapping @RequestMapping} annotated
 * {@link HandlerMethod HandlerMethods}.
 *
 * <p>
 * Support for custom argument and return value types can be added via {@link #setCustomArgumentResolvers} and
 * {@link #setCustomReturnValueHandlers}, or alternatively, to re-configure all argument and return value types, use
...
public class RequestMappingHandlerAdapter extends AbstractHandlerMethodAdapter
    ...
    public static final MethodFilter MODEL_ATTRIBUTE_METHODS =
        method -> (!AnnotatedElementUtils.hasAnnotation(method, RequestMapping.class)
     ...
     * <p>
     * <strong>Note:</strong> This method is available for backwards compatibility only. However, it is recommended to
     * re-write a {@code ModelAndViewResolver} as {@link HandlerMethodReturnValueHandler}. An adapter between the two
     ...
     * <p>
     * A {@code HandlerMethodReturnValueHandler} provides better access to the return type and controller method
    ...
    /**
     * Add one or more {@code RequestBodyAdvice} instances to intercept the request before it is read and converted for
     * {@code @RequestBody} and {@code HttpEntity} method arguments.
     */
    ...
    /**
     * Add one or more {@code ResponseBodyAdvice} instances to intercept the response before {@code @ResponseBody} or
     * {@code ResponseEntity} return values are written to the response body.
     */
     ...
     * <p>
     * By default a {@link SimpleAsyncTaskExecutor} instance is used. It's recommended to change that default in
    ...
    /**
     * Specify the amount of time, in milliseconds, before concurrent handling should time out. In Servlet 3, the
     * timeout begins after the main request processing thread has exited and ends when the request is dispatched again
     ...
     * a controller method can declare a {@link RedirectAttributes} argument and use it to provide attributes for a
     * redirect.
     * <p>
     ...
     * {@link org.springframework.web.bind.support.DefaultSessionAttributeStore}, storing session attributes in the
     * HttpSession with the same attribute name as in the model.
     */
     ...
     * <li>1 or higher: "Cache-Control: max-age=seconds" will ask to cache content; not advised when dealing with
     * session attributes</li>
     * </ul>
     * <p>
     * In contrast to the "cacheSeconds" property which will apply to all general handlers (but not to
     * {@code @SessionAttributes} annotated handlers), this setting will apply to {@code @SessionAttributes} handlers
     * only.
     * 
     * @see #setCacheSeconds
    ...
    /**
     * This implementation always returns -1. An {@code @RequestMapping} method can calculate the lastModified value,
     * call {@link WebRequest#checkNotModified(long)}, and return {@code null} if the result of that call is
     * {@code true}.
     */
    @Override
     ...
     * 
     * @param handlerMethod
     *            the {@link HandlerMethod} definition
     ...
     * <p>
     * The default implementation creates a ServletRequestDataBinderFactory. This can be overridden for custom
