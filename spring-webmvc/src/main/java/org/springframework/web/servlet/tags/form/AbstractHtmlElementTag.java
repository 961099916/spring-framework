...
/**
 * Base class for databinding-aware JSP tags that render HTML element. Provides a set of properties corresponding to the
 * set of HTML attributes that are common across elements.
 *
 * <p>
 * Additionally, this base class allows for rendering non-standard attributes as part of the tag's output. These
 * attributes are accessible to subclasses if needed via the {@link AbstractHtmlElementTag#getDynamicAttributes()
...
public abstract class AbstractHtmlElementTag extends AbstractDataBoundFormElementTag implements DynamicAttributes {
    ...
    /**
     * Writes the default attributes configured via this base class to the supplied {@link TagWriter}. Subclasses should
     * call this when they want the base attribute set to be written to the output.
     */
    @Override
    ...
    /**
     * Writes the optional attributes configured via this base class to the supplied {@link TagWriter}. The set of
     * optional attributes that will be rendered includes any non-standard dynamic attributes. Called by
