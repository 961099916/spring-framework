 ...
 *
 * <p>
 * Supports mapped {@linkplain #setMappedHandlers handlers} and {@linkplain #setMappedHandlerClasses handler classes}
 * that the resolver should be applied to and implements the {@link Ordered} interface.
 *
...
public abstract class AbstractHandlerExceptionResolver implements HandlerExceptionResolver, Ordered {
     ...
     * <p>
     * If no handlers or handler classes are set, the exception mappings and the default error view will apply to all
     ...
     * <p>
     * The exception mappings and the default error view will only apply to handlers of the specified types; the
     ...
     * Commons Logging, getting interpreted as a log category according to the logger's configuration. If {@code null}
     * or empty String is passed, warn logging is turned off.
     * <p>
     * By default there is no warn logging although subclasses like
     * {@link org.springframework.web.servlet.mvc.support.DefaultHandlerExceptionResolver} can change that default.
     * Specify this setting to activate warn logging into a specific category. Alternatively, override the
     * {@link #logException} method for custom logging.
     * 
     ...
     * <p>
     * Default is {@code false}. Switch this to {@code true} in order to automatically generate HTTP response headers
     * that suppress response caching.
     */
    ...
    /**
     * Check whether this resolver is supposed to apply (i.e. if the supplied handler matches any of the configured
     * {@linkplain #setMappedHandlers handlers} or {@linkplain #setMappedHandlerClasses handler classes}), and then
     * delegate to the {@link #doResolveException} template method.
     */
    @Override
    @Nullable
     ...
     * <p>
     * The default implementation checks against the configured {@linkplain #setMappedHandlers handlers} and
     ...
     * <p>
     * The default implementation prevents the response from being cached, if the {@link #setPreventResponseCaching
     * "preventResponseCaching"} property has been set to "true".
     * 
     * @param ex
     *            the exception that got thrown during handler execution
     * @param response
    ...
    /**
     * Prevents the response from being cached, through setting corresponding HTTP {@code Cache-Control: no-store}
     * header.
     * 
     * @param response
     *            current HTTP response
    ...
    /**
     * Actually resolve the given exception that got thrown during handler execution, returning a {@link ModelAndView}
