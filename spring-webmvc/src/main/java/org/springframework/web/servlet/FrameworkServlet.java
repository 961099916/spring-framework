...
/**
 * Base servlet for Spring's web framework. Provides integration with a Spring application context, in a JavaBean-based
 * overall solution.
 *
 * <p>
 * This class offers the following functionality:
 * <ul>
 * <li>Manages a {@link org.springframework.web.context.WebApplicationContext WebApplicationContext} instance per
 * servlet. The servlet's configuration is determined by beans in the servlet's namespace.
 * <li>Publishes events on request processing, whether or not a request is successfully handled.
 * </ul>
 *
 * <p>
 * Subclasses must implement {@link #doService} to handle requests. Because this extends {@link HttpServletBean} rather
 * than HttpServlet directly, bean properties are automatically mapped onto it. Subclasses can override
 * {@link #initFrameworkServlet()} for custom initialization.
 *
 * <p>
 * Detects a "contextClass" parameter at the servlet init-param level, falling back to the default context class,
 * {@link org.springframework.web.context.support.XmlWebApplicationContext XmlWebApplicationContext}, if not found. Note
 * that, with the default {@code FrameworkServlet}, a custom context class needs to implement the
 * {@link org.springframework.web.context.ConfigurableWebApplicationContext ConfigurableWebApplicationContext} SPI.
 *
 * <p>
 * Accepts an optional "contextInitializerClasses" servlet init-param that specifies one or more
 * {@link org.springframework.context.ApplicationContextInitializer ApplicationContextInitializer} classes. The managed
 * web application context will be delegated to these initializers, allowing for additional programmatic configuration,
 * e.g. adding property sources or activating profiles against the
 * {@linkplain org.springframework.context.ConfigurableApplicationContext#getEnvironment() context's environment}. See
 * also {@link org.springframework.web.context.ContextLoader} which supports a "contextInitializerClasses" context-param
 * with identical semantics for the "root" web application context.
 *
 * <p>
 * Passes a "contextConfigLocation" servlet init-param to the context instance, parsing it into potentially multiple
 * file paths which can be separated by any number of commas and spaces, like "test-servlet.xml, myServlet.xml". If not
 ...
 * files, at least when using Spring's default ApplicationContext implementation. This can be leveraged to deliberately
 * override certain bean definitions via an extra XML file.
 *
 * <p>
 * The default namespace is "'servlet-name'-servlet", e.g. "test-servlet" for a servlet-name "test" (leading to a
 * "/WEB-INF/test-servlet.xml" default location with XmlWebApplicationContext). The namespace can also be set explicitly
 * via the "namespace" servlet init-param.
 *
 * <p>
 * As of Spring 3.1, {@code FrameworkServlet} may now be injected with a web application context, rather than creating
 * its own internally. This is useful in Servlet 3.0+ environments, which support programmatic registration of servlet
 * instances. See {@link #FrameworkServlet(WebApplicationContext)} Javadoc for details.
 *
 * @author Rod Johnson
...
public abstract class FrameworkServlet extends HttpServletBean implements ApplicationContextAware {
    ...
    /**
     * Suffix for WebApplicationContext namespaces. If a servlet of this class is given the name "test" in a context,
     * the namespace used by the servlet will resolve to "test-servlet".
     */
    ...
    /**
     * Create a new {@code FrameworkServlet} that will create its own internal web application context based on defaults
     * and values provided through servlet init-params. Typically used in Servlet 2.5 or earlier environments, where the
     * only option for servlet registration is through {@code web.xml} which requires the use of a no-arg constructor.
     * <p>
     * Calling {@link #setContextConfigLocation} (init-param 'contextConfigLocation') will dictate which XML files will
     * be loaded by the {@linkplain #DEFAULT_CONTEXT_CLASS default XmlWebApplicationContext}
     * <p>
     * Calling {@link #setContextClass} (init-param 'contextClass') overrides the default
     * {@code XmlWebApplicationContext} and allows for specifying an alternative class, such as
     * {@code AnnotationConfigWebApplicationContext}.
     * <p>
     * Calling {@link #setContextInitializerClasses} (init-param 'contextInitializerClasses') indicates which
     * {@link ApplicationContextInitializer} classes should be used to further configure the internal application
     * context prior to refresh().
     * 
    ...
    /**
     * Create a new {@code FrameworkServlet} with the given web application context. This constructor is useful in
     * Servlet 3.0+ environments where instance-based registration of servlets is possible through the
     ...
     * </ul>
     * <p>
     * The given web application context may or may not yet be {@linkplain ConfigurableApplicationContext#refresh()
     * refreshed}. If it (a) is an implementation of {@link ConfigurableWebApplicationContext} and (b) has
     * <strong>not</strong> already been refreshed (the recommended approach), then the following will occur:
     * <ul>
     * <li>If the given context does not already have a {@linkplain ConfigurableApplicationContext#setParent parent},
     ...
     * "contextInitializerClasses" init-param or through the {@link #setContextInitializers} property will be
     * applied.</li>
     ...
     * "test-servlet" for a servlet named "test".
     */
    ...
    /**
     * Specify which {@link ApplicationContextInitializer} instances should be used to initialize the application
     * context used by this {@code FrameworkServlet}.
     * 
     * @see #configureAndRefreshWebApplicationContext
    ...
    /**
     * Specify the set of fully-qualified {@link ApplicationContextInitializer} class names, per the optional
    ...
    /**
     * Set whether to publish this servlet's context as a ServletContext attribute, available to all objects in the web
    ...
    /**
     * Set whether this servlet should publish a ServletRequestHandledEvent at the end of each request. Default is
     * "true"; can be turned off for a slight performance improvement, provided that no ApplicationListeners rely on
     * such events.
     * 
     ...
     * <p>
     * Default in the {@code FrameworkServlet} is "false", applying {@link javax.servlet.http.HttpServlet}'s default
     * behavior (i.e.enumerating all standard HTTP request methods as a response to the OPTIONS request). Note however
     * that as of 4.3 the {@code DispatcherServlet} sets this property to "true" by default due to its built-in support
     * for OPTIONS.
     * <p>
     ...
     * <p>
     * Note that HttpServlet's default OPTIONS processing will be applied in any case if your controllers happen to not
     * set the 'Allow' header (as required for an OPTIONS response).
     */
     ...
     * <p>
     * Note that HttpServlet's default TRACE processing will be applied in any case if your controllers happen to not
     * generate a response of content type 'message/http' (as required for a TRACE response).
     */
    ...
    /**
     * Called by Spring via {@link ApplicationContextAware} to inject the current application context. This method
     * allows FrameworkServlets to be registered as Spring beans inside an existing {@link WebApplicationContext} rather
     * than {@link #findWebApplicationContext() finding} a {@link org.springframework.web.context.ContextLoaderListener
    ...
    /**
     * Overridden method of {@link HttpServletBean}, invoked after any bean properties have been set. Creates this
     ...
     * {@link #setContextAttribute configured name}. The {@code WebApplicationContext} must have already been loaded and
     * stored in the {@code ServletContext} before this servlet gets initialized (or invoked).
     * <p>
     ...
     * 
     * @param parent
     *            the parent ApplicationContext to use, or {@code null} if none
     ...
     * Instantiate the WebApplicationContext for this servlet, either a default
     * {@link org.springframework.web.context.support.XmlWebApplicationContext} or a {@link #setContextClass custom
     * context class}, if set. Delegates to #createWebApplicationContext(ApplicationContext).
     * 
     * @param parent
     *            the parent WebApplicationContext to use, or {@code null} if none
     * @return the WebApplicationContext for this servlet
     * @see org.springframework.web.context.support.XmlWebApplicationContext
     ...
     * <p>
     * The default implementation is empty. {@code refresh()} will be called automatically after this method returns.
     * <p>
     * Note that this method is designed to allow subclasses to modify the application context, while
     * {@link #initWebApplicationContext} is designed to allow end-users to modify the context through the use of
    ...
    /**
     * Delegate the WebApplicationContext before it is refreshed to any {@link ApplicationContextInitializer} instances
     * specified by the "contextInitializerClasses" servlet init-param.
     * <p>
     * See also {@link #postProcessWebApplicationContext}, which is designed to allow subclasses (as opposed to
     * end-users) to modify the application context, and is called immediately before this method.
     * 
     * @param wac
     *            the configured WebApplicationContext (not refreshed yet)
     * @see #createWebApplicationContext
    ...
    /**
     * This method will be invoked after any bean properties have been set and the WebApplicationContext has been
     * loaded. The default implementation is empty; subclasses may override this method to perform any initialization
     * they require.
     * 
    ...
    /**
     * Template method which can be overridden to add servlet-specific refresh work. Called after successful context
     * refresh.
     * <p>
     ...
     * <p>
     * Applies HttpServlet's standard OPTIONS processing otherwise, and also if there is still no 'Allow' header set
     * after dispatching.
     * 
     * @see #doService
     */
     ...
     * <p>
     * The actual event handling is performed by the abstract {@link #doService} template method.
     */
     ...
     *            pre-bound RequestAttributes instance, if any
     * @return the ServletRequestAttributes to bind, or {@code null} to preserve the previously bound instance (or not
     *         binding any, if none bound before)
    ...
    /**
     * Subclasses must implement this method to do the work of request handling, receiving a centralized callback for
     * GET, POST, PUT and DELETE.
     * <p>
     * The contract is essentially the same as that for the commonly overridden {@code doGet} or {@code doPost} methods
     * of HttpServlet.
     * <p>
    ...
    /**
     * ApplicationListener endpoint that receives events from this servlet's WebApplicationContext only, delegating to
     * {@code onApplicationEvent} on the FrameworkServlet instance.
     */
     ...
     * LocaleContextHolder and RequestContextHolder.
     */
