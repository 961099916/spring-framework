 ...
 *
 * <p>
 * The XSLT Source object is supplied as a parameter in the model and then {@link #locateSource detected} during
 * response rendering. Users can either specify a specific entry in the model via the {@link #setSourceKey sourceKey}
 * property or have Spring locate the Source object. This class also provides basic conversion of objects into Source
 * implementations. See {@link #getSourceTypes() here} for more details.
 *
 * <p>
 * All model parameters are passed to the XSLT Transformer as parameters. In addition the user can configure
 * {@link #setOutputProperties output properties} to be passed to the Transformer.
 *
 * @author Rob Harrop
...
public class XsltView extends AbstractUrlBasedView {
    ...
    /**
     * Set the name of the model attribute that represents the XSLT Source. If not specified, the model map will be
     * searched for a matching value type.
     * <p>
     * The following source types are supported out of the box: {@link Source}, {@link Document}, {@link Node},
    ...
    /**
     * Set an implementation of the {@link javax.xml.transform.ErrorListener} interface for custom handling of
     * transformation errors and warnings.
     * <p>
     * If not set, a default {@link org.springframework.util.xml.SimpleTransformErrorListener} is used that simply logs
     * warnings using the logger instance of the view class, and rethrows errors to discontinue the XML transformation.
     * 
     ...
     * <p>
     * Default is {@code true} (on); set this to {@code false} (off) to not specify an "indent" key, leaving the choice
     ...
     * <p>
     * The default value is "true". Only set this to "false" in development, where caching does not seriously impact
     * performance.
     */
     ...
     * <p>
     * The default implementation simply calls {@link javax.xml.transform.TransformerFactory#newInstance()}. If a
    ...
    protected TransformerFactory
        newTransformerFactory(@Nullable Class<? extends TransformerFactory> transformerFactoryClass) {
     ...
     * <p>
     * The default implementation creates a {@link StreamResult} wrapping the supplied HttpServletResponse's
    ...
    /**
     * <p>
     * Locate the {@link Source} object in the supplied model, converting objects as required. The default
     * implementation first attempts to look under the configured {@link #setSourceKey source key}, if any, before
     * attempting to locate an object of {@link #getSourceTypes() supported type}.
     * 
     * @param model
     *            the merged model Map
     * @return the XSLT Source object (or {@code null} if none found)
     * @throws Exception
     *             if an error occurred during locating the source
     * @see #setSourceKey
     * @see #convertSource
     */
    @Nullable
    ...
    /**
     * Return the array of {@link Class Classes} that are supported when converting to an XSLT {@link Source}.
     * <p>
     * Currently supports {@link Source}, {@link Document}, {@link Node}, {@link Reader}, {@link InputStream} and
     * {@link Resource}.
     * 
     ...
     * {@link Transformer#setParameter parameter set}. This implementation also copies the {@link #setOutputProperties
     * output properties} into the {@link Transformer} {@link Transformer#setOutputProperty output properties}.
     * Indentation properties are set as well.
     * 
     * @param model
     *            merged output Map (never {@code null})
     * @param response
     *            current HTTP response
     * @param transformer
    ...
    /**
     * Copy the configured output {@link Properties}, if any, into the {@link Transformer#setOutputProperty output
     * property set} of the supplied {@link Transformer}.
     * 
     * @param transformer
     *            the target transformer
     */
    ...
    /**
     * Copy all entries from the supplied Map into the {@link Transformer#setParameter(String, Object) parameter set} of
     * the supplied {@link Transformer}.
     * 
     * @param model
     *            merged output Map (never {@code null})
     * @param transformer
     ...
     * <p>
     * The default implementation of this method sets the {@link HttpServletResponse#setContentType content type} and
     * {@link HttpServletResponse#setCharacterEncoding encoding} from the "media-type" and "encoding" output properties
     ...
     * <p>
     * The default implementation simply calls {@link Templates#newTransformer()}, and configures the
     * {@link Transformer} with the custom {@link URIResolver} if specified.
     * 
     * @param templates
