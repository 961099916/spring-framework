 ...
 * like RMI, but with the JMS provider as communication infrastructure.
 *
 * <p>
 * To be configured with a {@link javax.jms.QueueConnectionFactory} and a target queue (either as
...
public class JmsInvokerClientInterceptor implements MethodInterceptor, InitializingBean {
     ...
     * <p>
     * The default resolver is a {@code DynamicDestinationResolver}. Specify a {@code JndiDestinationResolver} for
     ...
     * {@link org.springframework.remoting.support.RemoteInvocation} objects into request messages, as well as response
     * messages into {@link org.springframework.remoting.support.RemoteInvocationResult} objects.
     * <p>
     * Default is a {@link SimpleMessageConverter}, using a standard JMS {@link javax.jms.ObjectMessage} for each
     * invocation / invocation result object.
     * <p>
     * Custom implementations may generally adapt {@link java.io.Serializable} objects into special kinds of messages,
     * or might be specifically tailored for translating {@code RemoteInvocation(Result)s} into specific kinds of
     * messages.
     */
     ...
     * <p>
     * Can be overridden in subclasses to provide custom {@code RemoteInvocation} subclasses, containing additional
     * invocation parameters like user credentials. Note that it is preferable to use a custom
     * {@code RemoteInvocationFactory} which is a reusable strategy.
     * 
    ...
    /**
     * Execute the given remote invocation, sending an invoker request message to this accessor's target queue and
     * waiting for a corresponding response.
     * 
     * @param invocation
     ...
     * <p>
     * The default implementation creates a JMS {@link javax.jms.ObjectMessage} for the given RemoteInvocation object.
     * 
     * @param session
     *            the current JMS Session
     * @param invocation
    ...
    /**
     * Actually execute the given request, sending the invoker request message to the specified target queue and waiting
     * for a corresponding response.
     * <p>
     * The default implementation is based on standard JMS send/receive, using a {@link javax.jms.TemporaryQueue} for
     * receiving the response.
     * 
     * @param session
     *            the JMS Session to use
     * @param queue
     *            the resolved target Queue to send to
     ...
     * {@link RemoteInvocationResult} object. If an invalid response message is encountered, the
     * {@code onInvalidResponse} callback gets invoked.
     * 
    ...
    /**
     * Callback that is invoked by {@link #executeRequest} when the receive timeout has expired for the specified
     * {@link RemoteInvocation}.
     * <p>
     * By default, an {@link RemoteTimeoutException} is thrown. Sub-classes can choose to either throw a more dedicated
     * exception or even return a default {@link RemoteInvocationResult} as a fallback.
     * 
     * @param invocation
     *            the invocation
    ...
    /**
     * Callback that is invoked by {@link #extractInvocationResult} when it encounters an invalid response message.
     * <p>
