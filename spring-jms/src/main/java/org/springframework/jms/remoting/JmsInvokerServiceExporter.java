...
/**
 * JMS message listener that exports the specified service bean as a JMS service endpoint, accessible via a JMS invoker
 * proxy.
 *
 * <p>
 * Note that this class implements Spring's {@link org.springframework.jms.listener.SessionAwareMessageListener}
 * interface, since it requires access to the active JMS Session. Hence, this class can only be used with message
 * listener containers which support the SessionAwareMessageListener interface (e.g. Spring's
...
public class JmsInvokerServiceExporter extends RemoteInvocationBasedExporter
     ...
     * {@link org.springframework.remoting.support.RemoteInvocationResult} objects into response messages.
     * <p>
     * Default is a {@link org.springframework.jms.support.converter.SimpleMessageConverter}, using a standard JMS
     * {@link javax.jms.ObjectMessage} for each invocation / invocation result object.
     * <p>
     * Custom implementations may generally adapt Serializables into special kinds of messages, or might be specifically
     * tailored for translating RemoteInvocation(Result)s into specific kinds of messages.
     */
     ...
     * <p>
     * Switch this flag to "false" to throw an exception back to the listener container. This will typically lead to
     * redelivery of the message, which is usually undesirable - since the message content will be the same (that is,
     * still invalid).
     */
     ...
     * @return the RemoteInvocation object (or {@code null} in case of an invalid message that will simply be ignored)
     ...
     * <p>
     * The default implementation creates a JMS ObjectMessage for the given RemoteInvocationResult object. It sets the
     * response's correlation id to the request message's correlation id, if any; otherwise to the request message id.
     * 
     * @param request
     ...
     * @throws javax.jms.JMSException
     *             if creating the message failed
     */
    ...
    /**
     * Callback that is invoked by {@link #readRemoteInvocation} when it encounters an invalid request message.
     * <p>
     * The default implementation either discards the invalid message or throws a MessageFormatException - according to
     * the "ignoreInvalidRequests" flag, which is set to "true" (that is, discard invalid messages) by default.
     * 
     * @param requestMessage
     *            the invalid request message
     * @return the RemoteInvocation to expose for the invalid request (typically {@code null} in case of an invalid
     *         message that will simply be ignored)
     * @throws javax.jms.JMSException
