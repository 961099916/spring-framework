 ...
 *
 * <p>
 * If you want to use dynamic destination creation, you must specify the type of JMS destination to create, using the
 * "pubSubDomain" property. For other operations, this is not necessary. Point-to-Point (Queues) is the default domain.
 *
 * <p>
 * Default settings for JMS Sessions are "not transacted" and "auto-acknowledge". As defined by the Java EE
 * specification, the transaction and acknowledgement parameters are ignored when a JMS Session is created inside an
 * active transaction, no matter if a JTA transaction or a Spring-managed transaction. To configure them for native JMS
 * usage, specify appropriate values for the "sessionTransacted" and "sessionAcknowledgeMode" bean properties.
 *
 * <p>
 * This template uses a {@link org.springframework.jms.support.destination.DynamicDestinationResolver} and a
 * {@link org.springframework.jms.support.converter.SimpleMessageConverter} as default strategies for resolving a
 * destination name or converting a message, respectively. These defaults can be overridden through the
 * "destinationResolver" and "messageConverter" bean properties.
 *
 * <p>
 * <b>NOTE: The {@code ConnectionFactory} used with this template should return pooled Connections (or a single shared
 * Connection) as well as pooled Sessions and MessageProducers. Otherwise, performance of ad-hoc JMS operations is going
 * to suffer.</b> The simplest option is to use the Spring-provided
 * {@link org.springframework.jms.connection.SingleConnectionFactory} as a decorator for your target
 * {@code ConnectionFactory}, reusing a single JMS Connection in a thread-safe fashion; this is often good enough for
 * the purpose of sending messages via this template. In a Java EE environment, make sure that the
 * {@code ConnectionFactory} is obtained from the application's environment naming context via JNDI; application servers
...
public class JmsTemplate extends JmsDestinationAccessor implements JmsOperations {
     ...
     * JmsTemplate via a BeanFactory, typically setting the ConnectionFactory via setConnectionFactory.
     * 
     * @see #setConnectionFactory
     */
    public JmsTemplate() {
    ...
    /**
     * Initialize the default implementations for the template's strategies: DynamicDestinationResolver and
    ...
    /**
     * Set the destination name to be used on send/receive operations that do not have a destination parameter. The
     * specified name will be dynamically resolved via the DestinationResolver.
     * <p>
     ...
     * <p>
     * The default converter is a SimpleMessageConverter, which is able to handle BytesMessages, TextMessages and
     * ObjectMessages.
     *
     * @see #convertAndSend
     ...
     * <p>
     * Specify {@link #RECEIVE_TIMEOUT_NO_WAIT} (or any other negative value) to indicate that a receive operation
     ...
     *
     * @return true if overriding default values of QOS parameters (deliveryMode, priority, and timeToLive)
     * @see #setDeliveryMode
     * @see #setPriority
     * @see #setTimeToLive
     */
    ...
    /**
     * Set whether message delivery should be persistent or non-persistent, specified as boolean value ("true" or
     * "false"). This will set the delivery mode accordingly, to either "PERSISTENT" (2) or "NON_PERSISTENT" (1).
     * <p>
     ...
     * <p>
     * Since a default value may be defined administratively, this is only used when "isExplicitQosEnabled" equals
     * "true".
     *
     * @param deliveryMode
    ...
    /**
     * Execute the action specified by the given action object within a JMS Session. Generalized version of
     * {@code execute(SessionCallback)}, allowing the JMS Connection to be started on the fly.
     * <p>
     * Use {@code execute(SessionCallback)} for the general case. Starting the JMS Connection is just necessary for
     * receiving messages, which is preferably achieved through the {@code receive} methods.
     * 
     * @param action
     ...
     *            the message selector for this consumer (can be {@code null})
     ...
     * 
     * @param message
     *            the JMS Message to convert (can be {@code null})
    ...
    /**
     * Send a request message to the given {@link Destination} and block until a reply has been received on a temporary
     * queue created on-the-fly.
     * <p>
    ...
    /**
     * A variant of {@link #execute(SessionCallback, boolean)} that explicitly creates a non-transactional
     * {@link Session}. The given {@link SessionCallback} does not participate in an existing transaction.
     */
    @Nullable
    ...
    /**
     * Create a JMS MessageProducer for the given Session and Destination, configuring it to disable message ids and/or
