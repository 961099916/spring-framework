...
/**
 * Common base class for all containers which need to implement listening based on a JMS Connection (either shared or
 * freshly obtained for each attempt). Inherits basic Connection and Session configuration handling from the
 * {@link org.springframework.jms.support.JmsAccessor} base class.
 *
 * <p>
 * This class provides basic lifecycle management, in particular management of a shared JMS Connection. Subclasses are
 * supposed to plug into this lifecycle, implementing the {@link #sharedConnectionEnabled()} as well as the
 * {@link #doInitialize()} and {@link #doShutdown()} template methods.
 *
 * <p>
 * This base class does not assume any specific listener programming model or listener invoker mechanism. It just
 * provides the general runtime lifecycle management needed for any kind of JMS-based listening mechanism that operates
 * on a JMS Connection/Session.
 *
 * <p>
 * For a concrete listener programming model, check out the {@link AbstractMessageListenerContainer} subclass. For a
 * concrete listener invoker mechanism, check out the {@link DefaultMessageListenerContainer} class.
 *
...
public abstract class AbstractJmsListeningContainer extends JmsDestinationAccessor
     ...
     * <p>
     * Creates a JMS Connection, starts the {@link javax.jms.Connection} (if {@link #setAutoStartup(boolean)
     * "autoStartup"} hasn't been turned off), and calls {@link #doInitialize()}.
     * 
     ...
     * <p>
     * This implementation always returns {@code true}; the default 'running' state is purely determined by
     ...
     * running. In other words, they may apply further restrictions to the 'running' state, returning {@code false} if
     * such a restriction prevents listeners from running.
     */
     ...
     *             if this container does not maintain a shared Connection, or if the Connection hasn't been initialized
     *             yet
     * @see #sharedConnectionEnabled()
     */
    ...
    /**
     * Take the given task object and reschedule it, either immediately if this container is currently running, or later
     * once this container has been restarted.
     * <p>
     * If this container has already been shut down, the task will not get rescheduled at all.
     * 
     * @param task
     *            the task object to reschedule
     * @return whether the task has been rescheduled (either immediately or for a restart of this container)
     ...
     * <p>
     * To be implemented by subclasses if they ever call {@code rescheduleTaskIfNecessary}. This implementation throws
     ...
     * @param ex
     *            the exception thrown from {@link #doRescheduleTask}
     */
    ...
    /**
     * Exception that indicates that the initial setup of this container's shared JMS Connection failed. This is
