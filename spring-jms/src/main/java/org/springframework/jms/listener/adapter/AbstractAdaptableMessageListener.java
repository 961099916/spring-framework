...
/**
 * An abstract JMS {@link MessageListener} adapter providing the necessary infrastructure to extract the payload of a
 * JMS {@link Message}.
 *
...
public abstract class AbstractAdaptableMessageListener
    ...
    /**
     * Set the default destination to send response messages to. This will be applied in case of a request message that
     * does not carry a "JMSReplyTo" field.
     * <p>
     ...
     * <p>
     * Alternatively, specify a "defaultResponseQueueName" or "defaultResponseTopicName", to be dynamically resolved via
    ...
    /**
     * Set the name of the default response queue to send response messages to. This will be applied in case of a
    ...
    /**
     * Set the name of the default response topic to send response messages to. This will be applied in case of a
     * request message that does not carry a "JMSReplyTo" field.
     * <p>
     * Alternatively, specify a JMS Destination object as "defaultResponseDestination".
     * 
     ...
     * <p>
     * The default resolver is a DynamicDestinationResolver. Specify a JndiDestinationResolver for resolving destination
     * names as JNDI locations.
     *
    ...
    /**
     * Return the converter that will convert incoming JMS messages to listener method arguments, and objects returned
     * from listener methods back to JMS messages.
     */
    @Nullable
    ...
    /**
     * Set the converter that will convert incoming JMS messages to listener method arguments, and objects returned from
     * listener methods back to JMS messages.
     * <p>
     * The default converter is a {@link SimpleMessageConverter}, which is able to handle {@link javax.jms.BytesMessage
     * BytesMessages}, {@link javax.jms.TextMessage TextMessages} and {@link javax.jms.ObjectMessage ObjectMessages}.
     */
    ...
    /**
     * Set the {@link JmsHeaderMapper} implementation to use to map the standard JMS headers. By default, a
     * {@link SimpleJmsHeaderMapper} is used.
     * 
    ...
    /**
     * Return the {@link MessagingMessageConverter} for this listener, being able to convert
    ...
    /**
     * Return the {@link QosSettings} to use when sending a response, or {@code null} if the defaults should be used.
     *
     * @since 5.0
     */
    @Nullable
    ...
    /**
     * Set the {@link QosSettings} to use when sending a response. Can be set to {@code null} to indicate that the
     * broker's defaults should be used.
     *
     * @param responseQosSettings
     *            the QoS settings to use when sending a response or {@code null} to use the default values.
     * @since 5.0
     */
     ...
     * Use the {@link SessionAwareMessageListener} entry point (typically through a Spring message listener container)
     ...
     * error level.
     * <p>
     * This method only applies when used as standard JMS {@link MessageListener}. In case of the Spring
     * {@link SessionAwareMessageListener} mechanism, exceptions get handled by the caller instead.
     * 
     * @param ex
    ...
    protected Message buildMessage(Session session, Object result) throws JMSException {
        Object content =
            preProcessResponse(result instanceof JmsResponse ? ((JmsResponse<?>)result).getResponse() : result);
     ...
     * @return the payload response to handle, either the {@code result} argument or any other object (for instance
     *         wrapping the result).
     * @since 4.3
     */
     ...
     * <p>
     * The default implementation first checks the JMS Reply-To {@link Destination} of the supplied request; if that is
     * not {@code null} it is returned; if it is {@code null}, then the configured
     * {@link #resolveDefaultResponseDestination default response destination} is returned; if this too is {@code null},
     * then an {@link javax.jms.InvalidDestinationException} is thrown.
     * 
     * @param request
     *            the original incoming JMS message
     * @param response
    ...
    private class MessagingMessageConverterAdapter extends MessagingMessageConverter {
        ...
        protected class LazyResolutionMessage implements org.springframework.messaging.Message<Object> {
            ...
            /**
             * Extract the payload of the current message. Since we deferred the resolution of the payload, a custom
             * converter may still return a full message for it. In this case, its payload is returned.
             *
