...
/**
 * Extension of the generic JCA 1.5 {@link org.springframework.jca.endpoint.GenericMessageEndpointManager}, adding
 * JMS-specific support for ActivationSpec configuration.
 *
 * <p>
 * Allows for defining a common {@link JmsActivationSpecConfig} object that gets converted into a provider-specific JCA
 * 1.5 ActivationSpec object for activating the endpoint.
 *
 * <p>
 * <b>NOTE:</b> This JCA-based endpoint manager supports standard JMS {@link javax.jms.MessageListener} endpoints only.
 * It does <i>not</i> support Spring's {@link org.springframework.jms.listener.SessionAwareMessageListener} variant,
 * simply because the JCA endpoint management contract does not allow for obtaining the current JMS
...
public class JmsMessageEndpointManager extends GenericMessageEndpointManager
     ...
     * Consult your resource provider's ActivationSpec documentation for the local transaction options of your
     * particular provider.
     * <p>
     * This is a shortcut for configuring a dedicated JmsMessageEndpointFactory.
     * 
     ...
     * <p>
     * This factory is dependent on the concrete JMS provider, e.g. on ActiveMQ. The default implementation simply
     * guesses the ActivationSpec class name from the provider's class name (e.g. "ActiveMQResourceAdapter" ->
     * "ActiveMQActivationSpec" in the same package), and populates the ActivationSpec properties as suggested by the
     * JCA 1.5 specification (plus a couple of autodetected vendor-specific properties).
     * 
    ...
    /**
     * Return the {@link JmsActivationSpecConfig} object that this endpoint manager should use for activating its
     * listener. Return {@code null} if none is set.
     */
    @Nullable
    ...
    /**
     * Specify the {@link JmsActivationSpecConfig} object that this endpoint manager should use for activating its
     * listener.
     * <p>
     * This config object will be turned into a concrete JCA 1.5 ActivationSpec object through a
    ...
    /**
     * Set the DestinationResolver to use for resolving destination names into the JCA 1.5 ActivationSpec "destination"
     * property.
     * <p>
     * If not specified, destination names will simply be passed in as Strings. If specified, destination names will be
     * resolved into Destination objects first.
     * <p>
     * Note that a DestinationResolver is usually specified on the JmsActivationSpecFactory (see
     * {@link StandardJmsActivationSpecFactory#setDestinationResolver}). This is simply a shortcut for parameterizing
     * the default JmsActivationSpecFactory; it will replace any custom JmsActivationSpecFactory that might have been
     * set before.
     *
