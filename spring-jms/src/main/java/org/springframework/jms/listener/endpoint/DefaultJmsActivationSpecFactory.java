...
/**
 * Default implementation of the {@link JmsActivationSpecFactory} interface. Supports the standard JMS properties as
 * defined by the JCA 1.5 specification, as well as Spring's extended "maxConcurrency" and "prefetchSize" settings
 * through autodetection of well-known vendor-specific provider properties.
 *
 * <p>
 * An ActivationSpec factory is effectively dependent on the concrete JMS provider, e.g. on ActiveMQ. This default
 * implementation simply guesses the ActivationSpec class name from the provider's class name ("ActiveMQResourceAdapter"
 * -> "ActiveMQActivationSpec" in the same package, or "ActivationSpecImpl" in the same package as the ResourceAdapter
 * class), and populates the ActivationSpec properties as suggested by the JCA 1.5 specification (Appendix B). Specify
 * the 'activationSpecClass' property explicitly if these default naming rules do not apply.
 *
 * <p>
 * Note: ActiveMQ, JORAM and WebSphere are supported in terms of extended settings (through the detection of their bean
 * property naming conventions). The default ActivationSpec class detection rules may apply to other JMS providers as
 * well.
 *
 * <p>
...
public class DefaultJmsActivationSpecFactory extends StandardJmsActivationSpecFactory {
     ...
     * "ActiveMQResourceAdapter" -> "ActiveMQActivationSpec" in the same package, or a class named "ActivationSpecImpl"
     * in the same package as the ResourceAdapter class.
     */
    @Override
    ...
    /**
     * This implementation supports Spring's extended "maxConcurrency" and "prefetchSize" settings through detecting
     * corresponding ActivationSpec properties: "maxSessions"/"maxNumberOfWorks" and
     * "maxMessagesPerSessions"/"maxMessages", respectively (following ActiveMQ's and JORAM's naming conventions).
     */
    @Override
