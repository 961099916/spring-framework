 ...
 *
 * <p>
 * By default, the content of incoming JMS messages gets extracted before being passed into the target listener method,
 * to let the target method operate on message content types such as String or byte array instead of the raw
 * {@link Message}. Message type conversion is delegated to a Spring JMS {@link MessageConverter}. By default, a
 * {@link SimpleMessageConverter} will be used. (If you do not want such automatic message conversion taking place, then
 * be sure to set the {@link #setMessageConverter MessageConverter} to {@code null}.)
 *
 * <p>
 * If a target listener method returns a non-null object (typically of a message content type such as {@code String} or
 * byte array), it will get wrapped in a JMS {@code Message} and sent to the response destination (either the JMS
 * "reply-to" destination or a {@link #setDefaultResponseDestination(javax.jms.Destination) specified default
 * destination}).
 *
 * <p>
 * <b>Note:</b> The sending of response messages is only available when using the {@link SessionAwareMessageListener}
 * entry point (typically through a Spring message listener container). Usage as standard JMS {@link MessageListener}
 ...
 * {@code Message} types and gets passed the contents of each {@code Message} type as an argument. No {@code Message}
 * will be sent back as all of these methods return {@code void}.
 *
 * <pre class="code">
 ...
 * </pre>
 *
 * This next example handles all {@code Message} types and gets passed the actual (raw) {@code Message} as an argument.
 * Again, no {@code Message} will be sent back as all of these methods return {@code void}.
 *
 * <pre class="code">
 ...
 * </pre>
 *
 * This next example illustrates a {@code Message} delegate that just consumes the {@code String} contents of
 * {@link javax.jms.TextMessage TextMessages}. Notice also how the name of the {@code Message} handling method is
 * different from the {@link #ORIGINAL_DEFAULT_LISTENER_METHOD original} (this will have to be configured in the
 * attendant bean definition). Again, no {@code Message} will be sent back as the method returns {@code void}.
 *
 * <pre class="code">
 ...
 * </pre>
 *
 * This final example illustrates a {@code Message} delegate that just consumes the {@code String} contents of
 * {@link javax.jms.TextMessage TextMessages}. Notice how the return type of this method is {@code String}: This will
 * result in the configured {@link MessageListenerAdapter} sending a {@link javax.jms.TextMessage} in response.
 *
 * <pre class="code">
 ...
 * (and it's attendant XML configuration) in detail.
 *
...
public class MessageListenerAdapter extends AbstractAdaptableMessageListener implements SubscriptionNameProvider {
     ...
     * target method returns a non-null object, wrap in a JMS message and send it back.
     * 
     * @param message
     ...
     *            the converted JMS request message, to be passed into the listener method as argument
