...
/**
 * Bean post-processor that registers methods annotated with {@link JmsListener} to be invoked by a JMS message listener
 * container created under the cover by a {@link org.springframework.jms.config.JmsListenerContainerFactory} according
 ...
 *
 * <p>
 * This post-processor is automatically registered by Spring's {@code <jms:annotation-driven>} XML element, and also by
 * the {@link EnableJms} annotation.
 *
 * <p>
 * Autodetects any {@link JmsListenerConfigurer} instances in the container, allowing for customization of the registry
 * to be used, the default container factory or for fine-grained control over endpoints registration. See the
...
public class JmsListenerAnnotationBeanPostProcessor
    ...
    /**
     * Set the {@link JmsListenerEndpointRegistry} that will hold the created endpoint and manage the lifecycle of the
     * related listener container.
     */
    ...
    /**
     * Making a {@link BeanFactory} available is optional; if not set, {@link JmsListenerConfigurer} beans won't get
     * autodetected and an {@link #setEndpointRegistry endpoint registry} has to be explicitly configured.
     */
    @Override
        ...
        if (!this.nonAnnotatedClasses.contains(targetClass)
            ...
            if (annotatedMethods.isEmpty()) {
                ...
                annotatedMethods.forEach(
                    (method, listeners) -> listeners.forEach(listener -> processJmsListener(listener, method, bean)));
    ...
    /**
     * Process the given {@link JmsListener} annotation on the given method, registering a corresponding endpoint for
        ...
        if (StringUtils.hasText(containerFactoryBeanName)) {
            ...
            try {
                ...
                throw new BeanInitializationException("Could not register JMS listener endpoint on ["
                    + mostSpecificMethod + "], no " + JmsListenerContainerFactory.class.getSimpleName() + " with id '"
    ...
    /**
     * Instantiate an empty {@link MethodJmsListenerEndpoint} for further configuration with provided parameters in
     * {@link #processJmsListener}.
     * 
    ...
    /**
     * A {@link MessageHandlerMethodFactory} adapter that offers a configurable underlying instance to use. Useful if
     * the factory to use is determined once the endpoints have been registered but not created yet.
     * 
