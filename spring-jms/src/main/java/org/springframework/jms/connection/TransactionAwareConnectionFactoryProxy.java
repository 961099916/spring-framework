...
/**
 * Proxy for a target JMS {@link javax.jms.ConnectionFactory}, adding awareness of Spring-managed transactions. Similar
 * to a transactional JNDI ConnectionFactory as provided by a Java EE application server.
 *
 * <p>
 * Messaging code which should remain unaware of Spring's JMS support can work with this proxy to seamlessly participate
 * in Spring-managed transactions. Note that the transaction manager, for example {@link JmsTransactionManager}, still
 * needs to work with the underlying ConnectionFactory, <i>not</i> with this proxy.
 *
 * <p>
 * <b>Make sure that TransactionAwareConnectionFactoryProxy is the outermost ConnectionFactory of a chain of
 * ConnectionFactory proxies/adapters.</b> TransactionAwareConnectionFactoryProxy can delegate either directly to the
 * target factory or to some intermediary adapter like {@link UserCredentialsConnectionFactoryAdapter}.
 *
 * <p>
 * Delegates to {@link ConnectionFactoryUtils} for automatically participating in thread-bound transactions, for example
 * managed by {@link JmsTransactionManager}. {@code createSession} calls and {@code close} calls on returned Sessions
 * will behave properly within a transaction, that is, always work on the transactional Session. If not within a
 * transaction, normal ConnectionFactory behavior applies.
 *
 * <p>
 * Note that transactional JMS Sessions will be registered on a per-Connection basis. To share the same JMS Session
 * across a transaction, make sure that you operate on the same JMS Connection handle - either through reusing the
 * handle or through configuring a {@link SingleConnectionFactory} underneath.
 *
 * <p>
 * Returned transactional Session proxies will implement the {@link SessionProxy} interface to allow for access to the
 * underlying target Session. This is only intended for accessing vendor-specific Session API or for testing purposes
 * (e.g. to perform manual transaction control). For typical application purposes, simply use the standard JMS Session
 * interface.
 *
 * <p>
...
public class TransactionAwareConnectionFactoryProxy
    ...
    /**
     * Set whether to allow for a local JMS transaction that is synchronized with a Spring-managed transaction (where
     * the main transaction might be a JDBC-based one for a specific DataSource, for example), with the JMS transaction
     * committing right after the main transaction. If not allowed, the given ConnectionFactory needs to handle
     * transaction enlistment underneath the covers.
     * <p>
     * Default is "false": If not within a managed transaction that encompasses the underlying JMS ConnectionFactory,
     * standard Sessions will be returned. Turn this flag on to allow participation in any Spring-managed transaction,
    ...
    /**
     * Wrap the given Connection with a proxy that delegates every method call to it but handles Session lookup in a
     * transaction-aware fashion.
     * 
     * @param target
