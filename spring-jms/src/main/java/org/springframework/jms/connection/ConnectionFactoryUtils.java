...
/**
 * Helper class for managing a JMS {@link javax.jms.ConnectionFactory}, in particular for obtaining transactional JMS
 * resources for a given ConnectionFactory.
 *
 * <p>
 * Mainly for internal use within the framework. Used by {@link org.springframework.jms.core.JmsTemplate} as well as
...
public abstract class ConnectionFactoryUtils {
     ...
     * <p>
     * Checks {@link SmartConnectionFactory#shouldStop}, if available. This is essentially a more sophisticated version
     * of {@link org.springframework.jms.support.JmsUtils#closeConnection}.
     * 
     * @param con
     *            the Connection to release (if this is {@code null}, the call will be ignored)
     * @param cf
     *            the ConnectionFactory that the Connection was obtained from (may be {@code null})
     * @param started
    ...
    /**
     * Return the innermost target Session of the given Session. If the given Session is a proxy, it will be unwrapped
     * until a non-proxy Session is found. Otherwise, the passed-in Session will be returned as-is.
     * 
     * @param session
    ...
    /**
     * Determine whether the given JMS Session is transactional, that is, bound to the current thread by Spring's
     ...
     * @param existingCon
     *            the existing JMS Connection to obtain a Session for (may be {@code null})
     * @param synchedLocalTransactionAllowed
     *            whether to allow for a local JMS transaction that is synchronized with a Spring-managed transaction
     *            (where the main transaction might be a JDBC-based one for a specific DataSource, for example), with
     *            the JMS transaction committing right after the main transaction. If not allowed, the given
    ...
    public static QueueSession getTransactionalQueueSession(final QueueConnectionFactory cf,
        @Nullable final QueueConnection existingCon, final boolean synchedLocalTransactionAllowed) throws JMSException {
    ...
    public static TopicSession getTransactionalTopicSession(final TopicConnectionFactory cf,
        @Nullable final TopicConnection existingCon, final boolean synchedLocalTransactionAllowed) throws JMSException {
     ...
     * Obtain a JMS Session that is synchronized with the current transaction, if any.
     * <p>
     * This {@code doGetTransactionalSession} variant always starts the underlying JMS Connection, assuming that the
     * Session will be used for receiving messages.
     * 
     * @param connectionFactory
     *            the JMS ConnectionFactory to bind for (used as TransactionSynchronizationManager key)
     * @param resourceFactory
     *            the ResourceFactory to use for extracting or creating JMS resources
     * @return the transactional Session, or {@code null} if none found
     * @throws JMSException
     *             in case of JMS failure
     ...
     *            whether the underlying JMS Connection approach should be started in order to allow for receiving
     *            messages. Note that a reused Connection may already have been started before, even if this flag is
     *            {@code false}.
     * @return the transactional Session, or {@code null} if none found
     * @throws JMSException
     *             in case of JMS failure
    ...
    /**
     * Callback interface for resource creation. Serving as argument for the {@code doGetTransactionalSession} method.
     */
    public interface ResourceFactory {
         ...
         * (where the main transaction might be a JDBC-based one for a specific DataSource, for example), with the JMS
    ...
    /**
     * Callback for resource cleanup at the end of a non-native JMS transaction (e.g. when participating in a
