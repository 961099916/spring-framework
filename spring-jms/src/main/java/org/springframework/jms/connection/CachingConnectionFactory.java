 ...
 * {@link javax.jms.MessageProducer} caching. This ConnectionFactory also switches the {@link #setReconnectOnException
 * "reconnectOnException" property} to "true" by default, allowing for automatic recovery of the underlying Connection.
 *
 * <p>
 * By default, only one single Session will be cached, with further requested Sessions being created and disposed on
 * demand. Consider raising the {@link #setSessionCacheSize "sessionCacheSize" value} in case of a high-concurrency
 * environment.
 *
 * <p>
 * When using the JMS 1.0.2 API, this ConnectionFactory will switch into queue/topic mode according to the JMS API
 * methods used at runtime: {@code createQueueConnection} and {@code createTopicConnection} will lead to queue/topic
 * mode, respectively; generic {@code createConnection} calls will lead to a JMS 1.1 connection which is able to serve
 * both modes.
 *
 * <p>
 * As of Spring Framework 5, this class supports JMS 2.0 {@code JMSContext} calls and therefore requires the JMS 2.0 API
 * to be present at runtime. It may nevertheless run against a JMS 1.1 driver (bound to the JMS 2.0 API) as long as no
 * actual JMS 2.0 calls are triggered by the application's setup.
 *
 * <p>
 * <b>NOTE: This ConnectionFactory requires explicit closing of all Sessions obtained from its shared Connection.</b>
 * This is the usual recommendation for native JMS access code anyway. However, with this ConnectionFactory, its use is
 * mandatory in order to actually allow for Session reuse.
 *
 * <p>
 * Note also that MessageConsumers obtained from a cached Session won't get closed until the Session will eventually be
 * removed from the pool. This may lead to semantic side effects in some cases. For a durable subscriber, the logical
 * {@code Session.close()} call will also close the subscription. Re-registering a durable consumer for the same
 * subscription on the same Session handle is not supported; close and reobtain a cached Session first.
 *
...
public class CachingConnectionFactory extends SingleConnectionFactory {
     ...
     * <p>
     * This cache size is the maximum limit for the number of cached Sessions per session acknowledgement type (auto,
     * client, dups_ok, transacted). As a consequence, the actual number of cached Sessions may be up to four times as
     * high as the specified value - in the unlikely case of mixing and matching different acknowledgement types.
     * <p>
     * Default is 1: caching a single Session, (re-)creating further ones on demand. Specify a number like 10 if you'd
     * like to raise the number of cached Sessions; that said, 1 may be sufficient for low-concurrency scenarios.
     *
    ...
    /**
     * Specify whether to cache JMS MessageProducers per JMS Session instance (more specifically: one MessageProducer
     * per Destination and Session).
     * <p>
    ...
    /**
     * Specify whether to cache JMS MessageConsumers per JMS Session instance (more specifically: one MessageConsumer
     * per Destination, selector String and Session). Note that durable subscribers will only be cached until logical
    ...
    /**
     * Wrap the given Session with a proxy that delegates every method call to it but adapts close calls. This is useful
     * for allowing application code to handle a special framework Session just like an ordinary Session.
     * 
     * @param target
    ...
    /**
     * Simple wrapper class around a Destination and other consumer attributes. Used as the cache key when caching
    ...
    private class CachedSessionInvocationHandler implements InvocationHandler {
        ...
        private MessageConsumer getCachedConsumer(Destination dest, @Nullable String selector,
            ...
            if (consumer != null) {
                ...
                if (dest instanceof Topic) {
                    if (noLocal == null) {
                        consumer =
                            (durable ? this.target.createSharedDurableConsumer((Topic)dest, subscription, selector)
                                : this.target.createSharedConsumer((Topic)dest, subscription, selector));
                    } else {
                        consumer =
                            (durable ? this.target.createDurableSubscriber((Topic)dest, subscription, selector, noLocal)
        ...
        private void logicalClose(Session proxy) throws JMSException {
            ...
            for (Iterator<Map.Entry<ConsumerCacheKey, MessageConsumer>> it = this.cachedConsumers.entrySet().iterator();
                it.hasNext();) {
