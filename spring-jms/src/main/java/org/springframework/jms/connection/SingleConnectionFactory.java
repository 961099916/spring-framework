...
/**
 * A JMS ConnectionFactory adapter that returns the same Connection from all {@link #createConnection()} calls, and
 * ignores calls to {@link javax.jms.Connection#close()}. According to the JMS Connection model, this is perfectly
 * thread-safe (in contrast to e.g. JDBC). The shared Connection can be automatically recovered in case of an Exception.
 *
 * <p>
 ...
 *
 * <p>
 * Note that when using the JMS 1.0.2 API, this ConnectionFactory will switch into queue/topic mode according to the JMS
 * API methods used at runtime: {@code createQueueConnection} and {@code createTopicConnection} will lead to queue/topic
 ...
 *
 * <p>
 * Useful for testing and standalone environments in order to keep using the same Connection for multiple
 * {@link org.springframework.jms.core.JmsTemplate} calls, without having a pooling ConnectionFactory underneath. This
 * may span any number of transactions, even concurrently executing transactions.
 *
 * <p>
 * Note that Spring's message listener containers support the use of a shared Connection within each listener container
 * instance. Using SingleConnectionFactory in combination only really makes sense for sharing a single JMS Connection
 * <i>across multiple listener containers</i>.
 *
