...
/**
 * An adapter for a target JMS {@link javax.jms.ConnectionFactory}, applying the given user credentials to every
 * standard {@code createConnection()} call, that is, implicitly invoking {@code createConnection(username, password)}
 * on the target. All other methods simply delegate to the corresponding methods of the target ConnectionFactory.
 *
 * <p>
 * Can be used to proxy a target JNDI ConnectionFactory that does not have user credentials configured. Client code can
 * work with the ConnectionFactory without passing in username and password on every {@code createConnection()} call.
 *
 * <p>
 * In the following example, client code can simply transparently work with the preconfigured "myConnectionFactory",
 * implicitly accessing "myTargetConnectionFactory" with the specified user credentials.
 *
 * <pre class="code">
 ...
 *
 * <p>
 * If the "username" is empty, this proxy will simply delegate to the standard {@code createConnection()} method of the
 * target ConnectionFactory. This can be used to keep a UserCredentialsConnectionFactoryAdapter bean definition just for
 * the <i>option</i> of implicitly passing in user credentials if the particular target ConnectionFactory requires it.
 *
 * <p>
 * As of Spring Framework 5, this class delegates JMS 2.0 {@code JMSContext} calls and therefore requires the JMS 2.0
 * API to be present at runtime. It may nevertheless run against a JMS 1.1 driver (bound to the JMS 2.0 API) as long as
 * no actual JMS 2.0 calls are triggered by the application's setup.
 *
...
public class UserCredentialsConnectionFactoryAdapter
    ...
    /**
     * Set user credententials for this proxy and the current thread. The given username and password will be applied to
     * all subsequent {@code createConnection()} calls on this ConnectionFactory proxy.
     * <p>
     * This will override any statically specified user credentials, that is, values of the "username" and "password"
     * bean properties.
     * 
     * @param username
     *            the username to apply
     * @param password
    ...
    /**
     * Remove any user credentials for this proxy from the current thread. Statically specified user credentials apply
     * again afterwards.
     * 
    ...
    /**
     * Determine whether there are currently thread-bound credentials, using them if available, falling back to the
     * statically specified username and password (i.e. values of the bean properties) otherwise.
     * 
    ...
    /**
     * This implementation delegates to the {@code createConnection(username, password)} method of the target
     * ConnectionFactory, passing in the specified user credentials. If the specified username is empty, it will simply
     * delegate to the standard {@code createConnection()} method of the target ConnectionFactory.
     * 
     * @param username
     *            the username to use
     * @param password
    ...
    /**
     * Determine whether there are currently thread-bound credentials, using them if available, falling back to the
     * statically specified username and password (i.e. values of the bean properties) else.
     * 
    ...
    /**
     * This implementation delegates to the {@code createQueueConnection(username, password)} method of the target
     * QueueConnectionFactory, passing in the specified user credentials. If the specified username is empty, it will
     * simply delegate to the standard {@code createQueueConnection()} method of the target ConnectionFactory.
     * 
     * @param username
     *            the username to use
     * @param password
    ...
    /**
     * This implementation delegates to the {@code createTopicConnection(username, password)} method of the target
     * TopicConnectionFactory, passing in the specified user credentials. If the specified username is empty, it will
     * simply delegate to the standard {@code createTopicConnection()} method of the target ConnectionFactory.
     * 
     * @param username
     *            the username to use
     * @param password
