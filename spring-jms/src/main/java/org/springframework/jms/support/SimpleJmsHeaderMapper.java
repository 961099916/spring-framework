 ...
 *
 * <p>
 * This implementation copies JMS API headers (e.g. JMSReplyTo) to and from {@link org.springframework.messaging.Message
 * Messages}. Any user-defined properties will also be copied from a JMS Message to a Message, and any other headers on
 * a Message (beyond the JMS API headers) will likewise be copied to a JMS Message. Those other headers will be copied
 * to the general properties of a JMS Message whereas the JMS API headers are passed to the appropriate setter methods
 * (e.g. setJMSReplyTo).
 *
 * <p>
 * Constants for the JMS API headers are defined in {@link JmsHeaders}. Note that most of the JMS headers are read-only:
 * the JMSDestination, JMSDeliveryMode, JMSExpiration, JMSMessageID, JMSPriority, JMSRedelivered and JMSTimestamp flags
 * are only copied <em>from</em> a JMS Message. Those values will <em>not</em> be passed along from a Message to an
...
public class SimpleJmsHeaderMapper extends AbstractHeaderMapper<Message> implements JmsHeaderMapper {
            ...
            if (jmsPropertyNames != null) {
                while (jmsPropertyNames.hasMoreElements()) {
                    ...
                    try {
                        ...
                        if (logger.isDebugEnabled()) {
                            logger.debug(
                                "Error occurred while mapping JMS property '" + propertyName + "' to Message header",
                                ex);
                        }
                    }
                }
     ...
     * <p>
     * Convert {@link MessageHeaders#CONTENT_TYPE} to {@code content_type} for JMS compliance.
     * 
     ...
     * <p>
     * Convert the JMS-compliant {@code content_type} to {@link MessageHeaders#CONTENT_TYPE}.
     * 
     * @see #CONTENT_TYPE_PROPERTY
     */
    @Override
