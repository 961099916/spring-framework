...
/**
 * Base class for {@link org.springframework.jms.core.JmsTemplate} and other JMS-accessing gateway helpers, defining
 * common properties such as the JMS {@link ConnectionFactory} to operate on. The subclass
 * {@link org.springframework.jms.support.destination.JmsDestinationAccessor} adds further, destination-related
 * properties.
 *
 * <p>
...
public abstract class JmsAccessor implements InitializingBean {
     ...
     * {@code create(Queue/Topic)Session(boolean transacted, int acknowledgeMode)} method are not taken into account.
     * Depending on the Java EE transaction context, the container makes its own decisions on these values. Analogously,
     * these parameters are not taken into account within a locally managed transaction either, since the accessor
    ...
    /**
     * Set the JMS acknowledgement mode by the name of the corresponding constant in the JMS {@link Session} interface,
     ...
     * 
     * @param constantName
     *            the name of the {@link Session} acknowledge mode constant
     ...
     * {@code create(Queue/Topic)Session(boolean transacted, int acknowledgeMode)} method are not taken into account.
     * Depending on the transaction context in the EJB, the container makes its own decisions on these values. See
     ...
     * <p>
     * The default implementation delegates to the {@link JmsUtils#convertJmsAccessException} method.
     * 
     * @param ex
     *            the original checked {@link JMSException} to convert
