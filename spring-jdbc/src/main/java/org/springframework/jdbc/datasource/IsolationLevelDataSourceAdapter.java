...
/**
 * An adapter for a target {@link javax.sql.DataSource}, applying the current Spring transaction's isolation level (and
 * potentially specified user credentials) to every {@code getConnection} call. Also applies the read-only flag, if
 * specified.
 *
 * <p>
 * Can be used to proxy a target JNDI DataSource that does not have the desired isolation level (and user credentials)
 * configured. Client code can work with this DataSource as usual, not worrying about such settings.
 *
 * <p>
 * Inherits the capability to apply specific user credentials from its superclass
 * {@link UserCredentialsDataSourceAdapter}; see the latter's javadoc for details on that functionality (e.g.
 * {@link #setCredentialsForCurrentThread}).
 *
 * <p>
 * <b>WARNING:</b> This adapter simply calls {@link java.sql.Connection#setTransactionIsolation} and/or
 * {@link java.sql.Connection#setReadOnly} for every Connection obtained from it. It does, however, <i>not</i> reset
 * those settings; it rather expects the target DataSource to perform such resetting as part of its connection pool
 * handling. <b>Make sure that the target DataSource properly cleans up such transaction state.</b>
 *
...
public class IsolationLevelDataSourceAdapter extends UserCredentialsDataSourceAdapter {
     ...
     * {@link org.springframework.transaction.TransactionDefinition}, e.g. "ISOLATION_SERIALIZABLE".
     * <p>
     * If not specified, the target DataSource's default will be used. Note that a transaction-specific isolation value
     * will always override any isolation setting specified at the DataSource level.
     * 
     * @param constantName
     ...
     * {@link java.sql.Connection} constants (equivalent to the corresponding Spring
    ...
    /**
     * Determine the current isolation level: either the transaction's isolation level or a statically defined isolation
     * level.
     * 
