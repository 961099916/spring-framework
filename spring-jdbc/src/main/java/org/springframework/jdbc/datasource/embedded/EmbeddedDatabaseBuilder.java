 ...
 * 
 * <pre class="code">
 * EmbeddedDatabase db = new EmbeddedDatabaseBuilder().generateUniqueName(true).setType(H2).setScriptEncoding("UTF-8")
 *     .ignoreFailedDrops(true).addScript("schema.sql").addScripts("user_data.sql", "country_data.sql").build();
 *
...
public class EmbeddedDatabaseBuilder {
     ...
     *            the {@code ResourceLoader} to delegate to
     */
     ...
     * <p>
     * If the configuration for this builder is reused across multiple application contexts within a single JVM, this
     * flag should be <em>enabled</em> (i.e., set to {@code true}) in order to ensure that each application context gets
     * its own embedded database.
     * <p>
     * Enabling this flag overrides any explicit name set via {@link #setName}.
     * 
     * @param flag
     *            {@code true} if a unique database name should be generated
     ...
     * <p>
     * Will be overridden if the {@code generateUniqueName} flag has been set to {@code true}.
     * 
     * @param databaseName
     *            the name of the embedded database to build
     * @return {@code this}, to facilitate method chaining
     ...
     * 
     * @param databaseType
     *            the type of embedded database to build
     * @return {@code this}, to facilitate method chaining
     */
    ...
    /**
     * Set the factory to use to create the {@link DataSource} instance that connects to the embedded database.
     * <p>
     * Defaults to {@link SimpleDriverDataSourceFactory} but can be overridden, for example to introduce connection
     * pooling.
     * 
     * @return {@code this}, to facilitate method chaining
     * @since 4.0.3
     ...
     * <p>
     * The default scripts are {@code "schema.sql"} to create the database schema and {@code "data.sql"} to populate the
     * database with data.
     * 
     * @return {@code this}, to facilitate method chaining
     */
     ...
     * 
     * @param commentPrefix
     *            the prefix for single-line comments
     * @return {@code this}, to facilitate method chaining
     * @since 4.0.3
    ...
    /**
     * Specify that all failures which occur while executing SQL scripts should be logged but should not cause a
     * failure.
     * <p>
     * Defaults to {@code false}.
     * 
     * @param flag
     *            {@code true} if script execution should continue on error
     * @return {@code this}, to facilitate method chaining
     * @since 4.0.3
     */
     ...
     * <p>
     * This is useful for a database whose SQL dialect does not support an {@code IF EXISTS} clause in a {@code DROP}
     * statement.
     * <p>
     * The default is {@code false} so that {@link #build building} will fail fast if a script starts with a
     * {@code DROP} statement.
     * 
     * @param flag
     *            {@code true} if failed drop statements should be ignored
     * @return {@code this}, to facilitate method chaining
     * @since 4.0.3
     */
