...
public abstract class ScriptUtils {
     ...
     * {@link #executeSqlScript(Connection, EncodedResource, boolean, boolean, String, String, String, String)} to
     * denote that an SQL script contains a single statement (potentially spanning multiple lines) with no explicit
     ...
     * 
     * @param connection
     *            the JDBC connection to use to execute the script; already configured and ready to use
     * @param resource
     *            the resource to load the SQL script from; encoded with the current platform's default encoding
     ...
     *            whether or not to continue in the event of specifically an error on a {@code DROP} statement
     * @param commentPrefix
     *            the prefix that identifies single-line comments in the SQL script (typically "--")
     * @param separator
     *            the script statement separator; defaults to {@value #DEFAULT_STATEMENT_SEPARATOR} if not specified and
     *            falls back to {@value #FALLBACK_STATEMENT_SEPARATOR} as a last resort; may be set to
     ...
     *            the <em>start</em> block comment delimiter
     ...
     *            the prefixes that identify single-line comments in the SQL script (typically "--")
     * @param separator
     *            the script statement separator; defaults to {@value #DEFAULT_STATEMENT_SEPARATOR} if not specified and
     *            falls back to {@value #FALLBACK_STATEMENT_SEPARATOR} as a last resort; may be set to
     *            {@value #EOF_STATEMENT_SEPARATOR} to signal that the script contains a single statement without a
    ...
    /**
     * Read a script from the provided resource, using the supplied comment prefixes and statement separator, and build
     * a {@code String} containing the lines.
     * <p>
     ...
     * 
     * @param resource
     *            the {@code EncodedResource} containing the script to be processed
     * @param separator
     *            the statement separator in the SQL script (typically ";")
     * @param commentPrefixes
     *            the prefixes that identify comments in the SQL script (typically "--")
     * @param blockCommentEndDelimiter
     *            the <em>end</em> block comment delimiter
     ...
     *            the {@code LineNumberReader} containing the script to be processed
     * @param commentPrefix
     ...
     * @throws IOException
     *             in case of I/O errors
     * @deprecated as of Spring Framework 5.2.16 with no plans for replacement. This is an internal API and will likely
     *             be removed in Spring Framework 6.0.
     */
    @Deprecated
     ...
     * <p>
     * This method is intended to be used to find the string delimiting each SQL statement &mdash; for example, a ';'
     * character.
     * <p>
     * Any occurrence of the delimiter within the script will be ignored if it is within a <em>literal</em> block of
     * text enclosed in single quotes ({@code '}) or double quotes ({@code "}), if it is escaped with a backslash
     * ({@code \}), or if it is within a single-line comment or block comment.
     * 
     * @param script
     *            the SQL script to search within
     * @param delimiter
     *            the statement delimiter to search for
     ...
     * <p>
     * This method is intended to be used to find the string separating each SQL statement &mdash; for example, a ';'
     * character.
     * <p>
     * Any occurrence of the separator within the script will be ignored if it is within a <em>literal</em> block of
     * text enclosed in single quotes ({@code '}) or double quotes ({@code "}), if it is escaped with a backslash
     * ({@code \}), or if it is within a single-line comment or block comment.
     * 
     * @param resource
     *            the resource from which the script was read, or {@code null} if unknown
     * @param script
     *            the SQL script to search within
     * @param separator
     *            the statement separator to search for
     * @param commentPrefixes
     *            the prefixes that identify single-line comments (typically {@code "--"})
     * @param blockCommentStartDelimiter
     *            the <em>start</em> block comment delimiter (typically {@code "/*"})
     * @param blockCommentEndDelimiter
     *            the <em>end</em> block comment delimiter (typically <code>"*&#47;"</code>)
     * @since 5.2.16
     */
    private static boolean containsStatementSeparator(@Nullable EncodedResource resource, String script,
        String separator, String[] commentPrefixes, String blockCommentStartDelimiter, String blockCommentEndDelimiter)
        throws ScriptException {
     ...
     * <p>
     * Within the script, {@value #DEFAULT_COMMENT_PREFIX} will be used as the comment prefix; any text beginning with
     * the comment prefix and extending to the end of the line will be omitted from the output. Similarly,
     * {@value #DEFAULT_BLOCK_COMMENT_START_DELIMITER} and {@value #DEFAULT_BLOCK_COMMENT_END_DELIMITER} will be used as
     * the <em>start</em> and <em>end</em> block comment delimiters: any text enclosed in a block comment will be
     * omitted from the output. In addition, multiple adjacent whitespace characters will be collapsed into a single
     * space.
     * 
     * @param script
     *            the SQL script
     * @param separator
     *            character separating each statement (typically a ';')
     * @param statements
    ...
    /**
     * Split an SQL script into separate statements delimited by the provided separator string. Each individual
     * statement will be added to the provided {@code List}.
     * <p>
     * Within the script, {@value #DEFAULT_COMMENT_PREFIX} will be used as the comment prefix; any text beginning with
     * the comment prefix and extending to the end of the line will be omitted from the output. Similarly,
     ...
     *            the SQL script
     * @param separator
     *            text separating each statement (typically a ';' or newline character)
     * @param statements
     *            the list that will contain the individual statements
     * @throws ScriptException
     *             if an error occurred while splitting the SQL script
     ...
     *            text separating each statement (typically a ';' or newline character)
     * @param commentPrefix
     *            the prefix that identifies SQL line comments (typically "--")
     * @param blockCommentStartDelimiter
     *            the <em>start</em> block comment delimiter; never {@code null} or empty
     * @param blockCommentEndDelimiter
     ...
     * statement will be added to the provided {@code List}.
     * <p>
     * Within the script, the provided {@code commentPrefixes} will be honored: any text beginning with one of the
     * comment prefixes and extending to the end of the line will be omitted from the output. Similarly, the provided
     * {@code blockCommentStartDelimiter} and {@code blockCommentEndDelimiter} delimiters will be honored: any text
     * enclosed in a block comment will be omitted from the output. In addition, multiple adjacent whitespace characters
     * will be collapsed into a single space.
     * 
     ...
     *            text separating each statement (typically a ';' or newline character)
     * @param commentPrefixes
     *            the prefixes that identify SQL line comments (typically "--")
     * @param blockCommentStartDelimiter
     *            the <em>start</em> block comment delimiter; never {@code null} or empty
     * @param blockCommentEndDelimiter
     *            the <em>end</em> block comment delimiter; never {@code null} or empty
