 ...
 *
 * <p>
 * The target {@link DataSource} is returned instead of an {@link EmbeddedDatabase} proxy since the {@link FactoryBean}
 * will manage the initialization and destruction lifecycle of the embedded database instance.
 *
 * <p>
 * Implements {@link DisposableBean} to shutdown the embedded database when the managing Spring container is being
 * closed.
 *
 * @author Keith Donald
...
public class EmbeddedDatabaseFactoryBean extends EmbeddedDatabaseFactory
    ...
    /**
     * Set a script execution to be run in the bean destruction callback, cleaning up the database and leaving it in a
     * known state for others.
     * 
     * @param databaseCleaner
     *            the database script executor to run on destroy
