 ...
 *
 * <p>
 * <b>If you configure both a LazyConnectionDataSourceProxy and a TransactionAwareDataSourceProxy, make sure that the
 * latter is the outermost DataSource.</b> In such a scenario, data access code will talk to the transaction-aware
 ...
 *
 * <p>
 * This DataSource proxy gives you behavior analogous to JTA and a transactional JNDI DataSource (as provided by the
 * Java EE server), even with a local transaction strategy like DataSourceTransactionManager or
 * HibernateTransactionManager. It does not add value with Spring's JtaTransactionManager as transaction strategy.
 *
 * <p>
 * Lazy fetching of JDBC Connections is also recommended for read-only operations with Hibernate, in particular if the
 * chances of resolving the result in the second-level cache are high. This avoids the need to communicate with the
 ...
 * interface) in order to handle lazy fetching of an actual JDBC Connection. Use {@link Connection#unwrap} to retrieve
...
public class LazyConnectionDataSourceProxy extends DelegatingDataSource {
     ...
     * <p>
     * If not specified, the default gets determined by checking a target Connection on startup. If that check fails,
     * the default will be determined lazily on first access of a Connection.
     * 
     ...
     * <p>
     * This property accepts the int constant value (e.g. 8) as defined in the {@link java.sql.Connection} interface; it
     * is mainly intended for programmatic use. Consider using the "defaultTransactionIsolationName" property for
     * setting the value by name (e.g. "TRANSACTION_SERIALIZABLE").
     * <p>
     * If not specified, the default gets determined by checking a target Connection on startup. If that check fails,
     * the default will be determined lazily on first access of a Connection.
     * 
    ...
    private class LazyConnectionInvocationHandler implements InvocationHandler {
        ...
        private Connection getTargetConnection(Method operation) throws SQLException {
            if (this.target == null) {
                ...
                this.target =
                    (this.username != null) ? obtainTargetDataSource().getConnection(this.username, this.password)
