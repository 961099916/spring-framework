 ...
 * Includes special support for Spring-managed transactional Connections, e.g. managed by
 * {@link DataSourceTransactionManager} or {@link org.springframework.transaction.jta.JtaTransactionManager}.
 *
 * <p>
 * Used internally by Spring's {@link org.springframework.jdbc.core.JdbcTemplate}, Spring's JDBC operation objects and
 * the JDBC {@link DataSourceTransactionManager}. Can also be used directly in application code.
 *
 * @author Rod Johnson
...
public abstract class DataSourceUtils {
    ...
    /**
     * Obtain a Connection from the given DataSource. Translates SQLExceptions into the Spring hierarchy of unchecked
     * generic data access exceptions, simplifying calling code and making any exception that is thrown more meaningful.
     * <p>
     * Is aware of a corresponding Connection bound to the current thread, for example when using
     * {@link DataSourceTransactionManager}. Will bind a Connection to the thread if transaction synchronization is
     * active, e.g. when running within a {@link org.springframework.transaction.jta.JtaTransactionManager JTA}
     * transaction).
     * 
     * @param dataSource
    ...
    /**
     * Actually obtain a JDBC Connection from the given DataSource. Same as {@link #getConnection}, but throwing the
     * original SQLException.
     * <p>
     * Is aware of a corresponding Connection bound to the current thread, for example when using
     * {@link DataSourceTransactionManager}. Will bind a Connection to the thread if transaction synchronization is
     * active (e.g. if in a JTA transaction).
     * <p>
    ...
    /**
     * Actually fetch a {@link Connection} from the given {@link DataSource}, defensively turning an unexpected
     * {@code null} return value from {@link DataSource#getConnection()} into an {@link IllegalStateException}.
     * 
     * @param dataSource
     *            the DataSource to obtain Connections from
     ...
     *             if the DataSource returned a null value
     ...
     * @param resetReadOnly
     *            whether to reset the connection's read-only flag
     * @since 5.2.1
     ...
     * @param previousIsolationLevel
     *            the isolation level to restore, if any
     * @deprecated as of 5.1.11, in favor of {@link #resetConnectionAfterTransaction(Connection, Integer, boolean)}
     */
    @Deprecated
    public static void resetConnectionAfterTransaction(Connection con, @Nullable Integer previousIsolationLevel) {
        Assert.notNull(con, "No Connection specified");
    ...
    /**
     * Determine whether the given JDBC Connection is transactional, that is, bound to the current thread by Spring's
     * transaction facilities.
     * 
     * @param con
     *            the Connection to check
     * @param dataSource
     *            the DataSource that the Connection was obtained from (may be {@code null})
    ...
    /**
     * Close the given Connection, obtained from the given DataSource, if it is not managed externally (that is, not
     * bound to the thread).
     * 
     * @param con
     *            the Connection to close if necessary (if this is {@code null}, the call will be ignored)
     * @param dataSource
     *            the DataSource that the Connection was obtained from (may be {@code null})
     * @see #getConnection
     */
    ...
    /**
     * Actually close the given Connection, obtained from the given DataSource. Same as {@link #releaseConnection}, but
     * throwing the original SQLException.
     * <p>
     * Directly accessed by {@link TransactionAwareDataSourceProxy}.
     * 
     * @param con
    ...
    /**
     * Determine whether the given two Connections are equal, asking the target Connection in case of a proxy. Used to
     * detect equality even if the user passed in a raw target Connection while the held one is a proxy.
     * 
     * @param conHolder
     *            the ConnectionHolder for the held Connection (potentially a proxy)
     * @param passedInCon
     *            the Connection passed-in by the user (potentially a target Connection without proxy)
    ...
    /**
     * Return the innermost target Connection of the given Connection. If the given Connection is a proxy, it will be
     * unwrapped until a non-proxy Connection is found. Otherwise, the passed-in Connection will be returned as-is.
     * 
     * @param con
    ...
    /**
     * Determine the connection synchronization order to use for the given DataSource. Decreased for every level of
     * nesting that a DataSource has, checked through the level of DelegatingDataSource nesting.
     * 
     * @param dataSource
    ...
    /**
     * Callback for resource cleanup at the end of a non-native JDBC transaction (e.g. when participating in a
