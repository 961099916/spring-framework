...
/**
 * {@link DataSource} implementation that delegates all calls to a WebSphere target {@link DataSource}, typically
 * obtained from JNDI, applying a current isolation level and/or current user credentials to every Connection obtained
 * from it.
 *
 * <p>
 * Uses IBM-specific API to get a JDBC Connection with a specific isolation level (and read-only flag) from a WebSphere
 * DataSource (<a href=
 ...
 * It's also possible to specify a default isolation level, to be applied when the current Spring-managed transaction
 * does not define a specific isolation level.
 *
 * <p>
 * Usage example, defining the target DataSource as an inner-bean JNDI lookup (of course, you can link to any WebSphere
 * DataSource through a bean reference):
 *
 * <pre class="code">
...
public class WebSphereDataSourceAdapter extends IsolationLevelDataSourceAdapter {
    ...
    /**
     * This constructor retrieves the WebSphere JDBC connection spec API, so we can get obtain specific WebSphere
     * Connections using reflection.
     */
    ...
    protected Connection doGetConnection(@Nullable String username, @Nullable String password) throws SQLException {
        ...
        Connection con =
            (Connection)invokeJdbcMethod(this.wsDataSourceGetConnectionMethod, obtainTargetDataSource(), connSpec);
     ...
     * <p>
     * The default implementation uses reflection to apply the given settings. Can be overridden in subclasses to
     * customize the JDBCConnectionSpec object (<a href=
     ...
     * javadoc</a>;
     * <a href="https://www.ibm.com/developerworks/websphere/library/techarticles/0404_tang/0404_tang.html">IBM
     * developerWorks article</a>).
     *
     * @param isolationLevel
     *            the isolation level to apply (or {@code null} if none)
     * @param readOnlyFlag
     *            the read-only flag to apply (or {@code null} if none)
     * @param username
     *            the username to apply ({@code null} or empty indicates the default)
     * @param password
     *            the password to apply (may be {@code null} or empty)
     * @throws SQLException
     *             if thrown by JDBCConnectionSpec API methods
     * @see com.ibm.websphere.rsadapter.JDBCConnectionSpec
     */
