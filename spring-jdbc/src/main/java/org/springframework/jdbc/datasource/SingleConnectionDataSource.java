...
/**
 * Implementation of {@link SmartDataSource} that wraps a single JDBC Connection which is not closed after use.
 ...
 *
 * <p>
 * If client code will call {@code close()} in the assumption of a pooled Connection, like when using persistence tools,
 * set "suppressClose" to "true". This will return a close-suppressing proxy instead of the physical Connection.
 *
 * <p>
 * This is primarily intended for testing. For example, it enables easy testing outside an application server, for code
 * that expects to work on a DataSource. In contrast to {@link DriverManagerDataSource}, it reuses the same Connection
...
public class SingleConnectionDataSource extends DriverManagerDataSource implements SmartDataSource, DisposableBean {
     ...
     * @param suppressClose
     *            if the returned Connection should be a close-suppressing proxy or the physical Connection
     ...
     * @param suppressClose
     *            if the Connection should be wrapped with a Connection that suppresses {@code close()} calls (to allow
     *            for normal {@code close()} usage in applications that expect a pooled Connection but do not know our
    ...
    /**
     * Specifying a custom username and password doesn't make sense with a single Connection. Returns the single
     * Connection if given the same username and password; throws an SQLException else.
     */
    @Override
