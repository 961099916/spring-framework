...
/**
 * Interface specifying the API for a Simple JDBC Call implemented by {@link SimpleJdbcCall}. This interface is not
 * often used directly, but provides the option to enhance testability, as it can easily be mocked or stubbed.
 *
...
public interface SimpleJdbcCallOperations {
     ...
     * <p>
     * To provide consistency with the Oracle DatabaseMetaData, this is used to specify the package name if the
     * procedure is declared as part of a package.
     * 
     * @param catalogName
     *            the catalog or package name
     * @return the instance of this SimpleJdbcCall
     */
    ...
    /**
     * Specify one or more parameters if desired. These parameters will be supplemented with any parameter information
     * retrieved from the database meta-data.
     * <p>
     * Note that only parameters declared as {@code SqlParameter} and {@code SqlInOutParameter} will be used to provide
     * input values. This is different from the {@code StoredProcedure} class which - for backwards compatibility
     * reasons - allows input values to be provided for parameters declared as {@code SqlOutParameter}.
     * 
     * @param sqlParameters
     ...
     * {@link RowMapper}. The results will be returned using the parameter name specified. Multiple ResultSets must be
     * declared in the correct order.
     * <p>
     * If the database you are using uses ref cursors then the name specified must match the name of the parameter
     * declared for the procedure in the database.
     * 
     * @param parameterName
     *            the name of the returned results and/or the name of the ref cursor parameter
     * @param rowMapper
     ...
     *            the type of the value to return
     * @param args
     *            a Map containing the parameter values to be used in the call
     */
     ...
     *            the type of the value to return
     * @param args
     *            the MapSqlParameterSource containing the parameter values to be used in the call
     */
    <T> T executeFunction(Class<T> returnType, SqlParameterSource args);

    /**
     * Execute the stored procedure and return the single out parameter as an Object of the specified return type. In
     * the case where there are multiple out parameters, the first one is returned and additional out parameters are
     * ignored.
     * 
     * @param returnType
     *            the type of the value to return
     ...
     * 
     * @param args
     *            the SqlParameterSource containing the parameter values to be used in the call
     * @return a Map of output params
     */
