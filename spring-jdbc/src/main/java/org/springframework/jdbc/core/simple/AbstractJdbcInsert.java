...
public abstract class AbstractJdbcInsert {
    ...
    /**
     * Has this operation been compiled? Compilation means at least checking that a DataSource or JdbcTemplate has been
     * provided.
     */
    ...
    /**
     * Compile this JdbcInsert using provided parameters and meta-data plus other settings. This finalizes the
     * configuration for this object and subsequent attempts to compile are ignored. This will be implicitly called the
     * first time an un-compiled insert is executed.
     * 
     ...
     * <p>
     * Subclasses can override this template method to perform their own compilation. Invoked after this base class's
    ...
    /**
     * Method to check whether we are allowed to make any configuration changes at this time. If the class has been
     ...
     * 
     * @param args
     *            a Map with parameter names and values to be used in insert
    ...
    /**
     * Method that provides execution of the insert using the passed-in {@link SqlParameterSource} and returning a
     * generated key.
     * 
     * @param parameterSource
     *            parameter names and values to be used in insert
    ...
    /**
     * Method that provides execution of the insert using the passed-in Map of parameters and returning all generated
     * keys.
     * 
     * @param args
     *            a Map with parameter names and values to be used in insert
    ...
    /**
     * Method that provides execution of the insert using the passed-in {@link SqlParameterSource} and returning all
     * generated keys.
     * 
     * @param parameterSource
     *            parameter names and values to be used in insert
    ...
    private KeyHolder executeInsertAndReturnKeyHolderInternal(final List<?> values) {
        ...
        if (this.tableMetaDataContext.isGetGeneratedKeysSupported()) {
            ...
            final String keyQuery =
                this.tableMetaDataContext.getSimpleQueryForGetGeneratedKey(getTableName(), getGeneratedKeyNames()[0]);
            ...
            if (keyQuery.toUpperCase().startsWith("RETURNING")) {
                Long key =
                    getJdbcTemplate().queryForObject(getInsertString() + " " + keyQuery, values.toArray(), Long.class);
    ...
    /**
     * Match the provided in parameter values with registered parameters and parameters defined via meta-data
     * processing.
     * 
     * @param parameterSource
     *            the parameter values provided as a {@link SqlParameterSource}
