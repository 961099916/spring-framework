 ...
 *
 * <p>
 * This is an alternative to the classic {@link org.springframework.jdbc.core.JdbcOperations} interface, implemented by
 * {@link NamedParameterJdbcTemplate}. This interface is not often used directly, but provides a useful option to
...
public interface NamedParameterJdbcOperations {
     ...
     * <p>
     * Note: In contrast to the JdbcOperations method with the same signature, this query variant always uses a
     * PreparedStatement. It is effectively equivalent to a query call with an empty parameter Map.
     * 
     * @param sql
     *            the SQL query to execute
     * @param rse
    ...
    /**
     * Query given SQL to create a prepared statement from SQL, reading the ResultSet on a per-row basis with a
     * RowCallbackHandler.
     * <p>
     * Note: In contrast to the JdbcOperations method with the same signature, this query variant always uses a
     * PreparedStatement. It is effectively equivalent to a query call with an empty parameter Map.
    ...
    /**
     * Query given SQL to create a prepared statement from SQL and a list of arguments to bind to the query, expecting a
     * result Map. The queryForMap() methods defined by this interface are appropriate when you don't have a domain
     * model. Otherwise, consider using one of the queryForObject() methods.
     * <p>
     * The query is expected to be a single row query; the result row will be mapped to a Map (one entry for each
     * column, using the column name as the key).
     * 
     ...
     *            a {@link KeyHolder} that will hold the generated keys
     * @return the number of rows affected
     * @throws DataAccessException
     *             if there is any problem issuing the update
     ...
     *            the SQL statement to execute
     * @param batchArgs
     *            the array of {@link SqlParameterSource} containing the batch of arguments for the query
     * @return an array containing the numbers of rows affected by each update in the batch (may also contain special
     *         JDBC-defined negative values for affected rows such as
     *         {@link java.sql.Statement#SUCCESS_NO_INFO}/{@link java.sql.Statement#EXECUTE_FAILED})
     * @throws DataAccessException
