...
/**
 * Object to represent an SQL BLOB/CLOB value parameter. BLOBs can either be an InputStream or a byte array. CLOBs can
 * be in the form of a Reader, InputStream or String. Each CLOB/BLOB value will be stored together with its length. The
 * type is based on which constructor is used. Objects of this class are immutable except for the LobCreator reference.
 * Use them and discard them.
 *
 * <p>
 * This class holds a reference to a LocCreator that must be closed after the update has completed. This is done via a
 * call to the closeLobCreator method. All handling of the LobCreator is done by the framework classes that use it - no
 * need to set or close the LobCreator for end users of this class.
 *
 * <p>
 * A usage example:
 *
 ...
 *
 * jdbcTemplate.update(
 *     "INSERT INTO imagedb (image_name, content, description) VALUES (?, ?, ?)", new Object[] {name,
 *         new SqlLobValue(contentStream, contentLength, lobHandler), new SqlLobValue(description, lobHandler)},
...
public class SqlLobValue implements DisposableSqlTypeValue {
     ...
     * 
     * @param bytes
     *            the byte array containing the BLOB value
     ...
     * 
     * @param content
     *            the String containing the CLOB value
     * @see org.springframework.jdbc.support.lob.DefaultLobHandler
     */
     ...
     * 
     * @param stream
     *            the stream containing the LOB value
     * @param length
     *            the length of the LOB value
     * @see org.springframework.jdbc.support.lob.DefaultLobHandler
     */
