...
/**
 * <b>This is the central class in the JDBC core package.</b> It simplifies the use of JDBC and helps to avoid common
 * errors. It executes core JDBC workflow, leaving application code to provide SQL and extract results. This class
 * executes SQL queries or updates, initiating iteration over ResultSets and catching JDBC exceptions and translating
 * them to the generic, more informative exception hierarchy defined in the {@code org.springframework.dao} package.
 *
 * <p>
 * Code using this class need only implement callback interfaces, giving them a clearly defined contract. The
 * {@link PreparedStatementCreator} callback interface creates a prepared statement given a Connection, providing SQL
 * and any necessary parameters. The {@link ResultSetExtractor} interface extracts values from a ResultSet. See also
 * {@link PreparedStatementSetter} and {@link RowMapper} for two popular alternative callback interfaces.
 *
 * <p>
 * Can be used within a service implementation via direct instantiation with a DataSource reference, or get prepared in
 * an application context and given to services as bean reference. Note: The DataSource should always be configured as a
 * bean in the application context, in the first case given to the service directly, in the second case to the prepared
 * template.
 *
 * <p>
 * Because this class is parameterizable by the callback interfaces and the
 * {@link org.springframework.jdbc.support.SQLExceptionTranslator} interface, there should be no need to subclass it.
 *
 * <p>
...
public class JdbcTemplate extends JdbcAccessor implements JdbcOperations {
    ...
    /**
     * If this variable is set to a non-negative value, it will be used for setting the fetchSize property on statements
     * used for query processing.
     */
    private int fetchSize = -1;

    /**
     * If this variable is set to a non-negative value, it will be used for setting the maxRows property on statements
     * used for query processing.
     */
    private int maxRows = -1;

    /**
     * If this variable is set to a non-negative value, it will be used for setting the queryTimeout property on
    ...
    /**
     * If this variable is set to true then all results from a stored procedure call that don't have a corresponding
     * SqlOutParameter declaration will be bypassed. All other results processing will be take place unless the variable
     * {@code skipResultsProcessing} is set to {@code true}.
     */
    private boolean skipUndeclaredResults = false;

    /**
     * If this variable is set to true then execution of a CallableStatement will return the results in a Map that uses
     ...
     *
     * @param sqlProvider
     *            object which is potentially an SqlProvider
     ...
     * <p>
     * Default is "true", swallowing and logging all warnings. Switch this flag to "false" to make the JdbcTemplate
    ...
    /**
     * Set the fetch size for this JdbcTemplate. This is important for processing large result sets: Setting this higher
     * than the default value will increase processing speed at the cost of memory consumption; setting this lower can
     * avoid transferring row data that will never be read by the application.
     * <p>
     * Default is -1, indicating to use the JDBC driver's default configuration (i.e. to not pass a specific fetch size
     * setting on to the driver).
     * <p>
     * Note: As of 4.3, negative values other than -1 will get passed on to the driver, since e.g. MySQL supports
     * special behavior for {@code Integer.MIN_VALUE}.
     *
    ...
    /**
     * Set the maximum number of rows for this JdbcTemplate. This is important for processing subsets of large result
     * sets, avoiding to read and hold the entire result set in the database or in the JDBC driver if we're never
     ...
     * <p>
     * Default is -1, indicating to use the JDBC driver's default configuration (i.e. to not pass a specific max rows
     * setting on to the driver).
     * <p>
     * Note: As of 4.3, negative values other than -1 will get passed on to the driver, in sync with
     * {@link #setFetchSize}'s support for special MySQL values.
     *
     ...
     * <p>
     * Default is -1, indicating to use the JDBC driver's default (i.e. to not pass a specific query timeout setting on
     * the driver).
     * <p>
     * Note: Any timeout specified here will be overridden by the remaining transaction timeout when executing within a
     * transaction that has a timeout specified at the transaction level.
     *
    ...
    /**
     * Set whether results processing should be skipped. Can be used to optimize callable statement processing when we
     * know that no results are being passed back - the processing of out parameter will still take place. This can be
     * used to avoid a bug in some older Oracle JDBC drivers like 10.1.0.2.
     */
    ...
    /**
     * Return whether execution of a CallableStatement will return the results in a Map that uses case insensitive names
     * for the parameters.
     */
    ...
    /**
     * Set whether execution of a CallableStatement will return the results in a Map that uses case insensitive names
     * for the parameters.
     */
     ...
     * <p>
     * The proxy also prepares returned JDBC Statements, applying statement settings such as fetch size, max rows, and
     * query timeout.
     *
     * @param con
    ...
    /**
     * Query using a prepared statement, allowing for a PreparedStatementCreator and a PreparedStatementSetter. Most
     * other query methods use this method, but application code will always work with either a creator or a setter.
     *
     * @param psc
     *            a callback that creates a PreparedStatement given a Connection
     * @param pss
     *            a callback that knows how to set values on the prepared statement. If this is {@code null}, the SQL
     *            will be assumed to contain no bind parameters.
     * @param rse
     *            a callback that will extract results
                    ...
                    if (batchSupported) {
                        ...
                        if (n % batchSize == 0 || n == batchArgs.size()) {
                            if (logger.isTraceEnabled()) {
                                ...
                                int items =
                                    n - ((n % batchSize == 0) ? n / batchSize - 1 : (n / batchSize)) * batchSize;
     ...
     *
     * @param cs
     *            a JDBC wrapper for the stored procedure
     * @param updateCountParameters
     *            the parameter list of declared update count parameters for the stored procedure
     * @param resultSetParameters
     *            the parameter list of declared resultSet parameters for the stored procedure
     ...
     *
     * @param cs
     *            the JDBC wrapper for the stored procedure
     * @param parameters
    ...
    protected Map<String, Object> extractOutputParameters(CallableStatement cs, List<SqlParameter> parameters)
        ...
        for (SqlParameter param : parameters) {
            if (param instanceof SqlOutParameter) {
                ...
                if (returnType != null) {
                    Object out =
                        returnType.getTypeValue(cs, sqlColIndex, outParam.getSqlType(), outParam.getTypeName());
     ...
     * <p>
     * If {@link #resultsMapCaseInsensitive} has been set to true, a {@link LinkedCaseInsensitiveMap} will be created;
     * otherwise, a {@link LinkedHashMap} will be created.
     *
     ...
     * <p>
     * By default, we'll create an {@link ArgumentPreparedStatementSetter}. This method allows for the creation to be
     ...
     * <p>
     * By default, we'll create an {@link ArgumentTypePreparedStatementSetter}. This method allows for the creation to
     ...
     *
     * @param warning
     *            the warnings object from the current statement. May be {@code null}, in which case this method does
     *            nothing.
     * @throws SQLWarningException
     ...
     * @param sql
     *            the SQL query or update that caused the problem (may be {@code null})
     * @param ex
