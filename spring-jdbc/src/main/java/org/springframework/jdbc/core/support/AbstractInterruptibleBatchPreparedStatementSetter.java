...
/**
 * Abstract implementation of the {@link InterruptibleBatchPreparedStatementSetter} interface, combining the check for
 * available values and setting of those into a single callback method {@link #setValuesIfAvailable}.
 *
...
public abstract class AbstractInterruptibleBatchPreparedStatementSetter
    ...
    /**
     * This implementation returns {@code Integer.MAX_VALUE}. Can be overridden in subclasses to lower the maximum batch
     * size.
     */
    @Override
    ...
    /**
     * Check for available values and set them on the given PreparedStatement. If no values are available anymore,
     * return {@code false}.
     * 
     * @param ps
     *            the PreparedStatement we'll invoke setter methods on
     * @param i
     *            index of the statement we're issuing in the batch, starting from 0
     * @return whether there were values to apply (that is, whether the applied parameters should be added to the batch
     *         and this method should be called for a further iteration)
     * @throws SQLException
