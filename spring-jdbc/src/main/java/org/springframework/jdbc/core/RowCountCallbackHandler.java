 ...
 *
 * <p>
 * We can either use this on its own (for example, in a test case, to ensure that our result sets have valid
 * dimensions), or use it as a superclass for callback handlers that actually do something, and will benefit from the
...
public class RowCountCallbackHandler implements RowCallbackHandler {
    ...
    /**
     * Implementation of ResultSetCallbackHandler. Work out column size if this is the first row, otherwise just count
     * rows.
     * <p>
     * Subclasses can perform custom extraction or processing by overriding the {@code processRow(ResultSet, int)}
     * method.
     * 
    ...
    /**
     * Subclasses may override this to perform custom extraction or processing. This class's implementation does
     * nothing.
     * 
     * @param rs
     *            the ResultSet to extract data from. This method is invoked for each row
     * @param rowNum
    ...
    /**
     * Return the number of columns in this result set. Valid once we've seen the first row, so subclasses can use it
     * during processing
     * 
