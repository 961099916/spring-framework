 ...
 * on a {@code java.sql.ResultSet} that just contains a single column.
 *
 * <p>
 * The type of the result value for each row can be specified. The value for the single column will be extracted from
 * the {@code ResultSet} and converted into the specified target type.
 *
...
public class SingleColumnRowMapper<T> implements RowMapper<T> {
     ...
     *            the {@link ConversionService} for converting a fetched value, or {@code null} for none
     * @since 5.0.4
     ...
     * <p>
     * The default implementation calls {@link JdbcUtils#getResultSetValue(java.sql.ResultSet, int, Class)}. If no
     * required type has been specified, this method delegates to {@code getColumnValue(rs, index)}, which basically
     * calls {@code ResultSet.getObject(index)} but applies some additional default conversion to appropriate value
     * types.
     *
     * @param rs
     ...
     *            is the column index
     * @param requiredType
     *            the type that each result object is expected to match (or {@code null} if none specified)
     ...
     * <p>
     * The default implementation delegates to {@code JdbcUtils.getResultSetValue()}, which uses the
     * {@code ResultSet.getObject(index)} method. Additionally, it includes a "hack" to get around Oracle returning a
     * non-standard object for their TIMESTAMP datatype. See the {@code JdbcUtils#getResultSetValue()} javadoc for
     * details.
     *
     * @param rs
     *            is the ResultSet holding the data
     ...
     * match already.
     * <p>
     * If the required type is String, the value will simply get stringified via {@code toString()}. In case of a
     * Number, the value will be converted into a Number, either through number conversion or through String parsing
     * (depending on the value type). Otherwise, the value will be converted to a required type using the
     * {@link ConversionService}.
     *
     * @param value
     *            the column value as extracted from {@code getColumnValue()} (never {@code null})
     * @param requiredType
     *            the type that each result object is expected to match (never {@code null})
