...
/**
 * Object to represent an SQL parameter value, including parameter meta-data such as the SQL type and the scale for
 * numeric values.
 *
 * <p>
 * Designed for use with {@link JdbcTemplate}'s operations that take an array of argument values: Each such argument
 * value may be a {@code SqlParameterValue}, indicating the SQL type (and optionally the scale) instead of letting the
 * template guess a default type. Note that this only applies to the operations with a 'plain' argument array, not to
 * the overloaded variants with an explicit type array.
 *
...
public class SqlParameterValue extends SqlParameter {
     ...
     * 
     * @param sqlType
     *            the SQL type of the parameter according to {@code java.sql.Types}
     * @param value
     *            the value object
     */
