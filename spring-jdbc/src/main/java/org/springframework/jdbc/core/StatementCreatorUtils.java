...
public abstract class StatementCreatorUtils {
    ...
    /**
     * System property that instructs Spring to ignore {@link java.sql.ParameterMetaData#getParameterType} completely,
     * i.e. to never even attempt to retrieve {@link PreparedStatement#getParameterMetaData()} for
     ...
     * {@link PreparedStatement#setNull} / {@link PreparedStatement#setObject} calls based on well-known behavior of
     * common databases.
     * <p>
     * Consider switching this flag to "true" if you experience misbehavior at runtime, e.g. with connection pool issues
     * in case of an exception thrown from {@code getParameterType} (as reported on JBoss AS 7) or in case of
     * performance problems (as reported on PostgreSQL).
     */
     ...
     * @param inValue
     *            the value to set (plain value or an SqlTypeValue)
     * @throws SQLException
     *             if thrown by PreparedStatement methods
     * @see SqlTypeValue
     */
    ...
    private static boolean isDateValue(Class<?> inValueType) {
        return (java.util.Date.class.isAssignableFrom(inValueType)
            && !(java.sql.Date.class.isAssignableFrom(inValueType) || java.sql.Time.class.isAssignableFrom(inValueType)
                || java.sql.Timestamp.class.isAssignableFrom(inValueType)));
    }

    /**
     * Clean up all resources held by parameter values which were passed to an execute method. This is for example
     * important for closing LOB values.
     * 
     * @param paramValues
     *            parameter values supplied. May be {@code null}.
