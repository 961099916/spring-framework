...
/**
 * Interface specifying a basic set of JDBC operations. Implemented by {@link JdbcTemplate}. Not often used directly,
 * but a useful option to enhance testability, as it can easily be mocked or stubbed.
 *
 * <p>
 * Alternatively, the standard JDBC infrastructure can be mocked. However, mocking this interface constitutes
...
public interface JdbcOperations {
    ...
    /**
     * Execute a JDBC data access operation, implemented as callback action working on a JDBC Connection. This allows
     * for implementing arbitrary data access operations, within Spring's managed JDBC environment: that is,
     * participating in Spring-managed transactions and converting JDBC SQLExceptions into Spring's DataAccessException
     * hierarchy.
     * <p>
     * The callback action can return a result object, for example a domain object or a collection of domain objects.
     * 
     * @param action
     *            a callback object that specifies the action
    ...
    /**
     * Execute a JDBC data access operation, implemented as callback action working on a JDBC Statement. This allows for
     * implementing arbitrary data access operations on a single Statement, within Spring's managed JDBC environment:
     * that is, participating in Spring-managed transactions and converting JDBC SQLExceptions into Spring's
     * DataAccessException hierarchy.
     * <p>
     * The callback action can return a result object, for example a domain object or a collection of domain objects.
     * 
     * @param action
     *            a callback that specifies the action
     * @return a result object returned by the action, or {@code null} if none
     * @throws DataAccessException
     *             if there is any problem
     */
     ...
     * @param rse
     *            a callback that will extract all rows of results
     ...
     *            the SQL query to execute
     * @param rch
     *            a callback that will extract results, one row at a time
     * @throws DataAccessException
     *             if there is any problem executing the query
     ...
     * <p>
     * Uses a JDBC Statement, not a PreparedStatement. If you want to execute a static query with a PreparedStatement,
     * use the overloaded {@code queryForList} method with {@code null} as argument array.
     * <p>
     ...
     *            the SQL query to execute
     * @param elementType
     *            the required type of element in the result list (for example, {@code Integer.class})
     ...
     * name as the key). Each element in the list will be of the form returned by this interface's {@code queryForMap}
     * methods.
     * 
     * @param sql
     *            the SQL query to execute
     ...
     * <p>
     * Uses a JDBC Statement, not a PreparedStatement. If you want to execute a static query with a PreparedStatement,
     * use the overloaded {@code queryForRowSet} method with {@code null} as argument array.
     * <p>
     * The results will be mapped to an SqlRowSet which holds the data in a disconnected fashion. This wrapper will
     * translate any SQLExceptions thrown.
     * <p>
     * Note that, for the default implementation, JDBC RowSet support needs to be available at runtime: by default,
     * Sun's {@code com.sun.rowset.CachedRowSetImpl} class is used, which is part of JDK 1.5+ and also available
     * separately as part of Sun's JDBC RowSet Implementations download (rowset.jar).
     * 
     * @param sql
     *            the SQL query to execute
     * @return an SqlRowSet representation (possibly a wrapper around a {@code javax.sql.rowset.CachedRowSet})
     * @throws DataAccessException
     *             if there is any problem executing the query
     ...
     * <p>
     * A PreparedStatementCreator can either be implemented directly or configured through a
     ...
     *            will be assumed to contain no bind parameters. Even if there are no bind parameters, this callback may
     *            be used to set the fetch size and other performance options.
     * @param rse
     *            a callback that will extract results
     * @return an arbitrary result object, as returned by the ResultSetExtractor
     * @throws DataAccessException
     ...
     * @param argTypes
     *            the SQL types of the arguments (constants from {@code java.sql.Types})
     * @param rse
     *            a callback that will extract results
     * @return an arbitrary result object, as returned by the ResultSetExtractor
     * @throws DataAccessException
     ...
     *            the SQL query to execute
     * @param args
     *            arguments to bind to the query (leaving it to the PreparedStatement to guess the corresponding SQL
     *            type); may also contain {@link SqlParameterValue} objects which indicate not only the argument value
     *            but also the SQL type and optionally the scale
     * @param rse
     *            a callback that will extract results
     * @return an arbitrary result object, as returned by the ResultSetExtractor
     * @throws DataAccessException
    ...
    /**
     * Query given SQL to create a prepared statement from SQL and a PreparedStatementSetter implementation that knows
     * how to bind values to the query, reading the ResultSet on a per-row basis with a RowCallbackHandler.
     * 
     * @param sql
     *            the SQL query to execute
     * @param pss
    ...
    /**
     * Query given SQL to create a prepared statement from SQL and a PreparedStatementSetter implementation that knows
     * how to bind values to the query, mapping each row to a result object via a RowMapper.
     * 
     * @param sql
     *            the SQL query to execute
     * @param pss
     ...
     * single result row to a result object via a RowMapper.
     * 
     * @param sql
     *            the SQL query to execute
     * @param args
     ...
     * result map.
     * <p>
     * The {@code queryForMap} methods defined by this interface are appropriate when you don't have a domain model.
     * Otherwise, consider using one of the {@code queryForObject} methods.
     * <p>
     * The query is expected to be a single row query; the result row will be mapped to a Map (one entry for each
     * column, using the column name as the key).
     * 
     ...
     * result list.
     * <p>
     * The results will be mapped to a List (one entry for each row) of Maps (one entry for each column, using the
     * column name as the key). Each element in the list will be of the form returned by this interface's
    ...
    /**
     * Query given SQL to create a prepared statement from SQL and a list of arguments to bind to the query, expecting
     * an SqlRowSet.
     * <p>
     * The results will be mapped to an SqlRowSet which holds the data in a disconnected fashion. This wrapper will
     * translate any SQLExceptions thrown.
     ...
     * 
     * @param psc
     *            a callback that provides SQL and any necessary parameters
     * @return the number of rows affected
     * @throws DataAccessException
     ...
     * keys will be put into the given KeyHolder.
     * <p>
     ...
     *            a KeyHolder that will hold the generated keys
     * @return the number of rows affected
     * @throws DataAccessException
     *             if there is any problem issuing the update
     * @see PreparedStatementCreatorFactory
     ...
     * 
     * @param sql
     *            the SQL containing bind parameters
     * @param pss
     *            helper that sets bind parameters. If this is {@code null} we run an update with static SQL.
     * @return the number of rows affected
     * @throws DataAccessException
     *             if there is any problem issuing the update
     */
     ...
     * <p>
     * Will fall back to separate updates on a single PreparedStatement if the JDBC driver does not support batch
     * updates.
     * 
     * @param sql
     ...
     * @return an array of the number of rows affected by each statement (may also contain special JDBC-defined negative
     *         values for affected rows such as
     ...
     * values will be set using the ParameterizedPreparedStatementSetter. Each batch should be of size indicated in
     * 'batchSize'.
     * 
     * @param sql
     *            the SQL statement to execute.
     ...
     *            batch size
     * @param pss
     *            the ParameterizedPreparedStatementSetter to use
     * @return an array containing for each batch another array containing the numbers of rows affected by each update
     *         in the batch (may also contain special JDBC-defined negative values for affected rows such as
     *         {@link java.sql.Statement#SUCCESS_NO_INFO}/{@link java.sql.Statement#EXECUTE_FAILED})
     * @throws DataAccessException
     *             if there is any problem issuing the update
     * @since 3.1
    ...
    /**
     * Execute a JDBC data access operation, implemented as callback action working on a JDBC CallableStatement. This
     * allows for implementing arbitrary data access operations on a single Statement, within Spring's managed JDBC
     * environment: that is, participating in Spring-managed transactions and converting JDBC SQLExceptions into
     * Spring's DataAccessException hierarchy.
     * <p>
     ...
     * 
     * @param csc
     *            a callback that creates a CallableStatement given a Connection
     * @param action
     *            a callback that specifies the action
     * @return a result object returned by the action, or {@code null} if none
     * @throws DataAccessException
