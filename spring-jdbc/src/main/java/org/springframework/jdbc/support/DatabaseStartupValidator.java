...
/**
 * Bean that checks if a database has already started up. To be referenced via "depends-on" from beans that depend on
 * database startup, like a Hibernate SessionFactory or custom data access objects that access a DataSource directly.
 *
 * <p>
 * Useful to defer application initialization until a database has started up. Particularly appropriate for waiting on a
 * slowly starting Oracle database.
 *
...
public class DatabaseStartupValidator implements InitializingBean {
    ...
    /**
     * Check whether the validation query can be executed on a Connection from the specified DataSource, with the
     * specified interval between checks, until the specified timeout.
     */
    @Override
