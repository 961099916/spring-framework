...
/**
 * Base class for {@link SQLExceptionTranslator} implementations that allow for fallback to some other
...
public abstract class AbstractFallbackSQLExceptionTranslator implements SQLExceptionTranslator {
     ...
     * <p>
     * The passed-in arguments will have been pre-checked. Furthermore, this method is allowed to return {@code null} to
     * indicate that no exception match has been found and that fallback translation should kick in.
     * 
     * @param task
     ...
     * @return the DataAccessException, wrapping the {@code SQLException}; or {@code null} if no exception match found
     */
    @Nullable
     ...
     *            readable text describing the task being attempted
     * @param sql
     *            the SQL statement that caused the problem
     * @param ex
     *            the offending {@code SQLException}
