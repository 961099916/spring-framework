...
/**
 * {@link DataFieldMaxValueIncrementer} that increments the maximum value of a given MySQL table with the equivalent of
 * an auto-increment column. Note: If you use this class, your MySQL key column should <i>NOT</i> be auto-increment, as
 * the sequence table does the job.
 *
 * <p>
 * The sequence is kept in a table; there should be one sequence table per table that needs an auto-generated key. The
 * storage engine used by the sequence table can be MYISAM or INNODB since the sequences are allocated using a separate
 ...
 * </pre>
 *
 * If "cacheSize" is set, the intermediate values are served without querying the database. If the server or your
 * application is stopped or crashes or a transaction is rolled back, the unused values will never be served. The
 * maximum hole size in numbering is consequently the value of cacheSize.
 *
 * <p>
 * It is possible to avoid acquiring a new connection for the incrementer by setting the "useNewConnection" property to
 * false. In this case you <i>MUST</i> use a non-transactional storage engine like MYISAM when defining the incrementer
 * table.
 *
...
public class MySQLMaxValueIncrementer extends AbstractColumnMaxValueIncrementer {
     ...
     * <p>
     * {@code true} is necessary to support transactional storage engines, using an isolated separate transaction for
     * the increment operation. {@code false} is sufficient if the storage engine of the sequence table is
     * non-transactional (like MYISAM), avoiding the effort of acquiring an extra {@code Connection} for the increment
     * operation.
     * <p>
            ...
            try {
                ...
                try {
                    ...
                    throw new DataAccessResourceFailureException(
                        "Could not increment " + columnName + " for " + getIncrementerName() + " sequence table", ex);
                }
