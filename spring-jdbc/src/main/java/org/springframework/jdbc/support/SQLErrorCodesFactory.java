 ...
 *
 * <p>
 * Returns {@code SQLErrorCodes} populated with vendor codes defined in a configuration file named
 * "sql-error-codes.xml". Reads the default file in this package if not overridden by a file in the root of the class
 * path (for example in the "/WEB-INF/classes" directory).
 *
...
public class SQLErrorCodesFactory {
    ...
    /**
     * Map to hold error codes for all databases defined in the config file. Key is the database product name, value is
     * the SQLErrorCodes instance.
     */
     ...
     * <p>
     * Not public to enforce Singleton design pattern. Would be private except to allow testing via overriding the
     ...
     * <p>
     * <b>Not to be overridden by application developers, who should obtain instances of this class from the static
     * {@link #getInstance()} method.</b>
     * <p>
     * Protected for testability.
     * 
     * @param path
     *            resource path; either a custom path or one of either {@link #SQL_ERROR_CODE_DEFAULT_PATH} or
     ...
     * 
     * @param databaseName
     *            the database name (must not be {@code null})
     * @return the {@code SQLErrorCodes} instance for the given database (never {@code null}; potentially empty)
     * @throws IllegalArgumentException
     *             if the supplied database name is {@code null}
     */
    ...
    /**
     * Return {@link SQLErrorCodes} for the given {@link DataSource}, evaluating "databaseProductName" from the
     * {@link java.sql.DatabaseMetaData}, or an empty error codes instance if no {@code SQLErrorCodes} were found.
     * 
     * @param dataSource
     *            the {@code DataSource} identifying the database
     * @return the corresponding {@code SQLErrorCodes} object (never {@code null}; potentially empty)
     * @see java.sql.DatabaseMetaData#getDatabaseProductName()
     */
    ...
    /**
     * Return {@link SQLErrorCodes} for the given {@link DataSource}, evaluating "databaseProductName" from the
     * {@link java.sql.DatabaseMetaData}, or {@code null} if case of a JDBC meta-data access problem.
     * 
     * @param dataSource
     *            the {@code DataSource} identifying the database
     * @return the corresponding {@code SQLErrorCodes} object, or {@code null} in case of a JDBC meta-data access
     *         problem
     * @since 5.2.9
     * @see java.sql.DatabaseMetaData#getDatabaseProductName()
     */
     ...
     *            the {@code DataSource} identifying the database
     * @param databaseName
     *            the corresponding database name as stated in the error codes definition file (must not be
     *            {@code null})
     ...
     * @param dataSource
     *            the {@code DataSource} identifying the database
     * @return the corresponding {@code SQLErrorCodes} object that got removed, or {@code null} if not registered
     * @since 4.3.5
    ...
    /**
     * Build an identification String for the given {@link DataSource}, primarily for logging purposes.
     * 
     * @param dataSource
     *            the {@code DataSource} to introspect
