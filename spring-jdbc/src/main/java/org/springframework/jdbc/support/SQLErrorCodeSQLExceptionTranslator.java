...
/**
 * Implementation of {@link SQLExceptionTranslator} that analyzes vendor-specific error codes. More precise than an
 ...
 * meta-data.
 * <li>Fallback to a fallback translator. {@link SQLStateSQLExceptionTranslator} is the default fallback translator,
 * analyzing the exception's SQL state only. On Java 6 which introduces its own {@code SQLException} subclass hierarchy,
 * we will use {@link SQLExceptionSubclassTranslator} by default, which in turns falls back to Spring's own SQL state
 * translation when not encountering specific subclasses.
 * </ul>
 *
 * <p>
 * The configuration file named "sql-error-codes.xml" is by default read from this package. It can be overridden through
 * a file of the same name in the root of the class path (e.g. in the "/WEB-INF/classes" directory), as long as the
...
public class SQLErrorCodeSQLExceptionTranslator extends AbstractFallbackSQLExceptionTranslator {
    ...
    /**
     * Create an SQL error code translator for the given DataSource. Invoking this constructor will cause a Connection
     * to be obtained from the DataSource to get the meta-data.
     * 
     * @param dataSource
     *            the DataSource to use to find meta-data and establish which error codes are usable
     * @see SQLErrorCodesFactory
     */
    ...
    /**
     * Create an SQL error code translator for the given database product name. Invoking this constructor will avoid
    ...
    /**
     * Create an SQLErrorCode translator given these error codes. Does not require a database meta-data lookup to be
     ...
     * @return {@code null} if no custom translation applies, otherwise a {@link DataAccessException} resulting from
     *         custom translation. This exception should include the {@code sqlEx} parameter as a nested root cause.
     *         This implementation always returns {@code null}, meaning that the translator always falls back to the
     *         default error codes.
     */
    @Nullable
    ...
    /**
     * Create a custom {@link DataAccessException}, based on a given exception class from a
     ...
     *            the exception class to use, as defined in the {@link CustomSQLErrorCodesTranslation} definition
     * @return {@code null} if the custom exception could not be created, otherwise the resulting
     *         {@link DataAccessException}. This exception should include the {@code sqlEx} parameter as a nested root
     *         cause.
        ...
        try {
            ...
            for (Constructor<?> constructor : constructors) {
                ...
                if (parameterTypes.length == 3 && String.class == parameterTypes[0] && String.class == parameterTypes[1]
                    && Throwable.class == parameterTypes[2] && constructorType < MESSAGE_SQL_THROWABLE_CONSTRUCTOR) {
