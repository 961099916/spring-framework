...
/**
 * Generic utility methods for working with JDBC. Mainly for internal use within the framework, but also useful for
...
public abstract class JdbcUtils {
     ...
     * or Clob object but rather a byte array or String representation, respectively.
     * <p>
     ...
     * <p>
     * This method will open a connection to the database and retrieve its meta-data. Since this method is called before
     * the exception translation feature is configured for a DataSource, this method can not rely on SQLException
     * translation itself.
     * <p>
     * Any exceptions will be wrapped in a MetaDataAccessException. This is a checked exception and any calling code
     * should catch and handle this exception. You can just log the error and hope for the best, but there is probably a
     * more serious error that will reappear when you try to access the database again.
     * 
     * @param dataSource
     *            the DataSource to extract meta-data for
     * @param action
     *            callback that will do the actual work
     * @return object containing the extracted information, as returned by the DatabaseMetaDataCallback's
     *         {@code processMetaData} method
     * @throws MetaDataAccessException
     *             if meta-data access failed
     ...
     * @throws MetaDataAccessException
     *             if we couldn't access the DatabaseMetaData or failed to invoke the specified method
     * @see java.sql.DatabaseMetaData
     * @deprecated as of 5.2.9, in favor of {@link #extractDatabaseMetaData(DataSource, DatabaseMetaDataCallback)} with
     *             a lambda expression or method reference and a generically typed result
     */
    @Deprecated
    ...
    public static <T> T extractDatabaseMetaData(DataSource dataSource, final String metaDataMethodName)
        ...
        return (T)extractDatabaseMetaData(dataSource, dbmd -> {
            try {
                ...
                throw new MetaDataAccessException(
                    "No method named '" + metaDataMethodName + "' found on DatabaseMetaData instance [" + dbmd + "]",
                    ex);
     ...
     * <p>
     * Logs a warning if the "supportsBatchUpdates" methods throws an exception and simply returns {@code false} in that
     * case.
     * 
     * @param con
    ...
    /**
     * Extract a common name for the target database in use even if various drivers/platforms provide varying names at
     * runtime.
     * 
     * @param source
     *            the name as provided in database meta-data
     ...
     * 
     * @param sqlType
     *            the SQL type to resolve
     * @return the corresponding constant name in {@link java.sql.Types} (e.g. "VARCHAR"/"NUMERIC"), or {@code null} if
     *         not resolvable
     * @since 5.2
     */
    @Nullable
     ...
     * <p>
     * <i>columnLabel - the label for the column specified with the SQL AS clause. If the SQL AS clause was not
     * specified, then the label is the name of the column</i>.
     * 
