...
/**
 * SQL "function" wrapper for a query that returns a single row of results. The default behavior is to return an int,
 * but that can be overridden by using the constructor with an extra return type parameter.
 *
 * <p>
 * Intended to use to call SQL functions that return a single result using a query like "select user()" or "select
 * sysdate from dual". It is not intended for calling more complex stored functions or for using a CallableStatement to
 * invoke a stored procedure or stored function. Use StoredProcedure or SqlCall for this type of processing.
 *
 * <p>
 * This is a concrete class, which there is often no need to subclass. Code using this package can create an object of
 * this type, declaring SQL and parameters, and then invoke the appropriate {@code run} method repeatedly to execute the
 * function. Subclasses are only supposed to add specialized {@code run} methods for specific parameter and return
 * types.
 *
 * <p>
...
public class SqlFunction<T> extends MappingSqlQuery<T> {
    ...
    /**
     * Constructor to allow use as a JavaBean. A DataSource, SQL and any parameters must be supplied before invoking the
     * {@code compile} method and using this object.
     * 
     * @see #setDataSource
     * @see #setSql
     * @see #compile
     ...
     *            the SQL types of the parameters, as defined in the {@code java.sql.Types} class
     * @param resultType
     *            the type that the result object is required to match
    ...
    /**
     * This implementation of this method extracts a single value from the single row returned by the function. If there
     * are a different number of rows returned, this is treated as an error.
     */
    @Override
    @Nullable
    ...
    /**
     * Analogous to the SqlQuery.execute([]) method. This is a generic method to execute a query, taken a number of
     * arguments.
     * 
     * @param parameters
     *            array of parameters. These will be objects or object wrapper types for primitives.
     * @return the value of the function
     */
    ...
    /**
     * Analogous to the {@code SqlQuery.findObject(Object[])} method. This is a generic method to execute a query, taken
