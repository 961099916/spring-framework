 ...
 *
 * <p>
 * Subclasses must implement the {@link #newRowMapper} method to provide an object that can extract the results of
 * iterating over the {@code ResultSet} created during the execution of the query.
 *
 * <p>
 * This class provides a number of public {@code execute} methods that are analogous to the different convenient JDO
 * query execute methods. Subclasses can either rely on one of these inherited methods, or can add their own custom
 * execution methods, with meaningful names and typed parameters (definitely a best practice). Each custom query method
 * will invoke one of this class's untyped query methods.
 *
 * <p>
 * Like all {@code RdbmsOperation} classes that ship with the Spring Framework, {@code SqlQuery} instances are
 * thread-safe after their initialization is complete. That is, after they are constructed and configured via their
...
public abstract class SqlQuery<T> extends SqlOperation {
     ...
     * 
     * @param ds
     *            the {@code DataSource} to use to get connections
     * @param sql
     *            the SQL to execute; SQL can also be supplied at runtime by overriding the {@link #getSql()} method.
     */
     ...
     * <p>
     * This can be used to ensure efficient storage of results. The default behavior is not to expect any specific
     * number of rows.
     */
     ...
     * 
     * @param params
     *            parameters, similar to JDO query parameters. Primitive parameters must be represented by their Object
     *            wrapper type. The ordering of parameters is significant.
     * @param context
     *            the contextual information passed to the {@code mapRow} callback method. The JDBC operation itself
     *            doesn't rely on this parameter, but it can be useful for creating the objects of the result list.
     * @return a List of objects, one per row of the ResultSet. Normally all these will be of the same class, although
     ...
     * 
     * @param params
     *            parameters for the query. Primitive parameters must be represented by their Object wrapper type. The
     *            ordering of parameters is significant.
     */
     ...
     * 
     * @param context
     *            the contextual information for object creation
     */
     ...
     * 
     * @param paramMap
     *            parameters associated with the name specified while declaring the SqlParameters. Primitive parameters
     *            must be represented by their Object wrapper type. The ordering of parameters is not significant since
     *            they are supplied in a SqlParameterMap which is an implementation of the Map interface.
     * @param context
     *            the contextual information passed to the {@code mapRow} callback method. The JDBC operation itself
     *            doesn't rely on this parameter, but it can be useful for creating the objects of the result list.
     * @return a List of objects, one per row of the ResultSet. Normally all these will be of the same class, although
     ...
     * @param paramMap
     *            parameters associated with the name specified while declaring the SqlParameters. Primitive parameters
     *            must be represented by their Object wrapper type. The ordering of parameters is not significant.
     */
    ...
    /**
     * Generic object finder method, used by all other {@code findObject} methods. Object finder methods are like EJB
     * entity bean finders, in that it is considered an error if they return more than one result.
     * 
     * @return the result object, or {@code null} if not found. Subclasses may choose to treat this as an error and
     *         throw an exception.
     ...
     * 
     * @param paramMap
     *            a Map of parameter name to parameter object, matching named parameters specified in the SQL statement.
    ...
    /**
     * Subclasses must implement this method to extract an object per row, to be returned by the {@code execute} method
     * as an aggregated {@link List}.
     * 
     * @param parameters
     *            the parameters to the {@code execute()} method, in case subclass is interested; may be {@code null} if
