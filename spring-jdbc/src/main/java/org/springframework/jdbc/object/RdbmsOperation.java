...
/**
 * An "RDBMS operation" is a multi-threaded, reusable object representing a query, update, or stored procedure call. An
 * RDBMS operation is <b>not</b> a command, as a command is not reusable. However, execute methods may take commands as
 * arguments. Subclasses should be JavaBeans, allowing easy configuration.
 *
 * <p>
 * This class and subclasses throw runtime exceptions, defined in the {@code org.springframework.dao} package (and as
 * thrown by the {@code org.springframework.jdbc.core} package, which the classes in this package use under the hood to
 * perform raw JDBC operations).
 *
 * <p>
 * Subclasses should set SQL and add parameters before invoking the {@link #compile()} method. The order in which
 * parameters are added is significant. The appropriate {@code execute} or {@code update} method can then be invoked.
 *
 * @author Rod Johnson
...
public abstract class RdbmsOperation implements InitializingBean {
    ...
    private String sql;
    /**
     * Has this operation been compiled? Compilation means at least checking that a DataSource and sql have been
     ...
     * {@link JdbcTemplate} in multiple {@code RdbmsOperations}. This is appropriate if the {@code JdbcTemplate} has
     * special configuration such as a {@link org.springframework.jdbc.support.SQLExceptionTranslator} to be reused.
     */
    ...
    /**
     * Set the fetch size for this RDBMS operation. This is important for processing large result sets: Setting this
    ...
    /**
     * Set the maximum number of rows for this RDBMS operation. This is important for processing subsets of large result
     * sets, avoiding to read and hold the entire result set in the database or in the JDBC driver.
     * <p>
     * Default is -1, indicating to use the driver's default.
     * 
     ...
     * {@link #setSql} method or in a subclass constructor.
     */
    @Nullable
    ...
    /**
     * Add anonymous parameters, specifying only their SQL types as defined in the {@code java.sql.Types} class.
     * <p>
     * Parameter ordering is significant. This method is an alternative to the {@link #declareParameter} method, which
     * should normally be preferred.
     * 
     * @param types
     *            array of SQL types as defined in the {@code java.sql.Types} class
     ...
     * <p>
     * The order in which this method is called is significant when using positional parameters. It is not significant
     * when using named parameters with named SqlParameter objects here; it remains significant when using named
     * parameters in combination with unnamed SqlParameter objects here.
     * 
     * @param param
     *            the SqlParameter to add. This will specify SQL type and (optionally) the parameter's name. Note that
     *            you typically use the {@link SqlParameter} class itself here, not any of its subclasses.
     * @throws InvalidDataAccessApiUsageException
    ...
    /**
     * Add one or more declared parameters. Used for configuring this operation when used in a bean factory. Each
     * parameter will specify SQL type and (optionally) the parameter's name.
     * 
     * @param parameters
     *            an array containing the declared {@link SqlParameter} objects
    ...
    /**
     * Is this operation "compiled"? Compilation, as in JDO, means that the operation is fully configured, and ready to
     * use. The exact meaning of compilation will vary between subclasses.
     * 
    ...
    /**
     * Validate the parameters passed to an execute method based on declared parameters. Subclasses should invoke this
     * method before every {@code executeQuery()} or {@code update()} method.
     * 
     * @param parameters
     *            the parameters supplied (may be {@code null})
     * @throws InvalidDataAccessApiUsageException
    ...
    /**
     * Validate the named parameters passed to an execute method based on declared parameters. Subclasses should invoke
     * this method before every {@code executeQuery()} or {@code update()} method.
     * 
     * @param parameters
     *            parameter Map supplied (may be {@code null})
     * @throws InvalidDataAccessApiUsageException
     *             if the parameters are invalid
     */
    ...
    /**
     * Subclasses must implement this template method to perform their own compilation. Invoked after this base class's
    ...
    /**
     * Return whether this operation accepts additional parameters that are given but not actually used. Applies in
     * particular to parameter Maps.
     * <p>
