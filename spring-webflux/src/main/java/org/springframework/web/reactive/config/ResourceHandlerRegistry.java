...
/**
 * Stores registrations of resource handlers for serving static resources such as images, css files and others through
 * Spring WebFlux including setting cache headers optimized for efficient loading in a web browser. Resources can be
 * served out of locations under web application root, from the classpath, and others.
 *
 * <p>
 * To create a resource handler, use {@link #addResourceHandler(String...)} providing the URL path patterns for which
 * the handler should be invoked to serve static resources (e.g. {@code "/resources/**"}).
 *
 * <p>
 * Then use additional methods on the returned {@link ResourceHandlerRegistration} to add one or more locations from
 * which to serve static content from (e.g. {{@code "/"}, {@code "classpath:/META-INF/public-web-resources/"}}) or to
 * specify a cache period for served resources.
 *
...
public class ResourceHandlerRegistry {
     ...
     *            the resource URL provider to use
     * @since 5.0.11
     */
    ...
    /**
     * Add a resource handler for serving static resources based on the specified URL path patterns. The handler will be
     * invoked for every incoming request that matches to one of the specified path patterns.
     * <p>
     * Patterns like {@code "/static/**"} or {@code "/css/{filename:\\w+\\.css}"} are allowed. See
    ...
    /**
     * Specify the order to use for resource handling relative to other {@code HandlerMapping}s configured in the Spring
     * configuration.
     * <p>
