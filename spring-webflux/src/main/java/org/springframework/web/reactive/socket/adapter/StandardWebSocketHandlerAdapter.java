...
/**
 * Adapter for Java WebSocket API (JSR-356) that delegates events to a reactive {@link WebSocketHandler} and its
 * session.
 *
...
public class StandardWebSocketHandlerAdapter extends Endpoint {
    ...
    public void onOpen(Session session, EndpointConfig config) {
        ...
        this.delegateHandler.handle(this.delegateSession)
            .checkpoint(session.getRequestURI() + " [StandardWebSocketHandlerAdapter]").subscribe(this.delegateSession);
    }
