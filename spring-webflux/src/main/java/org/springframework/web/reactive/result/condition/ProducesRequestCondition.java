...
/**
 * A logical disjunction (' || ') request condition to match a request's 'Accept' header to a list of media type
 * expressions. Two kinds of media type expressions are supported, which are described in
 * {@link RequestMapping#produces()} and {@link RequestMapping#headers()} where the header name is 'Accept'. Regardless
 * of which syntax is used, the semantics are the same.
 *
...
public final class ProducesRequestCondition extends AbstractRequestCondition<ProducesRequestCondition> {
    ...
    /**
     * Creates a new instance from "produces" expressions. If 0 expressions are provided in total, this condition will
     * match to any request.
     * 
     * @param produces
     *            expressions with syntax defined by {@link RequestMapping#produces()}
     */
    ...
    /**
     * Creates a new instance with "produces" and "header" expressions. "Header" expressions where the header name is
     * not 'Accept' or have no header value defined are ignored. If 0 expressions are provided in total, this condition
     * will match to any request.
     * 
     * @param produces
     *            expressions with syntax defined by {@link RequestMapping#produces()}
     * @param headers
     *            expressions with syntax defined by {@link RequestMapping#headers()}
     */
    ...
    /**
     * Private constructor for internal use to create matching conditions. Note the expressions List is neither sorted,
     * nor deep copied.
     */
    ...
    /**
     * Use this to clear {@link #MEDIA_TYPES_ATTRIBUTE} that contains the parsed, requested media types.
     *
     * @param exchange
    ...
    /**
     * Returns the "other" instance if it has any expressions; returns "this" instance otherwise. Practically that means
     * a method-level "produces" overrides a type-level "produces" condition.
     */
    @Override
    ...
    /**
     * Checks if any of the contained media type expressions match the given request 'Content-Type' header and returns
     * an instance that is guaranteed to contain matching expressions only. The match is performed via
     * {@link MediaType#isCompatibleWith(MediaType)}.
     *
     * @param exchange
     *            the current exchange
     * @return the same instance if there are no expressions; or a new condition with matching expressions; or
     ...
     * <ol>
     * <li>Sort 'Accept' header media types by quality value via {@link MediaType#sortByQualityValue(List)} and iterate
     * the list.
     * <li>Get the first index of matching media types in each "produces" condition first matching with
     * {@link MediaType#equals(Object)} and then with {@link MediaType#includes(MediaType)}.
     ...
     * </ol>
     * <p>
     * It is assumed that both instances have been obtained via {@link #getMatchingCondition(ServerWebExchange)} and
     * each instance contains the matching producible media type expression only or is otherwise empty.
     */
    @Override
    ...
    /**
     * Return the contained "produces" expressions or if that's empty, a list with a {@value MediaType#ALL_VALUE}
     * expression.
     */
