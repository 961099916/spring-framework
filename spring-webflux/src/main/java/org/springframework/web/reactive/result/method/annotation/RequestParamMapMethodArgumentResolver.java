...
/**
 * Resolver for {@link Map} method arguments annotated with {@link RequestParam @RequestParam} where the annotation does
 * not specify a request parameter name. See {@link RequestParamMethodArgumentResolver} for resolving {@link Map} method
 * arguments with a request parameter name.
 *
 * <p>
 * The created {@link Map} contains all request parameter name-value pairs. If the method parameter type is
 * {@link MultiValueMap} instead, the created map contains all request parameters and all there values for cases where
