...
/**
 * {@code WebSocketService} implementation that handles a WebSocket HTTP handshake request by delegating to a
 * {@link RequestUpgradeStrategy} which is either auto-detected (no-arg constructor) from the classpath but can also be
...
public class HandshakeWebSocketService implements WebSocketService, Lifecycle {
    ...
    public Mono<Void> handleRequest(ServerWebExchange exchange, WebSocketHandler handler) {
        ...
        if (HttpMethod.GET != method) {
            return Mono
                .error(new MethodNotAllowedException(request.getMethodValue(), Collections.singleton(HttpMethod.GET)));
        }
