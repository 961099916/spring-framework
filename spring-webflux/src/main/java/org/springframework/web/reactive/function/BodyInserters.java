...
public abstract class BodyInserters {
     ...
     *             if {@code body} is a {@link Publisher} or an instance of a type supported by
     *             {@link ReactiveAdapterRegistry#getSharedInstance()}, for which
     *             {@link #fromPublisher(Publisher, Class)} or {@link #fromProducer(Object, Class)} should be used.
    ...
    /**
     * Inserter to write the given producer of value(s) which must be a {@link Publisher} or another producer adaptable
     * to a {@code Publisher} via {@link ReactiveAdapterRegistry}.
     * <p>
     ...
     *            the type of the body
     * @param producer
     *            the source of body value(s).
     * @param elementClass
     *            the class of values to be produced
     ...
     * @param elementClass
     *            the class of elements in the publisher
     * @param <T>
     *            the type of the elements contained in the publisher
     * @param <P>
     *            the {@code Publisher} type
     ...
     * 
     * @param resource
     *            the resource to write to the output message
     * @param <T>
     *            the type of the {@code Resource}
     * @return the inserter to write a {@code Publisher}
     */
     ...
     * <p>
     * Alternatively, you can provide event data objects via {@link #fromPublisher(Publisher, Class)} or
     * {@link #fromProducer(Object, Class)}, and set the "Content-Type" to {@link MediaType#TEXT_EVENT_STREAM
     * text/event-stream}.
     * 
     * @param eventsPublisher
     *            the {@code ServerSentEvent} publisher to write to the response body
     * @param <T>
     *            the type of the data elements in the {@link ServerSentEvent}
     * @return the inserter to write a {@code ServerSentEvent} publisher
     * @see <a href="https://www.w3.org/TR/eventsource/">Server-Sent Events W3C recommendation</a>
     */
    // Parameterized for server-side use
    public static <T, S extends Publisher<ServerSentEvent<T>>> BodyInserter<S, ServerHttpResponse>
    ...
    /**
     * Return a {@link FormInserter} to write the given {@code MultiValueMap} as URL-encoded form data. The returned
     * inserter allows for additional entries to be added via {@link FormInserter#with(String, Object)}.
     * <p>
     * Note that you can also use the {@code bodyValue(Object)} method in the request builders of both the
     * {@code WebClient} and {@code WebTestClient}. In that case the setting of the request content type is also not
     * required, just be sure the map contains String values only or otherwise it would be interpreted as a multipart
     * request.
     * 
     * @param formData
     *            the form data to write to the output message
    ...
    /**
     * Return a {@link FormInserter} to write the given key-value pair as URL-encoded form data. The returned inserter
     * allows for additional entries to be added via {@link FormInserter#with(String, Object)}.
     * 
     * @param name
     *            the key to add to the form
     * @param value
     *            the value to add to the form
     * @return the inserter that allows adding more form data
     */
    ...
    /**
     * Return a {@link MultipartInserter} to write the given {@code MultiValueMap} as multipart data. Values in the map
     * can be an Object or an {@link HttpEntity}.
     * <p>
     * Note that you can also build the multipart data externally with {@link MultipartBodyBuilder}, and pass the
     * resulting map directly to the {@code bodyValue(Object)} shortcut method in {@code WebClient}.
     * 
     * @param multipartData
     *            the form data to write to the output message
    ...
    /**
     * Return a {@link MultipartInserter} to write the given parts, as multipart data. Values in the map can be an
     ...
     *            the part name
     * @param value
     *            the part value, an Object or {@code HttpEntity}
     * @return the inserter that allows adding more parts
     */
     ...
     *            the part name
     * @param publisher
     *            the publisher that forms the part value
     * @param elementClass
     *            the class contained in the {@code publisher}
     * @return the inserter that allows adding more parts
     */
     ...
     * {@link ParameterizedTypeReference} for the element type, which allows specifying generic type information.
     * <p>
     * Note that you can also build the multipart data externally with {@link MultipartBodyBuilder}, and pass the
     * resulting map directly to the {@code bodyValue(Object)} shortcut method in {@code WebClient}.
     * 
     ...
     *            the publisher that forms the part value
     * @param typeReference
     *            the type contained in the {@code publisher}
     * @return the inserter that allows adding more parts
     */
    public static <T, P extends Publisher<T>> MultipartInserter fromMultipartAsyncData(String name, P publisher,
     ...
     * 
     * @param publisher
     *            the data buffer publisher to write
     * @param <T>
     ...
     */
    public static <T extends Publisher<DataBuffer>> BodyInserter<T, ReactiveHttpOutputMessage>
    ...
    public interface MultipartInserter extends FormInserter<Object> {
        ...
        /**
         * Variant of {@link #withPublisher(String, Publisher, Class)} that accepts a {@link ParameterizedTypeReference}
         * for the element type, which allows specifying generic type information.
         * 
         * @param name
         *            the key to be added
         * @param publisher
         *            the publisher to be added as value
         * @param typeReference
         *            the type of elements contained in {@code publisher}
         * @return this inserter for adding more parts
         */
