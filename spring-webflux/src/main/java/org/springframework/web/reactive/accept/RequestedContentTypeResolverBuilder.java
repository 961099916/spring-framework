...
/**
 * Builder for a composite {@link RequestedContentTypeResolver} that delegates to other resolvers each implementing a
 * different strategy to determine the requested content type -- e.g. Accept header, query parameter, or other.
 *
 * <p>
 * Use builder methods to add resolvers in the desired order. For a given request he first resolver to return a list
 * that is not empty and does not consist of just {@link MediaType#ALL}, will be used.
 *
 * <p>
 * By default, if no resolvers are explicitly configured, the builder will add {@link HeaderContentTypeResolver}.
 *
...
public class RequestedContentTypeResolverBuilder {
    ...
    /**
     * Add a resolver to get the requested content type from a query parameter. By default the query parameter name is
     * {@code "format"}.
     */
    ...
    /**
     * Build a {@link RequestedContentTypeResolver} that delegates to the list of resolvers configured through this
     * builder.
     */
    public RequestedContentTypeResolver build() {
        List<RequestedContentTypeResolver> resolvers =
            (!this.candidates.isEmpty() ? this.candidates.stream().map(Supplier::get).collect(Collectors.toList())
