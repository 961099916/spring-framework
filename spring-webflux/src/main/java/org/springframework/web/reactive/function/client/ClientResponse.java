...
/**
 * Represents an HTTP response, as returned by {@link WebClient} and also {@link ExchangeFunction}. Provides access to
 * the response status and headers, and also methods to consume the response body.
 *
 * <p>
 * <strong>NOTE:</strong> When using a {@link ClientResponse} through the {@code WebClient}
 * {@link WebClient.RequestHeadersSpec#exchange() exchange()} method, you have to make sure that the body is consumed or
 * released by using one of the following methods:
 * <ul>
 ...
 * <li>{@link #toEntityList(Class)} or {@link #toEntityList(ParameterizedTypeReference)}</li>
 * <li>{@link #toBodilessEntity()}</li>
 * <li>{@link #releaseBody()}</li>
 * </ul>
 * You can also use {@code bodyToMono(Void.class)} if no response content is expected. However keep in mind the
 * connection will be closed, instead of being placed back in the pool, if any content does arrive. This is in contrast
 * to {@link #releaseBody()} which does consume the full body and releases any content received.
 *
 * @author Brian Clozel
...
public interface ClientResponse {
     ...
     *
     * @param extractor
     *            the {@code BodyExtractor} that reads from the response
     * @param <T>
     ...
     *            the type reference of element in the {@code Mono}
     * @param <T>
     *            the element type
     * @return a mono containing the body of the given type {@code T}
     */
     ...
     *
     * @param elementClass
     *            the class of elements in the {@code Flux}
     * @param <T>
     *            the element type
     ...
     *
     * @param elementTypeRef
     *            the type reference of elements in the {@code Flux}
     * @param <T>
     *            the element type
     * @return a flux containing the body of the given type {@code T}
     */
    ...
    /**
     * Return this response as a delayed {@code ResponseEntity} containing status and headers, but no body. Calling this
     * method will {@linkplain #releaseBody() release} the body of the response.
     *
    ...
    /**
     * Create a {@link WebClientResponseException} that contains the response status, headers, body, and the originating
     * request.
     *
     ...
     * <p>
     * The prefix is based on {@linkplain ClientRequest#logPrefix()}, which itself is based on the value of the
     * {@link ClientRequest#LOG_ID_ATTRIBUTE LOG_ID_ATTRIBUTE} request attribute, further surrounded with "[" and "]".
     *
     * @return the log message prefix or an empty String if the {@link ClientRequest#LOG_ID_ATTRIBUTE LOG_ID_ATTRIBUTE}
     *         is not set
     * @since 5.2.3
     */
    String logPrefix();
