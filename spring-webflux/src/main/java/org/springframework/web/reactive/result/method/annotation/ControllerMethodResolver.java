...
/**
 * Package-private class to assist {@link RequestMappingHandlerAdapter} with resolving, initializing, and caching
 * annotated methods declared in {@code @Controller} and {@code @ControllerAdvice} components.
 *
 * <p>
...
class ControllerMethodResolver {
    ...
    /**
     * Return an {@link InvocableHandlerMethod} for the given {@code @RequestMapping} method initialized with argument
     * resolvers.
     */
    ...
    /**
     * Find {@code @InitBinder} methods in {@code @ControllerAdvice} components or in the controller of the given
     * {@code @RequestMapping} method.
     */
    ...
    /**
     * Find {@code @ModelAttribute} methods in {@code @ControllerAdvice} components or in the controller of the given
     * {@code @RequestMapping} method.
     */
    public List<InvocableHandlerMethod> getModelAttributeMethods(HandlerMethod handlerMethod) {
        ...
        this.modelAttributeMethodCache.computeIfAbsent(handlerType,
            clazz -> MethodIntrospector.selectMethods(handlerType, MODEL_ATTRIBUTE_METHODS)).forEach(method -> {
                Object bean = handlerMethod.getBean();
    ...
    /**
     * Find an {@code @ExceptionHandler} method in {@code @ControllerAdvice} components or in the controller of the
     * given {@code @RequestMapping} method.
     */
    @Nullable
    public InvocableHandlerMethod getExceptionHandlerMethod(Throwable ex, HandlerMethod handlerMethod) {
        ...
        if (targetMethod == null) {
            ...
            for (Map.Entry<ControllerAdviceBean,
                ExceptionHandlerMethodResolver> entry : this.exceptionHandlerAdviceCache.entrySet()) {
    ...
    /**
     * Return the handler for the type-level {@code @SessionAttributes} annotation based on the given controller method.
     */
