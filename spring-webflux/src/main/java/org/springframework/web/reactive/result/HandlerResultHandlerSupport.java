...
/**
 * Base class for {@link org.springframework.web.reactive.HandlerResultHandler HandlerResultHandler} with support for
 * content negotiation and access to a {@code ReactiveAdapter} registry.
 *
...
public abstract class HandlerResultHandlerSupport implements Ordered {
     ...
     * <p>
     * By default set to {@link Ordered#LOWEST_PRECEDENCE}, however see Javadoc of sub-classes which may change this
     * default.
     *
     * @param order
     *            the order
     ...
     * @param exchange
     *            the current request
     * @param producibleTypesSupplier
     *            the media types that can be produced for the current request
    ...
    protected MediaType selectMediaType(ServerWebExchange exchange, Supplier<List<MediaType>> producibleTypesSupplier) {
        ...
        if (selected != null) {
            ...
            logger
                .debug(exchange.getLogPrefix() + "No match for " + acceptableTypes + ", supported: " + producibleTypes);
        }
