...
/**
 * <strong>Central entry point to Spring's functional web framework.</strong> Exposes routing functionality, such as to
 * {@linkplain #route() create} a {@code RouterFunction} using a discoverable builder-style API, to
 ...
 * {@linkplain #nest(RequestPredicate, RouterFunction) subrouting} on an existing routing function.
 *
 * <p>
 * Additionally, this class can {@linkplain #toHttpHandler(RouterFunction) transform} a {@code RouterFunction} into an
 * {@code HttpHandler}, which can be run in Servlet 3.1+, Reactor, or Undertow.
 *
...
public abstract class RouterFunctions {
    ...
    /**
     * Name of the {@link ServerWebExchange} attribute that contains the URI templates map, mapping variable names to
     * values.
     */
    public static final String URI_TEMPLATE_VARIABLES_ATTRIBUTE =
        RouterFunctions.class.getName() + ".uriTemplateVariables";
    /**
     * Name of the {@link ServerWebExchange#getAttributes() attribute} that contains the matching pattern, as a
     ...
     * <p>
     * For instance, the following example routes GET requests for "/user" to the {@code listUsers} method in
     * {@code userController}:
     * 
     * <pre class="code">
     ...
     *            the handler function to route to if the predicate applies
     * @param <T>
     *            the type of response returned by the handler function
     * @return a router function that routes to {@code handlerFunction} if {@code predicate} evaluates to {@code true}
    ...
    /**
     * Route to the given router function if the given request predicate applies. This method can be used to create
     * <strong>nested routes</strong>, where a group of routes share a common path (prefix), header, or other request
     * predicate.
     * <p>
     * For instance, the following example first creates a composed route that resolves to {@code listUsers} for a GET,
     * and {@code createUser} for a POST. This composed route then gets nested with a "/user" path predicate, so that
     * GET requests for "/user" will list users, and POST request for "/user" will create a new user.
     * 
     * <pre class="code">
     ...
     * RouterFunction&lt;ServerResponse&gt; nestedRoute = RouterFunctions.nest(RequestPredicates.path("/user"), userRoutes);
     * </pre>
     * 
     * @param predicate
     *            the predicate to test
     * @param routerFunction
     *            the nested router function to delegate to if the predicate applies
     * @param <T>
     *            the type of response returned by the handler function
     * @return a router function that routes to {@code routerFunction} if {@code predicate} evaluates to {@code true}
     * @see RequestPredicates
     */
     ...
     * @param location
     *            the location directory relative to which resources should be resolved
    ...
    /**
     * Returns the resource lookup function used by {@link #resources(String, Resource)}. The returned function can be
     * {@linkplain Function#andThen(Function) composed} on, for instance to return a default resource when the lookup
    ...
    /**
     * Route to resources using the provided lookup function. If the lookup function provides a {@link Resource} for the
     * given request, it will be it will be exposed using a {@link HandlerFunction} that handles GET, HEAD, and OPTIONS
     * requests.
     * 
     * @param lookupFunction
     *            the function to provide a {@link Resource} given the {@link ServerRequest}
     * @return a router function that routes to resources
     */
    ...
    /**
     * Convert the given {@linkplain RouterFunction router function} into a {@link HttpHandler}. This conversion uses
     * {@linkplain HandlerStrategies#builder() default strategies}.
     * <p>
     * The returned handler can be adapted to run in
     * <ul>
     * <li>Servlet 3.1+ using the {@link org.springframework.http.server.reactive.ServletHttpHandlerAdapter},</li>
     * <li>Reactor using the {@link org.springframework.http.server.reactive.ReactorHttpHandlerAdapter},</li>
     * <li>Undertow using the {@link org.springframework.http.server.reactive.UndertowHttpHandlerAdapter}.</li>
     * </ul>
     * <p>
     * Note that {@code HttpWebHandlerAdapter} also implements {@link WebHandler}, allowing for additional filter and
     * exception handler registration through {@link WebHttpHandlerBuilder}.
     * 
     * @param routerFunction
     *            the router function to convert
    ...
    /**
     * Convert the given {@linkplain RouterFunction router function} into a {@link HttpHandler}, using the given
     * strategies.
     * <p>
    ...
    /**
     * Convert the given {@linkplain RouterFunction router function} into a {@link WebHandler}. This conversion uses
     * {@linkplain HandlerStrategies#builder() default strategies}.
     * 
     * @param routerFunction
     *            the router function to convert
    ...
    /**
     * Convert the given {@linkplain RouterFunction router function} into a {@link WebHandler}, using the given
     * strategies.
     * 
     * @param routerFunction
     *            the router function to convert
    ...
    /**
     * Represents a discoverable builder for router functions. Obtained via {@link RouterFunctions#route()}.
     * 
     * @since 5.1
     */
    public interface Builder {
        ...
        /**
         * Adds a route to the given handler function that handles all HTTP {@code GET} requests that match the given
         * pattern.
         * 
         * @param pattern
         *            the pattern to match to
         * @param handlerFunction
         *            the handler function to handle all {@code GET} requests that match {@code pattern}
         * @return this builder
         */
         ...
         * <p>
         * For instance, the following example routes GET requests for "/user" that accept JSON to the {@code listUsers}
         * method in {@code userController}:
         * 
         * <pre class="code">
         * RouterFunction&lt;ServerResponse&gt; route = RouterFunctions.route()
         *     .GET("/user", RequestPredicates.accept(MediaType.APPLICATION_JSON), userController::listUsers).build();
         * </pre>
         * 
         * @param pattern
         *            the pattern to match to
        ...
        /**
         * Adds a route to the given handler function that handles all HTTP {@code HEAD} requests that match the given
         * pattern.
         * 
         * @param pattern
         *            the pattern to match to
         * @param handlerFunction
         *            the handler function to handle all {@code HEAD} requests that match {@code pattern}
         * @return this builder
         */
        ...
        /**
         * Adds a route to the given handler function that handles all HTTP {@code POST} requests that match the given
         * pattern.
         * 
         * @param pattern
         *            the pattern to match to
         * @param handlerFunction
         *            the handler function to handle all {@code POST} requests that match {@code pattern}
         * @return this builder
         */
         ...
         * pattern and predicate.
         * <p>
         * For instance, the following example routes POST requests for "/user" that contain JSON to the {@code addUser}
         * method in {@code userController}:
         * 
         * <pre class="code">
         * RouterFunction&lt;ServerResponse&gt; route = RouterFunctions.route()
         *     .POST("/user", RequestPredicates.contentType(MediaType.APPLICATION_JSON), userController::addUser).build();
         * </pre>
         * 
         * @param pattern
         *            the pattern to match to
        ...
        /**
         * Adds a route to the given handler function that handles all HTTP {@code PUT} requests that match the given
         * pattern.
         * 
         * @param pattern
         *            the pattern to match to
         * @param handlerFunction
         *            the handler function to handle all {@code PUT} requests that match {@code pattern}
         * @return this builder
         */
         ...
         * pattern and predicate.
         * <p>
         * For instance, the following example routes PUT requests for "/user" that contain JSON to the {@code editUser}
         * method in {@code userController}:
         * 
         * <pre class="code">
         * RouterFunction&lt;ServerResponse&gt; route = RouterFunctions.route()
         *     .PUT("/user", RequestPredicates.contentType(MediaType.APPLICATION_JSON), userController::editUser).build();
         * </pre>
         * 
         * @param pattern
         *            the pattern to match to
        ...
        /**
         * Adds a route to the given handler function that handles all HTTP {@code PATCH} requests that match the given
         * pattern.
         * 
         * @param pattern
         *            the pattern to match to
         * @param handlerFunction
         *            the handler function to handle all {@code PATCH} requests that match {@code pattern}
         * @return this builder
         */
         ...
         * pattern and predicate.
         * <p>
         * For instance, the following example routes PATCH requests for "/user" that contain JSON to the
         * {@code editUser} method in {@code userController}:
         * 
         * <pre class="code">
         * RouterFunction&lt;ServerResponse&gt; route = RouterFunctions.route()
         *     .PATCH("/user", RequestPredicates.contentType(MediaType.APPLICATION_JSON), userController::editUser).build();
         * </pre>
         * 
         * @param pattern
         *            the pattern to match to
        ...
        /**
         * Adds a route to the given handler function that handles all HTTP {@code DELETE} requests that match the given
         * pattern.
         * 
         * @param pattern
         *            the pattern to match to
         * @param handlerFunction
         *            the handler function to handle all {@code DELETE} requests that match {@code pattern}
         * @return this builder
         */
        ...
        /**
         * Adds a route to the given handler function that handles all HTTP {@code OPTIONS} requests that match the
         * given pattern.
         * 
         * @param pattern
         *            the pattern to match to
         * @param handlerFunction
         *            the handler function to handle all {@code OPTIONS} requests that match {@code pattern}
         * @return this builder
         */
         ...
         * 
         * @param predicate
         *            the request predicate to match
         * @param handlerFunction
         *            the handler function to handle all requests that match the predicate
         * @return this builder
         * @since 5.2
         * @see RequestPredicates
         */
        ...
        /**
         * Adds the given route to this builder. Can be used to merge externally defined router functions into this
         * builder, or can be combined with {@link RouterFunctions#route(RequestPredicate, HandlerFunction)} to allow
         * for more flexible predicate matching.
         * <p>
         * For instance, the following example adds the router function returned from
         * {@code OrderController.routerFunction()}. to the {@code changeUser} method in {@code userController}:
         * 
         * <pre class="code">
         * RouterFunction&lt;ServerResponse&gt; route =
         ...
         * the given request, it will be it will be exposed using a {@link HandlerFunction} that handles GET, HEAD, and
         * OPTIONS requests.
         * 
         * @param lookupFunction
         *            the function to provide a {@link Resource} given the {@link ServerRequest}
        ...
        /**
         * Route to the supplied router function if the given request predicate applies. This method can be used to
         * create <strong>nested routes</strong>, where a group of routes share a common path (prefix), header, or other
         * request predicate.
         * <p>
         * For instance, the following example creates a nested route with a "/user" path predicate, so that GET
         * requests for "/user" will list users, and POST request for "/user" will create a new user.
         * 
         * <pre class="code">
         * RouterFunction&lt;ServerResponse&gt; nestedRoute = RouterFunctions.route().nest(RequestPredicates.path("/user"),
         *     () -> RouterFunctions.route().GET(this::listUsers).POST(this::createUser).build()).build();
         * </pre>
         * 
         * @param predicate
         *            the predicate to test
         * @param routerFunctionSupplier
         *            supplier for the nested router function to delegate to if the predicate applies
         * @return this builder
         * @see RequestPredicates
         */
        ...
        /**
         * Route to a built router function if the given request predicate applies. This method can be used to create
         * <strong>nested routes</strong>, where a group of routes share a common path (prefix), header, or other
         * request predicate.
         * <p>
         * For instance, the following example creates a nested route with a "/user" path predicate, so that GET
         * requests for "/user" will list users, and POST request for "/user" will create a new user.
         ...
         *     .nest(RequestPredicates.path("/user"), builder -> builder.GET(this::listUsers).POST(this::createUser))
         *     .build();
         * </pre>
         * 
         * @param predicate
         ...
         *            consumer for a {@code Builder} that provides the nested router function
         * @return this builder
         * @see RequestPredicates
         */
        ...
        /**
         * Route to the supplied router function if the given path prefix pattern applies. This method can be used to
         * create <strong>nested routes</strong>, where a group of routes share a common path prefix. Specifically, this
         * method can be used to merge externally defined router functions under a path prefix.
         * <p>
         * For instance, the following example creates a nested route with a "/user" path predicate that delegates to
         * the router function defined in {@code userController}, and with a "/order" path that delegates to
         * {@code orderController}.
         * 
         * <pre class="code">
         * RouterFunction&lt;ServerResponse&gt; nestedRoute = RouterFunctions.route()
         *     .path("/user", userController::routerFunction).path("/order", orderController::routerFunction).build();
         * </pre>
         * 
         * @param pattern
         *            the pattern to match to
         * @param routerFunctionSupplier
         *            supplier for the nested router function to delegate to if the pattern matches
         * @return this builder
         */
        Builder path(String pattern, Supplier<RouterFunction<ServerResponse>> routerFunctionSupplier);

        /**
         * Route to a built router function if the given path prefix pattern applies. This method can be used to create
         * <strong>nested routes</strong>, where a group of routes share a common path prefix.
         * <p>
         * For instance, the following example creates a nested route with a "/user" path predicate, so that GET
         * requests for "/user" will list users, and POST request for "/user" will create a new user.
         * 
         * <pre class="code">
         * RouterFunction&lt;ServerResponse&gt; nestedRoute = RouterFunctions.route()
         *     .path("/user", builder -> builder.GET(this::listUsers).POST(this::createUser)).build();
         * </pre>
         * 
         * @param pattern
         *            the pattern to match to
        ...
        /**
         * Filters all routes created by this builder with the given filter function. Filter functions are typically
         * used to address cross-cutting concerns, such as logging, security, etc.
         * <p>
         * For instance, the following example creates a filter that returns a 401 Unauthorized response if the request
         * does not contain the necessary authentication headers.
         * 
         * <pre class="code">
         * RouterFunction&lt;ServerResponse&gt; filteredRoute = RouterFunctions.route().GET("/user", this::listUsers)
         *     .filter((request, next) -> {
         ...
         *            the function to filter all routes built by this builder
         * @return this builder
         */
         ...
         * Filters are typically used to address cross-cutting concerns, such as logging, security, etc.
         * <p>
         * For instance, the following example creates a filter that logs the request before the handler function
         * executes.
         * 
         * <pre class="code">
         * RouterFunction&lt;ServerResponse&gt; filteredRoute =
         *     RouterFunctions.route().GET("/user", this::listUsers).before(request -> {
         *         log(request);
         *         return request;
         *     }).build();
         * </pre>
        ...
        /**
         * Filter the response object for all routes created by this builder with the given response processing
         * function. Filters are typically used to address cross-cutting concerns, such as logging, security, etc.
         * <p>
         * For instance, the following example creates a filter that logs the response after the handler function
         * executes.
         * 
         * <pre class="code">
         * RouterFunction&lt;ServerResponse&gt; filteredRoute =
         *     RouterFunctions.route().GET("/user", this::listUsers).after((request, response) -> {
         *         log(response);
         *         return response;
         *     }).build();
         * </pre>
         ...
         * <p>
         * For instance, the following example creates a filter that returns a 500 response status when an
         * {@code IllegalStateException} occurs.
         * 
         * <pre class="code">
         * RouterFunction&lt;ServerResponse&gt; filteredRoute =
         *     RouterFunctions.route().GET("/user", this::listUsers).onError(e -> e instanceof IllegalStateException,
         *         (e, request) -> ServerResponse.status(HttpStatus.INTERNAL_SERVER_ERROR).build()).build();
         * </pre>
         * 
         * @param predicate
         ...
         * <pre class="code">
         * RouterFunction&lt;ServerResponse&gt; filteredRoute =
         *     RouterFunctions.route().GET("/user", this::listUsers).onError(IllegalStateException.class,
         *         (e, request) -> ServerResponse.status(HttpStatus.INTERNAL_SERVER_ERROR).build()).build();
         * </pre>
         * 
         * @param exceptionType
        ...
        /**
         * Builds the {@code RouterFunction}. All created routes are {@linkplain RouterFunction#and(RouterFunction)
         * composed} with one another, and filters (if any) are applied to the result.
         * 
    ...
    public interface Visitor {
         ...
         * 
         * @param predicate
         *            the predicate that applies to the nested router functions
         ...
         * 
         * @param predicate
         *            the predicate that applies to the handler function
         * @param handlerFunction
         ...
         * 
         * @param lookupFunction
         *            the lookup function for the resources
         * @see RouterFunctions#resources(Function)
         */
        void resources(Function<ServerRequest, Mono<Resource>> lookupFunction);

        /**
         * Receive notification of an unknown router function. This method is called for router functions that were not
         * created via the various {@link RouterFunctions} methods.
         * 
         * @param routerFunction
         *            the router function
         */
    ...
    /**
     * A composed routing function that first invokes one function, and then invokes the another function (of the same
     * response type {@code T}) if this route had {@linkplain Mono#empty() no result}.
     * 
     * @param <T>
    ...
    /**
     * A composed routing function that first invokes one function, and then invokes another function (of a different
     * response type) if this route had {@linkplain Mono#empty() no result}.
     */
     ...
     * 
     * @param <T>
     *            the type of the {@linkplain HandlerFunction handler function} to filter
     * @param <S>
