...
/**
 * Simple adapter to expose the bind status of a field or object. Set as a variable by FreeMarker macros and other tag
 * libraries.
 *
 * <p>
 * Obviously, object status representations (i.e. errors at the object level rather than the field level) do not have an
 * expression and a value but only error codes and messages. For simplicity's sake and to be able to use the same tags
 * and macros, the same status class is used for both scenarios.
 *
...
public class BindStatus {
     ...
     * @param path
     *            the bean and property path for which values and errors will be resolved (e.g.
    ...
    /**
     * Return a bind expression that can be used in HTML forms as input name for the respective field, or {@code null}
     * if not field-specific.
     * <p>
     * Returns a bind path appropriate for resubmission, e.g. "address.street". Note that the complete bind path as
     * required by the bind tag is "customer.address.street", if bound to a "customer" bean.
     */
    @Nullable
    ...
    /**
     * Return the current value of the field, i.e. either the property value or a rejected update, or {@code null} if
     * not field-specific.
     * <p>
    ...
    /**
     * Get the '{@code Class}' type of the field. Favor this instead of '{@code getValue().getClass()}' since
     * '{@code getValue()}' may return '{@code null}'.
     */
    @Nullable
    ...
    /**
     * Return a suitable display value for the field, i.e. the stringified value if not null, and an empty string in
     * case of a null value.
     * <p>
     * This value will be an HTML-escaped String if the original value was non-null: the {@code toString} result of the
     * original value will get HTML-escaped.
     */
    ...
    /**
     * Return the resolved error messages for the field or object, if any. Returns an empty array instead of null if
     * none.
     */
    ...
    /**
     * Find a PropertyEditor for the given value class, associated with the property that this bound status is currently
     * bound to.
     * 
     * @param valueClass
