...
public abstract class BodyExtractors {
     ...
     * 
     * @param elementClass
     *            the class of the element type to decode to
     * @param <T>
     *            the element type to decode to
     ...
     *            the type reference for the type to decode to
     * @param <T>
     *            the element type to decode to
     * @return {@code BodyExtractor} for {@code Mono<T>}
     */
    public static <T> BodyExtractor<Mono<T>, ReactiveHttpInputMessage>
    ...
    private static <T, S extends Publisher<T>> S readWithMessageReaders(ReactiveHttpInputMessage message,
        ...
        MediaType contentType =
            Optional.ofNullable(message.getHeaders().getContentType()).orElse(MediaType.APPLICATION_OCTET_STREAM);
