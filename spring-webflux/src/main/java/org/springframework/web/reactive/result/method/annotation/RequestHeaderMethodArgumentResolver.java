...
/**
 * Resolves method arguments annotated with {@code @RequestHeader} except for {@link Map} arguments. See
 * {@link RequestHeaderMapMethodArgumentResolver} for details on {@link Map} arguments annotated with
 * {@code @RequestHeader}.
 *
 * <p>
 * An {@code @RequestHeader} is a named value resolved from a request header. It has a required flag and a default value
 * to fall back on when the request header does not exist.
 *
 * <p>
 * A {@link ConversionService} is invoked to apply type conversion to resolved request header values that don't yet
...
public class RequestHeaderMethodArgumentResolver extends AbstractNamedValueSyncArgumentResolver {
     ...
     *            default values; or {@code null} if default values are not expected to have expressions
     * @param registry
