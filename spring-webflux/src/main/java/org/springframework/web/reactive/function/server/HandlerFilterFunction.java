...
public interface HandlerFilterFunction<T extends ServerResponse, R extends ServerResponse> {
    ...
    /**
     * Adapt the given request processor function to a filter function that only operates on the {@code ServerRequest}.
     *
    ...
    static HandlerFilterFunction<?, ?>
        ofRequestProcessor(Function<ServerRequest, Mono<ServerRequest>> requestProcessor) {
    ...
    static <T extends ServerResponse, R extends ServerResponse> HandlerFilterFunction<T, R>
        ofResponseProcessor(Function<T, Mono<R>> responseProcessor) {
    ...
    /**
     * Apply this filter to the given handler function. The given {@linkplain HandlerFunction handler function}
     * represents the next entity in the chain, and can be {@linkplain HandlerFunction#handle(ServerRequest) invoked} in
     * order to proceed to this entity, or not invoked to block the chain.
     *
     * @param request
     *            the request
     * @param next
     *            the next handler or filter function in the chain
    ...
    /**
     * Return a composed filter function that first applies this filter, and then applies the {@code after} filter.
     *
     * @param after
     *            the filter to apply after this filter is applied
     * @return a composed filter that first applies this function and then applies the {@code after} function
     */
     ...
     *
     * @param handler
     *            the handler function to filter
