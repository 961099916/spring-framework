...
public abstract class AbstractMessageWriterResultHandler extends HandlerResultHandlerSupport {
     ...
     *            for serializing Objects to the response body stream
     ...
     *            the object to write
     * @param bodyParameter
     *            the {@link MethodParameter} of the body to write
     * @param exchange
     ...
     *            the {@link MethodParameter} of the body to write
     * @param actualParam
     *            the actual return type of the method that returned the value; could be different from
     *            {@code bodyParameter} when processing {@code HttpEntity} for example
     * @param exchange
     *            the current exchange
     * @return indicates completion or error
     * @since 5.0.2
            ...
            for (HttpMessageWriter<?> writer : getMessageWriters()) {
                if (writer.canWrite(actualElementType, bestMediaType)) {
                    return writer.write((Publisher)publisher, actualType, elementType, bestMediaType,
                        exchange.getRequest(), exchange.getResponse(), Hints.from(Hints.LOG_PREFIX_HINT, logPrefix));
                }
            }
        }
