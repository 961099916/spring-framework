...
/**
 * Resolver that delegates to the chain, and if a resource is found, it then attempts to find an encoded (e.g. gzip,
 * brotli) variant that is acceptable based on the "Accept-Encoding" request header.
 *
 * <p>
 * The list of supported {@link #setContentCodings(List) contentCodings} can be configured, in order of preference, and
 * each coding must be associated with {@link #setExtensions(Map) extensions}.
 *
 * <p>
 * Note that this resolver must be ordered ahead of a {@link VersionResourceResolver} with a content-based, version
 * strategy to ensure the version calculation is not impacted by the encoding.
 *
...
public class EncodedResourceResolver extends AbstractResourceResolver {
    ...
    /**
     * Configure the supported content codings in order of preference. The first coding that is present in the
     * {@literal "Accept-Encoding"} header for a given request, and that has a file present with the associated
     * extension, is used.
     * <p>
     * <strong>Note:</strong> Each coding must be associated with a file extension via {@link #registerExtension} or
     * {@link #setExtensions}. Also customizations to the list of codings here should be matched by customizations to
     * the same list in {@link CachingResourceResolver} to ensure encoded variants of a resource are cached under
     * separate keys.
     * <p>
    ...
    /**
     * Configure mappings from content codings to file extensions. A dot "." will be prepended in front of the extension
            ...
            for (String coding : this.contentCodings) {
                if (acceptEncoding.contains(coding)) {
                    try {
                        ...
                        logger.trace(
                            exchange.getLogPrefix() + "No " + coding + " resource for [" + resource.getFilename() + "]",
                            ex);
                    }
                }
            }
