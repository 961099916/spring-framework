...
public abstract class AbstractView implements View, BeanNameAware, ApplicationContextAware {
    ...
    /**
     * Get the default charset, used when the {@linkplain #setSupportedMediaTypes(List) content type} does not contain
     * one.
     */
    ...
    /**
     * Set the default charset for this view, used when the {@linkplain #setSupportedMediaTypes(List) content type} does
     * not contain one.
     * <p>
     ...
     *             if the ApplicationContext cannot be obtained
     ...
     * 
     * @param model
     *            a map with attribute names as keys and corresponding model objects as values (the map can also be
     *            {@code null} in case of an empty model)
     * @param contentType
     *            the content type selected to render with, which should match one of the
     ...
     * <p>
     * The default implementation creates a combined output Map that includes model as well as static attributes with
    ...
    /**
     * Use the configured {@link ReactiveAdapterRegistry} to adapt asynchronous attributes to {@code Mono<T>} or
     * {@code Mono<List<T>>} and then wait to resolve them into actual values. When the returned {@code Mono<Void>}
     * completes, the asynchronous attributes in the model will have been replaced with their corresponding resolved
     * values.
     * 
     ...
     * Use the configured {@link ReactiveAdapterRegistry} to adapt asynchronous attributes to {@code Mono<T>} or
     * {@code Mono<List<T>>} and then wait to resolve them into actual values. When the returned {@code Mono<Void>}
     * completes, the asynchronous attributes in the model would have been replaced with their corresponding resolved
     * values.
     * 
     ...
     *             {@link #resolveAsyncAttributes(Map, ServerWebExchange)} instead. It is invoked after this one and
    ...
    /**
     * Create a {@link RequestContext} to expose under the {@linkplain #setRequestContextAttribute specified attribute
     * name}.
     * <p>
     ...
     *            the current exchange
     * @param model
     *            a combined output Map (never {@code null}), with dynamic values taking precedence over static
     *            attributes
     ...
     * <p>
     * The default implementation looks in the {@link #getApplicationContext() ApplicationContext} for a
     * {@code RequestDataValueProcessor} bean with the name {@link #REQUEST_DATA_VALUE_PROCESSOR_BEAN_NAME}.
     * 
     * @return the {@code RequestDataValueProcessor}, or {@code null} if there is none in the application context
     */
    @Nullable
