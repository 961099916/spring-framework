...
/**
 * {@code HandlerResultHandler} that encapsulates the view resolution algorithm supporting the following return types:
 * <ul>
 ...
 *
 * <p>
 * A String-based view name is resolved through the configured {@link ViewResolver} instances into a {@link View} to use
 * for rendering. If a view is left unspecified (e.g. by returning {@code null} or a model-related return value), a
 * default view name is selected.
 *
 * <p>
 * By default this resolver is ordered at {@link Ordered#LOWEST_PRECEDENCE} and generally needs to be late in the order
 * since it interprets any String return value as a view name or any non-simple value type as a model attribute while
 * other result handlers may interpret the same otherwise based on the presence of annotations, e.g. for
 * {@code @ResponseBody}.
 *
...
public class ViewResolutionResultHandler extends HandlerResultHandlerSupport implements HandlerResultHandler, Ordered {
    ...
    /**
     * Set the default views to consider always when resolving view names and trying to satisfy the best matching
     * content type.
     */
    ...
    public Mono<Void> handleResult(ServerWebExchange exchange, HandlerResult result) {
        ...
        if (adapter != null) {
            ...
            valueType =
                (adapter.isNoValue() ? ResolvableType.forClass(Void.class) : result.getReturnType().getGeneric());
        } else {
    ...
    /**
     * Select a default view name when a controller did not specify it. Use the request path the leading and trailing
     * slash stripped.
     */
    ...
    private boolean isBindingCandidate(String name, @Nullable Object value) {
        return (!name.startsWith(BindingResult.MODEL_KEY_PREFIX) && value != null && !value.getClass().isArray()
            ...
            && getAdapterRegistry().getAdapter(null, value) == null && !BeanUtils.isSimpleValueType(value.getClass()));
    }
