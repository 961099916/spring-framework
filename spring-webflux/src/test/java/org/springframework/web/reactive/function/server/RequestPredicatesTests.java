...
public class RequestPredicatesTests {
    ...
    public void methodCorsPreFlight() {
        ...
        MockServerHttpRequest mockRequest = MockServerHttpRequest.options("https://example.com")
            .header("Origin", "https://example.com").header(HttpHeaders.ACCESS_CONTROL_REQUEST_METHOD, "PUT").build();
        ServerRequest request =
            new DefaultServerRequest(MockServerWebExchange.from(mockRequest), Collections.emptyList());
    ...
    public void headers() {
        ...
        RequestPredicate predicate =
            RequestPredicates.headers(headers -> headers.header(name).equals(Collections.singletonList(value)));
    ...
    public void contentType() {
        ...
        MockServerHttpRequest mockRequest =
            MockServerHttpRequest.get("https://example.com").header(HttpHeaders.CONTENT_TYPE, json.toString()).build();
        ServerRequest request =
            new DefaultServerRequest(MockServerWebExchange.from(mockRequest), Collections.emptyList());
        assertThat(predicate.test(request)).isTrue();

        mockRequest =
            MockServerHttpRequest.get("https://example.com").header(HttpHeaders.CONTENT_TYPE, "foo/bar").build();
        request = new DefaultServerRequest(MockServerWebExchange.from(mockRequest), Collections.emptyList());
        assertThat(predicate.test(request)).isFalse();
    }
    ...
    public void accept() {
        ...
        MockServerHttpRequest mockRequest =
            MockServerHttpRequest.get("https://example.com").header(HttpHeaders.ACCEPT, json.toString()).build();
        ServerRequest request =
            new DefaultServerRequest(MockServerWebExchange.from(mockRequest), Collections.emptyList());
        assertThat(predicate.test(request)).isTrue();
