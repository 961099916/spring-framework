...
public class NashornScriptTemplateTests {
    ...
    public void renderTemplateWithUrl() throws Exception {
        ...
        assertThat(response.getBodyAsString().block())
            .isEqualTo(("<html><head><title>Check url parameter</title></head><body><p>" + url + "</p></body></html>"));
    }
    ...
    public void subscribeWithoutDemand() throws Exception {
        ...
        ServerWebExchange exchange = new DefaultServerWebExchange(MockServerHttpRequest.get("/path").build(), response,
            new DefaultWebSessionManager(), ServerCodecConfigurer.create(), new AcceptHeaderLocaleContextResolver());
