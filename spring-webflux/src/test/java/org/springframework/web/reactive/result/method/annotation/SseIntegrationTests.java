...
class SseIntegrationTests extends AbstractHttpHandlerIntegrationTests {
    ...
    private void startServer(HttpServer httpServer, ClientHttpConnector connector) throws Exception {
        ...
        this.webClient =
            WebClient.builder().clientConnector(connector).baseUrl("http://localhost:" + this.port + "/sse").build();
    }
    ...
    void sseAsString(HttpServer httpServer, ClientHttpConnector connector) throws Exception {
        ...
        Flux<String> result =
            this.webClient.get().uri("/string").accept(TEXT_EVENT_STREAM).retrieve().bodyToFlux(String.class);
    ...
    void sseAsPerson(HttpServer httpServer, ClientHttpConnector connector) throws Exception {
        ...
        Flux<Person> result =
            this.webClient.get().uri("/person").accept(TEXT_EVENT_STREAM).retrieve().bodyToFlux(Person.class);
    ...
    void serverDetectsClientDisconnect(HttpServer httpServer, ClientHttpConnector connector) throws Exception {
        ...
        Flux<String> result =
            this.webClient.get().uri("/infinite").accept(TEXT_EVENT_STREAM).retrieve().bodyToFlux(String.class);
    ...
    static class SseController {
        ...
        Flux<ServerSentEvent<Person>> sse() {
            return INTERVAL.take(2).map(
                l -> ServerSentEvent.builder(new Person("foo " + l)).id(Long.toString(l)).comment("bar " + l).build());
        }
