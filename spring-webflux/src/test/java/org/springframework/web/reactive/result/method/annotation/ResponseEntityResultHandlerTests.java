...
/**
 * Unit tests for {@link ResponseEntityResultHandler}. When adding a test also consider whether the logic under test is
 * in a parent class, then see:
 * <ul>
...
public class ResponseEntityResultHandlerTests {
    ...
    public void handleReturnValueETagAndLastModified() throws Exception {
        ...
        MockServerWebExchange exchange =
            MockServerWebExchange.from(get("/path").ifNoneMatch(eTag).ifModifiedSince(currentTime.toEpochMilli()));
    ...
    public void handleWithPresetContentTypeShouldFailWithServerError() {
        ...
        StepVerifier
            .create(resultHandler.handleResult(exchange, result)).consumeErrorWith(ex -> assertThat(ex)
    ...
    private void assertResponseBody(MockServerWebExchange exchange, String responseBody) {
        StepVerifier.create(exchange.getResponse().getBody())
            .consumeNextWith(buf -> assertThat(buf.toString(UTF_8)).isEqualTo(responseBody)).expectComplete().verify();
    }
