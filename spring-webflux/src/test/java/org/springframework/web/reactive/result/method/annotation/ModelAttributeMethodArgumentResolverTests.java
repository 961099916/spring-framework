...
class ModelAttributeMethodArgumentResolverTests {
    ...
    void validationErrorWithoutBindingForMono() throws Exception {
        MethodParameter parameter =
            this.testMethod.annotPresent(ModelAttribute.class).arg(Mono.class, ValidatedPojo.class);
    ...
    void validationErrorWithoutBindingForSingle() throws Exception {
        MethodParameter parameter =
            this.testMethod.annotPresent(ModelAttribute.class).arg(Single.class, ValidatedPojo.class);
    ...
    void bindDataClass() throws Exception {
        ...
        Object value = createResolver()
            .resolveArgument(parameter, this.bindContext, postForm("name=Robert&age=25&count=1")).block(Duration.ZERO);
    ...
    private ServerWebExchange postForm(String formData) throws URISyntaxException {
        return MockServerWebExchange
            .from(MockServerHttpRequest.post("/").contentType(MediaType.APPLICATION_FORM_URLENCODED).body(formData));
    }

    @SuppressWarnings("unused")
    void handle(@ModelAttribute @Validated Pojo pojo, @ModelAttribute @Validated Mono<Pojo> mono,
        @ModelAttribute @Validated Single<Pojo> single, @ModelAttribute(binding = false) NonBindingPojo nonBindingPojo,
        ...
        @ModelAttribute(binding = false) @Validated Single<ValidatedPojo> singleValidatedPojo, Pojo pojoNotAnnotated,
        String stringNotAnnotated, Mono<Pojo> monoNotAnnotated, Mono<String> monoStringNotAnnotated,
