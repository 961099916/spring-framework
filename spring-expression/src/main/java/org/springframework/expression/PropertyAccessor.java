 ...
 *
 * <p>
 * This interface places no restrictions, and so implementors are free to access properties directly as fields or
 * through getters or in any other way they see as appropriate.
 *
 * <p>
 * A resolver can optionally specify an array of target classes for which it should be called. However, if it returns
 * {@code null} from {@link #getSpecificTargetClasses()}, it will be called for all property references and given a
 * chance to determine if it can read or write them.
 *
 * <p>
 * Property resolvers are considered to be ordered, and each will be called in turn. The only rule that affects the call
 * order is that any resolver naming the target class directly in {@link #getSpecificTargetClasses()} will be called
 * first, before the general resolvers.
 *
 * @author Andy Clement
 * @since 3.0
 */
public interface PropertyAccessor {
     ...
     * <p>
     * Returning {@code null} indicates this is a general resolver that can be called in an attempt to resolve a
     ...
     * 
     * @param context
     *            the evaluation context in which the access is being attempted
     * @param target
     *            the target object upon which the property is being accessed
     * @param name
    ...
    /**
     * Called to read a property from a specified target object. Should only succeed if {@link #canRead} also returns
     * {@code true}.
     * 
     * @param context
     *            the evaluation context in which the access is being attempted
    ...
    /**
     * Called to write to a property on a specified target object. Should only succeed if {@link #canWrite} also returns
     * {@code true}.
     * 
     * @param context
     *            the evaluation context in which the access is being attempted
