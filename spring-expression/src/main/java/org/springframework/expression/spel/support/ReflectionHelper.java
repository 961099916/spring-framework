...
/**
 * Utility methods used by the reflection resolver code to discover the appropriate methods/constructors and fields that
...
public abstract class ReflectionHelper {
     ...
     * 
     * @param expectedArgTypes
     *            the types the method/constructor is expecting
     * @param suppliedArgTypes
     *            the types that are being supplied at the point of invocation
     * @param typeConverter
     *            a registered type converter
     * @return a MatchInfo object indicating what kind of match it was, or {@code null} if it was not a match
     */
    @Nullable
    ...
    /**
     * Convert a supplied set of arguments into the requested types. If the parameterTypes are related to a varargs
     * method then the final entry in the parameterTypes array is going to be an array itself whose component type
     * should be used as the conversion target for extraneous arguments. (For example, if the parameterTypes are
     * {Integer, String[]} and the input arguments are {Integer, boolean, float} then both the boolean and float must be
     * converted to strings). This method does *not* repackage the arguments into a form suitable for the varargs
     * invocation - a subsequent call to setupArgumentsForVarargsInvocation handles that.
     * 
     * @param converter
     *            the converter to use for type conversions
     * @param arguments
     ...
     * 
     * @param converter
     *            the type converter to use for attempting conversions
     * @param arguments
     *            the actual arguments that need conversion
     * @param executable
     *            the target Method or Constructor
     * @param varargsPosition
     *            the known position of the varargs argument, if any ({@code null} if not varargs)
     ...
     * @param possibleArray
     *            an array object that may have the supplied value as the first element
     ...
     * parameterTypes is {@code (int, String[])} because the second parameter was declared {@code String...}, then if
     * arguments is {@code [1,"a","b"]} then it must be repackaged as {@code [1,new String[]{"a","b"}]} in order to
     * match the expected types.
     * 
     * @param requiredParameterTypes
     *            the types of the parameters for the invocation
     * @param args
    ...
    /**
     * An instance of ArgumentsMatchInfo describes what kind of match was achieved between two sets of arguments - the
     * set that a method/constructor is expecting and the set that are being supplied at the point of invocation. If the
     * kind indicates that conversion is required for some of the arguments then the arguments that require conversion
     * are listed in the argsRequiringConversion array.
     */
