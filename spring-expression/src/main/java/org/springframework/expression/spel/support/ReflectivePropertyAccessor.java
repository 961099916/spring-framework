...
/**
 * A powerful {@link PropertyAccessor} that uses reflection to access properties for reading and possibly also for
 * writing on a target instance.
 *
 * <p>
 * A property can be referenced through a public getter method (when being read) or a public setter method (when being
 * written), and also as a public field.
 *
 * @author Andy Clement
...
public class ReflectivePropertyAccessor implements PropertyAccessor {
     ...
     * 
     * @param allowWrite
     *            whether to allow write operations on a target instance
     * @since 4.3.15
     * @see #canWrite
     */
    ...
    protected Method findGetterForProperty(String propertyName, Class<?> clazz, boolean mustBeStatic) {
        Method method =
            findMethodForProperty(getPropertyMethodSuffixes(propertyName), "get", clazz, mustBeStatic, 0, ANY_TYPES);
        if (method == null) {
    ...
    /**
     * Determine whether the given {@code Method} is a candidate for property access on an instance of the given target
     * class.
     * <p>
    ...
    /**
     * Return the method suffixes for a given property name. The default implementation uses JavaBean conventions with
     * additional support for properties of the form 'xY' where the method 'getXY()' is used in preference to the
     * JavaBean convention of 'getxY()'.
     */
    ...
    /**
     * Attempt to create an optimized property accessor tailored for a property of a particular name on a particular
     * class. The general ReflectivePropertyAccessor will always work but is not optimal due to the need to lookup which
     * reflective member (method/field) to use each time read() is called. This method will just return the
     * ReflectivePropertyAccessor instance if it is unable to build a more optimal accessor.
     * <p>
     * Note: An optimal accessor is currently only usable for read attempts. Do not call this method if you need a
     * read-write accessor.
     * 
    ...
    /**
     * Captures the member (method/field) to call reflectively to access a property value and the type descriptor for
     * the value returned by the reflective call.
     */
    ...
    /**
     * An optimized form of a PropertyAccessor that will use reflection but only knows how to access a particular
     * property on a particular class. This is unlike the general ReflectivePropertyResolver which manages a cache of
     * methods/fields that may be invoked to access different properties on different classes. This optimal accessor
     * exists because looking up the appropriate reflective object by class/name on each read is not cheap.
     */
