...
public abstract class SpelNodeImpl implements SpelNode, Opcodes {
    ...
    /**
     * Indicates the type descriptor for the result of this expression node. This is set as soon as it is known. For a
     * literal node it is known immediately. For a property access or method invocation it is known after one evaluation
     * of that node.
     * <p>
     * The descriptor is like the bytecode form but is slightly easier to work with. It does not include the trailing
     * semicolon (for non array reference types). Some examples: Ljava/lang/String, I, [I
     */
    @Nullable
     ...
     *
     * @param mv
     *            the method visitor where code should be generated
     * @param cf
     *            the current codeflow
     * @param member
     *            the method or constructor for which arguments are being setup
     * @param arguments
    ...
    /**
     * Ask an argument to generate its bytecode and then follow it up with any boxing/unboxing/checkcasting to ensure it
     * matches the expected parameter descriptor.
     */
    ...
    /**
     * Check whether a node can be compiled to bytecode. The reasoning in each node may be different but will typically
     * involve checking whether the exit type descriptor of the node is known and any relevant child nodes are
     * compilable.
     *
    ...
    /**
     * Generate the bytecode for this node into the supplied visitor. Context info about the current expression being
     * compiled is available in the codeflow object, e.g. including information about the type of the object currently
     * on the stack.
     *
     * @param mv
     *            the ASM MethodVisitor into which code should be generated
     * @param cf
