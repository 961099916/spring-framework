...
public class ConstructorReference extends SpelNodeImpl {
        ...
        try {
            ...
            if (executorToUse instanceof ReflectiveConstructorExecutor) {
                this.exitTypeDescriptor = CodeFlow
                    .toDescriptor(((ReflectiveConstructorExecutor)executorToUse).getConstructor().getDeclaringClass());
     ...
     * 
     * @param typeName
     *            the type trying to be constructed
     * @param argumentTypes
     *            the types of the arguments supplied that the constructor must take
     * @param state
     *            the current state of the expression
     * @return a reusable ConstructorExecutor that can be invoked to run the constructor or null
     * @throws SpelEvaluationException
     *             if there is a problem locating the constructor
     */
     ...
     * @throws EvaluationException
     *             if there is a problem creating the array
     */
        ...
        if (!(intendedArrayType instanceof String)) {
            throw new SpelEvaluationException(getChild(0).getStartPosition(),
                SpelMessage.TYPE_NAME_EXPECTED_FOR_ARRAY_CONSTRUCTION, FormatHelper
                    .formatClassNameForMessage(intendedArrayType != null ? intendedArrayType.getClass() : null));
        }
    ...
    private void populateReferenceTypeArray(ExpressionState state, Object newArray, TypeConverter typeConverter,
        ...
        for (int i = 0; i < newObjectArray.length; i++) {
            ...
            newObjectArray[i] =
                typeConverter.convertValue(arrayEntry, TypeDescriptor.forObject(arrayEntry), toTypeDescriptor);
        }
    }
