...
/**
 * A {@code SpelExpression} represents a parsed (valid) expression that is ready to be evaluated in a specified context.
 * An expression can be evaluated standalone or in a specified context. During expression evaluation the context may be
 * asked to resolve references to types, beans, properties, and methods.
 *
 * @author Andy Clement
...
public class SpelExpression implements Expression {
    ...
    /**
     * Perform expression compilation. This will only succeed once exit descriptors for all nodes have been determined.
     * If the compilation fails and has failed more than 100 times the expression is no longer considered suitable for
     * compilation.
     * 
    ...
    /**
     * Cause an expression to revert to being interpreted if it has been using a compiled form. It also resets the
     * compilation attempt failure count (an expression is normally no longer considered compilable if it cannot be
     * compiled after 100 attempts).
     */
