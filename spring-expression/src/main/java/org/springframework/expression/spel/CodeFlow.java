 ...
 *
 * <p>
 * Records intermediate compilation state as the bytecode is generated. Also includes various bytecode generation helper
 * functions.
 *
 * @author Andy Clement
...
public class CodeFlow implements Opcodes {
    ...
    /**
     * Name of the class being generated. Typically used when generating code that accesses freshly generated fields on
     * the generated type.
     */
    ...
    /**
     * Record the type of what is on top of the bytecode stack (i.e. the type of the output from the previous expression
     * component). New scopes are used to evaluate sub-expressions like the expressions for the argument values in a
     * method invocation expression.
     */
    private final Deque<List<String>> compilationScopes;

    /**
     * As SpEL ast nodes are called to generate code for the main evaluation method they can register to add a field to
     * this class. Any registered FieldAdders will be called after the main evaluation function has finished being
     * generated.
     */
    @Nullable
    ...
    /**
     * As SpEL ast nodes are called to generate code for the main evaluation method they can register to add code to a
     * static initializer in the class. Any registered ClinitAdders will be called after the main evaluation function
    ...
    /**
     * When code generation requires holding a value in a class level field, this is used to track the next available
     * field id (used as a name suffix).
     */
    private int nextFieldId = 1;

    /**
     * When code generation requires an intermediate variable within a method, this method records the next available
     * variable (variable 0 is 'this').
     */
     ...
     * @param classWriter
     *            the corresponding ASM {@code ClassWriter}
     */
     ...
     * @param ch
     *            the primitive type desired as output
     * @param stackDescriptor
     ...
     *
     * @param mv
     *            the method visitor into which instructions should be placed
     * @param targetDescriptor
     *            the (primitive) descriptor of the target type
     * @param stackDescriptor
    ...
    /**
     * Create the JVM signature descriptor for a method. This consists of the descriptors for the method parameters
     * surrounded with parentheses, followed by the descriptor for the return type. Note the descriptors here are JVM
     * descriptors, unlike the other descriptor forms the compiler is using which do not include the trailing semicolon.
     *
     * @param method
     *            the method
     ...
     * parameters surrounded with parentheses, followed by the descriptor for the return type, which is always "V". Note
     * the descriptors here are JVM descriptors, unlike the other descriptor forms the compiler is using which do not
    ...
    /**
     * Determine the JVM descriptor for a specified class. Unlike the other descriptors used in the compilation process,
     * this is the one the JVM wants, so this one includes any necessary trailing semicolon (e.g. Ljava/lang/String;
     ...
     *
     * @param value
     *            an object (possibly {@code null})
     * @return the type descriptor for the object (descriptor is "Ljava/lang/Object" for {@code null} value)
     */
    ...
    /**
     * Determine whether boxing/unboxing can get from one type to the other. Assumes at least one of the types is in
     * boxed form (i.e. single char descriptor).
     *
    ...
    /**
     * Determine if the supplied descriptor is for a supported number type or boolean. The compilation process only
     * (currently) supports certain number types. These are double, float, long and int.
     *
     * @param descriptor
     *            the descriptor for a type
    ...
    /**
     * Determine if the supplied descriptor is for a supported number. The compilation process only (currently) supports
     * certain number types. These are double, float, long and int.
     *
     * @param descriptor
     *            the descriptor for a type
    ...
    /**
     * Determine whether the given number is to be considered as an integer for the purposes of a numeric operation at
     * the bytecode level.
     *
     * @param number
     *            the number to check
     ...
     *
     * @param descriptor
     *            a descriptor for a type that should have a primitive representation
     ...
     *
     * @param mv
     *            the target visitor into which the instruction should be inserted
     * @param descriptor
    ...
    /**
     * Determine the appropriate boxing instruction for a specific type (if it needs boxing) and insert the instruction
     * into the supplied visitor.
     *
     * @param mv
     *            the target visitor for the new instructions
     * @param descriptor
    ...
    /**
     * Deduce the descriptor for a type. Descriptors are like JVM type names but missing the trailing ';' so for Object
     * the descriptor is "Ljava/lang/Object" for int it is "I".
     *
     * @param type
     *            the type (may be primitive) for which to determine the descriptor
    ...
    /**
     * Create an array of descriptors representing the parameter types for the supplied method. Returns a zero sized
     * array if there are no parameters.
     *
     * @param method
     *            a Method
    ...
    /**
     * Create an array of descriptors representing the parameter types for the supplied constructor. Returns a zero
     * sized array if there are no parameters.
     *
     * @param ctor
     *            a Constructor
    ...
    /**
     * Produce appropriate bytecode to store a stack item in an array. The instruction to use varies depending on
     * whether the type is a primitive or reference type.
     *
     * @param mv
     *            where to insert the bytecode
    ...
    /**
     * Produce the correct bytecode to build an array. The opcode to use and the signature to pass along with the opcode
     * can vary depending on the signature of the array type.
     *
     * @param mv
     *            the methodvisitor into which code should be inserted
     * @param size
    ...
    /**
     * For use in mathematical operators, handles converting from a (possibly boxed) number on the stack to a primitive
     * numeric type.
     * <p>
     * For example, from a Integer to a double, just need to call 'Number.doubleValue()' but from an int to a double,
     * need to use the bytecode 'i2d'.
     *
     * @param mv
     *            the method visitor when instructions should be appended
     * @param stackDescriptor
     *            a descriptor of the operand on the stack
     * @param targetDescriptor
    ...
    /**
     * Enter a new compilation scope, usually due to nested expression evaluation. For example when the arguments for a
     * method invocation expression are being evaluated, each argument will be evaluated in a new scope.
     */
    ...
    /**
     * Exit a compilation scope, usually after a nested expression has been evaluated. For example after an argument for
     * a method invocation has been evaluated this method returns us to the previous (outer) scope.
     */
    ...
    /**
     * If the codeflow shows the last expression evaluated to java.lang.Boolean then insert the necessary instructions
     * to unbox that to a boolean primitive.
     *
     * @param mv
    ...
    /**
     * Called after the main expression evaluation method has been generated, this method will callback any registered
     * FieldAdders or ClinitAdders to add any extra information to the class representing the compiled expression.
     */
    public void finish() {
    ...
    /**
     * Register a FieldAdder which will add a new field to the generated class to support the code produced by an ast
     * nodes primary generateCode() method.
     */
    ...
    /**
     * Register a ClinitAdder which will add code to the static initializer in the generated class to support the code
