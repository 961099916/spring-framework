...
/**
 * A {@link ConcurrentHashMap} that uses {@link ReferenceType#SOFT soft} or {@linkplain ReferenceType#WEAK weak}
 * references for both {@code keys} and {@code values}.
 *
 * <p>
 * This class can be used as an alternative to {@code Collections.synchronizedMap(new WeakHashMap<K, Reference<V>>())}
 * in order to support better performance when accessed concurrently. This implementation follows the same design
 * constraints as {@link ConcurrentHashMap} with the exception that {@code null} values and {@code null} keys are
 * supported.
 *
 * <p>
 * <b>NOTE:</b> The use of references means that there is no guarantee that items placed into the map will be
 * subsequently available. The garbage collector may discard references at any time, so it may appear that an unknown
 * thread is silently removing entries.
 *
 * <p>
...
public class ConcurrentReferenceHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
     ...
     *            the initial capacity of the map
     * @param loadFactor
     *            the load factor. When the average number of references per table exceeds this value resize will be
     *            attempted
     */
                    ...
                    while (ref != null) {
                        if (!toPurge.contains(ref)) {
                            ...
                            if (entry != null) {
                                ...
                                restructured[index] =
                                    this.referenceManager.createReference(entry, ref.getHash(), restructured[index]);
                            }
                        }
                        ref = ref.getNext();
                    }
    ...
    /**
     * Strategy class used to manage {@link Reference References}. This class can be overridden if alternative reference
