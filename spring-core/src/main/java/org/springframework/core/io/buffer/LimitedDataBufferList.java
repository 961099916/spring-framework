...
/**
 * Custom {@link List} to collect data buffers with and enforce a limit on the total number of bytes buffered. For use
 * with "collect" or other buffering operators in declarative APIs, e.g. {@link Flux}.
 *
 * <p>
 * Adding elements increases the byte count and if the limit is exceeded, {@link DataBufferLimitException} is raised.
 * {@link #clear()} resets the count. Remove and set are not supported.
 *
 * <p>
 * <strong>Note:</strong> This class does not automatically release the buffers it contains. It is usually preferable to
 * use hooks such as {@link Flux#doOnDiscard} that also take care of cancel and error signals, or otherwise
 * {@link #releaseAndClear()} can be used.
 *
...
public class LimitedDataBufferList extends ArrayList<DataBuffer> {
    ...
    /**
     * Shortcut to {@link DataBufferUtils#release release} all data buffers and then {@link #clear()}.
     */
