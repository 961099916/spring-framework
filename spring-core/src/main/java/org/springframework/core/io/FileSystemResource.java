...
/**
 * {@link Resource} implementation for {@code java.io.File} and {@code java.nio.file.Path} handles with a file system
 * target. Supports resolution as a {@code File} and also as a {@code URL}. Implements the extended
 * {@link WritableResource} interface.
 *
 * <p>
 * Note: As of Spring Framework 5.0, this {@link Resource} implementation uses NIO.2 API for read/write interactions. As
 * of 5.1, it may be constructed with a {@link java.nio.file.Path} handle in which case it will perform all file system
...
public class FileSystemResource extends AbstractResource implements WritableResource {
    ...
    /**
     * Create a new {@code FileSystemResource} from a {@link Path} handle, performing all file system interactions via
     * NIO.2 instead of {@link File}.
     * <p>
     * In contrast to {@link PathResource}, this variant strictly follows the general {@link FileSystemResource}
     * conventions, in particular in terms of path cleaning and {@link #createRelative(String)} handling.
     * <p>
     * Note: When building relative resources via {@link #createRelative}, the relative path will apply <i>at the same
     * directory level</i>: e.g. Paths.get("C:/dir1"), relative path "dir2" -> "C:/dir2"! If you prefer to have relative
     * paths built underneath the given root directory, use the {@link #FileSystemResource(String) constructor with a
     * file path} to append a trailing slash to the root path: "C:/dir1/", which indicates this directory as root for
     * all relative paths. Alternatively, consider using {@link PathResource#PathResource(Path)} for
     * {@code java.nio.path.Path} resolution in {@code createRelative}, always nesting relative paths.
     * 
     * @param filePath
    ...
    /**
     * Create a new {@code FileSystemResource} from a {@link FileSystem} handle, locating the specified path.
     * <p>
     * This is an alternative to {@link #FileSystemResource(String)}, performing all file system interactions via NIO.2
     * instead of {@link File}.
     * 
     * @param fileSystem
