...
public abstract class DataBufferUtils {
    ...
    /**
     * Obtain a {@link InputStream} from the given supplier, and read it into a {@code Flux} of {@code DataBuffer}s.
     * Closes the input stream when the Flux is terminated.
     * 
     * @param inputStreamSupplier
     *            the supplier for the input stream to read from
     * @param bufferFactory
     *            the factory to create data buffers with
     * @param bufferSize
     *            the maximum size of the data buffers
    ...
    /**
     * Obtain a {@link ReadableByteChannel} from the given supplier, and read it into a {@code Flux} of
     * {@code DataBuffer}s. Closes the channel when the Flux is terminated.
     * 
     * @param channelSupplier
     *            the supplier for the channel to read from
     * @param bufferFactory
     *            the factory to create data buffers with
     * @param bufferSize
     *            the maximum size of the data buffers
    ...
    /**
     * Obtain a {@code AsynchronousFileChannel} from the given supplier, and read it into a {@code Flux} of
     * {@code DataBuffer}s. Closes the channel when the Flux is terminated.
     * 
     * @param channelSupplier
     *            the supplier for the channel to read from
    ...
    /**
     * Obtain a {@code AsynchronousFileChannel} from the given supplier, and read it into a {@code Flux} of
     * {@code DataBuffer}s, starting at the given position. Closes the channel when the Flux is terminated.
     * 
     * @param channelSupplier
     *            the supplier for the channel to read from
     * @param position
    ...
    /**
     * Read bytes from the given file {@code Path} into a {@code Flux} of {@code DataBuffer}s. The method ensures that
     * the file is closed when the flux is terminated.
     * 
     * @param path
     ...
     * <p>
     * If the resource is a file, it is read into an {@code AsynchronousFileChannel} and turned to {@code Flux} via
     * {@link #readAsynchronousFileChannel(Callable, DataBufferFactory, int)} or else fall back to
     * {@link #readByteChannel(Callable, DataBufferFactory, int)}. Closes the channel when the flux is terminated.
     * 
     * @param resource
    ...
    /**
     * Read the given {@code Resource} into a {@code Flux} of {@code DataBuffer}s starting at the given position.
     * <p>
     * If the resource is a file, it is read into an {@code AsynchronousFileChannel} and turned to {@code Flux} via
     * {@link #readAsynchronousFileChannel(Callable, DataBufferFactory, int)} or else fall back on
     * {@link #readByteChannel(Callable, DataBufferFactory, int)}. Closes the channel when the flux is terminated.
     * 
     * @param resource
     *            the resource to read from
        ...
        try {
            if (resource.isFile()) {
                ...
                return readAsynchronousFileChannel(
                    () -> AsynchronousFileChannel.open(file.toPath(), StandardOpenOption.READ), position, bufferFactory,
                    bufferSize);
            }
     ...
     * <strong>not</strong> close the output stream when the flux is terminated, and does <strong>not</strong>
     * {@linkplain #release(DataBuffer) release} the data buffers in the source. If releasing is required, then
     * subscribe to the returned {@code Flux} with a {@link #releaseConsumer()}.
     * <p>
     * Note that the writing process does not start until the returned {@code Flux} is subscribed to.
     * 
     * @param source
     *            the stream of data buffers to be written
     * @param outputStream
     *            the output stream to write to
     * @return a Flux containing the same buffers as in {@code source}, that starts the writing process when subscribed
     *         to, and that publishes any writing errors and the completion signal
     */
     ...
     * <strong>not</strong> close the channel when the flux is terminated, and does <strong>not</strong>
     * {@linkplain #release(DataBuffer) release} the data buffers in the source. If releasing is required, then
     * subscribe to the returned {@code Flux} with a {@link #releaseConsumer()}.
     * <p>
     * Note that the writing process does not start until the returned {@code Flux} is subscribed to.
    ...
    /**
     * Write the given stream of {@link DataBuffer DataBuffers} to the given {@code AsynchronousFileChannel}. Does
     * <strong>not</strong> close the channel when the flux is terminated, and does <strong>not</strong>
     * {@linkplain #release(DataBuffer) release} the data buffers in the source. If releasing is required, then
     * subscribe to the returned {@code Flux} with a {@link #releaseConsumer()}.
     * <p>
     ...
     *            the channel to write to
     * @param position
     *            the file position where writing is to begin; must be non-negative
     * @return a flux containing the same buffers as in {@code source}, that starts the writing process when subscribed
     *         to, and that publishes any writing errors and the completion signal
     */
    ...
    /**
     * Write the given stream of {@link DataBuffer DataBuffers} to the given file {@link Path}. The optional
     * {@code options} parameter specifies how the file is created or opened (defaults to
     * {@link StandardOpenOption#CREATE CREATE}, {@link StandardOpenOption#TRUNCATE_EXISTING TRUNCATE_EXISTING}, and
    ...
    /**
     * Relay buffers from the given {@link Publisher} until the total {@linkplain DataBuffer#readableByteCount() byte
     * count} reaches the given maximum byte count, or until the publisher is complete.
     * 
     * @param publisher
    ...
    /**
     * Skip buffers from the given {@link Publisher} until the total {@linkplain DataBuffer#readableByteCount() byte
     * count} reaches the given maximum byte count, or until the publisher is complete.
     * 
     * @param publisher
     *            the publisher to filter
    ...
    /**
     * Return a new {@code DataBuffer} composed from joining together the given {@code dataBuffers} elements. Depending
     * on the {@link DataBuffer} type, the returned buffer may be a single buffer containing all data of the provided
     * buffers, or it may be a zero-copy, composite with references to the given buffers.
     * <p>
     * If {@code dataBuffers} produces an error or if there is a cancel signal, then all accumulated buffers will be
     * {@linkplain #release(DataBuffer) released}.
     * <p>
     * Note that the given data buffers do <strong>not</strong> have to be released. They will be released as part of
     * the returned composite.
     * 
     * @param dataBuffers
     *            the data buffers that are to be composed
    ...
    /**
     * Variant of {@link #join(Publisher)} that behaves the same way up until the specified max number of bytes to
     * buffer. Once the limit is exceeded, {@link DataBufferLimitException} is raised.
     * 
     * @param buffers
     *            the data buffers that are to be composed
     * @param maxByteCount
     *            the max number of bytes to buffer, or -1 for unlimited
     * @return a buffer with the aggregated content, possibly an empty Mono if the max number of bytes to buffer is
     *         exceeded.
     * @throws DataBufferLimitException
     *             if maxByteCount is exceeded
     * @since 5.1.11
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Return a {@link Matcher} for the given delimiter. The matcher can be used to find the delimiters in a stream of
     * data buffers.
     * 
     * @param delimiter
     *            the delimiter bytes to find
     * @return the matcher
     * @since 5.2
     */
    ...
    /**
     * Return a {@link Matcher} for the given delimiters. The matcher can be used to find the delimiters in a stream of
     * data buffers.
     * 
     * @param delimiters
     *            the delimiters bytes to find
     * @return the matcher
     * @since 5.2
     */
    ...
    public interface Matcher {
        ...
        /**
         * Find the first matching delimiter and return the index of the last byte of the delimiter, or {@code -1} if
         * not found.
         */
    ...
    /**
     * Matcher that can be nested within {@link CompositeMatcher} where multiple matchers advance together using the
     * same index, one byte at a time.
     */
     ...
     * 
     * @see <a href="https://www.nayuki.io/page/knuth-morris-pratt-string-matching">Knuth-Morris-Pratt string
     *      matching</a>
     */
