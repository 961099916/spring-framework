 ...
 *
 * <p>
 * Reads a {@code spring.properties} file from the root of the Spring library classpath, and also allows for
 * programmatically setting properties through {@link #setProperty}. When checking a property, local entries are being
 * checked first, then falling back to JVM-level system properties through a {@link System#getProperty} check.
 *
 * <p>
 * This is an alternative way to set Spring-related system properties such as "spring.getenv.ignore" and
 * "spring.beaninfo.ignore", in particular for scenarios where JVM system properties are locked on the target platform
 * (e.g. WebSphere). See {@link #setFlag} for a convenient way to locally set such flags to "true".
 *
...
public final class SpringProperties {
    ...
    /**
     * Retrieve the property value for the given key, checking local Spring properties first and falling back to
     * JVM-level system properties.
     * 
     * @param key
     *            the property key
