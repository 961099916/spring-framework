...
/**
 * Decode from a data buffer stream to a {@code String} stream, either splitting or aggregating incoming data chunks to
 * realign along newlines delimiters and produce a stream of strings. This is useful for streaming but is also necessary
 * to ensure that that multibyte characters can be decoded correctly, avoiding split-character issues. The default
 * delimiters used by default are {@code \n} and {@code \r\n} but that can be customized.
 *
...
public final class StringDecoder extends AbstractDataBufferDecoder<String> {
     ...
     * @deprecated as of Spring 5.0.4, in favor of {@link #textPlainOnly()} or {@link #textPlainOnly(List, boolean)}
     */
    @Deprecated
     ...
     * @param stripDelimiter
     *            this flag is ignored
     * @deprecated as of Spring 5.0.4, in favor of {@link #allMimeTypes()} or {@link #allMimeTypes(List, boolean)}
     */
    @Deprecated
    ...
    public Flux<String> decode(Publisher<DataBuffer> input, ResolvableType elementType, @Nullable MimeType mimeType,
        ...
        return Flux.from(input).concatMapIterable(buffer -> processDataBuffer(buffer, matcher, chunks))
            ...
            })).doOnTerminate(chunks::releaseAndClear).doOnDiscard(PooledDataBuffer.class, PooledDataBuffer::release)
