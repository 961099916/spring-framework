 ...
 * {@link #getInterfaces() interfaces}, and {@link #getGeneric(int...) generic parameters} along with the ability to
 * ultimately {@link #resolve() resolve} to a {@link java.lang.Class}.
 *
 * <p>
 * {@code ResolvableTypes} may be obtained from {@link #forField(Field) fields}, {@link #forMethodParameter(Method, int)
 * method parameters}, {@link #forMethodReturnType(Method) method returns} or {@link #forClass(Class) classes}. Most
 * methods on this class will themselves return {@link ResolvableType ResolvableTypes}, allowing easy navigation. For
 * example:
 * 
 * <pre class="code">
...
public class ResolvableType implements Serializable {
    ...
    /**
     * {@code ResolvableType} returned when no value is available. {@code NONE} is used in preference to {@code null} so
     * that multiple method calls can be safely chained.
     */
    ...
    /**
     * Private constructor used to create a new {@link ResolvableType} for cache key purposes, with no upfront
     * resolution.
     */
    ...
    /**
     * Private constructor used to create a new {@link ResolvableType} for cache value purposes, with upfront resolution
     * and a pre-calculated hash.
     * 
     * @since 4.2
     */
    ...
    /**
     * Private constructor used to create a new {@link ResolvableType} for uncached purposes, with upfront resolution
     * but lazily calculated hash.
     */
    private ResolvableType(Type type, @Nullable TypeProvider typeProvider, @Nullable VariableResolver variableResolver,
    ...
    /**
     * Private constructor used to create a new {@link ResolvableType} on a {@link Class} basis. Avoids all
     * {@code instanceof} checks in order to create a straight {@link Class} wrapper.
     * 
     * @since 4.2
     */
     ...
     *
     * @param clazz
     *            the class to introspect ({@code null} is semantically equivalent to {@code Object.class} for typical
     *            use cases here)
    ...
    /**
     * Return a {@link ResolvableType} for the specified {@link Class}, doing assignability checks against the raw class
     * only (analogous to {@link Class#isAssignableFrom}, which this serves as a wrapper for. For example:
    ...
    /**
     * Return a {@link ResolvableType} for the specified base type (interface or base class) with a given implementation
     * class. For example: {@code ResolvableType.forClass(List.class, MyArrayList.class)}.
     *
     * @param baseType
     *            the base type (must not be {@code null})
     * @param implementationClass
     *            the implementation class
     * @return a {@link ResolvableType} for the specified base type backed by the given implementation class
     * @see #forClass(Class)
     * @see #forClassWithGenerics(Class, Class...)
     */
    ...
    /**
     * Return a {@link ResolvableType} for the specified instance. The instance does not convey generic information but
     * if it implements {@link ResolvableTypeProvider} a more precise {@link ResolvableType} can be used than the simple
     * one based on the {@link #forClass(Class) Class instance}.
     *
     * @param instance
     *            the instance
     ...
     * <p>
     * Use this variant when the class that declares the field includes generic parameter variables that are satisfied
     ...
     *            the source field
     * @param nestingLevel
     *            the nesting level (1 for the outer level; 2 for a nested generic type; etc)
     * @see #forField(Field)
     */
    ...
    /**
     * Return a {@link ResolvableType} for the specified {@link Field} with a given implementation and the given nesting
     * level.
     * <p>
     * Use this variant when the class that declares the field includes generic parameter variables that are satisfied
     * by the implementation class.
    ...
    /**
     * Return a {@link ResolvableType} for the specified {@link Constructor} parameter with a given implementation. Use
     * this variant when the class that declares the constructor includes generic parameter variables that are satisfied
     * by the implementation class.
     *
     * @param constructor
     *            the source constructor (must not be {@code null})
    ...
    /**
     * Return a {@link ResolvableType} for the specified {@link Method} return type. Use this variant when the class
     * that declares the method includes generic parameter variables that are satisfied by the implementation class.
     *
     * @param method
     *            the source for the method return type
     * @param implementationClass
    ...
    /**
     * Return a {@link ResolvableType} for the specified {@link Method} parameter with a given implementation. Use this
    ...
    /**
     * Return a {@link ResolvableType} for the specified {@link MethodParameter} with a given implementation type. Use
     * this variant when the class that declares the method includes generic parameter variables that are satisfied by
     * the implementation type.
     *
     * @param methodParameter
     *            the source method parameter (must not be {@code null})
    ...
    /**
     * Return a {@link ResolvableType} for the specified {@link MethodParameter}, overriding the target type to resolve
     * with a specific given type.
     *
     * @param methodParameter
     *            the source method parameter (must not be {@code null})
     * @param targetType
    ...
    /**
     * Return a {@link ResolvableType} for the specified {@link MethodParameter} at a specific nesting level, overriding
     * the target type to resolve with a specific given type.
     *
     * @param methodParameter
     *            the source method parameter (must not be {@code null})
     * @param targetType
    ...
    /**
     * Return the underlying source of the resolvable type. Will return a {@link Field}, {@link MethodParameter} or
     * {@link Type} depending on how the {@link ResolvableType} was constructed. With the exception of the {@link #NONE}
     * constant, this method will never return {@code null}. This method is primarily to provide access to additional
    ...
    /**
     * Return this type as a resolved {@code Class}, falling back to {@link java.lang.Object} if no specific class can
     * be resolved.
     *
     ...
     * @return {@code true} if the specified other type can be assigned to this {@code ResolvableType}; {@code false}
     *         otherwise
     */
    public boolean isAssignableFrom(ResolvableType other) {
    ...
    /**
     * Return the ResolvableType representing the component type of the array or {@link #NONE} if this type does not
     * represent an array.
     *
     * @see #isArray()
     */
    ...
    /**
     * Convenience method to return this type as a resolvable {@link Collection} type. Returns {@link #NONE} if this
     * type does not implement or extend {@link Collection}.
     *
     * @see #as(Class)
     * @see #asMap()
     */
    ...
    /**
     * Convenience method to return this type as a resolvable {@link Map} type. Returns {@link #NONE} if this type does
     * not implement or extend {@link Map}.
     *
     * @see #as(Class)
     * @see #asCollection()
     */
    ...
    /**
     * Return this type as a {@link ResolvableType} of the specified class. Searches {@link #getSuperType() supertype}
     * and {@link #getInterfaces() interface} hierarchies to find a match, returning {@link #NONE} if this type does not
     * implement or extend the specified class.
     *
     * @param type
     *            the required type (typically narrowed)
     * @return a {@link ResolvableType} representing this object as the specified type, or {@link #NONE} if not
     *         resolvable as that type
     * @see #asCollection()
     * @see #asMap()
     * @see #getSuperType()
     * @see #getInterfaces()
    ...
    /**
     * Return a {@link ResolvableType} representing the direct supertype of this type. If no supertype is available this
     * method returns {@link #NONE}.
     * <p>
     * Note: The resulting {@link ResolvableType} instance may not be {@link Serializable}.
     *
     * @see #getInterfaces()
    ...
    /**
     * Return a {@link ResolvableType} array representing the direct interfaces implemented by this type. If this type
    ...
    /**
     * Return {@code true} if this type contains unresolvable generics only, that is, no substitute for any of its
    ...
    /**
     * Determine whether the underlying type has any unresolvable generics: either through an unresolvable type variable
     * on the type itself or through implementing a generic interface in a raw fashion, i.e. without substituting that
     * interface's type variables. The result will be {@code true} only in those two scenarios.
     */
     ...
     * <p>
     * The nesting level refers to the specific generic parameter that should be returned. A nesting level of 1
     * indicates this type; 2 indicates the first nested generic; 3 the second; and so on. For example, given
     * {@code List<Set<Integer>>} level 1 refers to the {@code List}, level 2 the {@code Set}, and level 3 the
     * {@code Integer}.
     * <p>
     * The {@code typeIndexesPerLevel} map can be used to reference a specific generic for the given level. For example,
     * an index of 0 would refer to a {@code Map} key; whereas, 1 would refer to the value. If the map does not contain
     * a value for a specific level the last generic will be used (e.g. a {@code Map} value).
     * <p>
     ...
     *
     * @param nestingLevel
     *            the required nesting level, indexed from 1 for the current type, 2 for the first nested generic, 3 for
     *            the second and so on
     * @param typeIndexesPerLevel
     *            a map containing the generic index for a given nesting level (may be {@code null})
    ...
    /**
     * Return a {@link ResolvableType} representing the generic parameter for the given indexes. Indexes are zero based;
     * for example given the type {@code Map<Integer, List<String>>}, {@code getGeneric(0)} will access the
     * {@code Integer}. Nested generics can be accessed by specifying multiple indexes; for example
     * {@code getGeneric(1, 0)} will access the {@code String} from the nested {@code List}. For convenience, if no
     ...
     *
     * @param indexes
     *            the indexes that refer to the generic parameter (may be omitted to return the first generic)
    ...
    /**
     * Return an array of {@link ResolvableType ResolvableTypes} representing the generic parameters of this type. If no
     * generics are available an empty array is returned. If you need to access a specific generic consider using the
     * {@link #getGeneric(int...)} method as it allows access to nested generics and protects against
     * {@code IndexOutOfBoundsExceptions}.
     *
     * @return an array of {@link ResolvableType ResolvableTypes} representing the generic parameters (never
     *         {@code null})
     * @see #hasGenerics()
     * @see #getGeneric(int...)
     * @see #resolveGeneric(int...)
     ...
     *
     * @return an array of resolved generic parameters (the resulting array will never be {@code null}, but it may
    ...
    /**
     * Convenience method that will {@link #getGenerics() get} and {@link #resolve() resolve} generic parameters, using
     * the specified {@code fallback} if any type cannot be resolved.
     *
     * @param fallback
    ...
    /**
     * Resolve this type to a {@link java.lang.Class}, returning {@code null} if the type cannot be resolved. This
     * method will consider bounds of {@link TypeVariable TypeVariables} and {@link WildcardType WildcardTypes} if
     * direct resolution fails; however, bounds of {@code Object.class} will be ignored.
     * <p>
     * If this method returns a non-null {@code Class} and {@link #hasGenerics()} returns {@code false}, the given type
     * effectively wraps a plain {@code Class}, allowing for plain {@code Class} processing if desirable.
     *
    ...
    /**
     * Resolve this type to a {@link java.lang.Class}, returning the specified {@code fallback} if the type cannot be
     * resolved. This method will consider bounds of {@link TypeVariable TypeVariables} and {@link WildcardType
    ...
    private static class WildcardBounds {
        ...
        /**
         * Get a {@link WildcardBounds} instance for the specified type, returning {@code null} if the specified type
         * cannot be resolved to a {@link WildcardType}.
         *
         * @param type
         *            the source type
