...
/**
 * {@link ParameterNameDiscoverer} implementation which uses Kotlin's reflection facilities for introspecting parameter
 * names.
 *
 * Compared to {@link StandardReflectionParameterNameDiscoverer}, it allows in addition to determine interface parameter
 * names without requiring Java 8 -parameters compiler flag.
 *
...
public class KotlinReflectionParameterNameDiscoverer implements ParameterNameDiscoverer {
    ...
    private String[] getParameterNames(List<KParameter> parameters) {
        List<KParameter> filteredParameters = parameters.stream()
            // Extension receivers of extension methods must be included as they appear as normal method parameters in
            // Java
        ...
        for (int i = 0; i < filteredParameters.size(); i++) {
            ...
            String name =
                KParameter.Kind.EXTENSION_RECEIVER.equals(parameter.getKind()) ? "$receiver" : parameter.getName();
            if (name == null) {
                return null;
            }
