...
public final class ReactiveTypeDescriptor {
    ...
    /**
     * The same as {@link #singleOptionalValue(Class, Supplier)} but for a non-deferred, async type such as
    ...
    /**
     * Return {@code true} if the reactive type can produce more than 1 value can be produced and is therefore a good
     * fit to adapt to {@code Flux}. A {@code false} return value implies the reactive type can produce 1 value at most
     * and is therefore a good fit to adapt to {@code Mono}.
     */
    ...
    /**
     * Return {@code true} if the reactive type does not produce any values and only provides completion and error
     * signals.
     */
    ...
    /**
     * Whether the underlying operation is deferred and needs to be started explicitly, e.g. via subscribing (or
     * similar), or whether it is triggered without the consumer having any control.
     *
     * @since 5.2.7
     */
