...
public interface Profiles {
     ...
     * <p>
     * The returned instance will {@linkplain Profiles#matches(Predicate) match} if any one of the given profile strings
     * matches.
     * <p>
     * A profile string may contain a simple profile name (for example {@code "production"}) or a profile expression. A
     ...
     * </ul>
     * <p>
     * Please note that the {@code &} and {@code |} operators may not be mixed without using parentheses. For example
     * {@code "a & b | c"} is not a valid expression; it must be expressed as {@code "(a & b) | c"} or
     * {@code "a & (b | c)"}.
     * <p>
     * As of Spring Framework 5.1.17, two {@code Profiles} instances returned by this method are considered equivalent
     * to each other (in terms of {@code equals()} and {@code hashCode()} semantics) if they are created with identical
     * <em>profile strings</em>.
     *
     * @param profiles
     *            the <em>profile strings</em> to include
     ...
     *            a predicate that tests whether a given profile is currently active
     */
