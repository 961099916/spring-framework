...
/**
 * A single merged annotation returned from a {@link MergedAnnotations} collection. Presents a view onto an annotation
 * where attribute values may have been "merged" from different source values.
 *
 * <p>
 * Attribute values may be accessed using the various {@code get} methods. For example, to access an {@code int}
 * attribute the {@link #getInt(String)} method would be used.
 *
 * <p>
 * Note that attribute values are <b>not</b> converted when accessed. For example, it is not possible to call
 * {@link #getString(String)} if the underlying attribute is an {@code int}. The only exception to this rule is
 * {@code Class} and {@code Class[]} values which may be accessed as {@code String} and {@code String[]} respectively to
 * prevent potential early class initialization.
 *
 * <p>
 * If necessary, a {@code MergedAnnotation} can be {@linkplain #synthesize() synthesized} back into an actual
...
public interface MergedAnnotation<A extends Annotation> {
    ...
    /**
     * Create a {@link MergedAnnotation} that represents a missing annotation (i.e. one that is not present).
     *
     ...
     *
     * @param source
     *            the source for the annotation. This source is used only for information and logging. It does not need
    ...
    /**
     * Create a new {@link MergedAnnotation} instance of the specified annotation type. The resulting annotation will
     * not have any attribute values but may still be used to query default values.
     *
    ...
    /**
     * Create a new {@link MergedAnnotation} instance of the specified annotation type with attribute values supplied by
     * a map.
     *
     * @param annotationType
     *            the annotation type
     * @param attributes
     *            the annotation attributes or {@code null} if just default values should be used
    ...
    /**
     * Determine if the annotation is present on the source. Considers {@linkplain #isDirectlyPresent() directly
     * present} and {@linkplain #isMetaPresent() meta-present} annotations within the context of the
     * {@link SearchStrategy} used.
     *
     ...
     * <p>
     * A directly present annotation is one that the user has explicitly declared and not one that is
     * {@linkplain #isMetaPresent() meta-present} or {@link Inherited @Inherited}.
     *
     ...
     * <p>
     * A meta-present annotation is an annotation that the user hasn't explicitly declared, but has been used as a
     ...
     * <p>
     * A directly declared annotation has a distance of {@code 0}, a meta-annotation has a distance of {@code 1}, a
     * meta-annotation on a meta-annotation has a distance of {@code 2}, etc. A {@linkplain #missing() missing}
     * annotation will always return a distance of {@code -1}.
     *
     ...
     * <p>
     * Can be used to reorder a stream of annotations, for example, to give a higher priority to annotations declared on
     * a superclass or interface. A {@linkplain #missing() missing} annotation will always return an aggregate index of
     * {@code -1}.
     *
    ...
    /**
     * Get the source that ultimately declared the root annotation, or {@code null} if the source is not known.
     * <p>
     * If this merged annotation was created {@link MergedAnnotations#from(AnnotatedElement) from} an
     * {@link AnnotatedElement} then this source will be an element of the same type. If the annotation was loaded
     * without using reflection, the source can be of any type, but should have a sensible {@code toString()}.
    ...
    /**
     * Get the source of the meta-annotation, or {@code null} if the annotation is not {@linkplain #isMetaPresent()
     * meta-present}.
     * <p>
    ...
    /**
     * Get the root annotation, i.e. the {@link #getDistance() distance} {@code 0} annotation as directly declared on
     * the source.
     *
     ...
     *            the attribute name
     * @param type
     *            the attribute type. Must be compatible with the underlying attribute type or {@code Object.class}.
     * @return an optional value or {@link Optional#empty()} if there is no matching attribute
     */
     ...
     * @param attributeName
     *            the attribute name
     * @return an optional of the default value or {@link Optional#empty()} if there is no matching attribute or no
     *         defined default
     */
     ...
     * <p>
     * Methods from this view will return attribute values with only alias mirroring rules applied. Aliases to
     * {@link #getMetaSource() meta-source} attributes will not be applied.
     *
     ...
     *
     * @param adaptations
     *            the adaptations that should be applied to the annotation values
    ...
    /**
     * Create a new {@link Map} instance of the given type that contains all the annotation attributes.
     * <p>
     * The {@link Adapt adaptations} may be used to change the way that values are added.
     *
     * @param factory
     ...
     * <p>
     * The result is synthesized using a JDK {@link Proxy} and as a result may incur a computational cost when first
     * invoked.
     * <p>
     * If this merged annotation was created {@linkplain #from(Annotation) from} an annotation instance, that annotation
     * will be returned unmodified if it is not <em>synthesizable</em>. An annotation is considered synthesizable if one
     * of the following is true.
     * <ul>
     * <li>The annotation declares attributes annotated with {@link AliasFor @AliasFor}.</li>
     * <li>The annotation is a composed annotation that relies on convention-based annotation attribute overrides in
     * meta-annotations.</li>
     * <li>The annotation declares attributes that are annotations or arrays of annotations that are themselves
     * synthesizable.</li>
     * </ul>
     *
     ...
     * invoked.
     * <p>
     * Consult the documentation for {@link #synthesize()} for an explanation of what is considered synthesizable.
     *
     * @param condition
     *            the test to determine if the annotation can be synthesized
     * @return an optional containing the synthesized version of the annotation or an empty optional if the condition
     *         doesn't match
     * @throws NoSuchElementException
     *             on a missing annotation
     * @see MergedAnnotationPredicates
    ...
    /**
     * Adaptations that can be applied to attribute values when creating {@linkplain MergedAnnotation#asMap(Adapt...)
     * Maps} or {@link MergedAnnotation#asAnnotationAttributes(Adapt...) AnnotationAttributes}.
     */
    enum Adapt {
