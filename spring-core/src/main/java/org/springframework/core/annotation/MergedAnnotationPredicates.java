...
/**
 * Predicate implementations that provide various test operations for {@link MergedAnnotation MergedAnnotations}.
 *
 * @author Phillip Webb
 * @since 5.2
 */
public abstract class MergedAnnotationPredicates {
     ...
     * {@linkplain MergedAnnotation#getType() merged annotation type} is contained in the specified array.
     * 
     * @param <A>
     *            the annotation type
     * @param typeNames
    ...
    /**
     * Create a new {@link Predicate} that evaluates to {@code true} if the {@linkplain MergedAnnotation#getType()
     * merged annotation type} is contained in the specified array.
     * 
     * @param <A>
     *            the annotation type
     * @param types
    ...
    /**
     * Create a new {@link Predicate} that evaluates to {@code true} if the {@linkplain MergedAnnotation#getType()
     * merged annotation type} is contained in the specified collection.
     * 
     * @param <A>
     *            the annotation type
     * @param types
    ...
    /**
     * Create a new stateful, single use {@link Predicate} that matches only the first run of an extracted value. For
     * example, {@code MergedAnnotationPredicates.firstRunOf(MergedAnnotation::distance)} will match the first
     * annotation, and any subsequent runs that have the same distance.
     * <p>
     * NOTE: This predicate only matches the first run. Once the extracted value changes, the predicate always returns
     * {@code false}. For example, if you have a set of annotations with distances {@code [1, 1, 2, 1]} then only the
     * first two will match.
     * 
     ...
     */
    public static <A extends Annotation> Predicate<MergedAnnotation<A>>
    ...
    /**
     * Create a new stateful, single use {@link Predicate} that matches annotations that are unique based on the
     * extracted key. For example {@code MergedAnnotationPredicates.unique(MergedAnnotation::getType)} will match the
     ...
     */
    public static <A extends Annotation, K> Predicate<MergedAnnotation<A>>
    ...
    /**
     * {@link Predicate} implementation used for {@link MergedAnnotationPredicates#firstRunOf(Function)}.
     */
