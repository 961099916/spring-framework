...
/**
 * {@link Collector} implementations that provide various reduction operations for {@link MergedAnnotation} instances.
 *
 * @author Phillip Webb
 * @author Sam Brannen
 * @since 5.2
...
public abstract class MergedAnnotationCollectors {
    ...
    /**
     * Create a new {@link Collector} that accumulates merged annotations to a {@link LinkedHashSet} containing
     ...
     * {@code Collectors.mapping(MergedAnnotation::synthesize, Collectors.toCollection(LinkedHashSet::new))} but avoids
     * the creation of a composite collector.
     * 
     * @param <A>
     *            the annotation type
    ...
    /**
     * Create a new {@link Collector} that accumulates merged annotations to an {@link Annotation} array containing
     * {@linkplain MergedAnnotation#synthesize() synthesized} versions.
     * 
     * @param <A>
     *            the annotation type
     * @return a {@link Collector} which collects and synthesizes the annotations into an {@code Annotation[]}
     ...
     * @return a {@link Collector} which collects and synthesizes the annotations into an annotation array
     * @see #toAnnotationArray
     */
    public static <R extends Annotation, A extends R> Collector<MergedAnnotation<A>, ?, R[]>
    ...
    /**
     * Create a new {@link Collector} that accumulates merged annotations to a {@link MultiValueMap} with items
     * {@linkplain MultiValueMap#add(Object, Object) added} from each merged annotation
     ...
     * @return a {@link Collector} which collects and synthesizes the annotations into a {@link LinkedMultiValueMap}
     * @see #toMultiValueMap(Function, MergedAnnotation.Adapt...)
     */
    public static <A extends Annotation> Collector<MergedAnnotation<A>, ?, MultiValueMap<String, Object>>
    ...
    public static <A extends Annotation> Collector<MergedAnnotation<A>, ?, MultiValueMap<String, Object>>
        ...
        Characteristics[] characteristics =
            (isSameInstance(finisher, Function.identity()) ? IDENTITY_FINISH_CHARACTERISTICS : NO_CHARACTERISTICS);
        return Collector.of(LinkedMultiValueMap::new,
            (map, annotation) -> annotation.asMap(adaptations).forEach(map::add), MergedAnnotationCollectors::combiner,
