...
/**
 * Unit tests demonstrating that the reflection-based {@link StandardAnnotationMetadata} and ASM-based
 * {@code SimpleAnnotationMetadata} produce <em>almost</em> identical output.
 *
...
class AnnotationMetadataTests {
    ...
    /**
     * In order to preserve backward-compatibility, {@link StandardAnnotationMetadata} defaults to return nested
     * annotations and annotation arrays as actual Annotation instances. It is recommended for compatibility with
     * ASM-based AnnotationMetadata implementations to set the 'nestedAnnotationsAsMap' flag to 'true' as is done in the
     * main test above.
     */
    @Test
    @Deprecated
    ...
    void metaAnnotationOverridesUsingAnnotationMetadataReadingVisitor() throws Exception {
        MetadataReaderFactory metadataReaderFactory = new SimpleMetadataReaderFactory();
        MetadataReader metadataReader =
            metadataReaderFactory.getMetadataReader(ComposedConfigurationWithAttributeOverridesClass.class.getName());
        AnnotationMetadata metadata = metadataReader.getAnnotationMetadata();
        assertMetaAnnotationOverrides(metadata);
    }
