...
class CollectionFactoryTests {
     ...
     * <p>
     * Specifically, the parameterized type {@code E} is not bound to the type of elements contained in the
     * {@code collection} argument passed to {@code createApproximateCollection()}. Thus casting the value returned by
     * {@link EnumSet#copyOf(EnumSet)} to {@code (Collection<E>)} cannot guarantee that the returned collection actually
    ...
    /**
     * The test demonstrates that the generics-based API for {@link CollectionFactory#createApproximateMap(Object, int)}
     * is not type-safe.
     * <p>
