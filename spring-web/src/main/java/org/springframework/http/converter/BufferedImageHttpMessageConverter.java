 ...
 * {@linkplain ImageIO#getReaderMIMETypes() registered image readers}, and writes using the media type of the first
 * available {@linkplain javax.imageio.ImageIO#getWriterMIMETypes() registered image writer}. The latter can be
 * overridden by setting the {@link #setDefaultContentType defaultContentType} property.
 *
 * <p>
 * If the {@link #setCacheDir cacheDir} property is set, this converter will cache image data.
 *
 * <p>
 * The {@link #process(ImageReadParam)} and {@link #process(ImageWriteParam)} template methods allow subclasses to
        ...
        try {
            ...
            if (imageReaders.hasNext()) {
                ...
                throw new HttpMessageNotReadableException(
                    "Could not find javax.imageio.ImageReader for Content-Type [" + contentType + "]", inputMessage);
            }
        } finally {
            if (imageReader != null) {
                imageReader.dispose();
