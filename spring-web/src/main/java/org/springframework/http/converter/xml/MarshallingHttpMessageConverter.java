 ...
 *
 * <p>
 * This converter requires a {@code Marshaller} and {@code Unmarshaller} before it can be used. These can be injected by
 * the {@linkplain #MarshallingHttpMessageConverter(Marshaller) constructor} or {@linkplain #setMarshaller(Marshaller)
 * bean properties}.
 *
 * <p>
 * By default, this converter supports {@code text/xml} and {@code application/xml}. This can be overridden by setting
 * the {@link #setSupportedMediaTypes(java.util.List) supportedMediaTypes} property.
 *
...
public class MarshallingHttpMessageConverter extends AbstractXmlHttpMessageConverter<Object> {
    ...
    /**
     * Construct a new {@code MarshallingHttpMessageConverter} with no {@link Marshaller} or {@link Unmarshaller} set.
     * The Marshaller and Unmarshaller must be set after construction by invoking {@link #setMarshaller(Marshaller)} and
     ...
     * <p>
     * If the given {@link Marshaller} also implements the {@link Unmarshaller} interface, it is used for both
     * marshalling and unmarshalling. Otherwise, an exception is thrown.
     * <p>
     * Note that all {@code Marshaller} implementations in Spring also implement the {@code Unmarshaller} interface, so
