 ...
 *
 * <p>
 * Adding Cache-Control directives to HTTP responses can significantly improve the client experience when interacting
 ...
 *
 * <ul>
 * <li>Caching HTTP responses with {@code CacheControl cc = CacheControl.maxAge(1, TimeUnit.HOURS)} will result in
 ...
 *
 * <p>
 * Note that to be efficient, Cache-Control headers should be written along HTTP validators such as "Last-Modified" or
 * "ETag" headers.
 *
 * @author Brian Clozel
...
public class CacheControl {
     ...
     * <p>
     * This directive is well suited for publicly caching resources, knowing that they won't change within the
     * configured amount of time. Additional directives can be also used, in case resources shouldn't be cached
     * ({@link #cachePrivate()}) or transformed ({@link #noTransform()}) by shared caches.
     * <p>
     * In order to prevent caches to reuse the cached response even when it has become stale (i.e. the "max-age" delay
     * is passed), the "must-revalidate" directive should be set ({@link #mustRevalidate()}
     * 
     * @param maxAge
     ...
     * <p>
     * This directive is well suited for telling caches that the response can be reused only if the client revalidates
     * it with the server. This directive won't disable cache altogether and may result with clients sending conditional
     * requests (with "ETag", "If-Modified-Since" headers) and the server responding with "304 - Not Modified" status.
     * <p>
     * In order to disable caching and minimize requests/responses exchanges, the {@link #noStore()} directive should be
     * used instead of {@code #noCache()}.
     * 
     * @return {@code this}, to facilitate method chaining
     ...
     * <p>
     * This directive indicates that once it has become stale, a cache MUST NOT use the response to satisfy subsequent
     ...
     * <p>
     * This directive indicates that intermediaries (caches and others) should not transform the response content. This
     * can be useful to force caches and CDNs not to automatically gzip or optimize the response content.
     * 
     * @return {@code this}, to facilitate method chaining
     ...
     * <p>
     * This directive indicates that any cache MAY store the response, even if the response would normally be
     * non-cacheable or cacheable only within a private cache.
     * 
     * @return {@code this}, to facilitate method chaining
     ...
     * <p>
     * This directive indicates that the response message is intended for a single user and MUST NOT be stored by a
     * shared cache.
     * 
     * @return {@code this}, to facilitate method chaining
     ...
     * <p>
     * This directive has the same meaning as the "must-revalidate" directive, except that it does not apply to private
     * caches (i.e. browsers, HTTP clients).
     * 
     * @return {@code this}, to facilitate method chaining
     ...
     * <p>
     * This directive indicates that, in shared caches, the maximum age specified by this directive overrides the
     * maximum age specified by other directives.
     * 
     * @param sMaxAge
     *            the maximum time the response should be cached
     * @param unit
     ...
     * <p>
     * This directive indicates that caches MAY serve the response in which it appears after it becomes stale, up to the
     * indicated number of seconds. If a cached response is served stale due to the presence of this extension, the
     * cache SHOULD attempt to revalidate it while still serving stale responses (i.e. without blocking).
     * 
     ...
     * <p>
     * This directive indicates that when an error is encountered, a cached stale response MAY be used to satisfy the
     * request, regardless of other freshness information.
     * 
     * @param staleIfError
