 ...
 * {@link ConnectionProvider} for the connection pool, within the lifecycle of a Spring {@code ApplicationContext}.
 *
 * <p>
 * This factory implements {@link InitializingBean} and {@link DisposableBean} and is expected typically to be declared
...
public class ReactorResourceFactory implements InitializingBean, DisposableBean {
    ...
    /**
     * Whether this factory exposes the global {@link reactor.netty.http.HttpResources HttpResources} holder.
     */
     ...
     * <p>
     * Default is "true" in which case this factory initializes and stops the global Reactor Netty resources within
     * Spring's {@code ApplicationContext} lifecycle. If set to "false" the factory manages its resources independent of
     * the global ones.
     *
     * @param useGlobalResources
     *            whether to expose and manage the global resources
    ...
    /**
     * Add a Consumer for configuring the global Reactor Netty resources on startup. When this option is used,
     * {@link #setUseGlobalResources} is also enabled.
     * 
     * @param consumer
    ...
    public void addGlobalResourcesConsumer(Consumer<HttpResources> consumer) {
        ...
        this.globalResourcesConsumer =
            (this.globalResourcesConsumer != null ? this.globalResourcesConsumer.andThen(consumer) : consumer);
    }

    /**
     * Use this when you don't want to participate in global resources and you want to customize the creation of the
     * managed {@code ConnectionProvider}.
     * <p>
    ...
    /**
     * Use this when you don't want to participate in global resources and you want to customize the creation of the
     * managed {@code LoopResources}.
     * <p>
     * By default, {@code LoopResources.create("reactor-http")} is used.
     * <p>
     * Note that this option is ignored if {@code userGlobalResources=false} or {@link #setLoopResources(LoopResources)}
     * is set.
     * 
     * @param supplier
     *            the supplier to use
    ...
    /**
     * Configure the amount of time we'll wait before shutting down resources. If a task is submitted during the
     * {@code shutdownQuietPeriod}, it is guaranteed to be accepted and the {@code shutdownQuietPeriod} will start over.
     * <p>
     * By default, this is set to {@link LoopResources#DEFAULT_SHUTDOWN_QUIET_PERIOD} which is 2 seconds but can also be
     * overridden with the system property {@link reactor.netty.ReactorNetty#SHUTDOWN_QUIET_PERIOD
    ...
    /**
     * Configure the maximum amount of time to wait until the disposal of the underlying resources regardless if a task
     * was submitted during the {@code shutdownQuietPeriod}.
     * <p>
     * By default, this is set to {@link LoopResources#DEFAULT_SHUTDOWN_TIMEOUT} which is 15 seconds but can also be
     * overridden with the system property {@link reactor.netty.ReactorNetty#SHUTDOWN_TIMEOUT
