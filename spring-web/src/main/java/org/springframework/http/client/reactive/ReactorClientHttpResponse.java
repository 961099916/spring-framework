...
class ReactorClientHttpResponse implements ClientHttpResponse {
    ...
    public MultiValueMap<String, ResponseCookie> getCookies() {
        ...
        this.response.cookies().values().stream().flatMap(Collection::stream)
            .forEach(c -> result.add(c.name(), ResponseCookie.fromClientResponse(c.name(), c.value()).domain(c.domain())
                .path(c.path()).maxAge(c.maxAge()).secure(c.isSecure()).httpOnly(c.isHttpOnly()).build()));
        return CollectionUtils.unmodifiableMultiValueMap(result);
    }

    /**
     * Called by {@link ReactorClientHttpConnector} when a cancellation is detected but the content has not been
     * subscribed to. If the subscription never materializes then the content will remain not drained. Or it could still
     * materialize if the cancellation happened very early, or the response reading was delayed for some reason.
     */
