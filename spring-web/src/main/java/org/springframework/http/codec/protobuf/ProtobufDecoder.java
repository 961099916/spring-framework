 ...
 *
 * <p>
 * Flux deserialized via {@link #decode(Publisher, ResolvableType, MimeType, Map)} are expected to use
 * <a href="https://developers.google.com/protocol-buffers/docs/techniques?hl=en#streaming"> delimited Protobuf
 * messages</a> with the size of each message specified before the message itself. Single values deserialized via
 * {@link #decodeToMono(Publisher, ResolvableType, MimeType, Map)} are expected to use regular Protobuf message format
 ...
 * {@code Mono.just(Msg.getDefaultInstance())} sent over the network will be deserialized as an empty {@link Mono}.
 *
 * <p>
 * To generate {@code Message} Java classes, you need to install the {@code protoc} binary.
 *
 * <p>
 * This decoder requires Protobuf 3 or higher, and supports {@code "application/x-protobuf"} and
 * {@code "application/octet-stream"} with the official {@code "com.google.protobuf:protobuf-java"} library.
 *
...
public class ProtobufDecoder extends ProtobufCodecSupport implements Decoder<Message> {
    ...
    /**
     * Construct a new {@code ProtobufDecoder} with an initializer that allows the registration of message extensions.
     * 
     * @param extensionRegistry
     *            a message extension registry
     */
    ...
    private class MessageDecoderFunction implements Function<DataBuffer, Iterable<? extends Message>> {
            ...
            try {
                ...
                do {
                    ...
                    if (this.messageBytesToRead == 0) {
                        ...
                        this.output = null;
                        Message message =
                            getMessageBuilder(this.elementType.toClass()).mergeFrom(stream, extensionRegistry).build();
        ...
        /**
         * Parse message size as a varint from the input stream, updating {@code messageBytesToRead} and {@code offset}
         * fields if needed to allow processing of upcoming chunks. Inspired from
