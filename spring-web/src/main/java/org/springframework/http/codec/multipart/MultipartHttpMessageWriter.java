...
/**
 * {@link HttpMessageWriter} for writing a {@code MultiValueMap<String, ?>} as multipart form data, i.e.
 * {@code "multipart/form-data"}, to the body of a request.
 *
 * <p>
 * The serialization of individual parts is delegated to other writers. By default only {@link String} and
 * {@link Resource} parts are supported but you can configure others through a constructor argument.
 *
 * <p>
 * This writer can be configured with a {@link FormHttpMessageWriter} to delegate to. It is the preferred way of
 * supporting both form data and multipart data (as opposed to registering each writer separately) so that when the
 * {@link MediaType} is not specified and generics are not present on the target element type, we can inspect the values
 * in the actual map and decide whether to write plain form data (String values only) or otherwise.
 *
...
public class MultipartHttpMessageWriter extends LoggingCodecSupport
    ...
    /**
     * Constructor with explicit list of writers for serializing parts and a writer for plain form data to fall back
     * when no media type is specified and the actual map consists of String values only.
     * 
     * @param partWriters
     *            the writers for serializing parts
     * @param formWriter
     *            the fallback writer for form data, {@code null} by default
     */
    public MultipartHttpMessageWriter(List<HttpMessageWriter<?>> partWriters,
    ...
    private <T> Flux<DataBuffer> encodePart(byte[] boundary, String name, T value, DataBufferFactory bufferFactory) {
        ...
        Optional<HttpMessageWriter<?>> writer =
            this.partWriters.stream().filter(partWriter -> partWriter.canWrite(finalBodyType, contentType)).findFirst();
