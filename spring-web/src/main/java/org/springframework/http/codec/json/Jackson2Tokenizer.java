...
/**
 * {@link Function} to transform a JSON stream of arbitrary size, byte array chunks into a {@code Flux<TokenBuffer>}
 * where each token buffer is a well-formed JSON object.
 *
...
final class Jackson2Tokenizer {
     ...
     * @param tokenizeArrays
     *            if {@code true} and the "top level" JSON object is an array, each element is returned individually
     *            immediately after it is received
     * @param forceUseOfBigDecimal
     *            if {@code true}, any floating point values encountered in source will use {@link java.math.BigDecimal}
