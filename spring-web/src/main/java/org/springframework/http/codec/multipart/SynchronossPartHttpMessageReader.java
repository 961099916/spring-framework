...
/**
 * {@code HttpMessageReader} for parsing {@code "multipart/form-data"} requests to a stream of {@link Part}'s using the
 * Synchronoss NIO Multipart library.
 *
 * <p>
 * This reader can be provided to {@link MultipartHttpMessageReader} in order to aggregate all parts into a Map.
 *
...
public class SynchronossPartHttpMessageReader extends LoggingCodecSupport implements HttpMessageReader<Part> {
     ...
     *
     * @param byteCount
     *            the in-memory limit in bytes; if set to -1 this limit is not enforced, and all parts may be written to
     *            disk and are limited only by the {@link #setMaxDiskUsagePerPart(long) maxDiskUsagePerPart} property.
     * @since 5.1.11
     */
     ...
     *            the disk limit in bytes, or -1 for unlimited
     * @since 5.1.11
     */
    ...
    public Flux<Part> read(ResolvableType elementType, ReactiveHttpInputMessage message, Map<String, Object> hints) {
        return getFileStorageDirectory()
            .flatMapMany(directory -> Flux.create(new SynchronossPartGenerator(message, directory)).doOnNext(part -> {
    ...
    /**
     * Subscribe to the input stream and feed the Synchronoss parser. Then listen for parser output, creating parts, and
     * pushing them into the FluxSink.
     */
        ...
        public void increaseByteCount(long byteCount) {
            ...
            if (maxDiskUsagePerPart > 0 && this.isFilePart && this.partSize > maxDiskUsagePerPart) {
                throw new DecodingException(
                    "Part[" + this.index + "] " + "exceeded the disk usage limit of " + maxDiskUsagePerPart + " bytes");
            }
        }
