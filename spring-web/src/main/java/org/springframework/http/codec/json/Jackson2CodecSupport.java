...
public abstract class Jackson2CodecSupport {
     ...
     * (server-side only). Currently set when the method argument has generics because in case of reactive types, use of
     * {@code ResolvableType.getGeneric()} means no MethodParameter source and no knowledge of the containing class.
     */
    ...
    private static final List<MimeType> DEFAULT_MIME_TYPES = Collections
        .unmodifiableList(Arrays.asList(new MimeType("application", "json"), new MimeType("application", "*+json")));
