...
/**
 * Defines a common interface for configuring either client or server HTTP message readers and writers. This is used as
 * follows:
 * <ul>
 * <li>Use {@link ClientCodecConfigurer#create()} or {@link ServerCodecConfigurer#create()} to create an instance.
 * <li>Use {@link #defaultCodecs()} to customize HTTP message readers or writers registered by default.
 * <li>Use {@link #customCodecs()} to add custom HTTP message readers or writers.
 * <li>Use {@link #getReaders()} and {@link #getWriters()} to obtain the list of configured HTTP message readers and
 * writers.
 * </ul>
 *
 * <p>
...
public interface CodecConfigurer {
    ...
    /**
     * Provides a way to completely turn off registration of default HTTP message readers and writers, and instead rely
     * only on the ones provided via {@link #customCodecs()}.
     * <p>
     * By default this is set to {@code "true"} in which case default registrations are made; setting this to
     * {@code false} disables default registrations.
     */
    ...
    /**
     * Create a copy of this {@link CodecConfigurer}. The returned clone has its own lists of default and custom codecs
     * and generally can be configured independently. Keep in mind however that codec instances (if any are configured)
     * are themselves not cloned.
     * 
     * @since 5.1.12
     */
    ...
    /**
     * Customize or replace the HTTP message readers and writers registered by default. The options are further extended
     * by {@link ClientCodecConfigurer.ClientDefaultCodecs ClientDefaultCodecs} and
    ...
    interface DefaultCodecs {
        ...
        /**
         * Configure a limit on the number of bytes that can be buffered whenever the input stream needs to be
         * aggregated. This can be a result of decoding to a single {@code DataBuffer}, {@link java.nio.ByteBuffer
         * ByteBuffer}, {@code byte[]}, {@link org.springframework.core.io.Resource Resource}, {@code String}, etc. It
         * can also occur when splitting the input stream, e.g. delimited text, in which case the limit applies to data
         * buffered between delimiters.
         * <p>
         * By default this is not set, in which case individual codec defaults apply. All codecs are limited to 256K by
         * default.
         * 
         * @param byteCount
    ...
    interface CustomCodecs {
        ...
        /**
         * Variant of {@link #register(Object)} that also applies the below properties, if configured, via
         * {@link #defaultCodecs()}:
         * <ul>
         ...
         * </ul>
         * <p>
         * The properties are applied every time {@link #getReaders()} or {@link #getWriters()} are used to obtain the
         * list of configured readers or writers.
         * 
         * @param codec
         *            the codec to register and apply default config to
         * @since 5.1.13
         */
        void registerWithDefaultConfig(Object codec);

        /**
         * Variant of {@link #register(Object)} that also allows the caller to apply the properties from
         * {@link DefaultCodecConfig} to the given codec. If you want to apply all the properties, prefer using
         * {@link #registerWithDefaultConfig(Object)}.
         * <p>
         * The consumer is called every time {@link #getReaders()} or {@link #getWriters()} are used to obtain the list
         * of configured readers or writers.
         * 
         * @param codec
         *            the codec to register
         * @param configConsumer
         *            consumer of the default config
         * @since 5.1.13
         */
         ...
         * @param decoder
         *            the decoder to add
         * @deprecated as of 5.1.13, use {@link #register(Object)} or {@link #registerWithDefaultConfig(Object)}
         *             instead.
         */
        @Deprecated
        ...
        /**
         * Add a custom {@link HttpMessageReader}. For readers of type {@link DecoderHttpMessageReader} consider using
         * the shortcut {@link #decoder(Decoder)} instead.
         * 
         * @param reader
         *            the reader to add
         * @deprecated as of 5.1.13, use {@link #register(Object)} or {@link #registerWithDefaultConfig(Object)}
        ...
        /**
         * Add a custom {@link HttpMessageWriter}. For writers of type {@link EncoderHttpMessageWriter} consider using
         * the shortcut {@link #encoder(Encoder)} instead.
         * 
         * @param writer
         *            the writer to add
         * @deprecated as of 5.1.13, use {@link #register(Object)} or {@link #registerWithDefaultConfig(Object)}
        ...
        /**
         * Register a callback for the {@link DefaultCodecConfig configuration} applied to default codecs. This allows
         * custom codecs to follow general guidelines applied to default ones, such as logging details and limiting the
         * amount of buffered data.
         * 
         * @param codecsConfigConsumer
    ...
    /**
     * Exposes the values of properties configured through {@link #defaultCodecs()} that are applied to default codecs.
     * The main purpose of this interface is to provide access to them so they can also be applied to custom codecs if
     * needed.
     * 
     * @since 5.1.12
    ...
    interface DefaultCodecConfig {
        ...
        /**
         * Get the configured limit on the number of bytes that can be buffered whenever the input stream needs to be
         * aggregated.
         */
        @Nullable
