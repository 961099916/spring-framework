...
public class Jaxb2XmlDecoder extends AbstractDecoder<Object> {
    ...
    /**
     * Set the max number of bytes that can be buffered by this decoder. This is either the size of the entire input
     * when decoding as a whole, or when using async parsing with Aalto XML, it is the size of one top-level XML tree.
    ...
    public Flux<Object> decode(Publisher<DataBuffer> inputStream, ResolvableType elementType,
        ...
        Flux<XMLEvent> xmlEventFlux =
            this.xmlEventDecoder.decode(inputStream, ResolvableType.forClass(XMLEvent.class), mimeType, hints);
    ...
    QName toQName(Class<?> outputClass) {
        ...
        if (outputClass.isAnnotationPresent(XmlRootElement.class)) {
            ...
            throw new IllegalArgumentException(
                "Output class [" + outputClass.getName() + "] is neither annotated with @XmlRootElement nor @XmlType");
        }
    ...
    /**
     * Split a flux of {@link XMLEvent XMLEvents} into a flux of XMLEvent lists, one list for each branch of the tree
     * that starts with the given qualified name. That is, given the XMLEvents shown {@linkplain XmlEventDecoder here},
     * and the {@code desiredName} "{@code child}", this method returns a flux of two lists, each of which containing
     * the events of a particular branch of the tree that starts with "{@code child}".
     * <ol>
