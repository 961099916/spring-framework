 ...
 * messages</a> with the size of each message specified before the message itself. Single values are serialized using
 ...
 *
 * <p>
 * This encoder requires Protobuf 3 or higher, and supports {@code "application/x-protobuf"} and
...
public class ProtobufEncoder extends ProtobufCodecSupport implements HttpMessageEncoder<Message> {
    ...
    private static final List<MediaType> streamingMediaTypes =
        MIME_TYPES.stream().map(mimeType -> new MediaType(mimeType.getType(), mimeType.getSubtype(),
            Collections.singletonMap(DELIMITED_KEY, DELIMITED_VALUE))).collect(Collectors.toList());
