 ...
 *
 * <p>
 * Note that this decoder is not registered by default but is used internally by other decoders which are registered by
 * default.
 *
...
public class XmlEventDecoder extends AbstractDecoder<XMLEvent> {
    ...
    private static final boolean aaltoPresent =
        ClassUtils.isPresent("com.fasterxml.aalto.AsyncXMLStreamReader", XmlEventDecoder.class.getClassLoader());
     ...
     * decoding as a whole, or when using async parsing via Aalto XML, it is size one top-level XML tree. When the limit
