...
/**
 * Base class providing support methods for Jackson 2.9 encoding. For non-streaming use cases, {@link Flux} elements are
 * collected into a {@link List} before serialization for performance reason.
 *
...
public abstract class AbstractJackson2Encoder extends Jackson2CodecSupport implements HttpMessageEncoder<Object> {
    ...
    private ObjectWriter createObjectWriter(ResolvableType valueType, @Nullable MimeType mimeType,
        ...
        ObjectWriter writer =
            (jsonView != null ? getObjectMapper().writerWithView(jsonView) : getObjectMapper().writer());
     ...
     *
     * @param mimeType
     *            the mime type as requested by the caller
    ...
    /**
     * Configure "streaming" media types for which flushing should be performed automatically vs at the end of the
     * stream.
     * <p>
     * By default this is set to {@link MediaType#APPLICATION_STREAM_JSON}.
     *
     * @param mediaTypes
     *            one or more media types to add to the list
