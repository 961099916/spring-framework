 ...
 *
 * <p>
 * Also a {@code HttpMessageWriter} that pre-resolves encoding hints from the extra information available on the server
 * side such as the request or controller method annotations.
 *
...
public class EncoderHttpMessageWriter<T> implements HttpMessageWriter<T> {
    ...
    public Mono<Void> write(Publisher<? extends T> inputStream, ResolvableType elementType,
        ...
        if (isStreamingMediaType(contentType)) {
            return message.writeAndFlushWith(
                body.map(buffer -> Mono.just(buffer).doOnDiscard(PooledDataBuffer.class, DataBufferUtils::release)));
        }
    ...
    /**
     * Get additional hints for encoding for example based on the server request or annotations from controller method
     * parameters. By default, delegate to the encoder if it is an instance of {@link HttpMessageEncoder}.
     */
