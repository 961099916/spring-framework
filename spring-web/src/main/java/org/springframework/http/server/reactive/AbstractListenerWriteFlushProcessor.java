...
/**
 * An alternative to {@link AbstractListenerWriteProcessor} but instead writing a {@code Publisher<Publisher<T>>} with
 * flush boundaries enforces after the completion of each nested Publisher.
 *
...
public abstract class AbstractListenerWriteFlushProcessor<T> implements Processor<Publisher<? extends T>, Void> {
    ...
    /**
     * Invoked when flushing is possible, either in the same thread after a check via {@link #isWritePossible()}, or as
     * a callback from the underlying container.
     */
    ...
    /**
     * Invoked when an error happens while flushing. Sub-classes may choose to ignore this if they know the underlying
     * API will provide an error notification in a container thread.
     * <p>
     * Defaults to no-op.
     */
    ...
    private enum State {
        ...
        /**
         * Subscriber to receive and delegate completion notifications for from the current Publisher, i.e. for the
         * current flush boundary.
         */
