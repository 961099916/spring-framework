...
/**
 * Abstract base class for {@code Processor} implementations that bridge between event-listener write APIs and Reactive
 * Streams.
 *
 * <p>
 * Specifically a base class for writing to the HTTP response body with Servlet 3.1 non-blocking I/O and Undertow XNIO
 * as well for writing WebSocket messages through the Java WebSocket API (JSR-356), Jetty, and Undertow.
 *
 ...
 * @since 5.0
 * @param <T>
 *            the type of element signaled to the {@link Subscriber}
 */
public abstract class AbstractListenerWriteProcessor<T> implements Processor<T, Void> {
    ...
    /**
     * Indicates we're waiting for one last isReady-onWritePossible cycle after "onComplete" because some Servlet
     * containers expect this to take place prior to calling AsyncContext.complete(). See
    ...
    /**
     * Error signal from the upstream, write Publisher. This is also used by sub-classes to delegate error notifications
     * from the container.
     */
    @Override
    ...
    /**
     * Completion signal from the upstream, write Publisher. This is also used by sub-classes to delegate completion
    ...
    /**
     * Invoked when writing is possible, either in the same thread after a check via {@link #isWritePossible()}, or as a
     * callback from the underlying container.
     */
    ...
    /**
     * Template method invoked after a data item to write is received via {@link Subscriber#onNext(Object)}. The default
     * implementation saves the data item for writing once that is possible.
     */
     ...
     * <p>
     * <strong>Note:</strong> Sub-classes are responsible for releasing any data buffer associated with the item, once
     * fully written, if pooled buffers apply to the underlying container.
     * 
     * @param data
     *            the item to write
     * @return {@code true} if the current data item was written completely and a new item requested, or {@code false}
     *         if it was written partially and we'll need more write callbacks before it is fully written
     */
    protected abstract boolean write(T data) throws IOException;

    /**
     * Invoked after the current data has been written and before requesting the next item from the upstream, write
     * Publisher.
     * <p>
     * The default implementation is a no-op.
     * 
     * @deprecated originally introduced for Undertow to stop write notifications when no data is available, but
     *             deprecated as of as of 5.0.6 since constant switching on every requested item causes a significant
     *             slowdown.
     */
    @Deprecated
    ...
    /**
     * Invoked when an I/O error occurs during a write. Sub-classes may choose to ignore this if they know the
     * underlying API will provide an error notification in a container thread.
     * <p>
     * Defaults to no-op.
     */
    ...
    /**
     * Invoked after any error (either from the upstream write Publisher, or from I/O operations to the underlying
     * server) and cancellation to discard in-flight data that was in the process of being written when the error took
     * place.
     * 
     * @param data
