...
public abstract class AbstractServerHttpRequest implements ServerHttpRequest {
    ...
    /**
     * A method for parsing of the query into name-value pairs. The return value is turned into an immutable map and
     * cached.
     * <p>
     * Note that this method is invoked lazily on first access to {@link #getQueryParams()}. The invocation is not
     * synchronized but the parsing is thread-safe nevertheless.
     */
    ...
    /**
     * Obtain the cookies from the underlying "native" request and adapt those to an {@link HttpCookie} map. The return
     * value is turned into an immutable map and cached.
     * <p>
     * Note that this method is invoked lazily on access to {@link #getCookies()}. Sub-classes should synchronize cookie
     * initialization if the underlying "native" request does not provide thread-safe access to cookie data.
     */
