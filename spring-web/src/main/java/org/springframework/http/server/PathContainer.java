...
/**
 * Structured representation of a URI path parsed via {@link #parsePath(String)} into a sequence of {@link Separator}
 * and {@link PathSegment} elements.
 *
 * <p>
 * Each {@link PathSegment} exposes its content in decoded form and with path parameters removed. This makes it safe to
 * match one path segment at a time without the risk of decoded reserved characters altering the structure of the path.
 *
...
public interface PathContainer {
    ...
    /**
     * Parse the path value into a sequence of {@code "/"} {@link Separator Separator} and {@link PathSegment
    ...
    /**
     * Parse the path value into a sequence of {@link Separator Separator} and {@link PathSegment PathSegment} elements.
     *
     * @param path
     *            the encoded, raw path value to parse
     * @param options
    ...
    /**
     * Extract a sub-path from the given start offset into the element list (inclusive) and to the end offset
     * (exclusive).
     *
     * @param startIndex
     *            the start element index (inclusive)
    ...
    class Options {
         ...
         * <p>
         * Separator '.' with neither URL decoding nor parsing of path parameters. Escape sequences for the separator
         * character in segment values are still decoded.
         */
         ...
         *
         * @param separator
         *            the separator for parsing the path into segments; currently this must be slash or dot.
         * @param decodeAndParseSegments
         *            whether to URL decode path segment values and parse path parameters. If set to false, only escape
