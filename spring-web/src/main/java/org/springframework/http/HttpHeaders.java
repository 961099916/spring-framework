...
/**
 * A data structure representing HTTP request or response headers, mapping String header names to a list of String
 * values, also offering accessors for common application-level data types.
 *
 * <p>
 * In addition to the regular methods defined by {@link Map}, this class offers many common convenience methods, for
 * example:
 * <ul>
...
public class HttpHeaders implements MultiValueMap<String, String>, Serializable {
    ...
    private static final DateTimeFormatter[] DATE_PARSERS = new DateTimeFormatter[] {
        DateTimeFormatter.RFC_1123_DATE_TIME, DateTimeFormatter.ofPattern("EEEE, dd-MMM-yy HH:mm:ss zzz", Locale.US),
     ...
     * <p>
     * This constructor is available as an optimization for adapting to existing headers map structures, primarily for
     ...
     * {@link #readOnlyHttpHeaders(HttpHeaders)}.
     *
     * @param headers
     *            the headers to expose
    ...
    /**
     * Helps to format HTTP header values, as HTTP header values themselves can contain comma-separated values, can
     * become confusing with regular {@link Map} formatting that also uses commas between entries.
     *
     * @param headers
     ...
     * <p>
     * The encoded credentials returned by this method can be supplied to {@link #setBasicAuth(String)} to set the Basic
     ...
     *            the password
     * @param charset
     *            the charset to use to convert the credentials into an octet sequence. Defaults to
     *            {@linkplain StandardCharsets#ISO_8859_1 ISO-8859-1}.
     * @throws IllegalArgumentException
     *             if {@code username} or {@code password} contains characters that cannot be encoded to the given
     *             charset
     * @since 5.2
     ...
     * <p>
     * If you only need sorted, preferred locales only use {@link #getAcceptLanguageAsLocales()} or if you need to
     * filter based on a list of supported locales you can pass the returned list to
    ...
    public void setAcceptLanguage(List<Locale.LanguageRange> languages) {
        ...
        List<String> values =
            languages.stream().map(range -> range.getWeight() == Locale.LanguageRange.MAX_WEIGHT ? range.getRange()
                : range.getRange() + ";q=" + decimal.format(range.getWeight())).collect(Collectors.toList());
        set(ACCEPT_LANGUAGE, toCommaDelimitedString(values));
    }

    /**
     * A variant of {@link #getAcceptLanguage()} that converts each {@link java.util.Locale.LanguageRange} to a
     * {@link Locale}.
     *
    ...
    /**
     * Set the value of the {@linkplain #AUTHORIZATION Authorization} header to Basic Authentication based on the given
     * username and password.
     * <p>
     * Note that this method only supports characters in the {@link StandardCharsets#ISO_8859_1 ISO-8859-1} character
     * set.
     *
     * @param username
     *            the username
     ...
     *            the password
     * @throws IllegalArgumentException
     *             if either {@code user} or {@code password} contain characters that cannot be encoded to ISO-8859-1
     * @since 5.1
     * @see #setBasicAuth(String)
     * @see #setBasicAuth(String, String, Charset)
    ...
    /**
     * Set the value of the {@linkplain #AUTHORIZATION Authorization} header to Basic Authentication based on the given
     * {@linkplain #encodeBasicAuth encoded credentials}.
     * <p>
     * Favor this method over {@link #setBasicAuth(String, String)} and {@link #setBasicAuth(String, String, Charset)}
     * if you wish to cache the encoded credentials.
     *
    ...
    /**
     * Set a configured {@link CacheControl} instance as the new value of the {@code Cache-Control} header.
     *
     * @since 5.0.5
     */
    ...
    /**
     * Set the {@code Content-Disposition} header when creating a {@code "multipart/form-data"} request.
     * <p>
     * Applications typically would not set this header directly but rather prepare a
     * {@code MultiValueMap<String, Object>}, containing an Object or a {@link org.springframework.core.io.Resource} for
     * each part, and then pass that to the {@code RestTemplate} or {@code WebClient}.
     *
     * @param name
     *            the control name
     * @param filename
     ...
     * <p>
     * This could be used on a response to indicate if the content is expected to be displayed inline in the browser or
     * as an attachment to be saved locally.
     * <p>
     * It can also be used for a {@code "multipart/form-data"} request. For more details see notes on
     * {@link #setContentDispositionFormData}.
     *
     * @since 5.0
     * @see #getContentDisposition()
     */
    ...
    /**
     * Set the {@link Locale} of the content language, as specified by the {@literal Content-Language} header.
     * <p>
     ...
     * <p>
     * The date is returned as the number of milliseconds since January 1, 1970 GMT. Returns -1 when the date is
     * unknown.
     *
     * @throws IllegalArgumentException
    ...
    /**
     * Set the date and time at which the message was created, as specified by the {@code Date} header.
     *
     * @since 5.2
     */
    ...
    /**
     * Return the date and time at which the message is no longer valid, as specified by the {@code Expires} header.
     * <p>
     * The date is returned as the number of milliseconds since January 1, 1970 GMT. Returns -1 when the date is
     * unknown.
     *
    ...
    /**
     * Set the date and time at which the message is no longer valid, as specified by the {@code Expires} header.
     *
     * @since 5.2
     */
     ...
     * <p>
     * If the header value does not contain a port, the {@linkplain InetSocketAddress#getPort() port} in the returned
     * address will be {@code 0}.
     *
     * @since 5.0
     */
    @Nullable
     ...
     * <p>
     * If the given {@linkplain InetSocketAddress#getPort() port} is {@code 0}, the host header will only contain the
    ...
    /**
     * Return the time the resource was last changed, as specified by the {@code Last-Modified} header.
     * <p>
     * The date is returned as the number of milliseconds since January 1, 1970 GMT. Returns -1 when the date is
     * unknown.
     *
    ...
    /**
     * Set the request header names (e.g. "Accept-Language") for which the response is subject to content negotiation
     * and variances based on the value of those request headers.
     *
    ...
    /**
     * Set the given date under the given header name after formatting it as a string using the RFC-1123 date-time
     * formatter. The equivalent of {@link #set(String, String)} but for date headers.
     *
     * @since 5.0
     */
    ...
    /**
     * Parse the first header value for the given header name as a date, return -1 if there is no value or also in case
     * of an invalid value (if {@code rejectInvalid=false}), or raise {@link IllegalArgumentException} if the value
     * cannot be parsed as a date.
     *
     * @param headerName
     *            the header name
     * @param rejectInvalid
     *            whether to reject invalid values with an {@link IllegalArgumentException} ({@code true}) or rather
    ...
    /**
     * Parse the first header value for the given header name as a date, return {@code null} if there is no value, or
    ...
    /**
     * Parse the first header value for the given header name as a date, return {@code null} if there is no value or
     * also in case of an invalid value (if {@code rejectInvalid=false}), or raise {@link IllegalArgumentException} if
