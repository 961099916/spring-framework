...
/**
 * An {@code HttpCookie} subclass with the additional attributes allowed in the "Set-Cookie" response header. To build
 * an instance use the {@link #from} static method.
 *
...
public final class ResponseCookie extends HttpCookie {
    ...
    /**
     * Factory method to obtain a builder for a server-defined cookie that starts with a name-value pair and may also
     * include attributes.
     *
     * @param name
     *            the cookie name
    ...
    /**
     * Factory method to obtain a builder for a server-defined cookie. Unlike {@link #from(String, String)} this option
     * assumes input from a remote server, which can be handled more leniently, e.g. ignoring a empty domain name with
     * double quotes.
     *
     * @param name
     *            the cookie name
     ...
     * <p>
     * A positive value indicates when the cookie expires relative to the current time. A value of 0 means the cookie
     * should expire immediately. A negative value means no "Max-Age" attribute in which case the cookie is removed when
     ...
     * <p>
     * This limits the scope of the cookie such that it will only be attached to same site requests if {@code "Strict"}
     * or cross-site requests if {@code "Lax"}.
     *
    ...
    public interface ResponseCookieBuilder {
         ...
         *
         * <p>
         * A positive value indicates when the cookie should expire relative to the current time. A value of 0 means the
         * cookie should expire immediately. A negative value results in no "Max-Age" attribute in which case the cookie
         ...
         * <p>
         * This limits the scope of the cookie such that it will only be attached to same site requests if
         * {@code "Strict"} or cross-site requests if {@code "Lax"}.
         * 
         * @since 5.1
         * @see <a href="https://tools.ietf.org/html/draft-ietf-httpbis-rfc6265bis#section-4.1.2.7">RFC6265 bis</a>
         */
