...
public abstract class AbstractHttpInvokerRequestExecutor implements HttpInvokerRequestExecutor, BeanClassLoaderAware {
    ...
    /**
     * Return whether to accept GZIP encoding, that is, whether to send the HTTP "Accept-Encoding" header with "gzip" as
     * value.
     */
    ...
    /**
     * Set whether to accept GZIP encoding, that is, whether to send the HTTP "Accept-Encoding" header with "gzip" as
     * value.
     * <p>
     * Default is "true". Turn this flag off if you do not want GZIP response compression even if enabled on the HTTP
     * server.
     */
     ...
     * <p>
     * The default implementation gives {@code decorateOutputStream} a chance to decorate the stream first (for example,
     * for custom encryption or compression). Creates an {@code ObjectOutputStream} for the final stream and calls
    ...
    /**
     * Return the OutputStream to use for writing remote invocations, potentially decorating the given original
     * OutputStream.
     * <p>
     ...
     * <p>
     * The default implementation simply calls {@code writeObject}. Can be overridden for serialization of a custom
     * wrapper object rather than the plain invocation, for example an encryption-aware holder.
     * 
     * @param invocation
     *            the RemoteInvocation object
     * @param oos
     ...
     * <p>
     * Implementations will usually call {@code readRemoteInvocationResult} to deserialize a returned
     * RemoteInvocationResult object.
     * 
     * @param config
     *            the HTTP invoker configuration that specifies the target service
     * @param baos
     *            the ByteArrayOutputStream that contains the serialized RemoteInvocation object
     ...
     * <p>
     * Gives {@code decorateInputStream} a chance to decorate the stream first (for example, for custom encryption or
     * compression). Creates an {@code ObjectInputStream} via {@code createObjectInputStream} and calls
    ...
    /**
     * Return the InputStream to use for reading remote invocation results, potentially decorating the given original
     * InputStream.
     * <p>
     * The default implementation returns the given stream as-is. Can be overridden, for example, for custom encryption
     * or compression.
     ...
     *            the InputStream to read from
     * @param codebaseUrl
     *            the codebase URL to load classes from if not found locally (can be {@code null})
     ...
     * <p>
     * The default implementation simply calls {@code readObject}. Can be overridden for deserialization of a custom
     * wrapper object rather than the plain invocation, for example an encryption-aware holder.
     * 
     * @param ois
     ...
     *             if thrown by I/O methods
     * @throws ClassNotFoundException
     *             if the class name of a serialized object couldn't get resolved
     * @see java.io.ObjectOutputStream#writeObject
     */
