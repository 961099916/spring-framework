...
/**
 * {@link org.aopalliance.intercept.MethodInterceptor} for accessing a specific port of a JAX-WS service.
 *
 * <p>
 * Uses either {@link LocalJaxWsServiceFactory}'s facilities underneath, or takes an explicit reference to an existing
 * JAX-WS Service instance (e.g. obtained via {@link org.springframework.jndi.JndiObjectFactoryBean}).
 *
...
public class JaxWsPortClientInterceptor extends LocalJaxWsServiceFactory
     ...
     * {@link org.springframework.jndi.JndiObjectFactoryBean}. If not set, {@link LocalJaxWsServiceFactory}'s properties
    ...
    /**
     * Allow Map access to the custom properties to be set on the stub, with the option to add or override specific
     * entries.
     * <p>
     * Useful for specifying entries directly, for example via "customProperties[myKey]". This is particularly useful
     * for adding or overriding entries in child bean definitions.
     */
     ...
     * <p>
     * Default is "true". Turn this flag off to allow for late start of the target server. In this case, the JAX-WS
     * service will be lazily fetched on first access.
     */
     ...
     * {@link JaxWsPortProxyFactoryBean} subclass.
     */
    @Override
    ...
    /**
     * Initialize this client interceptor's properties from the given WebService annotation, if necessary and possible
     * (i.e. if "wsdlDocumentUrl", "namespaceUri", "serviceName" and "portName" haven't been set but corresponding
     * values are declared at the annotation level of the specified service interface).
     * 
     * @param ann
    ...
    /**
     * Return whether this client interceptor has already been prepared, i.e. has already looked up the JAX-WS service
     * and port.
     */
     ...
     * @param portQName
     *            the name of the desired port, if specified
    ...
    /**
     * Return the underlying JAX-WS port stub that this interceptor delegates to for each method invocation on the
     * proxy.
     */
    @Nullable
