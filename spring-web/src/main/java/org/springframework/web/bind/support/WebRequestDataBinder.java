...
/**
 * Special {@link org.springframework.validation.DataBinder} to perform data binding from web request parameters to
 * JavaBeans, including support for multipart files.
 *
 * <p>
 * See the DataBinder/WebDataBinder superclasses for customization options, which include specifying allowed/required
 * fields, and registering custom property editors.
 *
 * <p>
 * Can also used for manual data binding in custom web controllers or interceptors that build on Spring's
 * {@link org.springframework.web.context.request.WebRequest} abstraction: e.g. in a
 * {@link org.springframework.web.context.request.WebRequestInterceptor} implementation. Simply instantiate a
 * WebRequestDataBinder for each binding process, and invoke {@code bind} with the current WebRequest as argument:
 *
 * <pre class="code">
...
public class WebRequestDataBinder extends WebDataBinder {
     ...
     * <p>
     * Multipart files are bound via their parameter name, just like normal HTTP parameters: i.e. "uploadedFile" to an
     * "uploadedFile" bean property, invoking a "setUploadedFile" setter method.
     * <p>
     * The type of the target property for a multipart file can be Part, MultipartFile, byte[], or String. The latter
     * two receive the contents of the uploaded file; all metadata like original file name, content type, etc are lost
     * in those cases.
     * 
     * @param request
     ...
     * 
     * @param request
     *            the request with parameters to bind
     */
