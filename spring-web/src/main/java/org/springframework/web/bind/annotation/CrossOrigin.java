...
/**
 * Annotation for permitting cross-origin requests on specific handler classes and/or handler methods. Processed if an
 * appropriate {@code HandlerMapping} is configured.
 *
 * <p>
 * Both Spring Web MVC and Spring WebFlux support this annotation through the {@code RequestMappingHandlerMapping} in
 * their respective modules. The values from each type and method level pair of annotations are added to a
 * {@link CorsConfiguration} and then default values are applied via
 * {@link CorsConfiguration#applyPermitDefaultValues()}.
 *
 * <p>
 * The rules for combining global and local configuration are generally additive -- e.g. all global and all local
 * origins. For those attributes where only a single value can be accepted such as {@code allowCredentials} and
 * {@code maxAge}, the local overrides the global value. See {@link CorsConfiguration#combine(CorsConfiguration)} for
 * more details.
 *
...
public @interface CrossOrigin {
    ...
    /**
     * The list of request headers that are permitted in actual requests, possibly {@code "*"} to allow all headers.
     * <p>
    ...
    /**
     * The List of response headers that the user-agent will allow the client to access on an actual response, other
     * than "simple" headers, i.e. {@code Cache-Control}, {@code Content-Language}, {@code Content-Type},
     ...
     * <p>
     * Setting this to a reasonable value can reduce the number of preflight request/response interactions required by
     * the browser. A negative value means <em>undefined</em>.
     * <p>
