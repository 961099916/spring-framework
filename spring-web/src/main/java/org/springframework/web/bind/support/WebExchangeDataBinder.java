...
/**
 * Specialized {@link org.springframework.validation.DataBinder} to perform data binding from URL query parameters or
 * form data in the request data to Java objects.
 *
...
public class WebExchangeDataBinder extends WebDataBinder {
    ...
    private static void addBindValue(Map<String, Object> params, String key, List<?> values) {
        if (!CollectionUtils.isEmpty(values)) {
            values =
                values.stream().map(value -> value instanceof FormFieldPart ? ((FormFieldPart)value).value() : value)
