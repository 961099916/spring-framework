...
/**
 * A path element representing capturing a piece of the path as a variable. In the pattern '/foo/{bar}/goo' the {bar} is
 * represented as a {@link CaptureVariablePathElement}. There must be at least one character to bind to the variable.
 *
 * @author Andy Clement
 * @since 5.0
 */
        ...
        if (colon == -1) {
            ...
            if (caseSensitive) {
                this.constraintPattern =
                    Pattern.compile(new String(captureDescriptor, colon + 1, captureDescriptor.length - colon - 2));
            } else {
                this.constraintPattern =
                    Pattern.compile(new String(captureDescriptor, colon + 1, captureDescriptor.length - colon - 2),
