...
/**
 * {@code UriBuilderFactory} that relies on {@link UriComponentsBuilder} for the actual building of the URI.
 *
 * <p>
 * Provides options to create {@link UriBuilder} instances with a common base URI, alternative encoding mode strategies,
 * among others.
 *
...
public class DefaultUriBuilderFactory implements UriBuilderFactory {
     ...
     * <p>
     * The given URI template is parsed via {@link UriComponentsBuilder#fromUriString} and then applied as a base URI to
     * every UriBuilder via {@link UriComponentsBuilder#uriComponents} unless the UriBuilder itself was created with a
     * URI template that already has a target address.
     * 
     ...
     * <p>
     * <strong>Note:</strong> Prior to 5.1 the default was {@link EncodingMode#URI_COMPONENT EncodingMode.URI_COMPONENT}
     * therefore the {@code WebClient} {@code RestTemplate} have switched their default behavior.
     *
     * @param encodingMode
     ...
     * {@link EncodingMode#URI_COMPONENT EncodingMode.URI_COMPONENT}, which ensures that URI variables in the path are
    ...
    /**
     * Whether to parse the path into path segments if the encoding mode is set to {@link EncodingMode#URI_COMPONENT
    ...
    public enum EncodingMode {
         ...
         * rules:
         * <ul>
         * <li>For the URI template replace <em>only</em> non-ASCII and illegal (within a given URI component type)
         * characters with escaped octets.
         * <li>For URI variables do the same and also replace characters with reserved meaning.
         * </ul>
         * <p>
         * For most cases, this mode is most likely to give the expected result because in treats URI variables as
         * opaque data to be fully encoded, while {@link #URI_COMPONENT} by comparison is useful only if intentionally
         * expanding URI variables with reserved characters.
         * 
         * @since 5.0.8
        ...
        /**
         * Expand URI variables first, and then encode the resulting URI component values, replacing <em>only</em>
         * non-ASCII and illegal (within a given URI component type) characters, but not characters with reserved
         * meaning.
         * 
