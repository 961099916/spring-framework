...
/**
 * Helper class for URL path matching. Provides support for URL paths in {@code RequestDispatcher} includes and support
 * for consistent URL decoding.
 *
 * <p>
 * Used by {@link org.springframework.web.servlet.handler.AbstractUrlHandlerMapping} and
 * {@link org.springframework.web.servlet.support.RequestContext} for path matching and/or URI determination.
 *
...
public class UrlPathHelper {
    ...
    /**
     * Special WebSphere request attribute, indicating the original request URI. Preferable over the standard Servlet
     * 2.4 forward attribute on WebSphere, simply because we need the very first URI in the request forwarding chain.
     */
     ...
     * <p>
     * If set to {@literal false} the path within the current servlet mapping is used instead if applicable (i.e. in the
     * case of a prefix based Servlet mapping such as "/myServlet/*").
     * <p>
    ...
    /**
     * Whether the context path and request URI should be decoded -- both of which are returned <i>undecoded</i> by the
     * Servlet API, in contrast to the servlet path.
     * <p>
     ...
     * <p>
     * <strong>Note:</strong> Be aware the servlet path will not match when compared to encoded paths. Therefore use of
     * {@code urlDecode=false} is not compatible with a prefix-based Servlet mapping and likewise implies also setting
    ...
    /**
     * Return the mapping lookup path for the given request, within the current servlet mapping if applicable, else
    ...
    /**
     * Variant of {@link #getLookupPathForRequest(HttpServletRequest)} that automates checking for a previously computed
     * lookupPath saved as a request attribute. The attribute is only used for lookup purposes.
     *
     * @param request
     *            current HTTP request
     * @param lookupPathAttributeName
     *            the request attribute to check
     * @return the lookup path
     * @since 5.2
    ...
    /**
     * Return the path within the servlet mapping for the given request, i.e. the part of the request's URL beyond the
     * part that called the servlet, or "" if the whole URL has been used to identify the servlet.
     *
     * @param request
     *            current HTTP request
     ...
     *            current HTTP request
     * @param pathWithinApp
     *            a precomputed path within the application
     * @return the path within the servlet mapping, or ""
     * @since 5.2.9
     * @see #getLookupPathForRequest
     */
    ...
    /**
     * Match the given "mapping" to the start of the "requestUri" and if there is a match return the extra part. This
     * method is needed because the context path and the servlet path returned by the HttpServletRequest are stripped of
     * semicolon content unlike the requestUri.
     */
    @Nullable
    ...
    /**
     * Return the request URI for the given request, detecting an include request URL if called within a
     * RequestDispatcher include.
     * <p>
     * As the value returned by {@code request.getRequestURI()} is <i>not</i> decoded by the servlet container, this
     * method will decode it.
     * <p>
     * The URI that the web container resolves <i>should</i> be correct, but some containers like JBoss/Jetty
    ...
    /**
     * Return the context path for the given request, detecting an include request URL if called within a
     * RequestDispatcher include.
     * <p>
     * As the value returned by {@code request.getContextPath()} is <i>not</i> decoded by the servlet container, this
     * method will decode it.
     *
     * @param request
     *            current HTTP request
    ...
    /**
     * Return the servlet path for the given request, regarding an include request URL if called within a
     * RequestDispatcher include.
     * <p>
     * As the value returned by {@code request.getServletPath()} is already decoded by the servlet container, this
     * method will not attempt to decode it.
     *
     * @param request
     *            current HTTP request
    ...
    /**
     * Return the request URI for the given request. If this is a forwarded request, correctly resolves to the request
    ...
    /**
     * Return the servlet path for the given request, detecting an include request URL if called within a
     * RequestDispatcher include.
     *
     * @param request
     *            current HTTP request
    ...
    /**
     * Return the query string part of the given request's URL. If this is a forwarded request, correctly resolves to
    ...
    /**
     * Decode the given source string with a URLDecoder. The encoding will be taken from the request, falling back to
    ...
    /**
     * Remove ";" (semicolon) content from the given request URI if the {@linkplain #setRemoveSemicolonContent
     * removeSemicolonContent} property is set to "true". Note that "jsessionid" is always removed.
     *
     * @param requestUri
     *            the request URI string to remove ";" content from
    ...
    /**
     * Decode the given URI path variables via {@link #decodeRequestString} unless {@link #setUrlDecode} is set to
     * {@code true} in which case it is assumed the URL path from which the variables were extracted is already decoded
    ...
    /**
     * Decode the given matrix variables via {@link #decodeRequestString} unless {@link #setUrlDecode} is set to
     * {@code true} in which case it is assumed the URL path from which the variables were extracted is already decoded
     * through a call to {@link #getLookupPathForRequest(HttpServletRequest)}.
     *
     * @param request
