 ...
 *
 * <p>
 * The {@link PathPatternParser} and {@link PathPattern} are specifically designed for use with HTTP URL paths in web
 * applications where a large number of URI path patterns, continuously matched against incoming requests, motivates the
 * need for efficient matching.
 *
 * <p>
...
public class PathPatternParser {
    ...
    /**
     * Whether a {@link PathPattern} produced by this parser should automatically match request paths with a trailing
     * slash.
     *
     * <p>
     * If set to {@code true} a {@code PathPattern} without a trailing slash will also match request paths with a
     * trailing slash. If set to {@code false} a {@code PathPattern} will only match request paths with a trailing
     * slash.
     *
     * <p>
     ...
     * <p>
     * {@link org.springframework.http.server.PathContainer.Options#HTTP_PATH} is used by default.
     *
     * @since 5.2
     */
    public void setPathOptions(PathContainer.Options pathOptions) {
    ...
    /**
     * Process the path pattern content, a character at a time, breaking it into path elements around separator
     * boundaries and verifying the structure at each stage. Produces a PathPattern object that can be used for fast
     * matching against paths. Each invocation of this method delegates to a new instance of the
