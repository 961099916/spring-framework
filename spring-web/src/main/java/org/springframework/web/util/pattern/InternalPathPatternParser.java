...
/**
 * Parser for URI template patterns. It breaks the path pattern into a number of {@link PathElement PathElements} in a
 * linked list. Instances are reusable but are not thread-safe.
 *
 * @author Andy Clement
 * @since 5.0
 */
class InternalPathPatternParser {
    ...
    /**
     * Just hit a ':' and want to jump over the regex specification for this variable. pos will be pointing at the ':',
     * we want to skip until the }.
     * <p>
    ...
    /**
     * After processing a separator, a quick peek whether it is followed by (and only before the end of the pattern or
     * the next separator).
     */
    ...
    /**
     * Used the knowledge built up whilst processing since the last path element to determine what kind of path element
     * to create.
     * 
    ...
    /**
     * For a path element representing a captured variable, locate the constraint pattern. Assumes there is a constraint
     * pattern.
     * 
     * @param data
     ...
     * @return the index of the character after the ':' within the pattern expression relative to the start of the whole
     *         expression
     */
