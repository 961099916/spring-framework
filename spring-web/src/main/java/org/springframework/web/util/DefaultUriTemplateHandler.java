...
/**
 * Default implementation of {@link UriTemplateHandler} based on the use of {@link UriComponentsBuilder} for expanding
 * and encoding variables.
 *
 * <p>
 * There are also several properties to customize how URI template handling is performed, including a {@link #setBaseUrl
 * baseUrl} to be used as a prefix for all URI templates and a couple of encoding related options &mdash;
 * {@link #setParsePath parsePath} and {@link #setStrictEncoding strictEncoding} respectively.
 *
...
public class DefaultUriTemplateHandler extends AbstractUriTemplateHandler {
     ...
     * <p>
     * If set to {@code true} the URI template path is immediately decomposed into path segments any URI variables
     * expanded into it are then subject to path segment encoding rules. In effect URI variables in the path have any
     * "/" characters percent encoded.
     * <p>
     * By default this is set to {@code false} in which case the path is kept as a full path and expanded URI variables
     * will preserve "/" characters.
     * 
     * @param parsePath
     ...
     * <a href="https://tools.ietf.org/html/rfc3986#section-2">RFC 3986 Section 2</a>. This ensures a URI variable value
     * will not contain any characters with a reserved purpose.
     * <p>
     * By default this is set to {@code false} in which case only characters illegal for the given URI component are
     * encoded. For example when expanding a URI variable into a path segment the "/" character is illegal and encoded.
     * The ";" character however is legal and not encoded even though it has a reserved purpose.
     * <p>
     * <strong>Note:</strong> this property supersedes the need to also set the {@link #setParsePath parsePath}
     * property.
     *
    ...
    /**
     * Create a {@code UriComponentsBuilder} from the URI template string. This implementation also breaks up the path
     * into path segments depending on whether {@link #setParsePath parsePath} is enabled.
     */
