...
public class UriComponentsBuilder implements UriBuilder, Cloneable {
    ...
    private static final Pattern URI_PATTERN =
        Pattern.compile("^(" + SCHEME_PATTERN + ")?" + "(//(" + USERINFO_PATTERN + "@)?" + HOST_PATTERN + "(:"
            + PORT_PATTERN + ")?" + ")?" + PATH_PATTERN + "(\\?" + QUERY_PATTERN + ")?" + "(#" + LAST_PATTERN + ")?");

    private static final Pattern HTTP_URL_PATTERN =
        Pattern.compile("^" + HTTP_PATTERN + "(//(" + USERINFO_PATTERN + "@)?" + HOST_PATTERN + "(:" + PORT_PATTERN
     ...
     * <p>
     * <strong>Note:</strong> the components in the resulting builder will be in fully encoded (raw) form and further
     * changes must also supply values that are fully encoded, for example via methods in {@link UriUtils}. In addition
     * please use {@link #build(boolean)} with a value of "true" to build the {@link UriComponents} instance in order to
     * indicate that the components are encoded.
     * 
     * @param uri
     ...
     * <p>
     * <strong>Note:</strong> The presence of reserved characters can prevent correct parsing of the URI string. For
     * example if a query parameter contains {@code '='} or {@code '&'} characters, the query string cannot be parsed
     * unambiguously. Such values should be substituted for URI variables to enable correct parsing:
     * 
     * <pre class="code">
    ...
    /**
     * Create a new {@code UriComponents} object from the URI associated with the given HttpRequest while also
     * overlaying with values from the headers "Forwarded" (<a href="https://tools.ietf.org/html/rfc7239">RFC 7239</a>),
     * or "X-Forwarded-Host", "X-Forwarded-Port", and "X-Forwarded-Proto" if "Forwarded" is not found.
     * 
     * @param request
     *            the source request
     * @return the URI components of the URI
     ...
     * <p>
     * In comparison to {@link UriComponents#encode()}, this method has the same effect on the URI template, i.e. each
     * URI component is encoded by replacing non-ASCII and illegal (within the URI component type) characters with
     * escaped octets. However URI variables are encoded more strictly, by also escaping characters with reserved
     * meaning.
     * <p>
     * For most cases, this method is more likely to give the expected result because in treats URI variables as opaque
     * data to be fully encoded, while {@link UriComponents#encode()} is useful only if intentionally expanding URI
     * variables that contain reserved characters.
     * <p>
     * For example ';' is legal in a path but has reserved meaning. This method replaces ";" with "%3B" in URI variables
     * but not in the URI template. By contrast, {@link UriComponents#encode()} never replaces ";" since it is a legal
     * character in a path.
     * 
     * @since 5.0.8
     */
    ...
    /**
     * Variant of {@link #build()} to create a {@link UriComponents} instance when components are already fully encoded.
     * This is useful for example if the builder was created via {@link UriComponentsBuilder#fromUri(URI)}.
     * 
     * @param encoded
     *            whether the components in this builder are already encoded
     * @return the URI components
    ...
    private UriComponents buildInternal(EncodingHint hint) {
        ...
        if (this.ssp != null) {
            ...
            HierarchicalUriComponents uric = new HierarchicalUriComponents(this.scheme, this.fragment, this.userInfo,
                this.host, this.port, this.pathBuilder.build(), this.queryParams, hint == EncodingHint.FULLY_ENCODED);
    ...
    /**
     * Build a {@code UriComponents} instance and replaces URI template variables with the values from a map. This is a
    ...
    /**
     * Build a {@code UriComponents} instance and replaces URI template variables with the values from an array. This is
     * a shortcut method which combines calls to {@link #build()} and then {@link UriComponents#expand(Object...)}.
     * 
     ...
     * </pre>
     * <p>
     * However if {@link #uriVariables(Map) URI variables} have been provided then the URI template is pre-encoded
     * separately from URI variables (see {@link #encode()} for details), i.e. equivalent to:
     * 
     * <pre>
    ...
    /**
     * Set or append individual URI components of this builder from the values of the given {@link UriComponents}
     * instance.
     * <p>
     * For the semantics of each component (i.e. set vs append) check the builder methods on this class. For example
     * {@link #host(String)} sets while {@link #path(String)} appends.
     * 
     * @param uriComponents
    ...
    /**
     * Set the URI scheme-specific-part. When invoked, this method overwrites {@linkplain #userInfo(String) user-info},
     * {@linkplain #host(String) host}, {@linkplain #port(int) port}, {@linkplain #path(String) path}, and
     ...
     * <p>
     * The provided variables may be a subset of all required ones. At build time, the available ones are expanded,
     * while unresolved URI placeholders are left in place and can still be expanded later.
     * <p>
     * In contrast to {@link UriComponents#expand(Map)} or {@link #buildAndExpand(Map)}, this method is useful when you
     * need to supply URI variables without building the {@link UriComponents} instance just yet, or perhaps pre-expand
     * some shared default values such as host and port.
     * 
     * @param uriVariables
    ...
    /**
     * Adapt this builder's scheme+host+port from the given headers, specifically "Forwarded"
     * (<a href="https://tools.ietf.org/html/rfc7239">RFC 7239</a>, or "X-Forwarded-Host", "X-Forwarded-Port", and
     * "X-Forwarded-Proto" if "Forwarded" is not found.
     * <p>
     * <strong>Note:</strong> this method uses values from forwarded headers, if present, in order to reflect the
     * client-originated protocol and address. Consider using the {@code ForwardedHeaderFilter} in order to choose from
     * a central place whether to extract and use, or to discard such headers. See the Spring Framework reference for
     * more on this filter.
     * 
     * @param headers
