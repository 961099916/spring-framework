...
/**
 * Representation of a parsed path pattern. Includes a chain of path elements for fast matching and accumulates computed
 * state for quick comparison of patterns.
 *
 * <p>
 ...
 * <li><code>{*spring}</code> matches zero or more <em>path segments</em> until the end of the path and captures it as a
 * variable named "spring"</li>
 * </ul>
 *
 * Notable behavior difference with {@code AntPathMatcher}:<br>
 * {@code **} and its capturing variant <code>{*spring}</code> cannot be used in the middle of a pattern string, only at
 * the end: {@code /pages/{**}} is valid, but {@code /pages/{**}/details} is not.
 *
 * <h3>Examples</h3>
 * <ul>
 * <li>{@code /pages/t?st.html} &mdash; matches {@code /pages/test.html} as well as {@code /pages/tXst.html} but not
 * {@code /pages/toast.html}</li>
 * <li>{@code /resources/*.png} &mdash; matches all {@code .png} files in the {@code resources} directory</li>
 * <li><code>/resources/&#42;&#42;</code> &mdash; matches all files underneath the {@code /resources/} path, including
 * {@code /resources/image.png} and {@code /resources/css/spring.css}</li>
 * <li><code>/resources/{&#42;path}</code> &mdash; matches all files underneath the {@code /resources/} path and
 * captures their relative path in a variable named "path"; {@code /resources/image.png} will match with "path" &rarr;
 * "/image.png", and {@code /resources/css/spring.css} will match with "path" &rarr; "/css/spring.css"</li>
 * <li><code>/resources/{filename:\\w+}.dat</code> will match {@code /resources/spring.dat} and assign the value
 * {@code "spring"} to the {@code filename} variable</li>
 * </ul>
 *
 * @author Andy Clement
...
public class PathPattern implements Comparable<PathPattern> {
    ...
    public static final Comparator<PathPattern> SPECIFICITY_COMPARATOR =
        Comparator.nullsLast(Comparator.<PathPattern>comparingInt(p -> p.isCatchAll() ? 1 : 0)
    ...
    /**
     * The normalized length is trying to measure the 'active' part of the pattern. It is computed by assuming all
     * captured variables have a normalized length of 1. Effectively this means changing your variable name lengths
     * isn't going to change the length of the active part of the pattern. Useful when comparing two patterns.
     */
    ...
    /**
     * Score is used to quickly compare patterns. Different pattern components are given different weights. A 'lower
     * score' is more specific. Current weights:
     * <ul>
    ...
    /**
     * Whether the pattern string contains pattern syntax that would require use of {@link #matches(PathContainer)}, or
     * if it is a regular String that could be compared directly to others.
     *
     * @since 5.2
     */
     ...
     *
     * @param pathContainer
     *            the candidate path to attempt to match against
    ...
    /**
     * Match this pattern to the given URI path and return extracted URI template variables as well as path parameters
     * (matrix variables).
     *
     * @param pathContainer
     *            the candidate path to attempt to match against
    ...
    /**
     * Match the beginning of the given path and return the remaining portion not covered by this pattern. This is
     * useful for matching nested routes where the path is matched incrementally at each level.
     *
     * @param pathContainer
     *            the candidate path to attempt to match against
     ...
     * <b>Notes:</b>
     * <ul>
     * <li>Assumes that {@link #matches} returns {@code true} for the same path but does <strong>not</strong> enforce
     * this.
     ...
     *
     * @param path
     *            a path that matches this pattern
    ...
    /**
     * Compare this pattern with a supplied pattern: return -1,0,+1 if this pattern is more specific, the same or less
     * specific than the supplied pattern. The aim is to sort more specific patterns first.
     */
    @Override
    ...
    public PathPattern combine(PathPattern pattern2string) {
        ...
        if (this.endsWithSeparatorWildcard) {
            return this.parser.parse(
                concat(this.patternString.substring(0, this.patternString.length() - 2), pattern2string.patternString));
        }
    ...
    /**
     * The normalized length is trying to measure the 'active' part of the pattern. It is computed by assuming all
     * capture variables have a normalized length of 1. Effectively this means changing your variable name lengths isn't
     * going to change the length of the active part of the pattern. Useful when comparing two patterns.
     */
    ...
    /**
     * Holder for the result of a match on the start of a pattern. Provides access to the remaining path not matched to
     * the pattern as well as any variables bound in that first part that was matched.
     */
    ...
    /**
     * Encapsulates context when attempting a match. Includes some fixed state like the candidate currently being
     * considered for a match but also some accumulators for extracted variables.
     */
