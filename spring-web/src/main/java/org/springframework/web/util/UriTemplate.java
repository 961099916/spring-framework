...
/**
 * Representation of a URI template that can be expanded with URI variables via {@link #expand(Map)},
 * {@link #expand(Object[])}, or matched to a URL via {@link #match(String)}. This class is designed to be thread-safe
 * and reusable, and allows any number of expand or match calls.
 *
 * <p>
 * <strong>Note:</strong> this class uses {@link UriComponentsBuilder} internally to expand URI templates, and is merely
 * a shortcut for already prepared URI templates. For more dynamic preparation and extra flexibility, e.g. around URI
 * encoding, consider using {@code UriComponentsBuilder} or the higher level {@link DefaultUriBuilderFactory} which adds
 * several encoding modes on top of {@code UriComponentsBuilder}. See the
 * <a href="https://docs.spring.io/spring/docs/current/spring-framework-reference/web.html#mvc-uri-building">reference
...
public class UriTemplate implements Serializable {
     ...
     *             if {@code uriVariables} is {@code null}; or if it does not contain values for all the variable names
     */
     ...
     * @return the expanded URI
     * @throws IllegalArgumentException
     *             if {@code uriVariables} is {@code null} or if it does not contain sufficient variables
     */
