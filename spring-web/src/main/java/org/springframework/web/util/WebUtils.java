...
public abstract class WebUtils {
     ...
     * <p>
     * If included via a {@code RequestDispatcher}, the current resource will see the originating request. Its own
     * request URI is exposed as a request attribute.
     */
     ...
     * <p>
     * If included via a {@code RequestDispatcher}, the current resource will see the originating context path. Its own
     * context path is exposed as a request attribute.
     */
     ...
     * <p>
     * If included via a {@code RequestDispatcher}, the current resource will see the originating servlet path. Its own
     * servlet path is exposed as a request attribute.
     */
     ...
     * <p>
     * If included via a {@code RequestDispatcher}, the current resource will see the originating path info. Its own
     * path info is exposed as a request attribute.
     */
     ...
     * <p>
     * If included via a {@code RequestDispatcher}, the current resource will see the originating query string. Its own
     * query string is exposed as a request attribute.
     */
     ...
     * <p>
     * If forwarded to via a RequestDispatcher, the current resource will see its own request URI. The originating
     * request URI is exposed as a request attribute.
     */
     ...
     * <p>
     * If forwarded to via a RequestDispatcher, the current resource will see its own context path. The originating
     * context path is exposed as a request attribute.
     */
     ...
     * <p>
     * If forwarded to via a RequestDispatcher, the current resource will see its own servlet path. The originating
     * servlet path is exposed as a request attribute.
     */
     ...
     * <p>
     * If forwarded to via a RequestDispatcher, the current resource will see its own path ingo. The originating path
     * info is exposed as a request attribute.
     */
     ...
     * <p>
     * If forwarded to via a RequestDispatcher, the current resource will see its own query string. The originating
     * query string is exposed as a request attribute.
     */
     ...
     * <p>
     * To be exposed to JSPs that are marked as error pages, when forwarding to them directly rather than through the
     * servlet container's error page resolution mechanism.
     */
    ...
    /**
     * Default character encoding to use when {@code request.getCharacterEncoding} returns {@code null}, according to
     * the Servlet spec.
     * 
    ...
    /**
     * Standard Servlet spec context attribute that specifies a temporary directory for the current web application, of
     * type {@code java.io.File}.
     */
    ...
    /**
     * Use of response encoding for HTML escaping parameter at the servlet context level (i.e. a context-param in
     * {@code web.xml}): "responseEncodedHtmlEscape".
     * 
     * @since 4.1.2
     */
     ...
     * <p>
     * Can be used for tools that support substitution with {@code System.getProperty} values, like log4j's "${key}"
     ...
     * <p>
     * This method differentiates between no param specified at all and an actual boolean value specified, allowing to
     * have a context-specific default in case of no setting at the global level.
     * 
     * @param servletContext
     *            the servlet context of the web application
     * @return whether default HTML escaping is enabled for the given application ({@code null} = no explicit default)
     */
    @Nullable
    ...
    /**
     * Return whether response encoding should be used when HTML escaping characters, thus only escaping XML markup
     * significant characters with UTF-* encodings. This option is enabled for the web application with a ServletContext
     * param, i.e. the value of the "responseEncodedHtmlEscape" context-param in {@code web.xml} (if any).
     * <p>
     * This method differentiates between no param specified at all and an actual boolean value specified, allowing to
     * have a context-specific default in case of no setting at the global level.
     * 
     * @param servletContext
     *            the servlet context of the web application
     * @return whether response encoding is to be used for HTML escaping ({@code null} = no explicit default)
     * @since 4.1.2
     */
    @Nullable
    ...
    /**
     * Check the given request for a session attribute of the given name. Returns null if there is no session or if the
     * session has no such attribute. Does not create a new session if none has existed before!
     * 
     * @param request
     *            current HTTP request
     * @param name
    ...
    /**
     * Check the given request for a session attribute of the given name. Throws an exception if there is no session or
     * if the session has no such attribute. Does not create a new session if none has existed before!
     * 
     * @param request
     *            current HTTP request
     * @param name
    ...
    /**
     * Set the session attribute with the given name to the given value. Removes the session attribute if value is null,
     * if a session existed at all. Does not create a new session if not necessary!
     * 
     * @param request
     *            current HTTP request
     * @param name
     ...
     * session.
     * <p>
     * Returns the session mutex attribute if available; usually, this means that the HttpSessionMutexListener needs to
     * be defined in {@code web.xml}. Falls back to the HttpSession itself if no mutex attribute found.
     * <p>
    ...
    /**
     * Determine whether the given request is an include request, that is, not a top-level HTTP request coming in from
     * the outside.
     * <p>
     * Checks the presence of the "javax.servlet.include.request_uri" request attribute. Could check any request
     * attribute that is only present in an include request.
     * 
     * @param request
    ...
    /**
     * Expose the Servlet spec's error attributes as {@link javax.servlet.http.HttpServletRequest} attributes under the
     * keys defined in the Servlet 2.3 specification, for error pages that are rendered directly rather than through the
     * Servlet container's error page resolution: {@code javax.servlet.error.status_code},
     ...
     * <p>
     * Does not override values if already present, to respect attribute values that have been exposed explicitly
     * before.
     * <p>
     * Exposes status code 200 by default. Set the "javax.servlet.error.status_code" attribute explicitly (before or
     * after) in order to expose a different status code.
     * 
     * @param request
     *            current servlet request
     * @param ex
    ...
    /**
     * Clear the Servlet spec's error attributes as {@link javax.servlet.http.HttpServletRequest} attributes under the
     * keys defined in the Servlet 2.3 specification: {@code javax.servlet.error.status_code},
     * {@code javax.servlet.error.exception_type}, {@code javax.servlet.error.message},
     * {@code javax.servlet.error.exception}, {@code javax.servlet.error.request_uri},
     * {@code javax.servlet.error.servlet_name}.
     * 
     ...
     * <ol>
     * <li>Try to get the parameter value using just the given <i>logical</i> name. This handles parameters of the form
     * <tt>logicalName = value</tt>. For normal parameters, e.g. submitted using a hidden HTML form field, this will
     * return the requested value.</li>
    ...
    /**
     * Parse the given string with matrix variables. An example string would look like this
     * {@code "q1=a;q1=b;q2=a,b,c"}. The resulting map would contain keys {@code "q1"} and {@code "q2"} with values
     * {@code ["a","b"]} and {@code ["a","b","c"]} respectively.
     * 
     ...
     *
     * <p>
     * <strong>Note:</strong> as of 5.1 this method ignores {@code "Forwarded"} and {@code "X-Forwarded-*"} headers that
     * specify the client-originated address. Consider using the {@code ForwardedHeaderFilter} to extract and use, or to
     * discard such headers.
     *
    ...
    /**
     * Check if the request is a same-origin one, based on {@code Origin}, {@code Host}, {@code Forwarded},
     * {@code X-Forwarded-Proto}, {@code X-Forwarded-Host} and {@code X-Forwarded-Port} headers.
     *
     * <p>
     * <strong>Note:</strong> as of 5.1 this method ignores {@code "Forwarded"} and {@code "X-Forwarded-*"} headers that
     * specify the client-originated address. Consider using the {@code ForwardedHeaderFilter} to extract and use, or to
