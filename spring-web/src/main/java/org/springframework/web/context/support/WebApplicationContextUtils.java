...
/**
 * Convenience methods for retrieving the root {@link WebApplicationContext} for a given {@link ServletContext}. This is
 * useful for programmatically accessing a Spring application context from within custom web views or MVC actions.
 *
 * <p>
 * Note that there are more convenient ways of accessing the root context for many web frameworks, either part of Spring
 * or available as an external library. This helper class is just the most generic way to access the root context.
 *
...
public abstract class WebApplicationContextUtils {
     ...
     * 
     * @param sc
     *            the ServletContext to find the web application context for
    ...
    /**
     * Register web-specific scopes ("request", "session", "globalSession", "application") with the given BeanFactory,
    ...
    /**
     * Convenient variant of {@link #initServletPropertySources(MutablePropertySources, ServletContext, ServletConfig)}
    ...
    /**
     * Replace {@code Servlet}-based {@link StubPropertySource stub property sources} with actual instances populated
     * with the given {@code servletContext} and {@code servletConfig} objects.
     * <p>
     * This method is idempotent with respect to the fact it may be called any number of times but will perform
     * replacement of stub property sources with their corresponding actual property sources once and only once.
     * 
     * @param sources
     *            the {@link MutablePropertySources} to initialize (must not be {@code null})
     * @param servletContext
     *            the current {@link ServletContext} (ignored if {@code null} or if the
     *            {@link StandardServletEnvironment#SERVLET_CONTEXT_PROPERTY_SOURCE_NAME servlet context property
     *            source} has already been initialized)
     * @param servletConfig
     *            the current {@link ServletConfig} (ignored if {@code null} or if the
     *            {@link StandardServletEnvironment#SERVLET_CONFIG_PROPERTY_SOURCE_NAME servlet config property source}
