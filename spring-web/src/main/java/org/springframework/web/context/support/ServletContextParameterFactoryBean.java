...
/**
 * {@link FactoryBean} that retrieves a specific ServletContext init parameter (that is, a "context-param" defined in
 * {@code web.xml}). Exposes that ServletContext init parameter when used as bean reference, effectively making it
 * available as named Spring bean instance.
 *
 * <p>
 * <b>NOTE:</b> As of Spring 3.0, you may also use the "contextParameters" default bean which is of type Map, and
 * dereference it using an "#{contextParameters.myKey}" expression to access a specific parameter by name.
 *
