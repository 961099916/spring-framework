...
/**
 * {@link LocaleContextResolver} implementation that simply uses the primary locale specified in the "Accept-Language"
 * header of the HTTP request (that is, the locale sent by the client browser, normally that of the client's OS).
 *
 * <p>
 * Note: Does not support {@link #setLocaleContext}, since the accept header can only be changed through changing the
 * client's locale settings.
 *
...
public class AcceptHeaderLocaleContextResolver implements LocaleContextResolver {
    ...
    /**
     * Configure a fixed default locale to fall back on if the request does not have an "Accept-Language" header (not
     * set by default).
     *
     * @param defaultLocale
