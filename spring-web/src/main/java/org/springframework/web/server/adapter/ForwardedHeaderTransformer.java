 ...
 *
 * <p>
 * Alternatively if {@link #setRemoveOnly removeOnly} is set to "true", then "Forwarded" and "X-Forwarded-*" headers are
 * only removed, and not used.
 *
 * <p>
 * An instance of this class is typically declared as a bean with the name "forwardedHeaderTransformer" and detected by
 * {@link WebHttpHandlerBuilder#applicationContext(ApplicationContext)}, or it can also be registered directly via
...
public class ForwardedHeaderTransformer implements Function<ServerHttpRequest, ServerHttpRequest> {
    ...
    /**
     * Enable mode in which any "Forwarded" or "X-Forwarded-*" headers are removed only and the information in them
     * ignored.
     *
     * @param removeOnly
