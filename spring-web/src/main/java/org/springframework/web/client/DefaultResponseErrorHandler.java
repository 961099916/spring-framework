 ...
 *
 * <p>
 * This error handler checks for the status code on the {@link ClientHttpResponse}. Any code in the 4xx or 5xx series is
 * considered to be an error. This behavior can be changed by overriding {@link #hasError(HttpStatus)}. Unknown status
...
public class DefaultResponseErrorHandler implements ResponseErrorHandler {
    ...
    /**
     * Delegates to {@link #hasError(HttpStatus)} (for a standard status enum value) or {@link #hasError(int)} (for an
     * unknown status code) with the response status code.
     * 
     ...
     * <p>
     * The default implementation checks {@link HttpStatus#isError()}. Can be overridden in subclasses.
     * 
     * @param statusCode
     ...
     * <ul>
     * <li>{@link HttpClientErrorException} if the status code is in the 4xx series, or one of its sub-classes such as
     * {@link HttpClientErrorException.BadRequest} and others.
     * <li>{@link HttpServerErrorException} if the status code is in the 5xx series, or one of its sub-classes such as
     * {@link HttpServerErrorException.InternalServerError} and others.
     * <li>{@link UnknownHttpStatusCodeException} for error status codes not in the {@link HttpStatus} enum range.
     * </ul>
     * 
    ...
    public void handleError(ClientHttpResponse response) throws IOException {
        ...
        if (statusCode == null) {
            ...
            String message =
                getErrorMessage(response.getRawStatusCode(), response.getStatusText(), body, getCharset(response));
     ...
     *
     * <p>
     * The default implementation delegates to {@link HttpClientErrorException#create} for errors in the 4xx range, to
     * {@link HttpServerErrorException#create} for errors in the 5xx range, or otherwise raises
     * {@link UnknownHttpStatusCodeException}.
     *
     * @since 5.0
     ...
     * @throws UnknownHttpStatusCodeException
     *             in case of an unknown status code that cannot be represented with the {@link HttpStatus} enum
     * @since 4.3.8
