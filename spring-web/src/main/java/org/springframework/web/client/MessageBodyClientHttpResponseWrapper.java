...
/**
 * Implementation of {@link ClientHttpResponse} that can not only check if the response has a message body, but also if
 * its length is 0 (i.e. empty) by actually reading the input stream.
 *
 * @author Brian Clozel
 * @since 4.1.5
...
class MessageBodyClientHttpResponseWrapper implements ClientHttpResponse {
    ...
    public boolean hasMessageBody() throws IOException {
        ...
        if (status != null
            && (status.is1xxInformational() || status == HttpStatus.NO_CONTENT || status == HttpStatus.NOT_MODIFIED)) {
            return false;
        }
