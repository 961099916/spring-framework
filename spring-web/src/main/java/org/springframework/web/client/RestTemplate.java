...
/**
 * Synchronous client to perform HTTP requests, exposing a simple, template method API over underlying HTTP client
 * libraries such as the JDK {@code HttpURLConnection}, Apache HttpComponents, and others.
 *
 * <p>
 * The RestTemplate offers templates for common scenarios by HTTP method, in addition to the generalized
 * {@code exchange} and {@code execute} methods that support of less frequent cases.
 *
 * <p>
 * <strong>NOTE:</strong> As of 5.0 this class is in maintenance mode, with only minor requests for changes and bugs to
 * be accepted going forward. Please, consider using the {@code org.springframework.web.reactive.client.WebClient} which
...
public class RestTemplate extends InterceptingHttpAccessor implements RestOperations {
    ...
    /**
     * Create a new instance of the {@link RestTemplate} using default settings. Default {@link HttpMessageConverter
     * HttpMessageConverters} are initialized.
     */
    public RestTemplate() {
     ...
     * <p>
     * By default, {@link DefaultUriBuilderFactory} is used and for backwards compatibility, the encoding mode is set to
     * {@link EncodingMode#URI_COMPONENT URI_COMPONENT}. As of 5.0.8, prefer using
     * {@link EncodingMode#TEMPLATE_AND_VALUES TEMPLATE_AND_VALUES}.
     * <p>
     * <strong>Note:</strong> in 5.0 the switch from {@link org.springframework.web.util.DefaultUriTemplateHandler
     * DefaultUriTemplateHandler} (deprecated in 4.3), as the default to use, to {@link DefaultUriBuilderFactory} brings
     * in a different default for the {@code parsePath} property (switching from false to true).
     *
     * @param handler
     ...
     * <p>
     * The {@link ClientHttpRequest} is processed using the {@link RequestCallback}; the response with the
     ...
     *            the HTTP method to execute (GET, POST, etc.)
     * @param response
     *            the resulting {@link ClientHttpResponse}
     * @throws IOException
    ...
    /**
     * Return a {@code RequestCallback} that sets the request {@code Accept} header based on the given response type,
     ...
     * <ol>
     * <li>Sets the request {@code Accept} header based on the given response type, cross-checked against the configured
     * message converters.
        ...
        public void doWithRequest(ClientHttpRequest request) throws IOException {
            if (this.responseType != null) {
                List<MediaType> allSupportedMediaTypes =
                    getMessageConverters().stream().filter(converter -> canReadResponse(this.responseType, converter))
