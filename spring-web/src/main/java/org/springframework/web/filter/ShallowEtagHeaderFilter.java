...
/**
 * {@link javax.servlet.Filter} that generates an {@code ETag} value based on the content on the response. This ETag is
 * compared to the {@code If-None-Match} header of the request. If these headers are equal, the response content is not
 * sent, but rather a {@code 304 "Not Modified"} status instead.
 *
 * <p>
 * Since the ETag is based on the response content, the response (e.g. a {@link org.springframework.web.servlet.View})
 * is still rendered. As such, this filter only saves bandwidth, not server performance.
 *
 * <p>
 * <b>NOTE:</b> As of Spring Framework 5.0, this filter uses request/response decorators built on the Servlet 3.1 API.
 *
...
public class ShallowEtagHeaderFilter extends OncePerRequestFilter {
    ...
    /**
     * This method can be used to suppress the content caching response wrapper of the ShallowEtagHeaderFilter. The main
     * reason for this is streaming scenarios which are not to be cached and do not need an eTag.
     * <p>
     * <strong>Note:</strong> This method must be called before the response is written to in order for the entire
     * response content to be written without caching.
     *
     * @since 4.2
     */
     ...
     * <p>
     * Should be configured using an {@code <init-param>} for parameter name "writeWeakETag" in the filter definition in
     * {@code web.xml}.
     *
     * @since 4.3
    ...
    /**
     * The default value is {@code false} so that the filter may delay the generation of an ETag until the last
    ...
    /**
     * Whether an ETag should be calculated for the given request and response exchange. By default this is {@code true}
    ...
    /**
     * Returns the raw OutputStream, instead of the one that does caching, if {@link #isContentCachingDisabled}.
     */
