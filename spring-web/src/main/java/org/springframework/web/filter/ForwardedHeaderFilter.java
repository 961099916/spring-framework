...
/**
 * Extract values from "Forwarded" and "X-Forwarded-*" headers, wrap the request and response, and make they reflect the
 * client-originated protocol and address in the following methods:
 * <ul>
 ...
 *
 * <p>
 * This filter can also be used in a {@link #setRemoveOnly removeOnly} mode where "Forwarded" and "X-Forwarded-*"
 * headers are eliminated, and not used.
 *
...
public class ForwardedHeaderFilter extends OncePerRequestFilter {
    ...
    /**
     * Enables mode in which any "Forwarded" or "X-Forwarded-*" headers are removed only and the information in them
     * ignored.
     * 
     * @param removeOnly
    ...
    /**
     * Use this property to enable relative redirects as explained in {@link RelativeRedirectFilter}, and also using the
     * same response wrapper as that filter does, or if both are configured, only one will wrap.
     * <p>
     * By default, if this property is set to false, in which case calls to
     * {@link HttpServletResponse#sendRedirect(String)} are overridden in order to turn relative into absolute URLs,
     * also taking into account forwarded headers.
     * 
     * @param relativeRedirects
     *            whether to use relative redirects
     * @since 4.3.10
     */
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if (this.removeOnly) {
            ...
            HttpServletResponse wrappedResponse =
                this.relativeRedirects ? RelativeRedirectResponseWrapper.wrapIfNecessary(response, HttpStatus.SEE_OTHER)
    ...
    /**
     * Responsible for the contextPath, requestURI, and requestURL with forwarded headers in mind, and also taking into
     * account changes to the path of the underlying delegate request (e.g. on a Servlet FORWARD).
     */
    private static class ForwardedPrefixExtractor {
         ...
         *            supplier for the current {@link HttpServletRequestWrapper#getRequest() delegate request} which may
         *            change during a forward (e.g. Tomcat.
         * @param baseUrl
         *            the host, scheme, and port based on forwarded headers
         */
    ...
    private static class ForwardedHeaderExtractingResponse extends HttpServletResponseWrapper {
        ...
        public void sendRedirect(String location) throws IOException {
            ...
            String result = UriComponentsBuilder.fromHttpRequest(new ServletServerHttpRequest(this.request))
                .replacePath(path).replaceQuery(uriComponents.getQuery()).fragment(uriComponents.getFragment()).build()
