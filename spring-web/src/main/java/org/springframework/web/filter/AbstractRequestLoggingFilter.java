 ...
 *
 * <p>
 * Subclasses are passed the message to write to the log in the {@code beforeRequest} and {@code afterRequest} methods.
 * By default, only the URI of the request is logged. However, setting the {@code includeQueryString} property to
 * {@code true} will cause the query string of the request to be included also; this can be further extended through
 * {@code includeClientInfo} and {@code includeHeaders}. The payload (body content) of the request can be logged via the
 * {@code includePayload} flag: Note that this will only log the part of the payload which has actually been read, not
 * necessarily the entire body of the request.
 *
 * <p>
 * Prefixes and suffixes for the before and after messages can be configured using the {@code beforeMessagePrefix},
 * {@code afterMessagePrefix}, {@code beforeMessageSuffix} and {@code afterMessageSuffix} properties.
 *
 * @author Rob Harrop
...
public abstract class AbstractRequestLoggingFilter extends OncePerRequestFilter {
     ...
     * <p>
     * Should be configured using an {@code <init-param>} for parameter name "includeQueryString" in the filter
     * definition in {@code web.xml}.
     */
     ...
     * <p>
     * Should be configured using an {@code <init-param>} for parameter name "includeClientInfo" in the filter
     * definition in {@code web.xml}.
     */
     ...
     * <p>
     * Should be configured using an {@code <init-param>} for parameter name "includeHeaders" in the filter definition
     * in {@code web.xml}.
     *
     * @since 4.3
     */
     ...
     * <p>
     * Should be configured using an {@code <init-param>} for parameter name "includePayload" in the filter definition
     * in {@code web.xml}.
     *
     * @since 3.0
     */
    ...
    /**
     * Configure a predicate for selecting which headers should be logged if {@link #setIncludeHeaders(boolean)} is set
     * to {@code true}.
     * <p>
     ...
     * {@link #createMessage(HttpServletRequest, String, String)} when {@link #isIncludePayload()} returns true.
     * 
     * @since 5.0.3
     */
    @Nullable
    ...
    /**
     * Determine whether to call the {@link #beforeRequest}/{@link #afterRequest} methods for the current request, i.e.
     * whether logging is currently active (and the log message is worth building).
     * <p>
     * The default implementation always returns {@code true}. Subclasses may override this with a log level check.
     * 
     * @param request
