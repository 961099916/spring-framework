...
/**
 * JSF PhaseListener implementation that delegates to one or more Spring-managed PhaseListener beans coming from the
 ...
 * </pre>
 *
 * The multicaster will delegate all {@code beforePhase} and {@code afterPhase} events to all target PhaseListener
 * beans. By default, those will simply be obtained by type: All beans in the Spring root WebApplicationContext that
 * implement the PhaseListener interface will be fetched and invoked.
 *
 * <p>
 * Note: This multicaster's {@code getPhaseId()} method will always return {@code ANY_PHASE}. <b>The phase id exposed by
 * the target listener beans will be ignored; all events will be propagated to all listeners.</b>
 *
 * <p>
 * This multicaster may be subclassed to change the strategy used to obtain the listener beans, or to change the
 * strategy used to access the ApplicationContext (normally obtained via
...
public class DelegatingPhaseListenerMulticaster implements PhaseListener {
     ...
     * <p>
     * The default implementation delegates to {@code getWebApplicationContext}. Can be overridden to provide an
     * arbitrary ListableBeanFactory reference to resolve against; usually, this will be a full Spring
     * ApplicationContext.
     * 
     * @param facesContext
     *            the current JSF context
