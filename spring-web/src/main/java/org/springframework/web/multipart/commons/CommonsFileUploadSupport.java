 ...
 * CommonsMultipartFile instances as representation of uploaded files and a String-based parameter Map as representation
 * of uploaded form fields.
 *
...
public abstract class CommonsFileUploadSupport {
    ...
    /**
     * Return the underlying {@code org.apache.commons.fileupload.disk.DiskFileItemFactory} instance. There is hardly
    ...
    /**
     * Return the underlying {@code org.apache.commons.fileupload.FileUpload} instance. There is hardly any need to
     * access this.
     * 
    ...
    /**
     * Set the maximum allowed size (in bytes) for each individual file before an upload gets rejected. -1 indicates no
     * limit (the default).
     * 
     * @param maxUploadSizePerFile
     *            the maximum upload size per file
     * @since 4.2
     ...
     * {@link CommonsMultipartFile#getOriginalFilename()}.
     * <p>
     * Default is "false", stripping off path information that may prefix the actual filename e.g. from Opera. Switch
     * this to "true" for preserving the client-specified filename as-is, including potential path separators.
     * 
     * @since 4.3.5
     ...
     * 
     * @param fileItems
     *            the Commons FileItems to parse
     * @param encoding
     ...
     *            a Collection of MultipartFile instances
        ...
        for (List<MultipartFile> files : multipartFiles.values()) {
            for (MultipartFile file : files) {
                if (file instanceof CommonsMultipartFile) {
                    ...
                    LogFormatUtils.traceDebug(logger, traceOn -> "Cleaning up part '" + cmf.getName() + "', filename '"
                        + cmf.getOriginalFilename() + "'" + (traceOn ? ", stored " + cmf.getStorageDescription() : ""));
                }
            }
        }
    }
