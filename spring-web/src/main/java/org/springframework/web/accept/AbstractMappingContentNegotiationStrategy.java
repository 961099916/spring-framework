 ...
 *
 * <p>
 * First a key (e.g. "json", "pdf") must be extracted from the request (e.g. file extension, query param). The key must
 * then be resolved to media type(s) through the base class {@link MappingMediaTypeFileExtensionResolver} which stores
 * such mappings.
 *
 * <p>
 * The method {@link #handleNoMatch} allow sub-classes to plug in additional ways of looking up media types (e.g.
 * through the Java Activation framework, or {@link javax.servlet.ServletContext#getMimeType}. Media types resolved via
 * base classes are then added to the base class {@link MappingMediaTypeFileExtensionResolver}, i.e. cached for new
 * lookups.
 *
...
public abstract class AbstractMappingContentNegotiationStrategy extends MappingMediaTypeFileExtensionResolver
    ...
    /**
     * Whether to ignore requests with unknown file extension. Setting this to {@code false} results in
    ...
    /**
     * Override to provide handling when a key is not resolved via. {@link #lookupMediaType}. Sub-classes can take
     * further steps to determine the media type(s). If a MediaType is returned from this method it will be added to the
