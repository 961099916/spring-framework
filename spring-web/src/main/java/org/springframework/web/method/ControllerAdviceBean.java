...
/**
 * Encapsulates information about an {@link ControllerAdvice @ControllerAdvice} Spring-managed bean without necessarily
 * requiring it to be instantiated.
 *
 * <p>
 * The {@link #findAnnotatedBeans(ApplicationContext)} method can be used to discover such beans. However, a
 * {@code ControllerAdviceBean} may be created from any object, including ones without an {@code @ControllerAdvice}
 * annotation.
 *
...
public class ControllerAdviceBean implements Ordered {
    ...
    /**
     * Reference to the resolved bean instance, potentially lazily retrieved via the {@code BeanFactory}.
     */
    @Nullable
     ...
     * @param beanFactory
     *            a {@code BeanFactory} to retrieve the bean type initially and later to resolve the actual bean
     */
     ...
     *            the {@code @ControllerAdvice} annotation for the bean, or {@code null} if not yet retrieved
     * @since 5.2
     */
    ...
    /**
     * Find beans annotated with {@link ControllerAdvice @ControllerAdvice} in the given {@link ApplicationContext} and
     * wrap them as {@code ControllerAdviceBean} instances.
     * <p>
     * As of Spring Framework 5.2, the {@code ControllerAdviceBean} instances in the returned list are sorted using
     * {@link OrderComparator#sort(List)}.
     *
     * @see #getOrder()
     * @see OrderComparator
     * @see Ordered
    ...
    private static HandlerTypePredicate createBeanTypePredicate(@Nullable ControllerAdvice controllerAdvice) {
        if (controllerAdvice != null) {
            return HandlerTypePredicate.builder().basePackage(controllerAdvice.basePackages())
                ...
                .assignableType(controllerAdvice.assignableTypes()).annotation(controllerAdvice.annotations()).build();
        }
     ...
     * <p>
     * As of Spring Framework 5.2, the order value is lazily retrieved using the following algorithm and cached. Note,
     * however, that a {@link ControllerAdvice @ControllerAdvice} bean that is configured as a scoped bean &mdash; for
     * example, as a request-scoped or session-scoped bean &mdash; will not be eagerly resolved. Consequently,
     * {@link Ordered} is not honored for scoped {@code @ControllerAdvice} beans.
     * <ul>
     * <li>If the {@linkplain #resolveBean resolved bean} implements {@link Ordered}, use the value returned by
     * {@link Ordered#getOrder()}.</li>
     * <li>If the {@linkplain #getBeanType() bean type} is known, use the value returned by
     * {@link OrderUtils#getOrder(Class, int)} with {@link Ordered#LOWEST_PRECEDENCE} used as the default order
     * value.</li>
     * <li>Otherwise use {@link Ordered#LOWEST_PRECEDENCE} as the default, fallback order value.</li>
     * </ul>
     *
     * @see #resolveBean()
     */
    ...
    /**
     * Get the bean instance for this {@code ControllerAdviceBean}, if necessary resolving the bean name through the
     * {@link BeanFactory}.
     * <p>
     * As of Spring Framework 5.2, once the bean instance has been resolved it will be cached if it is a singleton,
