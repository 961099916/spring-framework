...
/**
 * Resolve {@code @ModelAttribute} annotated method arguments and handle return values from {@code @ModelAttribute}
 * annotated methods.
 *
 * <p>
 * Model attributes are obtained from the model or created with a default constructor (and then added to the model).
 * Once created the attribute is populated via data binding to Servlet request parameters. Validation may be applied if
 * the argument is annotated with {@code @javax.validation.Valid}. or Spring's own
 * {@code @org.springframework.validation.annotation.Validated}.
 *
 * <p>
 * When this handler is created with {@code annotationNotRequired=true} any non-simple type argument and return value is
 * regarded as a model attribute with or without the presence of an {@code @ModelAttribute}.
 *
...
public class ModelAttributeMethodProcessor implements HandlerMethodArgumentResolver, HandlerMethodReturnValueHandler {
     ...
     *            if "true", non-simple method arguments and return values are considered model attributes with or
     *            without a {@code @ModelAttribute} annotation
     */
    ...
    /**
     * Returns {@code true} if the parameter is annotated with {@link ModelAttribute} or, if in default resolution mode,
     * for any method parameter that is not a simple type.
     */
    @Override
     ...
     *             if data binding and validation result in an error and the next method parameter is not of type
     *             {@link Errors}
     * @throws Exception
    ...
    /**
     * Extension point to create the model attribute if not found in the model, with subsequent parameter binding
     * through bean properties (unless suppressed).
     * <p>
     * The default implementation typically uses the unique public no-arg constructor if available but also handles a
     * "primary constructor" approach for data classes: It understands the JavaBeans {@link ConstructorProperties}
     * annotation as well as runtime-retained parameter names in the bytecode, associating request parameters with
     * constructor arguments by name. If no such constructor is found, the default constructor will be used (even if not
     * public), assuming subsequent bean property bindings through setter methods.
     * 
     * @param attributeName
     ...
     * @throws BindException
     *             in case of constructor argument binding failure
     * @throws Exception
     *             in case of constructor invocation failure
     ...
     * <p>
     * Called from {@link #createAttribute(String, MethodParameter, WebDataBinderFactory, NativeWebRequest)} after
     ...
     * {@link org.springframework.validation.annotation.Validated}, and custom annotations whose name starts with
     * "Valid".
     * 
     * @param binder
     ...
     * 
     * @param ann
     *            the annotation (potentially a validation annotation)
     * @return the validation hints to apply (possibly an empty array), or {@code null} if this annotation does not
    ...
    /**
     * Return {@code true} if there is a method-level {@code @ModelAttribute} or, in default resolution mode, for any
     * return value type that is not a simple type.
     */
    @Override
