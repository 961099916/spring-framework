 ...
 *
 * <p>
 * By default response content is consumed in full upon writing and cached for subsequent access, however it is also
 * possible to set a custom {@link #setWriteHandler(Function) writeHandler}.
 *
...
public class MockServerHttpResponse extends AbstractServerHttpResponse {
    ...
    private Flux<DataBuffer> body =
        Flux.error(new IllegalStateException("No content was written nor was setComplete() called on this response."));
     ...
     * <p>
     * By default, response body content is consumed in full and cached for subsequent access in tests. Use this option
     * to take control over how the response body is consumed.
     * 
     * @param writeHandler
     *            the write handler to use returning {@code Mono<Void>} when the body has been "written" (i.e.
     *            consumed).
     */
    ...
    public Mono<String> getBodyAsString() {
        ...
        Charset charset =
            Optional.ofNullable(getHeaders().getContentType()).map(MimeType::getCharset).orElse(StandardCharsets.UTF_8);
