...
public abstract class AbstractHttpHandlerIntegrationTests {
     ...
     *
     * <p>
     * If the test method throws an {@link HttpServerErrorException}, this extension will throw an
     * {@link AssertionError} that wraps the {@code HttpServerErrorException} using the
     * {@link HttpServerErrorException#getResponseBodyAsString() response body} as the failure message.
     *
     * <p>
     * This mechanism provides an actually meaningful failure message if the test fails due to an {@code AssertionError}
     * on the server.
     */
    @RegisterExtension
    ...
    /**
     * Return an interval stream of N number of ticks and buffer the emissions to avoid back pressure failures (e.g. on
     * slow CI server).
     *
     * <p>
     * Use this method as follows:
     * <ul>
     * <li>Tests that verify N number of items followed by verifyOnComplete() should set the number of emissions to N.
     * <li>Tests that verify N number of items followed by thenCancel() should set the number of buffered to an
     * arbitrary number greater than N.
     * </ul>
     */
    ...
    static Stream<HttpServer> httpServers() {
        return Stream.of(new JettyHttpServer(), new ReactorHttpServer(), new TomcatHttpServer(),
