...
public class MockClientHttpRequest extends AbstractClientHttpRequest implements HttpRequest {
    ...
    private Flux<DataBuffer> body = Flux.error(new IllegalStateException(
        "The body is not set. " + "Did handling complete with success? Is a custom \"writeHandler\" configured?"));
     ...
     *
     * <p>
     * The default write handler consumes and caches the request body so it may be accessed subsequently, e.g. in test
     * assertions. Use this property when the request body is an infinite stream.
     *
     * @param writeHandler
    ...
    /**
     * Return the request body, or an error stream if the body was never set or when {@link #setWriteHandler} is
     * configured.
     */
