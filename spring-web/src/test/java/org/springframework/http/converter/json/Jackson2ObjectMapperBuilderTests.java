...
public class Jackson2ObjectMapperBuilderTests {
    ...
    public void booleanSetters() {
        ObjectMapper objectMapper = Jackson2ObjectMapperBuilder.json()
            .featuresToEnable(MapperFeature.DEFAULT_VIEW_INCLUSION, DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES,
    ...
    public void customizeWellKnownModulesWithSerializer() throws JsonProcessingException, UnsupportedEncodingException {
        ...
        ObjectMapper objectMapper =
            Jackson2ObjectMapperBuilder.json().serializerByType(Integer.class, new CustomIntegerSerializer()).build();
        DateTime dateTime = new DateTime(1322903730000L, DateTimeZone.UTC);
        assertThat(new String(objectMapper.writeValueAsBytes(dateTime), "UTF-8")).isEqualTo("1322903730000");
        assertThat(new String(objectMapper.writeValueAsBytes(4), "UTF-8")).contains("customid");
    }
    ...
    public void registerMultipleModulesWithNullTypeId() {
        ...
        assertThat(StreamSupport.stream(getSerializerFactoryConfig(objectMapper).serializers().spliterator(), false)
            .filter(s -> s.findSerializer(null, SimpleType.construct(Foo.class), null) != null).count()).isEqualTo(1);
        assertThat(StreamSupport.stream(getSerializerFactoryConfig(objectMapper).serializers().spliterator(), false)
            .filter(s -> s.findSerializer(null, SimpleType.construct(Bar.class), null) != null).count()).isEqualTo(1);
    }
    ...
    public void serializerByType() {
        ...
        ObjectMapper objectMapper = Jackson2ObjectMapperBuilder.json().modules(new ArrayList<>()) // Disable well-known
                                                                                                  // modules detection
    ...
    public void filters() throws JsonProcessingException {
        ObjectMapper objectMapper =
            Jackson2ObjectMapperBuilder.json().filters(new SimpleFilterProvider().setFailOnUnknownId(false)).build();
    ...
    public void visibility() throws JsonProcessingException {
        ObjectMapper objectMapper =
            Jackson2ObjectMapperBuilder.json().visibility(PropertyAccessor.GETTER, Visibility.NONE)
