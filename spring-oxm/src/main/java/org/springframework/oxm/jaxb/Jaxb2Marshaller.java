 ...
 *
 * <p>
 * The typical usage will be to set either the "contextPath" or the "classesToBeBound" property on this bean, possibly
 * customize the marshaller and unmarshaller by setting properties, schemas, adapters, and listeners, and to refer to
 * it.
 *
...
public class Jaxb2Marshaller implements MimeMarshaller, MimeUnmarshaller, GenericMarshaller, GenericUnmarshaller,
    ...
    /**
     * Set multiple JAXB context paths. The given array of context paths gets converted to a colon-delimited string, as
     * supported by JAXB.
     */
     ...
     * <p>
     * Setting either this property, {@link #setClassesToBeBound "classesToBeBound"} or {@link #setPackagesToScan
     * "packagesToScan"} is required.
     */
     ...
     * <p>
     * Setting either this property, {@link #setContextPath "contextPath"} or {@link #setPackagesToScan
     * "packagesToScan"} is required.
     */
     ...
     * search and therefore analogous to Spring's component-scan feature
     * ({@link org.springframework.context.annotation.ClassPathBeanDefinitionScanner}).
     * <p>
     * Setting either this property, {@link #setContextPath "contextPath"} or {@link #setClassesToBeBound
     * "classesToBeBound"} is required.
     */
    ...
    /**
     * Set the {@code JAXBContext} properties. These implementation-specific properties will be set on the underlying
     * {@code JAXBContext}.
     */
     ...
     * <p>
     * These properties will be set on the underlying JAXB {@code Marshaller}, and allow for features such as
     * indentation.
     * 
     * @param properties
     *            the properties
    ...
    /**
     * Set the JAXB validation event handler. This event handler will be called by JAXB if any validation errors are
     * encountered during calls to any of the marshal APIs.
     */
    ...
    /**
     * Set whether to lazily initialize the {@link JAXBContext} for this marshaller. Default is {@code false} to
     * initialize on startup; can be switched to {@code true}.
     * <p>
    ...
    /**
     * Specify whether MTOM support should be enabled or not. Default is {@code false}: marshalling using XOP/MTOM not
     * being enabled.
     */
     ...
     * {@code JAXBElement} classes (though {@link #supports(Type)} can return {@code true}, since it can obtain the type
     * parameters of {@code JAXBElement}).
     * <p>
     ...
     * <p>
     * Default is {@code true}, meaning that {@code supports(Class)} will check for this annotation. However, some JAXB
     * implementations (i.e. EclipseLink MOXy) allow for defining the bindings in an external definition file, thus
     * keeping the classes annotations free. Setting this property to {@code false} supports these JAXB implementations.
     * 
     * @see #supports(Class)
     * @see #supports(Type)
     */
     ...
     * <p>
     * Default is {@code false}, meaning that external entities are not resolved. Note that processing of external
     * entities will only be enabled/disabled when the {@code Source} passed to {@link #unmarshal(Source)} is a
     * {@link SAXSource} or {@link StreamSource}. It has no effect for {@link DOMSource} or {@link StAXSource}
     * instances.
     * <p>
     * <strong>Note:</strong> setting this option to {@code true} also automatically sets {@link #setSupportDtd} to
     * {@code true}.
     */
    ...
    private Schema loadSchema(Resource[] resources, String schemaLanguage) throws IOException, SAXException {
        if (logger.isDebugEnabled()) {
            logger
                .debug("Setting validation schema to " + StringUtils.arrayToCommaDelimitedString(this.schemaResources));
        }
     ...
     * called after creation of JAXB {@code Marshaller}, and after the respective properties have been set.
     * <p>
     * The default implementation sets the {@link #setMarshallerProperties defined properties}, the
     ...
     * called after creation of JAXB {@code Marshaller}, and after the respective properties have been set.
     * <p>
     * The default implementation sets the {@link #setUnmarshallerProperties defined properties}, the
     * {@link #setValidationEventHandler validation event handler}, the {@link #setSchemas schemas},
     * {@link #setUnmarshallerListener listener}, and {@link #setAdapters adapters}.
     */
    ...
    /**
     * Convert the given {@code JAXBException} to an appropriate exception from the {@code org.springframework.oxm}
     * hierarchy.
     * 
     * @param ex
     *            {@code JAXBException} that occurred
