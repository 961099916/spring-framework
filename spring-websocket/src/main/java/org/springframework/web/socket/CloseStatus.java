...
/**
 * Represents a WebSocket close status code and reason. Status codes in the 1xxx range are pre-defined by the protocol.
 * Optionally, a status code may be sent with a reason.
 *
 * <p>
...
public final class CloseStatus {
    ...
    /**
     * A status code for use within the framework the indicate a session has become unreliable (e.g. timed out while
     * sending a message) and extra care should be exercised, e.g. avoid sending any further data to the client that may
     * be done during normal shutdown.
     * 
     * @since 4.0.3
     */
