...
public abstract class AbstractHttpSockJsSession extends AbstractSockJsSession {
    ...
    /**
     * Unlike WebSocket where sub-protocol negotiation is part of the initial handshake, in HTTP transports the same
     * negotiation must be emulated and the selected protocol set through this setter.
     *
     * @param protocol
    ...
    /**
     * Return the SockJS buffer for messages stored transparently between polling requests. If the polling request takes
     * longer than 5 seconds, the session is closed.
     * 
     ...
     * <p>
     * Long polling-based transports (e.g. "xhr", "jsonp") complete the request after writing the open frame.
     * Streaming-based transports ("xhr_streaming", "eventsource", and "htmlfile") leave the response open longer for
     * further streaming of message frames but will also close it eventually after some amount of data has been sent.
     * 
     * @param request
     *            the current request
     * @param response
     ...
     * <p>
     * Long polling-based transports (e.g. "xhr", "jsonp") complete the request after writing any buffered message
     * frames (or the next one). Streaming-based transports ("xhr_streaming", "eventsource", and "htmlfile") leave the
     * response open longer for further streaming of message frames but will also close it eventually after some amount
    ...
    /**
     * Called when the connection is active and ready to write to the response. Subclasses should only call this method
     * from a method where the "responseLock" is acquired.
     */
