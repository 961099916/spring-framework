...
/**
 * A central class for aggregating information about internal state and counters from key infrastructure components of
 * the setup that comes with {@code @EnableWebSocketMessageBroker} for Java config and
 * {@code <websocket:message-broker>} for XML.
 *
 * <p>
 * By default aggregated information is logged every 30 minutes at INFO level. The frequency of logging can be changed
 * via {@link #setLoggingPeriod(long)}.
 *
 * <p>
 * This class is declared as a Spring bean by the above configuration with the name "webSocketMessageBrokerStats" and
 * can be easily exported to JMX, e.g. with the {@link org.springframework.jmx.export.MBeanExporter MBeanExporter}.
 *
    ...
    private ScheduledFuture<?> initLoggingTask(long initialDelay) {
        if (this.sockJsTaskScheduler != null && this.loggingPeriod > 0 && logger.isInfoEnabled()) {
            return this.sockJsTaskScheduler.scheduleWithFixedDelay(
                () -> logger.info(WebSocketMessageBrokerStats.this.toString()), Instant.now().plusMillis(initialDelay),
