...
public abstract class AbstractXhrTransport implements XhrTransport {
    ...
    /**
     * An {@code XhrTransport} can support both the "xhr_streaming" and "xhr" SockJS server transports. From a client
     * perspective there is no implementation difference.
     * <p>
     * Typically an {@code XhrTransport} is used as "XHR streaming" first and then, if that fails, as "XHR". In some
     * cases however it may be helpful to suppress XHR streaming so that only XHR is attempted.
     * <p>
    ...
    public ListenableFuture<WebSocketSession> connect(TransportRequest request, WebSocketHandler handler) {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(
                "Starting XHR " + (isXhrStreamingDisabled() ? "Polling" : "Streaming") + "session url=" + receiveUrl);
        }
