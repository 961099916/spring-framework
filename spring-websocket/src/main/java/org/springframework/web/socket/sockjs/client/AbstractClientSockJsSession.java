...
/**
 * Base class for SockJS client implementations of {@link WebSocketSession}. Provides processing of incoming SockJS
 * message frames and delegates lifecycle events and messages to the (application) {@link WebSocketHandler}. Sub-classes
 * implement actual send as well as disconnect logic.
 *
...
public abstract class AbstractClientSockJsSession implements WebSocketSession {
    ...
    /**
     * Return a timeout cleanup task to invoke if the SockJS sessions is not fully established within the retransmission
     * timeout period calculated in {@code SockJsRequest} based on the duration of the initial SockJS "Info" request.
     */
