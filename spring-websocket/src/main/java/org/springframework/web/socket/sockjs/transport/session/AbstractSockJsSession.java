...
public abstract class AbstractSockJsSession implements SockJsSession {
     ...
     * <a href="https://github.com/eclipse-ee4j/servlet-api/issues/44">eclipse-ee4j/servlet-api#44</a>. Therefore
     * network IO failures may occur simply because a client has gone away, and that can fill the logs with unnecessary
     * stack traces.
     * <p>
     * We make a best effort to identify such network failures, on a per-server basis, and log them under a separate log
     * category. A simple one-line message is logged at DEBUG level, while a full stack trace is shown at TRACE level.
     *
    ...
    /**
     * Tomcat: ClientAbortException or EOFException Jetty: EofException WildFly, GlassFish: java.io.IOException "Broken
     * pipe" (already covered)
     * <p>
     * TODO: This definition is currently duplicated between HttpWebHandlerAdapter and AbstractSockJsSession. It is a
     * candidate for a common utility class.
     * 
     ...
     *            the session ID
     * @param config
     *            the SockJS service configuration options
     * @param handler
     *            the recipient of SockJS messages
     * @param attributes
     *            the attributes from the HTTP handshake to associate with the WebSocket session; the provided
    ...
    private static List<String> getUndelivered(String[] messages, int i) {
        switch (messages.length - i) {
            ...
            case 1:
                return (messages[i].trim().isEmpty() ? Collections.emptyList()
    ...
    /**
     * Polling and Streaming sessions periodically close the current HTTP request and wait for the next request to come
     * through. During this "downtime" the session is still open but inactive and unable to send messages and therefore
     * has to buffer them temporarily. A WebSocket session by contrast is stateful and remain active until closed.
     */
    public abstract boolean isActive();

    /**
     * Actually close the underlying WebSocket session or in the case of HTTP transports complete the underlying
     * request.
     */
        ...
        if (indicatesDisconnectedClient(ex)) {
            if (disconnectedClientLogger.isTraceEnabled()) {
                ...
                disconnectedClientLogger.debug(
                    "Looks like the client has gone away: " + ex + " (For a full stack trace, set the log category '"
                        + DISCONNECTED_CLIENT_LOG_CATEGORY + "' to TRACE level.)");
            }
        } else {
