...
public interface TransportHandler {
    ...
    /**
     * Check whether the type of the given session matches the transport type of this {@code TransportHandler} where
     * session id and the transport type are extracted from the SockJS URL.
     * 
     * @return {@code true} if the session matches (and would therefore get accepted by {@link #handleRequest}), or
     ...
     * @param handler
     *            the target WebSocketHandler (never {@code null})
     * @param session
     *            the SockJS session (never {@code null})
     * @throws SockJsException
     *             raised when request processing fails as explained in {@link SockJsService}
     */
