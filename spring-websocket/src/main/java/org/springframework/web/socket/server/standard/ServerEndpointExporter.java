...
/**
 * Detects beans of type {@link javax.websocket.server.ServerEndpointConfig} and registers with the standard Java
 * WebSocket runtime. Also detects beans annotated with {@link ServerEndpoint} and registers them as well. Although not
 * required, it is likely annotated endpoints should have their {@code configurator} property set to
 * {@link SpringConfigurator}.
 *
 * <p>
 * When this class is used, by declaring it in Spring configuration, it should be possible to turn off a Servlet
 * container's scan for WebSocket endpoints. This can be done with the help of the {@code <absolute-ordering>} element
 * in {@code web.xml}.
 *
...
public class ServerEndpointExporter extends WebApplicationObjectSupport
    ...
    /**
     * Explicitly list annotated endpoint types that should be registered on startup. This can be done if you wish to
     * turn off a Servlet container's scan for endpoints, which goes through all 3rd party jars in the, and rely on
     * Spring configuration instead.
     * 
     * @param annotatedEndpointClasses
     *            {@link ServerEndpoint}-annotated types
     */
    ...
    /**
     * Set the JSR-356 {@link ServerContainer} to use for endpoint registration. If not set, the container is going to
     * be retrieved via the {@code ServletContext}.
     */
