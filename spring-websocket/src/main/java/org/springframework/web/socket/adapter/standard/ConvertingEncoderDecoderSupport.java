...
/**
 * Base class that can be used to implement a standard {@link javax.websocket.Encoder} and/or
 * {@link javax.websocket.Decoder}. It provides encode and decode method implementations that delegate to a Spring
 * {@link ConversionService}.
 *
 * <p>
 * By default, this class looks up a {@link ConversionService} registered in the {@link #getApplicationContext() active
 * ApplicationContext} under the name {@code 'webSocketConversionService'}. This works fine for both client and server
 * endpoints, in a Servlet container environment. If not running in a Servlet container, subclasses will need to
 * override the {@link #getConversionService()} method to provide an alternative lookup strategy.
 *
 * <p>
 * Subclasses can extend this class and should also implement one or both of {@link javax.websocket.Encoder} and
 * {@link javax.websocket.Decoder}. For convenience {@link ConvertingEncoderDecoderSupport.BinaryEncoder},
 * {@link ConvertingEncoderDecoderSupport.BinaryDecoder}, {@link ConvertingEncoderDecoderSupport.TextEncoder} and
 * {@link ConvertingEncoderDecoderSupport.TextDecoder} subclasses are provided.
 *
 * <p>
 * Since JSR-356 only allows Encoder/Decoder to be registered by type, instances of this class are therefore managed by
 * the WebSocket runtime, and do not need to be registered as Spring Beans. They can, however, by injected with
 * Spring-managed dependencies via {@link Autowired @Autowire}.
 *
 * <p>
 * Converters to convert between the {@link #getType() type} and {@code String} or {@code ByteBuffer} should be
 * registered.
 *
 * @author Phillip Webb
 * @since 4.0
 ...
 * @param <M>
 *            the WebSocket message type ({@link String} or {@link ByteBuffer})
...
public abstract class ConvertingEncoderDecoderSupport<T, M> {
    ...
    /**
     * Strategy method used to obtain the {@link ConversionService}. By default this method expects a bean named
     * {@code 'webSocketConversionService'} in the {@link #getApplicationContext() active ApplicationContext}.
     * 
     ...
     * {@link ContextLoader#getCurrentWebApplicationContext()}, which finds the ApplicationContext loaded via
     * {@link ContextLoader} typically in a Servlet container environment. When not running in a Servlet container and
    ...
    public T decode(M message) throws DecodeException {
        try {
            ...
            if (message instanceof String) {
                throw new DecodeException((String)message, "Unable to decode websocket message using ConversionService",
                    ex);
            }
    ...
    /**
     * A binary {@link javax.websocket.Encoder.Binary javax.websocket.Encoder} that delegates to Spring's conversion
     * service. See {@link ConvertingEncoderDecoderSupport} for details.
     * 
     * @param <T>
     *            the type that this Encoder can convert to
     */
     ...
     * 
     * @param <T>
     *            the type that this Decoder can convert from
     */
    ...
    /**
     * A text {@link javax.websocket.Encoder.Text javax.websocket.Encoder} that delegates to Spring's conversion
     * service. See {@link ConvertingEncoderDecoderSupport} for details.
     * 
     * @param <T>
     *            the type that this Encoder can convert to
