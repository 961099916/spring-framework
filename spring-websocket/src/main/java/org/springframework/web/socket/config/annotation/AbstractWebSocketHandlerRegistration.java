...
public abstract class AbstractWebSocketHandlerRegistration<M> implements WebSocketHandlerRegistration {
    ...
    /**
     * Expose the {@code SockJsServiceRegistration} -- if SockJS is enabled or {@code null} otherwise -- so that it can
     * be configured with a TaskScheduler if the application did not provide one. This should be done prior to calling
     * {@link #getMappings()}.
     */
    @Nullable
