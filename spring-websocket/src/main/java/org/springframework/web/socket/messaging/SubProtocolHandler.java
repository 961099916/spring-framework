...
/**
 * A contract for handling WebSocket messages as part of a higher level protocol, referred to as "sub-protocol" in the
 * WebSocket RFC specification. Handles both {@link WebSocketMessage WebSocketMessages} from a client as well as
 * {@link Message Messages} to a client.
 *
 * <p>
 * Implementations of this interface can be configured on a {@link SubProtocolWebSocketHandler} which selects a
 * sub-protocol handler to delegate messages to based on the sub-protocol requested by the client through the
