...
/**
 * Wrap a {@link org.springframework.web.socket.WebSocketSession WebSocketSession} to guarantee only one thread can send
 * messages at a time.
 *
 * <p>
 * If a send is slow, subsequent attempts to send more messages from other threads will not be able to acquire the flush
 * lock and messages will be buffered instead. At that time, the specified buffer-size limit and send-time limit will be
 * checked and the session will be closed if the limits are exceeded.
 *
...
public class ConcurrentWebSocketSessionDecorator extends WebSocketSessionDecorator {
     ...
     * 
     * @param delegate
     *            the {@code WebSocketSession} to delegate to
     * @param sendTimeLimit
     *            the send-time limit (milliseconds)
    ...
    public enum OverflowStrategy {
        ...
        /**
         * Throw {@link SessionLimitExceededException} that would will result in the session being terminated.
         */
        TERMINATE,
